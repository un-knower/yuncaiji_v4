...
public class HttpCmdExecutor implements Runnable {
	...
	private static final ILogger LOGGER = LoggerManager.getLogger(HttpCmdExecutor.class);
    ...
    public void run() {
        ...
        try {
            ...
            HttpCmdProcessor httpCmdProc = context.getCmdProcessor(request.getNodeIdentity(), request.getCommand());

            Assert.notNull(httpCmdProc, "Can not find the command:[" + request.getCommand() + "]");

            sendResponse(HTTP_OK, JsonConvert.serialize(httpCmdProc.execute(request)));

        } catch (HttpCMDErrorException ignored) {
            // 忽略
        } catch (IllegalArgumentException e) {
            sendError(HTTP_BADREQUEST, JsonConvert.serialize(HttpCmdResponse.newResponse(false, e.getMessage())), false);
        } catch (Throwable t) {
            LOGGER.error("Error When Execute Command", t);
            sendError(HTTP_INTERNALERROR, JsonConvert.serialize(HttpCmdResponse.newResponse(false, "Error:" + t.getMessage())), false);
        }
    }
