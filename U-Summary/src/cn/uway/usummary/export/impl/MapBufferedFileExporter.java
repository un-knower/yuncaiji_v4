...
public class MapBufferedFileExporter extends AbstractExporter {
	...
	private static final Logger LOGGER = LoggerFactory.getLogger(MapBufferedFileExporter.class); // 日志
	 ...
	 */
	public MapBufferedFileExporter(ExporterArgs exporterArgs) {
		super(exporterArgs);
	...
	protected void checkLocalFile() throws IOException {
		File rootFile = new File(this.exportPath);
		if (!rootFile.exists() && rootFile.mkdirs())
			LOGGER.debug("文件根目录{}不存在.尝试创建目录成功!", this.exportPath);
	}
	...
	protected void checkLocalExportFile() throws IOException {
		...
		String outFileName = this.exportPath + File.separator + tempFile;
	...
	protected String getRealExportFileName(String exportFileName) {
		if (this.zipFlag)
			return exportFileName.substring(0, exportFileName.lastIndexOf(".")) + "."
					+ (compressFormat == null ? "zip" : compressFormat);
	...
	protected void writeFile(Map<String, String> outDate) {
		...
		try {
			...
			this.fail++;
			this.errorCode = 0;
			this.cause = "MapBufferedFileExporter输出失败!";
			LOGGER.error("MapBufferedFileExporter输出失败,dest=" + this.dest, e);
		}

		// 如果文件超过了指定的尺寸，则换行
		if (this.partFileMaxSize > 0 && this.partFileMaxSize > 0 && this.partFileSize >= this.partFileMaxSize) {
			try {
				this.pack();
		...
		} else if (this.batchNum > 0 && this.partFileNum > this.batchNum) {
			try {
				this.pack();
				this.completeWriteFile();
				this.checkLocalFile();
	...
	public void pack() {
		...
		String packFileName = this.exportPath + File.separator + getRealExportFileName(this.exportFileName);
		String outFileName = this.exportPath + File.separator + this.exportFileName;
		...
		try {
			...
			if (exportFile.exists())
				LOGGER.debug("已经存在同名文件={},尝试删除{}",
						new Object[] { exportFile, (deleteFlag = exportFile.delete()) ? "成功" : "失败" });
			if (!deleteFlag) {
			...
			if (this.zipFlag && ("tar.gz".equalsIgnoreCase(compressFormat) || "zip".equalsIgnoreCase(compressFormat))) {
				...
				if (!renameFileFlag) {
					LOGGER.debug("重命名文件{}至{}完成,结果={}",
							new Object[] { tmpFile, exportFile, renameFileFlag ? "成功" : "失败" });
					return;
				}
				// if(packFile.exists())
				// {
