package cn.uway.ucloude.data.descriptor;
...
public class QueryResult implements Serializable {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -6794299586946371655L;
...
private List<Map<String, Object>> copyForShardResult = new ArrayList<Map<String, Object>>();
    ...
    public <T> T as(Class<T> beanClass){
    	...
    	if(result != null){
    		...
    		return JsonConvert.deserialize(JsonConvert.serialize(result), beanClass);
    	}
        return null;
    }
    ...
    public <T> List<T> asList(Class<T> beanClass){
        if(resultList != null){
        	return JsonConvert.deSerializeArray(JsonConvert.serialize(resultList), beanClass);
        }
        return null;
    }
	...
	private Map<String, Object> getResultWithAliasName(Map<String, Object> result, Map<String, String> aliasName){
    	if(result != null && result.size() > 0 && null != aliasName){
    		for(Entry<String, String> item : aliasName.entrySet()){
    			if(StringUtil.isNotBlank(item.getKey()) && StringUtil.isNotBlank(item.getValue())){
