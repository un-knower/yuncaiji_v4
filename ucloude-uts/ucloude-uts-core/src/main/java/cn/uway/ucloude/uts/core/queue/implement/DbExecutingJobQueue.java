package cn.uway.ucloude.uts.core.queue.implement;

import java.util.Date;
import java.util.List;

import cn.uway.ucloude.data.dataaccess.builder.SqlBuilderFactory;
import cn.uway.ucloude.query.Pagination;
import cn.uway.ucloude.uts.core.ExtConfigKeys;
import cn.uway.ucloude.uts.core.queue.ExecutingJobQueue;
import cn.uway.ucloude.uts.core.queue.domain.JobPo;
import cn.uway.ucloude.uts.core.queue.domain.JobQueueReq;

/**
 * 执行中的任务队列
 * @author Uway-M3
 *
 */
public class DbExecutingJobQueue  extends AbstractDbJobQueue implements ExecutingJobQueue {

	public DbExecutingJobQueue(String connectionKey) {
		super(connectionKey);
		// TODO Auto-generated constructor stub
	}

	@Override
	public boolean add(JobPo jobPo) {
		return super.add(getTableName(), jobPo);
	}

	@Override
	public boolean remove(String jobId) {
		// TODO Auto-generated method stub
		return SqlBuilderFactory.getSqlFactory(getSqlTemplate()).getDeleteSql().delete()
                .from()
                .table(getTableName())
                .where("job_id = ?", jobId)
                .doDelete() == 1;
	}

	@Override
	public List<JobPo> getJobs(String taskTrackerIdentity) {
		// TODO Auto-generated method stub
		return SqlBuilderFactory.getSqlFactory(getSqlTemplate()).getSelectSql().select()
                .all()
                .from()
                .table(getTableName())
                .where("task_tracker_identity = ?", taskTrackerIdentity)
                .list(RshHolder.JOB_PO_LIST_RSH);
	}

	@Override
	public List<JobPo> getDeadJobs(long deadline) {
		// TODO Auto-generated method stub
		return SqlBuilderFactory.getSqlFactory(getSqlTemplate()).getSelectSql().select()
                .all()
                .from()
                .table(getTableName())
                .where("created_time < ?", deadline)
                .list(RshHolder.JOB_PO_LIST_RSH);
	}

	@Override
	public JobPo getJob(String taskTrackerNodeGroup, String taskId) {
		// TODO Auto-generated method stub
		return SqlBuilderFactory.getSqlFactory(getSqlTemplate()).getSelectSql().select()
                .all()
                .from()
                .table(getTableName())
                .where("task_id = ?", taskId)
                .and("task_tracker_node_group = ?", taskTrackerNodeGroup)
                .single(RshHolder.JOB_PO_RSH);
	}

	@Override
	protected String getTableName() {
		// TODO Auto-generated method stub
		return "uts_excuting_job";
	}
}
