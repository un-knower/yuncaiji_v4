package cn.uway.ucloude.uts.core.registry.zookeeper;
...
import cn.uway.ucloude.uts.core.registry.FailbackRegistry;
...
public class ZookeeperRegistry extends FailbackRegistry {
	...
	public ZookeeperRegistry(final UtsContext appContext) {
		super(appContext);
		// TODO Auto-generated constructor stub
		
		this.clusterName = appContext.getConfiguration().getClusterName();
        this.cachedChildrenNodeMap = new ConcurrentHashMap<String, List<String>>();
        ZookeeperTransporter zookeeperTransporter = ServiceFactory.load(ZookeeperTransporter.class, appContext.getConfiguration());
        this.zkClient = zookeeperTransporter.connect(appContext.getConfiguration().getRegistryAddress());
    ...
    protected void doSubscribe(Node node, NotifyListener listener) {
        ...
        if (CollectionUtil.isEmpty(listenNodeTypes)) {
            return;
        }
        ...
        if (zkListener == null) {
            ...
            listeners.putIfAbsent(listener, new ChildListener() {
                ...
                public void childChanged(String parentPath, List<String> currentChildren) {
                    ...
                    // 1. 找出增加的 节点
                    List<String> addChildren = CollectionUtil.getLeftDiff(currentChildren, oldChildren);
                    // 2. 找出减少的 节点
                    List<String> decChildren = CollectionUtil.getLeftDiff(oldChildren, currentChildren);
                    ...
                    if (CollectionUtil.isNotEmpty(decChildren)) {
                        List<Node> nodes = new ArrayList<Node>(addChildren.size());
