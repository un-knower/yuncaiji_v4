(function (f, define) {
    ...
    (function ($, undefined) {
        var ufa = window.ufa, Widget = ufa.ui.Widget, Popup = ufa.ui.Popup, isFunction = ufa.isFunction, isPlainObject = $.isPlainObject, extend = $.extend, proxy = $.proxy, DOCUMENT = $(document), isLocalUrl = ufa.isLocalUrl, ARIAIDSUFFIX = '_tt_active', DESCRIBEDBY = 'aria-describedby', SHOW = 'show', HIDE = 'hide', ERROR = 'error', CONTENTLOAD = 'contentLoad', REQUESTSTART = 'requestStart', KCONTENTFRAME = 'k-content-frame', TEMPLATE = '<div role="tooltip" class="k-widget k-tooltip#if (!autoHide) {# k-tooltip-closable#}#">#if (!autoHide) {# <div class="k-tooltip-button"><a href="\\#" class="k-icon k-i-close">close</a></div> #}#' + '<div class="k-tooltip-content"></div>' + '#if (callout){ #<div class="k-callout k-callout-#=dir#"></div>#}#' + '</div>', IFRAMETEMPLATE = ufa.template('<iframe frameborder=\'0\' class=\'' + KCONTENTFRAME + '\' ' + 'src=\'#= content.url #\'>' + 'This page requires frames in order to show content' + '</iframe>'), NS = '.ufaTooltip', POSITIONS = {
                bottom: {
                ...
                return e.target.data(ufa.ns + 'title');
            };
        ...
        function restoreTitleAttributeForElement(element) {
            var title = element.data(ufa.ns + 'title');
            if (title) {
                element.attr('title', title);
                element.removeData(ufa.ns + 'title');
            }
        }
        function saveTitleAttributeForElement(element) {
            var title = element.attr('title');
            if (title) {
                element.data(ufa.ns + 'title', title);
                element.attr('title', '');
            }
        }
        ...
        var Tooltip = Widget.extend({
            ...
            _ajaxRequest: function (options) {
                ...
                jQuery.ajax(extend({
                    ...
                    error: function (xhr, status) {
                        ufa.ui.progress(that.content, false);
                        that.trigger(ERROR, {
                            status: status,
                            xhr: xhr
                        });
            ...
            _show: function (target) {
                ...
                if (current && current[0] != target[0]) {
                    that.popup.close();
                    that.popup.element.ufaStop(true, true);
                }
                if (!current || current[0] != target[0]) {
            ...
            _initPopup: function () {
                var that = this, options = that.options, wrapper = $(ufa.template(TEMPLATE)({
