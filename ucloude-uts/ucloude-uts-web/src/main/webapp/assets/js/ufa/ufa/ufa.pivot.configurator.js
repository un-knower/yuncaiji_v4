(function (f, define) {
    define('ufa.pivot.configurator', ['ufa.dom'], f);
}(function () {
    var __meta__ = {
    ...
    (function ($, undefined) {
        var ufa = window.ufa, ui = ufa.ui, Widget = ui.Widget, ns = '.ufaPivotConfigurator', HOVEREVENTS = 'mouseenter' + ns + ' mouseleave' + ns, SETTING_CONTAINER_TEMPLATE = ufa.template('<p class="k-reset"><span class="k-icon #=icon#"></span>${name}</p>' + '<div class="k-list-container k-reset"/>');
        ...
        function settingTargetFromNode(node) {
            ...
            if (target.length) {
                return target.data('ufaPivotSettingTarget');
            }
            return null;
        }
        var PivotConfigurator = Widget.extend({
            ...
            _dataSource: function () {
                ...
                that.dataSource = ufa.data.PivotDataSource.create(that.options.dataSource);
            ...
            _treeViewDataSource: function () {
                var that = this;
                return ufa.data.HierarchicalDataSource.create({
                    schema: {
                        model: {
                            id: 'uniqueName',
            ...
            _fields: function () {
                ...
                this.treeView = $('<div/>').appendTo(container).ufaTreeView({
                    template: template,
            ...
            _createTarget: function (element, options) {
                var template = '<li class="k-item k-header" data-' + ufa.ns + 'name="${data.name}">${data.name}';
