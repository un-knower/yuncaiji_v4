(function (f, define) {
    ...
    (function ($) {
        ...
        var ufaDomElement = ufa.dom.element;
        var ufaTextElement = ufa.dom.text;
        var ufaHtmlElement = ufa.dom.html;
        ...
        var isRtl = false;
        var keys = ufa.keys;
        var Query = ufa.data.Query;
        var STRING = 'string';
        var NS = '.ufaGanttTimeline';
        var CLICK = 'click';
        ...
        var KEYDOWN = 'keydown';
        var DOT = '.';
        var TIME_HEADER_TEMPLATE = ufa.template('#=ufa.toString(start, \'t\')#');
        var DAY_HEADER_TEMPLATE = ufa.template('#=ufa.toString(start, \'ddd M/dd\')#');
        var WEEK_HEADER_TEMPLATE = ufa.template('#=ufa.toString(start, \'ddd M/dd\')# - #=ufa.toString(ufa.date.addDays(end, -1), \'ddd M/dd\')#');
        var MONTH_HEADER_TEMPLATE = ufa.template('#=ufa.toString(start, \'MMM\')#');
        var YEAR_HEADER_TEMPLATE = ufa.template('#=ufa.toString(start, \'yyyy\')#');
        var RESIZE_HINT = ufa.template('<div class="#=styles.marquee#">' + '<div class="#=styles.marqueeColor#"></div>' + '</div>');
        var RESIZE_TOOLTIP_TEMPLATE = ufa.template('<div style="z-index: 100002;" class="#=styles.tooltipWrapper#">' + '<div class="#=styles.tooltipContent#">' + '<div>#=messages.start#: #=ufa.toString(start, format)#</div>' + '<div>#=messages.end#: #=ufa.toString(end, format)#</div>' + '</div>' + '</div>');
        var PERCENT_RESIZE_TOOLTIP_TEMPLATE = ufa.template('<div style="z-index: 100002;" class="#=styles.tooltipWrapper#" >' + '<div class="#=styles.tooltipContent#">#=text#%</div>' + '<div class="#=styles.tooltipCallout#" style="left:13px;"></div>' + '</div>');
        var TASK_TOOLTIP_TEMPLATE = ufa.template('<div class="#=styles.taskDetails#">' + '<strong>#=task.title#</strong>' + '<div class="#=styles.taskDetailsPercent#">#=ufa.toString(task.percentComplete, "p0")#</div>' + '<ul class="#=styles.reset#">' + '<li>#=messages.start#: #=ufa.toString(task.start, "h:mm tt ddd, MMM d")#</li>' + '<li>#=messages.end#: #=ufa.toString(task.end, "h:mm tt ddd, MMM d")#</li>' + '</ul>' + '</div>');
        var SIZE_CALCULATION_TEMPLATE = '<table style=\'visibility: hidden;\'>' + '<tbody>' + '<tr style=\'height:{0}\'>' + '<td>&nbsp;</td>' + '</tr>' + '</tbody>' + '</table>';
        var defaultViews = {
            day: { type: 'ufa.ui.GanttDayView' },
            week: { type: 'ufa.ui.GanttWeekView' },
            month: { type: 'ufa.ui.GanttMonthView' },
            year: { type: 'ufa.ui.GanttYearView' }
        };
        ...
        var GanttView = ufa.ui.GanttView = Widget.extend({
            init: function (element, options) {
                ...
                this._taskTemplate = options.taskTemplate ? ufa.template(options.taskTemplate, extend({}, ufa.Template, options.templateSettings)) : null;
            ...
            createLayout: function (rows) {
                ...
                var header = ufaDomElement('thead', null, headers);
                var table = ufaDomElement('table', { style: { width: this._tableWidth + 'px' } }, [
                    colgroup,
                    header
                ]);
                tree.render([table]);
            ...
            _rowsTable: function (rowCount) {
                ...
                for (var i = 0; i < rowCount; i++) {
                    row = ufaDomElement('tr', attributes[i % 2], [ufaDomElement('td', null, [ufaTextElement('\xA0')])]);
                    rows.push(row);
                }
            ...
            _columnsTable: function () {
                ...
                for (var i = 0; i < slotsCount; i++) {
                    ...
                    cells.push(ufaDomElement('td', attributes, [ufaTextElement('\xA0')]));
                }
                row = ufaDomElement('tr', null, cells);
            ...
            _tasksTable: function (tasks) {
                ...
                for (var i = 0, l = tasks.length; i < l; i++) {
                    ...
                    row = ufaDomElement('tr', null);
                    cell = ufaDomElement('td', null, [this._renderTask(tasks[i], position)]);
                    if (task[resourcesField] && task[resourcesField].length) {
                        ...
                        cell.children.push(ufaDomElement('div', {
            ...
            _createTable: function (colspan, rows, styles) {
                ...
                for (var i = 0; i < colspan; i++) {
                    cols.push(ufaDomElement('col'));
                }
                colgroup = ufaDomElement('colgroup', null, cols);
                tbody = ufaDomElement('tbody', null, rows);
                if (!styles.style) {
                    styles.style = {};
                }
                styles.style.width = this._tableWidth + 'px';
                return ufaDomElement('table', styles, [
                    colgroup,
                    tbody
                ]);
            },
            ...
            _calculateTaskBorderWidth: function () {
                ...
                computedStyle = ufa.getComputedStyles(task[0], ['border-left-width']);
            ...
            _renderTask: function (task, position) {
                ...
                taskWrapper = ufaDomElement('div', taskWrapAttr, [taskElement]);
                if (editable) {
                    taskWrapper.children.push(ufaDomElement('div', { className: styles.taskDot + ' ' + styles.taskDotStart }));
                    taskWrapper.children.push(ufaDomElement('div', { className: styles.taskDot + ' ' + styles.taskDotEnd }));
                }
                if (!task.summary && !task.isMilestone() && editable && this._taskTemplate === null) {
                    ...
                    taskWrapper.children.push(ufaDomElement('div', {
            ...
            _renderSingleTask: function (task, position) {
                ...
                if (this._taskTemplate !== null) {
                    taskContent = ufaHtmlElement(this._taskTemplate(task));
                } else {
                    taskContent = ufaTextElement(task.title);
                    taskChildren.push(ufaDomElement('div', {
                        className: styles.taskComplete,
                        style: { width: progressWidth + 'px' }
                    }));
                }
                var content = ufaDomElement('div', { className: styles.taskContent }, [ufaDomElement('div', { className: styles.taskTemplate }, [taskContent])]);
                taskChildren.push(content);
                if (this.options.editable) {
                    content.children.push(ufaDomElement('span', { className: styles.taskActions }, [ufaDomElement('a', {
                            className: styles.link + ' ' + styles.taskDelete,
                            href: '#'
                        }, [ufaDomElement('span', { className: styles.icon + ' ' + styles.iconDelete })])]));
                    content.children.push(ufaDomElement('span', { className: styles.taskResizeHandle + ' ' + styles.taskResizeHandleWest }));
                    content.children.push(ufaDomElement('span', { className: styles.taskResizeHandle + ' ' + styles.taskResizeHandleEast }));
                }
                var element = ufaDomElement('div', {
            ...
            _renderResources: function (resources, className) {
                ...
                for (var i = 0, length = resources.length; i < length; i++) {
                    ...
                    children.push(ufaDomElement('span', {
                        className: className,
                        style: { 'color': resource.get('color') }
                    }, [ufaTextElement(resource.get('name'))]));
                }
                if (isRtl) {
                    children.reverse();
                }
            ...
            _line: function (className, styles) {
                return ufaDomElement('div', {
                    className: className,
                    style: styles
                });
            },
            _arrow: function (direction) {
                return ufaDomElement('span', { className: direction ? GanttView.styles.arrowWest : GanttView.styles.arrowEast });
            },
            _colgroup: function () {
                var slots = this._timeSlots();
                ...
                return ufaDomElement('colgroup', null, cols);
            },
            ...
            _updateResizeHint: function (start, end, resizeStart) {
                ...
                var tablesWidth = this._tableWidth - ufa.support.scrollbar();
            ...
            _createTaskTooltip: function (task, element, mouseLeft) {
                ...
                var contentScrollLeft = ufa.scrollLeft(content);
                var row = $(element).parents('tr').first();
                var rowOffset = row.offset();
                var template = options.tooltip && options.tooltip.template ? ufa.template(options.tooltip.template) : TASK_TOOLTIP_TEMPLATE;
                var left = isRtl ? mouseLeft - (contentOffset.left + contentScrollLeft + ufa.support.scrollbar()) : mouseLeft - (contentOffset.left - contentScrollLeft);
            ...
            _scrollTo: function (element) {
                ...
                var scrollbarWidth = ufa.support.scrollbar();
            ...
            _headers: function (columnLevels) {
                ...
                for (var levelIndex = 0, levelCount = columnLevels.length; levelIndex < levelCount; levelIndex++) {
                    ...
                    for (var columnIndex = 0, columnCount = level.length; columnIndex < columnCount; columnIndex++) {
                        ...
                        headerText = ufaTextElement(column.text);
                        headers.push(ufaDomElement('th', {
                            colspan: column.span,
                            className: styles.header + (column.isNonWorking ? ' ' + styles.nonWorking : '')
                        }, [headerText]));
                    }
                    rows.push(ufaDomElement('tr', null, headers));
                }
                return rows;
            },
            ...
            _days: function (start, end) {
                ...
                while (start < end) {
                    slotEnd = ufa.date.nextDay(start);
            ...
            _weeks: function (start, end) {
                ...
                while (start < end) {
                    slotEnd = ufa.date.dayOfWeek(ufa.date.addDays(start, 1), firstDay, 1);
                    if (slotEnd > end) {
                        slotEnd = end;
                    }
            ...
            calendarInfo: function () {
                return ufa.getCulture().calendars.standard;
            },
        ...
        ufa.ui.GanttDayView = GanttView.extend({
            name: 'day',
            options: {
            ...
            range: function (range) {
                this.start = ufa.date.getDate(range.start);
                this.end = ufa.date.getDate(range.end);
                if (ufa.date.getMilliseconds(range.end) > 0 || this.end.getTime() === this.start.getTime()) {
                    this.end = ufa.date.addDays(this.end, 1);
                }
            },
            ...
            _layout: function () {
                var rows = [];
                var options = this.options;
                rows.push(this._slotHeaders(this._slots[0], ufa.template(options.dayHeaderTemplate)));
                rows.push(this._slotHeaders(this._slots[1], ufa.template(options.timeHeaderTemplate)));
                return rows;
            }
        });
        ufa.ui.GanttWeekView = GanttView.extend({
            name: 'week',
            options: {
                dayHeaderTemplate: DAY_HEADER_TEMPLATE,
            ...
            range: function (range) {
                ...
                this.start = ufa.date.getDate(ufa.date.dayOfWeek(range.start, firstDay, -1));
                this.end = ufa.date.getDate(ufa.date.dayOfWeek(rangeEnd, firstDay, 1));
            },
            _createSlots: function () {
                var slots = [];
            ...
            _layout: function () {
                var rows = [];
                var options = this.options;
                rows.push(this._slotHeaders(this._slots[0], ufa.template(options.weekHeaderTemplate)));
                rows.push(this._slotHeaders(this._slots[1], ufa.template(options.dayHeaderTemplate)));
                return rows;
            }
        });
        ufa.ui.GanttMonthView = GanttView.extend({
            name: 'month',
            options: {
                weekHeaderTemplate: WEEK_HEADER_TEMPLATE,
            ...
            range: function (range) {
                this.start = ufa.date.firstDayOfMonth(range.start);
                this.end = ufa.date.addDays(ufa.date.getDate(ufa.date.lastDayOfMonth(range.end)), 1);
            },
            _createSlots: function () {
                var slots = [];
            ...
            _layout: function () {
                var rows = [];
                var options = this.options;
                rows.push(this._slotHeaders(this._slots[0], ufa.template(options.monthHeaderTemplate)));
                rows.push(this._slotHeaders(this._slots[1], ufa.template(options.weekHeaderTemplate)));
                return rows;
            }
        });
        ufa.ui.GanttYearView = GanttView.extend({
            name: 'year',
            options: {
            ...
            range: function (range) {
                this.start = ufa.date.firstDayOfMonth(new Date(range.start.setMonth(0)));
                this.end = ufa.date.firstDayOfMonth(new Date(range.end.setMonth(12)));
            },
            _createSlots: function () {
                var slots = [];
            ...
            _layout: function () {
                var rows = [];
                var options = this.options;
                rows.push(this._slotHeaders(this._slots[0], ufa.template(options.yearHeaderTemplate)));
                rows.push(this._slotHeaders(this._slots[1], ufa.template(options.monthHeaderTemplate)));
                return rows;
            }
        });
        var timelineStyles = {
        ...
        var GanttTimeline = ufa.ui.GanttTimeline = Widget.extend({
            init: function (element, options) {
                Widget.fn.init.call(this, element, options);
            ...
            _wrapper: function () {
                ...
                var calculateSize = function () {
                    ...
                    var table = $(ufa.format(SIZE_CALCULATION_TEMPLATE, rowHeight));
            ...
            _draggable: function () {
                ...
                this._moveDraggable = new ufa.ui.Draggable(this.wrapper, {
                    distance: 0,
                    filter: DOT + styles.task,
                    holdToDrag: ufa.support.mobileOS,
                ...
                }).bind('drag', ufa.throttle(function (e) {
            ...
            _resizable: function () {
                ...
                this._resizeDraggable = new ufa.ui.Draggable(this.wrapper, {
                    distance: 0,
            ...
            _percentResizeDraggable: function () {
                ...
                this._percentDraggable = new ufa.ui.Draggable(this.wrapper, {
                    distance: 0,
            ...
            _createDependencyDraggable: function () {
                ...
                this._dependencyDraggable = new ufa.ui.Draggable(this.wrapper, {
                    distance: 0,
                ...
                this._dependencyDraggable.bind('dragstart', function (e) {
                    ...
                    that.view()._removeDependencyDragHint();
                    var target = $(ufa.elementUnderCursor(e));
                    var tablesOffset = that.wrapper.find(DOT + styles.tasksWrapper).offset();
            ...
            _attachEvents: function () {
                ...
                if (this.options.editable) {
                    ...
                    if (!ufa.support.mobileOS) {
                        ...
                        this.touch = this.wrapper.ufaTouch({
                            filter: DOT + styles.task,
