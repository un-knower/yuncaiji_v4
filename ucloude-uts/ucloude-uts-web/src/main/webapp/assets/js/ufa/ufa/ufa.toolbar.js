(function (f, define) {
    ...
    (function ($, undefined) {
        var ufa = window.ufa, Class = ufa.Class, Widget = ufa.ui.Widget, proxy = $.proxy, isFunction = ufa.isFunction, keys = ufa.keys, TOOLBAR = 'k-toolbar', BUTTON = 'k-button', OVERFLOW_BUTTON = 'k-overflow-button', TOGGLE_BUTTON = 'k-toggle-button', BUTTON_GROUP = 'k-button-group', SPLIT_BUTTON = 'k-split-button', SEPARATOR = 'k-separator', POPUP = 'k-popup', RESIZABLE_TOOLBAR = 'k-toolbar-resizable', STATE_ACTIVE = 'k-state-active', STATE_DISABLED = 'k-state-disabled', STATE_HIDDEN = 'k-state-hidden', GROUP_START = 'k-group-start', GROUP_END = 'k-group-end', PRIMARY = 'k-primary', ICON = 'k-icon', ICON_PREFIX = 'k-i-', BUTTON_ICON = 'k-button-icon', BUTTON_ICON_TEXT = 'k-button-icontext', LIST_CONTAINER = 'k-list-container k-split-container', SPLIT_BUTTON_ARROW = 'k-split-button-arrow', OVERFLOW_ANCHOR = 'k-overflow-anchor', OVERFLOW_CONTAINER = 'k-overflow-container', FIRST_TOOLBAR_VISIBLE = 'k-toolbar-first-visible', LAST_TOOLBAR_VISIBLE = 'k-toolbar-last-visible', CLICK = 'click', TOGGLE = 'toggle', OPEN = 'open', CLOSE = 'close', OVERFLOW_OPEN = 'overflowOpen', OVERFLOW_CLOSE = 'overflowClose', OVERFLOW_NEVER = 'never', OVERFLOW_AUTO = 'auto', OVERFLOW_ALWAYS = 'always', OVERFLOW_HIDDEN = 'k-overflow-hidden', ufa_UID_ATTR = ufa.attr('uid');
        ufa.toolbar = {};
        var components = {
            overflowAnchor: '<div tabindex="0" class="k-overflow-anchor"></div>',
            overflowContainer: '<ul class="k-overflow-container k-list-container"></ul>'
        };
        ufa.toolbar.registerComponent = function (name, toolbar, overflow) {
            components[name] = {
                toolbar: toolbar,
                overflow: overflow
            };
        };
        var Item = ufa.Class.extend({
            addOverflowAttr: function () {
                this.element.attr(ufa.attr('overflow'), this.options.overflow || OVERFLOW_AUTO);
            },
            addUidAttr: function () {
                this.element.attr(ufa_UID_ATTR, this.options.uid);
            },
            addIdAttr: function () {
                if (this.options.id) {
            ...
            twin: function () {
                var uid = this.element.attr(ufa_UID_ATTR);
                if (this.overflow) {
                    return this.toolbar.element.find('[' + ufa_UID_ATTR + '=\'' + uid + '\']').data(this.options.type);
                } else if (this.toolbar.options.resizable) {
                    return this.toolbar.popup.element.find('[' + ufa_UID_ATTR + '=\'' + uid + '\']').data(this.options.type);
                }
            }
        });
        ufa.toolbar.Item = Item;
        var Button = Item.extend({
            init: function (options, toolbar) {
                ...
                if (options.url !== undefined && !options.useButtonTag) {
                    ...
                    if (options.mobile) {
                        element.attr(ufa.attr('role'), 'button');
                    }
                }
                if (options.group) {
                    element.attr(ufa.attr('group'), options.group);
        ...
        ufa.toolbar.ToolBarButton = ToolBarButton;
        ...
        ufa.toolbar.OverflowButton = OverflowButton;
        ufa.toolbar.registerComponent('button', ToolBarButton, OverflowButton);
        ...
        ufa.toolbar.ButtonGroup = ButtonGroup;
        ...
        ufa.toolbar.ToolBarButtonGroup = ToolBarButtonGroup;
        ...
        ufa.toolbar.OverflowButtonGroup = OverflowButtonGroup;
        ufa.toolbar.registerComponent('buttonGroup', ToolBarButtonGroup, OverflowButtonGroup);
        var ToolBarSplitButton = Item.extend({
            ...
            createPopup: function () {
                var options = this.options;
                var element = this.element;
                this.popupElement.attr('id', options.id + '_optionlist').attr(ufa_UID_ATTR, options.rootUid);
                if (options.mobile) {
                    this.popupElement = actionSheetWrap(this.popupElement);
                }
                this.popup = this.popupElement.ufaPopup({
                    ...
                    activate: function () {
                        this.element.find(':ufaFocusable').first().focus();
                    },
                    close: function () {
                        element.focus();
                    }
        ...
        ufa.toolbar.ToolBarSplitButton = ToolBarSplitButton;
        ...
        ufa.toolbar.OverflowSplitButton = OverflowSplitButton;
        ufa.toolbar.registerComponent('splitButton', ToolBarSplitButton, OverflowSplitButton);
        ...
        ufa.toolbar.registerComponent('separator', ToolBarSeparator, OverflowSeparator);
        ...
        ufa.toolbar.TemplateItem = TemplateItem;
        ...
        ufa.toolbar.OverflowTemplateItem = OverflowTemplateItem;
        function adjustPopupWidth() {
            var anchor = this.options.anchor, computedWidth = anchor.outerWidth(), width;
            ufa.wrap(this.element).addClass('k-split-wrapper');
        ...
        function findFocusableSibling(element, dir) {
            ...
            if (candidate.is(':ufaFocusable') || !candidate.length) {
                return candidate;
            }
            if (candidate.find(':ufaFocusable').length) {
                return getter.call(candidate.find(':ufaFocusable'));
            }
        ...
        var ToolBar = Widget.extend({
            init: function (element, options) {
                ...
                this.uid = ufa.guid();
                this._isRtl = ufa.support.isRtl(element);
                this._groups = {};
                element.attr(ufa_UID_ATTR, this.uid);
                ...
                if (options.resizable) {
                    ...
                    that.overflowUserEvents = new ufa.UserEvents(that.element, {
                        threshold: 5,
                        allowSelection: true,
                ...
                that.userEvents = new ufa.UserEvents(document, {
                    threshold: 5,
                    allowSelection: true,
                    filter: '[' + ufa_UID_ATTR + '=' + this.uid + '] a.' + BUTTON + ', ' + '[' + ufa_UID_ATTR + '=' + this.uid + '] .' + OVERFLOW_BUTTON,
            ...
            add: function (options) {
                ...
                if (options.menuButtons) {
                    for (var i = 0; i < options.menuButtons.length; i++) {
                        $.extend(options.menuButtons[i], { uid: ufa.guid() });
                    }
                }
            ...
            _renderOverflow: function () {
                ...
                that.popup = new ufa.ui.Popup(overflowContainer, {
                    ...
                    copyAnchorStyles: false,
                    open: function (e) {
                        var wrapper = ufa.wrap(that.popup.element).addClass('k-overflow-wrapper');
                        if (!that.isMobile) {
                ...
                that.popup.container.attr(ufa_UID_ATTR, this.uid);
            },
            ...
            _buttonClick: function (e) {
                ...
                if (splitContainer[0]) {
                    popup = splitContainer.data('ufaPopup');
                    (popup ? popup : splitContainer.parents('.km-popup-wrapper').data('ufaPopup')).close();
                }
            },
            _navigatable: function () {
                var that = this;
                that.element.attr('tabindex', 0).focus(function () {
                    var element = $(this).find(':ufaFocusable:first');
            ...
            _keydown: function (e) {
                ...
                if (keyCode === keys.TAB) {
                    ...
                    if (element.is('.' + OVERFLOW_ANCHOR)) {
                        ...
                        if (items.last().is(':ufaFocusable')) {
                            items.last().focus();
                        } else {
                            items.last().find(':ufaFocusable').last().focus();
                        }
                    }
                    ...
                    if (firstHasFocus) {
                        e.preventDefault();
                        this.wrapper.prev(':ufaFocusable').focus();
                    }
                }
