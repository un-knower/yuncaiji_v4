...
(function (f, define) {
    define('util/undoredostack', ['ufa.core'], f);
}(function () {
    (function (ufa) {
        var UndoRedoStack = ufa.Observable.extend({
            init: function (options) {
                ufa.Observable.fn.init.call(this, options);
                this.clear();
            },
            events: [
                'undo',
        ...
        ufa.deepExtend(ufa, { util: { UndoRedoStack: UndoRedoStack } });
    }(ufa));
}, typeof define == 'function' && define.amd ? define : function (a1, a2, a3) {
    (a3 || a2)();
}));
(function (f, define) {
    define('util/main', ['ufa.core'], f);
}(function () {
    (function () {
        var math = Math, ufa = window.ufa, deepExtend = ufa.deepExtend;
        ...
        function renderPos(pos) {
            var result = [];
            if (pos) {
                var parts = ufa.toHyphens(pos).split('-');
        ...
        ufa.drawing.util = ufa.util;
        ufa.dataviz.util = ufa.util;
    }());
    return window.ufa;
}, typeof define == 'function' && define.amd ? define : function (a1, a2, a3) {
    (a3 || a2)();
...
(function (f, define) {
    ...
    (function ($) {
        var doc = document, ufa = window.ufa, Class = ufa.Class, util = ufa.util, defined = util.defined;
        ...
        ufa.util.TextMetrics = TextMetrics;
        ufa.util.LRUCache = LRUCache;
        ufa.util.measureText = measureText;
    }(window.ufa.jQuery));
}, typeof define == 'function' && define.amd ? define : function (a1, a2, a3) {
    (a3 || a2)();
}));
(function (f, define) {
    ...
    ufa.util.parseXML = function parseXML() {
        try {
...
(function (f, define) {
    ...
    (function () {
        var ufa = window.ufa, deepExtend = ufa.deepExtend, fromCharCode = String.fromCharCode;
...
(function (f, define) {
    define('mixins/observers', ['ufa.core'], f);
}(function () {
    (function ($) {
        var math = Math, ufa = window.ufa, deepExtend = ufa.deepExtend, inArray = $.inArray;
        var ObserversMixin = {
            observers: function () {
        ...
        deepExtend(ufa, { mixins: { ObserversMixin: ObserversMixin } });
    }(window.ufa.jQuery));
    return window.ufa;
}, typeof define == 'function' && define.amd ? define : function (a1, a2, a3) {
    (a3 || a2)();
...
(function (f, define) {
    ...
    (function (ufa) {
        if (ufa.support.browser.msie && ufa.support.browser.version < 9) {
            return;
        }
        var $ = ufa.jQuery;
        var Command = ufa.spreadsheet.Command = ufa.Class.extend({
            init: function (options) {
        ...
        var PropertyChangeCommand = ufa.spreadsheet.PropertyChangeCommand = Command.extend({
            init: function (options) {
        ...
        ufa.spreadsheet.ClearContentCommand = Command.extend({
            exec: function () {
                this.getState();
                this.range().clearContent();
            }
        });
        ufa.spreadsheet.EditCommand = PropertyChangeCommand.extend({
            init: function (options) {
            ...
            exec: function () {
                ...
                try {
                    ...
                    if (ex1 instanceof ufa.spreadsheet.calc.ParseError) {
                        try {
                            range.input(value + ')');
                            var validationState = range._getValidationState();
                            if (validationState) {
                            ...
                            if (ex2 instanceof ufa.spreadsheet.calc.ParseError) {
                                range.input('\'' + value);
                                return {
        ...
        ufa.spreadsheet.TextWrapCommand = PropertyChangeCommand.extend({
            init: function (options) {
        ...
        ufa.spreadsheet.AdjustDecimalsCommand = Command.extend({
            init: function (options) {
            ...
            exec: function () {
                ...
                var formatting = ufa.spreadsheet.formatting;
                this.getState();
        ...
        ufa.spreadsheet.BorderChangeCommand = Command.extend({
            init: function (options) {
        ...
        ufa.spreadsheet.MergeCellCommand = Command.extend({
            init: function (options) {
                Command.fn.init.call(this, options);
        ...
        ufa.spreadsheet.FreezePanesCommand = Command.extend({
            init: function (options) {
                Command.fn.init.call(this, options);
                this._type = options.value;
            },
        ...
        ufa.spreadsheet.PasteCommand = Command.extend({
            init: function (options) {
                Command.fn.init.call(this, options);
        ...
        ufa.spreadsheet.AdjustRowHeightCommand = Command.extend({
            exec: function () {
        ...
        ufa.spreadsheet.ToolbarPasteCommand = Command.extend({
            exec: function () {
                if (ufa.support.clipboard.paste) {
        ...
        ufa.spreadsheet.CopyCommand = Command.extend({
            init: function (options) {
                Command.fn.init.call(this, options);
        ...
        ufa.spreadsheet.ToolbarCopyCommand = Command.extend({
            init: function (options) {
                Command.fn.init.call(this, options);
                this._clipboard = options.workbook.clipboard();
            },
        ...
        ufa.spreadsheet.CutCommand = Command.extend({
            init: function (options) {
                Command.fn.init.call(this, options);
                this._clipboard = options.workbook.clipboard();
            },
        ...
        ufa.spreadsheet.AutoFillCommand = Command.extend({
            init: function (options) {
                Command.fn.init.call(this, options);
            },
        ...
        ufa.spreadsheet.ToolbarCutCommand = Command.extend({
            init: function (options) {
                Command.fn.init.call(this, options);
                this._clipboard = options.workbook.clipboard();
            },
        ...
        ufa.spreadsheet.FilterCommand = Command.extend({
            undo: function () {
        ...
        ufa.spreadsheet.SortCommand = Command.extend({
            undo: function () {
                var sheet = this.range().sheet();
            ...
            expandRange: function () {
                var sheet = this.range().sheet();
                return new ufa.spreadsheet.Range(sheet._sheetRef, sheet);
            }
        });
        var ApplyFilterCommand = ufa.spreadsheet.ApplyFilterCommand = Command.extend({
            column: function () {
            ...
            exec: function () {
                ...
                if (this.options.valueFilter) {
                    filterRule = {
                        column: column,
                        filter: new ufa.spreadsheet.ValueFilter(this.options.valueFilter)
                    };
                } else if (this.options.customFilter) {
                    filterRule = {
                        column: column,
                        filter: new ufa.spreadsheet.CustomFilter(this.options.customFilter)
                    };
                }
                this.getState();
        ...
        ufa.spreadsheet.ClearFilterCommand = ApplyFilterCommand.extend({
            exec: function () {
                var range = this.range();
                var column = this.column();
                this.getState();
        ...
        ufa.spreadsheet.HideLineCommand = Command.extend({
            init: function (options) {
                Command.fn.init.call(this, options);
        ...
        ufa.spreadsheet.UnHideLineCommand = ufa.spreadsheet.HideLineCommand.extend({
            exec: function () {
                var sheet = this.range().sheet();
                this._state = sheet.getAxisState();
                if (this.axis == 'row') {
        ...
        var DeleteCommand = ufa.spreadsheet.DeleteCommand = Command.extend({
            undo: function () {
                var sheet = this.range().sheet();
                sheet.setState(this._state);
            }
        });
        ufa.spreadsheet.DeleteRowCommand = DeleteCommand.extend({
            exec: function () {
                var sheet = this.range().sheet();
                this._state = sheet.getState();
        ...
        ufa.spreadsheet.DeleteColumnCommand = DeleteCommand.extend({
            exec: function () {
                var sheet = this.range().sheet();
                this._state = sheet.getState();
        ...
        ufa.spreadsheet.AddColumnCommand = AddCommand.extend({
            exec: function () {
                var sheet = this.range().sheet();
                this._state = sheet.getState();
                if (this._value === 'left') {
        ...
        ufa.spreadsheet.AddRowCommand = AddCommand.extend({
            exec: function () {
                var sheet = this.range().sheet();
        ...
        ufa.spreadsheet.EditValidationCommand = Command.extend({
            init: function (options) {
                Command.fn.init.call(this, options);
                this._value = options.value;
            },
        ...
        ufa.spreadsheet.OpenCommand = Command.extend({
            exec: function () {
        ...
        ufa.spreadsheet.SaveAsCommand = Command.extend({
            exec: function () {
...
(function (f, define) {
    define('spreadsheet/formulabar', ['ufa.core'], f);
}(function () {
    (function (ufa) {
        if (ufa.support.browser.msie && ufa.support.browser.version < 9) {
            return;
        ...
        var FormulaBar = ufa.ui.Widget.extend({
            init: function (element, options) {
                ufa.ui.Widget.call(this, element, options);
                element = this.element.addClass(FormulaBar.classNames.wrapper);
                this.formulaInput = new ufa.spreadsheet.FormulaInput($('<div/>').appendTo(element));
            },
            destroy: function () {
        ...
        ufa.spreadsheet.FormulaBar = FormulaBar;
...
(function (f, define) {
    define('spreadsheet/formulainput', ['ufa.core'], f);
}(function () {
    (function (ufa, window) {
        if (ufa.support.browser.msie && ufa.support.browser.version < 9) {
            return;
        ...
        var $ = ufa.jQuery;
        var Widget = ufa.ui.Widget;
        var ns = '.ufaFormulaInput';
        var keys = ufa.keys;
        var classNames = {
        ...
        var FormulaInput = Widget.extend({
            ...
            _formulaSource: function () {
                var result = [];
                var value;
                for (var key in ufa.spreadsheet.calc.runtime.FUNCS) {
                ...
                this.formulaSource = new ufa.data.DataSource({ data: result });
            },
            _formulaList: function () {
                this.list = new ufa.ui.StaticList($('<ul />').addClass(FormulaInput.classNames.listWrapper).insertAfter(this.element), {
                    autoBind: false,
                    selectable: true,
            ...
            _popup: function () {
                this.popup = new ufa.ui.Popup(this.list.element, { anchor: this.element });
            },
            _blur: function () {
                this.popup.close();
            ...
            _tokenContext: function () {
                ...
                var tokens = ufa.spreadsheet.calc.tokenize(value, this.row(), this.col());
                var tok;
            ...
            _textContainer: function () {
                var computedStyles = ufa.getComputedStyles(this.element[0], styles);
            ...
            isActive: function () {
                return this.element[0] === ufa._activeElement();
            },
            ...
            _canInsertRef: function (isKeyboardAction) {
                ...
                if (point && this._isFormula()) {
                    ...
                    tokens = ufa.spreadsheet.calc.tokenize(this.value(), this.row(), this.col());
                    for (var i = 0; i < tokens.length; ++i) {
                        tok = tokens[i];
            ...
            _syntaxHighlight: function () {
                var pos = this.getPos();
                var value = this.value();
                var refClasses = ufa.spreadsheet.Pane.classNames.series;
                var highlightedRefs = [];
                var refIndex = 0;
                var parens = [];
                var tokens = [];
                ...
                if (!/^=/.test(value)) {
                    ...
                    tokens = ufa.spreadsheet.calc.tokenize(value, this.row(), this.col());
                    ...
                    tokens.reverse().forEach(function (tok) {
                        ...
                        var text = ufa.htmlEncode(value.substring(begin, end));
        ...
        function knownFunction(name) {
            return ufa.spreadsheet.calc.runtime.FUNCS[name.toLowerCase()];
        }
        ...
        ufa.spreadsheet.FormulaInput = FormulaInput;
...
(function (f, define) {
    define('spreadsheet/eventlistener', ['ufa.core'], f);
}(function () {
    (function (ufa) {
        if (ufa.support.browser.msie && ufa.support.browser.version < 9) {
            return;
        ...
        var EventListener = ufa.Class.extend({
        ...
        ufa.spreadsheet.EventListener = EventListener;
    }(window.ufa));
}, typeof define == 'function' && define.amd ? define : function (a1, a2, a3) {
    (a3 || a2)();
}));
(function (f, define) {
    define('spreadsheet/rangelist', ['ufa.core'], f);
}(function () {
    (function (ufa) {
        if (ufa.support.browser.msie && ufa.support.browser.version < 9) {
            return;
        }
        var RangeTreeNode = ufa.Class.extend({
        ...
        var RangeTree = ufa.Class.extend({
            init: function () {
                this.root = NilNode;
            },
        ...
        var RangeList = ufa.Class.extend({
        ...
        ufa.spreadsheet.RangeTree = RangeTree;
        ufa.spreadsheet.RangeList = RangeList;
        ufa.spreadsheet.SparseRangeList = SparseRangeList;
        ufa.spreadsheet.ValueRange = Range;
    }(ufa));
}, typeof define == 'function' && define.amd ? define : function (a1, a2, a3) {
    (a3 || a2)();
}));
(function (f, define) {
    define('spreadsheet/propertybag', ['ufa.core'], f);
}(function () {
    (function (ufa) {
        if (ufa.support.browser.msie && ufa.support.browser.version < 9) {
            return;
        ...
        var ValueProperty = Property.extend({
            ...
            set: function (start, end, value) {
                if (value instanceof Date) {
                    value = ufa.spreadsheet.dateToNumber(value);
                    this.formats.value(start, end, toExcelFormat(ufa.culture().calendar.patterns.d));
                }
                this.list.value(start, end, value);
            }
        });
        ...
        ufa.spreadsheet.PropertyBag = ufa.Class.extend({
            specs: [
                {
            ...
            init: function (cellCount) {
                ...
                this.specs.forEach(function (spec) {
                    this.lists[spec.name] = new ufa.spreadsheet.SparseRangeList(0, cellCount, spec.value);
                }, this);
                this.specs.forEach(function (spec) {
        ...
        ufa.spreadsheet.ALL_PROPERTIES = $.map(ufa.spreadsheet.PropertyBag.prototype.specs, function (spec) {
            return spec.name;
        });
    }(window.ufa));
}, typeof define == 'function' && define.amd ? define : function (a1, a2, a3) {
...
(function (f, define) {
    define('spreadsheet/references', ['ufa.core'], f);
}(function () {
    'use strict';
    if (ufa.support.browser.msie && ufa.support.browser.version < 9) {
        return;
    }
    var spreadsheet = ufa.spreadsheet;
    var Class = ufa.Class;
...
(function (f, define) {
    define('spreadsheet/autofillcalculator', ['ufa.core'], f);
}(function () {
    (function (ufa) {
        if (ufa.support.browser.msie && ufa.support.browser.version < 9) {
            return;
        }
        var RangeRef = ufa.spreadsheet.RangeRef;
        var CellRef = ufa.spreadsheet.CellRef;
        var AutoFillCalculator = ufa.Class.extend({
            init: function (grid) {
                this._grid = grid;
            },
        ...
        ufa.spreadsheet.AutoFillCalculator = AutoFillCalculator;
    }(ufa));
}, typeof define == 'function' && define.amd ? define : function (a1, a2, a3) {
    (a3 || a2)();
}));
(function (f, define) {
    ...
    (function (ufa) {
        ...
        var RangeRef = ufa.spreadsheet.RangeRef;
        var CellRef = ufa.spreadsheet.CellRef;
        var EdgeNavigator = ufa.Class.extend({
        ...
        var SheetNavigator = ufa.Class.extend({
            init: function (sheet) {
                this._sheet = sheet;
                this.columns = this._sheet._grid._columns;
                this.autoFillCalculator = new ufa.spreadsheet.AutoFillCalculator(sheet._grid);
        ...
        ufa.spreadsheet.SheetNavigator = SheetNavigator;
    }(ufa));
}, typeof define == 'function' && define.amd ? define : function (a1, a2, a3) {
    (a3 || a2)();
}));
(function (f, define) {
    define('spreadsheet/axismanager', ['ufa.core'], f);
}(function () {
    (function (ufa) {
        if (ufa.support.browser.msie && ufa.support.browser.version < 9) {
            return;
        }
        var AxisManager = ufa.Class.extend({
            init: function (sheet) {
                this._sheet = sheet;
            },
        ...
        ufa.spreadsheet.AxisManager = AxisManager;
    }(ufa));
}, typeof define == 'function' && define.amd ? define : function (a1, a2, a3) {
    (a3 || a2)();
}));
(function (f, define) {
    define('spreadsheet/clipboard', ['ufa.core'], f);
}(function () {
    (function (ufa) {
        if (ufa.support.browser.msie && ufa.support.browser.version < 9) {
            return;
        ...
        var RangeRef = ufa.spreadsheet.RangeRef;
        var CellRef = ufa.spreadsheet.CellRef;
        var Clipboard = ufa.Class.extend({
            init: function (workbook) {
                this.workbook = workbook;
                this.origin = ufa.spreadsheet.NULLREF;
            ...
            canCopy: function () {
                ...
                if (selection === ufa.spreadsheet.NULLREF) {
                    status.canCopy = false;
                }
                if (selection instanceof ufa.spreadsheet.UnionRef) {
                    status.canCopy = false;
            ...
            canPaste: function () {
                ...
                if (ref === ufa.spreadsheet.NULLREF) {
            ...
            _isInternal: function () {
                ...
                var internalHTML = $('<div/>').html(this._external.html).find('table.ufa-clipboard-' + this._uid).length ? true : false;
                var internalPlain = $('<div/>').html(this._external.plain).find('table.ufa-clipboard-' + this._uid).length ? true : false;
        ...
        ufa.spreadsheet.Clipboard = Clipboard;
    }(ufa));
}, typeof define == 'function' && define.amd ? define : function (a1, a2, a3) {
    (a3 || a2)();
}));
(function (f, define) {
    ...
    (function (ufa) {
        ...
        var $ = ufa.jQuery;
        var UnionRef = ufa.spreadsheet.UnionRef;
        var styles = [
            'color',
            'fontFamily',
            'underline',
        ...
        var Range = ufa.Class.extend({
            ...
            _collapsedBorder: function (property) {
                ...
                var direction = borders[property].direction;
                this._resizedRef(direction).forEach(function (ref) {
                    if (!result && ref !== ufa.spreadsheet.NULLREF) {
            ...
            input: function (value) {
                if (value !== undefined) {
                    ...
                    var x = ufa.spreadsheet.calc.parse(this._sheet.name(), tl.row, tl.col, value);
                    this._sheet.batch(function () {
                        var formula = null;
                        if (x.type == 'exp') {
                            formula = ufa.spreadsheet.calc.compile(x);
                        } else if (x.type == 'date') {
                            this.format(toExcelFormat(ufa.culture().calendar.patterns.d));
                    ...
                    var type = format && !formula && ufa.spreadsheet.formatting.type(value, format);
                    if (formula) {
                        value = '=' + formula;
                    } else if (type === 'date') {
                        value = ufa.toString(ufa.spreadsheet.numberToDate(value), ufa.culture().calendar.patterns.d);
            ...
            values: function (values) {
                ...
                if (this._ref === ufa.spreadsheet.NULLREF) {
                    if (values !== undefined) {
            ...
            isSortable: function () {
                return !(this._ref instanceof UnionRef || this._ref === ufa.spreadsheet.NULLREF);
            },
            sort: function (spec) {
                if (this._ref instanceof UnionRef) {
                    throw new Error('Unsupported for multiple ranges.');
            ...
            getState: function (propertyName) {
                ...
                var properties;
                if (!propertyName) {
                    properties = ufa.spreadsheet.ALL_PROPERTIES;
            ...
            _adjustRowHeight: function () {
                ...
                this.forEachRow(function (row) {
                    ...
                    row.forEachCell(function (rowIndex, colIndex, cell) {
                        ...
                        maxHeight = Math.max(maxHeight, ufa.spreadsheet.util.getTextHeight(cell.value, width, cell.fontSize, cell.wrap));
                    });
                ...
                this.forEachRow(function (range) {
                    ...
                    range.forEachCell(function (row, col, cell) {
                        ...
                        if (cell.value !== null && cell.value !== undefined) {
                            maxHeight = Math.max(maxHeight, ufa.spreadsheet.util.getTextHeight(cell.value, width, cell.fontSize, true));
                        }
                    });
            ...
            fontSize: function (size) {
                ...
                this.forEachRow(function (range) {
                    ...
                    range.forEachCell(function (row, col, cell) {
                        var width = this._sheet.columnWidth(col);
                        if (cell.value !== null && cell.value !== undefined) {
                            maxHeight = Math.max(maxHeight, ufa.spreadsheet.util.getTextHeight(cell.value, width, size, cell.wrap));
                        }
                    });
                    range.sheet().rowHeight(range.topLeft().row, maxHeight);
                }.bind(this));
        ...
        function looksLikeANumber(str) {
            return !/^=/.test(str) && /number|percent/.test(ufa.spreadsheet.calc.parse(null, 0, 0, str).type);
        }
        ...
        function getTextHeight(text, width, fontSize, wrap) {
            ...
            return ufa.util.measureText(text, styles, measureBox).height;
        }
        ufa.spreadsheet.util = { getTextHeight: getTextHeight };
        ufa.spreadsheet.Range = Range;
    }(window.ufa));
}, typeof define == 'function' && define.amd ? define : function (a1, a2, a3) {
    (a3 || a2)();
}));
(function (f, define) {
    ...
    function compileArgumentChecks(functionName, args) {
        ...
        function cond(type) {
            ...
            if (type == 'ref') {
                return '($' + name + ' instanceof ufa.spreadsheet.Ref)';
            }
            if (type == 'area') {
                return '($' + name + ' instanceof ufa.spreadsheet.CellRef || $' + name + ' instanceof ufa.spreadsheet.RangeRef)';
            }
            if (type == 'cell') {
                return '($' + name + ' instanceof ufa.spreadsheet.CellRef)';
            }
            if (type == 'null') {
                return '(' + force() + ' == null)';
            }
    ...
    exports.defineAlias = function (alias, name) {
        ...
        if (!orig.ufaSpreadsheetAliases) {
            orig.ufaSpreadsheetAliases = [name];
        }
        orig.ufaSpreadsheetAliases.push(alias);
        FUNCS[alias] = orig;
    };
    exports.FUNCS = FUNCS;
    var ARGS_NUMERIC = [
...
(function (f, define) {
    ...
    function compileValidation(sheet, row, col, validation) {
        ...
        if (validation.from) {
            if (validation.dataType === 'list') {
                validation.from = ufa.format(TRANSPOSE_FORMAT, validation.from);
            }
            if (validation.dataType === 'date') {
                parsedFromDate = calc.runtime.parseDate(validation.from);
                if (parsedFromDate) {
                    validation.from = ufa.format(DATE_FORMAT, validation.from);
        ...
        if (validation.to) {
            if (validation.dataType === 'date') {
                ...
                if (parsedToDate) {
                    validation.to = ufa.format(DATE_FORMAT, validation.to);
        ...
        if (!comparer) {
            throw ufa.format('\'{0}\' comparer is not implemented.', validation.comparerType);
        }
        ...
        return new ufa.spreadsheet.validation.Validation($.extend(validation, {
    ...
    var Validation = Class.extend({
        ...
        _formatMessages: function (format) {
            ...
            return ufa.format(format, from, to, fromFormula, toFormula, dataType, type, comparerType);
        },
...
(function (f, define) {
    ...
    (function (ufa) {
        ...
        var RangeRef = ufa.spreadsheet.RangeRef;
        var CellRef = ufa.spreadsheet.CellRef;
        var Range = ufa.spreadsheet.Range;
        var Color = ufa.Color;
        var Selection = ufa.Class.extend({
            init: function (sheet) {
                this._sheet = sheet;
                this.selection = ufa.spreadsheet.FIRSTREF.toRangeRef();
                this.originalSelection = ufa.spreadsheet.FIRSTREF.toRangeRef();
                this._activeCell = ufa.spreadsheet.FIRSTREF.toRangeRef();
                this.originalActiveCell = ufa.spreadsheet.FIRSTREF;
            },
        ...
        var Sheet = ufa.Observable.extend({
            init: function (rowCount, columnCount, rowHeight, columnWidth, headerHeight, headerWidth) {
                ufa.Observable.prototype.init.call(this);
                var cellCount = rowCount * columnCount - 1;
                this._rows = new ufa.spreadsheet.Axis(rowCount, rowHeight);
                this._columns = new ufa.spreadsheet.Axis(columnCount, columnWidth);
                this._mergedCells = [];
                this._frozenRows = 0;
                this._frozenColumns = 0;
                this._suspendChanges = false;
                this._filter = null;
                this._grid = new ufa.spreadsheet.Grid(this._rows, this._columns, rowCount, columnCount, headerHeight, headerWidth);
                this._sheetRef = this._grid.normalize(ufa.spreadsheet.SHEETREF);
                this._properties = new ufa.spreadsheet.PropertyBag(cellCount);
                this._sorter = new ufa.spreadsheet.Sorter(this._grid, this._properties.sortable());
            ...
            navigator: function () {
                if (!this._navigator) {
                    this._navigator = new ufa.spreadsheet.SheetNavigator(this);
                }
                return this._navigator;
            },
            axisManager: function () {
                if (!this._axisManager) {
                    this._axisManager = new ufa.spreadsheet.AxisManager(this);
                }
            ...
            setDataSource: function (dataSource, columns) {
                ...
                this.dataSourceBinder = new ufa.spreadsheet.SheetDataSourceBinder({
            ...
            _ref: function (row, column, numRows, numColumns) {
                var ref = null;
                if (row instanceof ufa.spreadsheet.Ref) {
                    return row;
                }
                if (row instanceof ufa.spreadsheet.Range) {
                    return row._ref.toRangeRef();
                }
                if (typeof row === 'string') {
                    ref = ufa.spreadsheet.calc.parseReference(row);
                } else {
                    if (!numRows) {
                        numRows = 1;
                    }
            ...
            toJSON: function () {
                ...
                this.forEach(ufa.spreadsheet.SHEETREF, function (row, col, cell) {
                    if (Object.keys(cell).length === 0) {
                        return;
                    }
            ...
            fromJSON: function (json) {
                this.batch(function () {
                    ...
                    if (json.filter) {
                        ...
                        if (!ref) {
                            ufa.logToConsole('Dropping filter for sheet \'' + json.name + '\' due to missing ref');
                        } else {
                            this._filter = {
                                ref: this._ref(ref),
                                columns: columns.map(function (column) {
                                    return {
                                        index: column.index,
                                        filter: ufa.spreadsheet.Filter.create(column)
                                    };
                                })
                            };
                            this._refreshFilter();
            ...
            _compileValidation: function (row, col, validation) {
                ...
                return ufa.spreadsheet.validation.compile(this._name(), row, col, validation);
            },
            _compileFormula: function (row, col, f) {
                f = f.replace(/^=/, '');
                f = ufa.spreadsheet.calc.parseFormula(this._name(), row, col, f);
                return ufa.spreadsheet.calc.compile(f);
            },
            ...
            _filterBy: function (ref, columns) {
                this.batch(function () {
                    ...
                    columns.forEach(function (column) {
                        ...
                        var cells = [];
                        if (columnRef === ufa.spreadsheet.NULLREF) {
                            return;
                        }
            ...
            setState: function (state) {
                ...
                this.triggerChange(ufa.spreadsheet.ALL_REASONS);
            },
            _merge: function (ref) {
                var mergedCells = this._mergedCells;
                var sheet = this;
                ...
                this.batch(function () {
                    mergedRef = ref.map(function (ref) {
                        if (ref instanceof ufa.spreadsheet.CellRef) {
                            return ref;
                        }
...
(function (f, define) {
    ...
    (function (ufa) {
        ...
        var SheetsBar = ufa.ui.Widget.extend({
            init: function (element, options) {
                var classNames = SheetsBar.classNames;
                ufa.ui.Widget.call(this, element, options);
                element = this.element;
                element.addClass(classNames.sheetsBarWrapper);
                this._tree = new ufa.dom.Tree(element[0]);
                this._tree.render([
                    this._addButton(),
            ...
            _createEditor: function () {
                ...
                this._editor = this.element.find(ufa.format('input{0}{1}', DOT, SheetsBar.classNames.sheetsBarEditor)).focus().on('keydown', this._onEditorKeydown.bind(this)).on('blur', this._onEditorBlur.bind(this));
            },
            ...
            _renderSheets: function (sheets, selectedIndex, isInEditMode) {
                ...
                that._isRtl = ufa.support.isRtl(that.element);
                that._sheets = sheets;
            ...
            _createSortable: function () {
                var classNames = SheetsBar.classNames;
                this._sortable = new ufa.ui.Sortable(this.element, {
                    filter: ufa.format('ul li.{0},ul li.{1}', classNames.sheetsBarActive, classNames.sheetsBarInactive),
        ...
        ufa.spreadsheet.SheetsBar = SheetsBar;
...
(function (f, define) {
    ...
    var ParseError = ufa.Class.extend({
...
(function (f, define) {
    ...
    var $ = ufa.jQuery;
    var parseXML = ufa.util.parseXML;
    var parseReference = ufa.spreadsheet.calc.parseReference;
    ...
    function readStyles(zip, theme) {
        ...
        function getColor(attrs) {
            if (attrs.rgb) {
                ...
                var color = ufa.parseColor(themeColor);
                if (attrs.tint) {
                    color = color.toHSL();
    ...
    ufa.spreadsheet.readExcel = readExcel;
    ufa.spreadsheet._readSheet = readSheet;
    ufa.spreadsheet._readStrings = readStrings;
    ufa.spreadsheet._readStyles = readStyles;
    ufa.spreadsheet._readTheme = readTheme;
    ufa.spreadsheet._readWorkbook = readWorkbook;
}, typeof define == 'function' && define.amd ? define : function (a1, a2, a3) {
    (a3 || a2)();
}));
(function (f, define) {
    ...
    (function (ufa) {
        ...
        var $ = ufa.jQuery;
        var Workbook = ufa.Observable.extend({
            init: function (options, view) {
                ...
                this.undoRedoStack = new ufa.util.UndoRedoStack();
                this.undoRedoStack.bind([
                    'undo',
                    'redo'
                ], this._onUndoRedo.bind(this));
                this._context = new ufa.spreadsheet.FormulaContext(this);
                this._validationContext = new ufa.spreadsheet.ValidationFormulaContext(this);
                this._names = Object.create(null);
                this.fromJSON(this.options);
            },
            clipboard: function () {
                if (!this._clipboard) {
                    this._clipboard = new ufa.spreadsheet.Clipboard(this);
                }
            ...
            _inputForRef: function (ref) {
                return new ufa.spreadsheet.Range(ref, this._sheet).input();
            },
            ...
            execute: function (options) {
                ...
                var command = new ufa.spreadsheet[options.command](commandOptions);
            ...
            activeSheet: function (sheet) {
                ...
                this._sheet = sheet;
                sheet.triggerChange(ufa.spreadsheet.ALL_REASONS);
            },
            ...
            insertSheet: function (options) {
                ...
                var sheet = new ufa.spreadsheet.Sheet(options.rows || this.options.rows, options.columns || this.options.columns, options.rowHeight || this.options.rowHeight, options.columnWidth || this.options.columnWidth, options.headerHeight || this.options.headerHeight, options.headerWidth || this.options.headerWidth);
            ...
            fromFile: function (file) {
                ...
                if (file && !this.trigger('excelImport', args)) {
                    ...
                    this._sheets = [];
                    this._sheetsSearchCache = {};
                    ufa.spreadsheet.readExcel(file, this, deferred);
                } else {
                    deferred.reject();
                }
                return promise;
            ...
            saveAsExcel: function (options) {
                ...
                if (!this.trigger('excelExport', { workbook: data })) {
                    var workbook = new ufa.ooxml.Workbook(data);
                    ufa.saveAs({
            ...
            draw: function (options, callback) {
                ...
                (function loop(i) {
                    if (i < sheets.length) {
                        sheets[i].draw(ufa.spreadsheet.SHEETREF, options, function (group) {
                            parts.push(group);
                            loop(i + 1);
                        });
                    } else {
            ...
            adjustNames: function (affectedSheet, forRow, start, delta) {
                ...
                Object.keys(this._names).forEach(function (name) {
                    ...
                    if (ref instanceof ufa.spreadsheet.Ref && ref.sheet.toLowerCase() == affectedSheet) {
        ...
        ufa.spreadsheet.Workbook = Workbook;
        if (ufa.PDFMixin) {
            ufa.PDFMixin.extend(Workbook.prototype);
            Workbook.prototype.saveAsPDF = function (options) {
                ...
                this._drawPDF(options, progress).then(function (root) {
                    return ufa.drawing.exportPDF(root);
...
(function (f, define) {
    define('spreadsheet/formulacontext', ['ufa.core'], f);
}(function () {
    if (ufa.support.browser.msie && ufa.support.browser.version < 9) {
        return;
    }
    ...
    var NameRef = spreadsheet.NameRef;
    var Ref = spreadsheet.Ref;
    var FormulaContext = ufa.Class.extend({
        init: function (workbook) {
            this.workbook = workbook;
        },
        getRefCells: function (ref, hiddenInfo) {
            ...
            if (ref instanceof CellRef) {
                ...
                if (!sheet || !ref.valid()) {
                    return [{ value: new ufa.spreadsheet.calc.runtime.CalcError('REF') }];
                }
            ...
            if (ref instanceof NameRef) {
                ...
                return [{ value: new ufa.spreadsheet.calc.runtime.CalcError('NAME') }];
            }
            return [];
        },
        ...
        onFormula: function (f) {
            ...
            if (value instanceof ufa.spreadsheet.calc.runtime.Matrix) {
...
(function (f, define) {
    define('spreadsheet/controller', ['ufa.core'], f);
}(function () {
    (function (ufa) {
        'use strict';
        if (ufa.support.browser.msie && ufa.support.browser.version < 9) {
        ...
        var Controller = ufa.Class.extend({
            init: function (view, workbook) {
                ...
                this.listener = new ufa.spreadsheet.EventListener(this.container, this, CONTAINER_EVENTS);
                this.keyListener = new ufa.spreadsheet.EventListener(this.clipboardElement, this, CLIPBOARD_EVENTS);
                this.barKeyListener = new ufa.spreadsheet.EventListener(this.editor.barElement(), this, FORMULABAR_EVENTS);
                this.inputKeyListener = new ufa.spreadsheet.EventListener(this.editor.cellElement(), this, FORMULAINPUT_EVENTS);
                if (this.sheetsbar) {
            ...
            onContextMenu: function (event) {
                ...
                var isComposite = this.navigator._sheet.select() instanceof ufa.spreadsheet.UnionRef;
            ...
            onCut: function (e) {
                if (e) {
                    var table = this.clipboardElement.find('table.ufa-clipboard-' + this.clipboard._uid).detach();
        ...
        ufa.spreadsheet.Controller = Controller;
    }(window.ufa));
}, typeof define == 'function' && define.amd ? define : function (a1, a2, a3) {
    (a3 || a2)();
}));
(function (f, define) {
    ...
    (function (ufa) {
        ...
        var VIEW_MESAGES = ufa.spreadsheet.messages.view = {
            errors: {
        ...
        function drawCell(collection, cell, cls, hBorders, vBorders, showGrid) {
            if (!cls && !ufa.spreadsheet.draw.shouldDrawCell(cell)) {
                return;
            }
            var left = cell.left;
            var top = cell.top;
            ...
            if (background) {
                ...
                if (showGrid) {
                    defaultBorder = ufa.parseColor(defaultBorder).toHSV();
                    defaultBorder.v *= 0.9;
            ...
            if (cell.format && data !== null) {
                data = ufa.spreadsheet.formatting.format(data, cell.format);
                if (data.__dataType) {
                    type = data.__dataType;
                }
            } else if (data !== null && data !== undefined) {
                data = ufa.dom.text(data);
            }
            if (!style.textAlign) {
                switch (type) {
                case 'number':
            ...
            if (verticalAlign && data) {
                data = ufa.dom.element('div', { className: 'k-vertical-align-' + verticalAlign }, [data]);
            }
            var children = data ? [data] : [];
            var properties = { style: style };
            var validation = cell.validation;
            if (validation && !validation.value) {
                children.push(ufa.dom.element('span', { className: 'k-dirty' }));
                classNames.push('k-dirty-cell');
                properties.title = validation._getOptions().messageTemplate;
            }
            properties.className = classNames.join(' ');
            var div = ufa.dom.element('div', properties, children);
            collection.push(div);
            return div;
        }
        ...
        var HtmlTable = ufa.Class.extend({
            init: function () {
                this.cols = [];
                this.trs = [];
                this._height = 0;
            ...
            addColumn: function (width) {
                this._width += width;
                var col = ufa.dom.element('col', { style: { width: width + 'px' } });
                col.visible = width > 0;
                this.cols.push(col);
            },
            addRow: function (height) {
                ...
                this._height += height;
                var tr = ufa.dom.element('tr', attr);
                tr.visible = height > 0;
                this.trs.push(tr);
            },
            addCell: function (rowIndex, text, style, className, validation) {
                ...
                if (!(text instanceof ufa.dom.Node)) {
                    text = ufa.dom.text(text);
                }
                var children = [text];
                var properties = { style: style };
                if (validation && !validation.value) {
                ...
                var td = ufa.dom.element('td', properties, children);
            ...
            toDomTree: function (x, y, className) {
                ...
                return ufa.dom.element('table', {
                    ...
                    ufa.dom.element('colgroup', null, this.cols),
                    ufa.dom.element('tbody', null, this.trs)
                ]);
            }
        });
        ...
        var View = ufa.Class.extend({
            init: function (element, options) {
                ...
                this.tree = new ufa.dom.Tree(this.container);
                this.clipboardContents = new ufa.dom.Tree(this.clipboard[0]);
                this.editor = new ufa.spreadsheet.SheetEditor(this);
                this._sheetsbar();
                ...
                this.cellContextMenu = new ufa.spreadsheet.ContextMenu(element.find(DOT + classNames.cellContextMenu), contextMenuConfig);
                this.colHeaderContextMenu = new ufa.spreadsheet.ContextMenu(element.find(DOT + classNames.colHeaderContextMenu), contextMenuConfig);
                this.rowHeaderContextMenu = new ufa.spreadsheet.ContextMenu(element.find(DOT + classNames.rowHeaderContextMenu), contextMenuConfig);
                var scrollbar = ufa.support.scrollbar();
                $(this.container).css({
            ...
            _chrome: function () {
                ...
                this.formulaBar = new ufa.spreadsheet.FormulaBar(formulaBar);
            ...
            _formulaInput: function () {
                ...
                this.formulaInput = new ufa.spreadsheet.FormulaInput(editor, { autoScale: true });
            },
            _sheetsbar: function () {
                if (this.options.sheetsbar) {
                    this.sheetsbar = new ufa.spreadsheet.SheetsBar(this.element.find(DOT + View.classNames.sheetsBar), $.extend(true, {}, this.options.sheetsbar));
                }
            },
            _tabstrip: function () {
                ...
                this.tabstrip = new ufa.spreadsheet.TabStrip($('<div />').prependTo(this.element), {
                    animation: false,
                    dataTextField: 'text',
            ...
            createFilterMenu: function (column) {
                ...
                var range = new ufa.spreadsheet.Range(ref, sheet);
                var element = $('<div />').appendTo(this.element);
                var options = {
                    column: column,
                    range: range
                };
                var filterMenu = new ufa.spreadsheet.FilterMenu(element, options);
            ...
            openDialog: function (name, options) {
                ...
                var range = new ufa.spreadsheet.Range(ref, sheet);
                var dialog = ufa.spreadsheet.dialogs.create(name, options);
                if (dialog) {
            ...
            render: function () {
                ...
                var topCorner = ufa.dom.element('div', {
                    style: {
            ...
            renderResizeHint: function () {
                ...
                return ufa.dom.element('div', {
                    className: classNames.resizeHint + (!horizontal ? ' ' + classNames.resizeHintVertical : ''),
                    style: style
                }, [
                    ufa.dom.element('div', { className: classNames.resizeHintHandle }),
                    ufa.dom.element('div', { className: classNames.resizeHintMarker })
                ]);
            },
            renderClipboardContents: function () {
                ...
                this.clipboardContents.render([table.toDomTree(0, 0, 'ufa-clipboard-' + this._workbook.clipboard()._uid)]);
                this.selectClipBoardContents();
            },
        ...
        var Pane = ufa.Class.extend({
            ...
            render: function (scrollLeft, scrollTop) {
                ...
                if (grid.hasRowHeader) {
                    var rowHeader = ufa.dom.element('div', {
                    ...
                    sheet.forEach(view.ref.leftColumn(), function (row) {
                        if (!sheet.isHiddenRow(row)) {
                            ...
                            rowHeader.children.push(ufa.dom.element('div', {
                                className: this.headerClassName(row, 'row'),
                                style: {
                                    width: grid.headerWidth + 'px',
                                    height: height + 'px'
                                }
                            }, [ufa.dom.element('div', { className: 'k-vertical-align-center' }, [ufa.dom.text(text + '')])]));
                        }
                    }.bind(this));
                }
                if (grid.hasColumnHeader) {
                    var columnHeader = ufa.dom.element('div', {
                    ...
                    sheet.forEach(view.ref.topRow(), function (row, col) {
                        if (!sheet.isHiddenColumn(col)) {
                            var text = ufa.spreadsheet.Ref.display(null, Infinity, col), width = sheet.columnWidth(col);
                            columnHeader.children.push(ufa.dom.element('div', {
            ...
            renderData: function () {
                ...
                var layout = ufa.spreadsheet.draw.doLayout(this._sheet, view.ref, { forScreen: true }), prev;
                ...
                if (showGridLines) {
                    ...
                    layout.xCoords.forEach(function (x) {
                        if (x !== prev) {
                            prev = x;
                            cont.children.push(ufa.dom.element('div', {
            ...
            filterIconRect: function (rect) {
                var BUTTON_SIZE = 16;
                var BUTTON_OFFSET = 3;
                return new ufa.spreadsheet.Rectangle(rect.right - BUTTON_SIZE - BUTTON_OFFSET, rect.top + BUTTON_OFFSET, BUTTON_SIZE, BUTTON_SIZE);
            },
            renderFilterHeaders: function () {
                ...
                function icon(className) {
                    return ufa.dom.element('span', { className: classNames.icon + ' ' + className });
                }
                function filterButton(classNames, position, index) {
                    ...
                    var button = ufa.dom.element('span', {
                        className: classes,
                        style: style
                ...
                return ufa.dom.element('div', { className: classNames.filterHeadersWrapper }, children);
            },
            renderEditorSelection: function () {
                ...
                return ufa.dom.element('div', { className: classNames.selectionWrapper }, selections);
            },
            ...
            renderAutoFill: function () {
                ...
                if (this._sheet.autoFillInProgress()) {
                    ...
                    if (punch) {
                        ...
                        var hint = ufa.dom.element('span', { className: 'k-tooltip' }, [ufa.dom.text(this._sheet._autoFillHint)]);
                ...
                return ufa.dom.element('div', { className: Pane.classNames.autoFillWrapper }, autoFillRectangle);
            },
        ...
        ufa.spreadsheet.View = View;
        ufa.spreadsheet.Pane = Pane;
        ufa.spreadsheet.drawCell = drawCell;
...
(function (f, define) {
    ...
    (function (ufa) {
        ...
        var RangeRef = ufa.spreadsheet.RangeRef;
        var UnionRef = ufa.spreadsheet.UnionRef;
        var Rectangle = ufa.Class.extend({
        ...
        var Grid = ufa.Class.extend({
            ...
            pane: function (options) {
                return new PaneGrid(new ufa.spreadsheet.PaneAxis(this._rows, options.row, options.rowCount, this._headerHeight), new ufa.spreadsheet.PaneAxis(this._columns, options.column, options.columnCount, this._headerWidth), this);
            },
        ...
        ufa.spreadsheet.Grid = Grid;
        ufa.spreadsheet.PaneGrid = PaneGrid;
        ufa.spreadsheet.Rectangle = Rectangle;
    }(ufa));
}, typeof define == 'function' && define.amd ? define : function (a1, a2, a3) {
    (a3 || a2)();
}));
(function (f, define) {
    define('spreadsheet/axis', ['ufa.core'], f);
}(function () {
    (function (ufa) {
        if (ufa.support.browser.msie && ufa.support.browser.version < 9) {
            return;
        }
        var Axis = ufa.Class.extend({
            init: function (count, value) {
                this._value = value;
                this._count = count;
                this.values = new ufa.spreadsheet.RangeList(0, count - 1, value);
                this._hidden = new ufa.spreadsheet.RangeList(0, count - 1, 0);
                this.scrollBarSize = ufa.support.scrollbar();
                this._refresh();
            },
            ...
            _refresh: function () {
                ...
                this._pixelValues = this.values.map(function (range) {
                    ...
                    var end = current - 1;
                    return new ufa.spreadsheet.ValueRange(start, end, range);
                });
                this.total = current;
            },
            getState: function () {
        ...
        ufa.spreadsheet.Axis = Axis;
        ufa.spreadsheet.PaneAxis = PaneAxis;
    }(ufa));
}, typeof define == 'function' && define.amd ? define : function (a1, a2, a3) {
    (a3 || a2)();
}));
(function (f, define) {
    ...
    (function (ufa) {
        ...
        var Filter = ufa.spreadsheet.Filter = ufa.Class.extend({
            prepare: function () {
            },
            value: function (cell) {
                return cell.value;
        ...
        Filter.create = function (options) {
            ...
            var constructor = ufa.spreadsheet[filter.charAt(0).toUpperCase() + filter.substring(1) + 'Filter'];
            if (!constructor) {
                throw new Error('Filter type not recognized.');
            }
            return new constructor(options);
        };
        ufa.spreadsheet.ValueFilter = Filter.extend({
            _values: [],
            _dates: [],
            _blanks: false,
            init: function (options) {
            ...
            value: function (cell) {
                ...
                if (this._dates.length > 0 && cell.format && typeof value === 'number') {
                    var type = ufa.spreadsheet.formatting.type(value, cell.format);
                    if (type === 'date') {
                        value = ufa.spreadsheet.numberToDate(value);
                    }
                }
                return value;
            },
        ...
        ufa.spreadsheet.CustomFilter = Filter.extend({
            _logic: 'and',
            init: function (options) {
                ...
                var expression = ufa.data.Query.filterExpr({
                    logic: this._logic,
            ...
            value: function (cell) {
                ...
                if (cell.format) {
                    valueType = ufa.spreadsheet.formatting.type(value, cell.format);
                }
                if (valueType != criterionType) {
                    if (criterionType == 'string') {
                        if (cell.format) {
                            value = ufa.spreadsheet.formatting.text(value, cell.format);
                        }
                        value = value + '';
                    }
        ...
        ufa.spreadsheet.TopFilter = Filter.extend({
            init: function (options) {
        ...
        ufa.spreadsheet.DynamicFilter = Filter.extend({
            init: function (options) {
                this._type = options.type;
            ...
            tomorrow: function (value) {
                if (value instanceof Date) {
                    var tomorrow = ufa.date.addDays(ufa.date.today(), 1);
                    return ufa.date.getDate(value).getTime() === tomorrow.getTime();
                }
                return false;
            },
            today: function (value) {
                if (value instanceof Date) {
                    return ufa.date.isToday(value);
                }
                return false;
            },
            yesterday: function (value) {
                if (value instanceof Date) {
                    var yesterday = ufa.date.addDays(ufa.date.today(), -1);
                    return ufa.date.getDate(value).getTime() === yesterday.getTime();
                }
                return false;
            },
            nextWeek: function (value) {
                return sameWeek(ufa.date.addDays(ufa.date.today(), 7), value);
            },
            thisWeek: function (value) {
                return sameWeek(ufa.date.today(), value);
            },
        ...
        [
            ...
            ufa.spreadsheet.DynamicFilter.prototype['quarter' + target] = function (value) {
                if (value instanceof Date) {
        ...
        ufa.cultures['en-US'].calendar.months.names.forEach(function (month, index) {
            ufa.spreadsheet.DynamicFilter.prototype[month.toLowerCase()] = function (value) {
                if (value instanceof Date) {
        ...
        function sameMonth(value, offset) {
            if (value instanceof Date) {
                var today = ufa.date.firstDayOfMonth(ufa.date.today());
                today.setMonth(today.getMonth() + offset, 1);
                return today.getTime() === ufa.date.firstDayOfMonth(value).getTime();
            }
            return false;
        }
        function sameWeek(a, b) {
            if (b instanceof Date) {
                var firstWeek = ufa.date.dayOfWeek(ufa.date.getDate(a), 1);
                var secondWeek = ufa.date.dayOfWeek(ufa.date.getDate(b), 1);
...
(function (f, define) {
    define('spreadsheet/sorter', ['ufa.core'], f);
}(function () {
    (function (ufa) {
        if (ufa.support.browser.msie && ufa.support.browser.version < 9) {
            return;
        }
        var Sorter = ufa.Class.extend({
        ...
        Sorter.ascendingComparer = function (a, b) {
            ...
            if (a instanceof ufa.spreadsheet.calc.runtime.CalcError) {
                if (b instanceof ufa.spreadsheet.calc.runtime.CalcError) {
                    return 0;
                } else {
                    return 1;
                }
...
(function (f, define) {
    ...
    function compileFormatPart(format) {
        ...
        var preamble = '';
        if (condition == 'text') {
            preamble = 'if (typeof value == \'string\' || value instanceof ufa.spreadsheet.CalcError) { ';
    ...
    function compile(format) {
        ...
        if (!f) {
            ...
            code = '\'use strict\'; return function(value, culture){ ' + 'if (!culture) culture = ufa.culture(); ' + 'var output = \'\', type = null, result = { body: [] }; ' + code + '; return result; };';
...
(function (f, define) {
    ...
    (function (ufa) {
        ...
        var BORDER_PALETTE_MESSAGES = ufa.spreadsheet.messages.borderPalette = {
        ...
        var ColorChooser = ufa.ui.Widget.extend({
            init: function (element, options) {
                ufa.ui.Widget.call(this, element, options);
                this.element = element;
            ...
            events: ['change'],
            destroy: function () {
                ufa.unbind(this.dialog.element.find('.k-action-buttons'));
                this.dialog.destroy();
            ...
            _colorPalette: function () {
                ...
                var colorPalette = this.colorPalette = $('<div />').ufaColorPalette({
                    palette: [
                        '#ffffff',
                        '#000000',
                        '#d6ecff',
            ...
            _customColorPalette: function () {
                ...
                var dialog = this.dialog = element.appendTo(document.body).ufaWindow({
                    animation: false,
                    scrollable: false,
                    resizable: false,
                    maximizable: false,
                ...
                dialog.one('activate', function () {
                    this.element.find('[data-role=flatcolorpicker]').data('ufaFlatColorPicker')._hueSlider.resize();
                });
                var flatColorPicker = this.flatColorPicker = dialog.element.children().first().ufaFlatColorPicker().data('ufaFlatColorPicker');
                var viewModel = ufa.observable({
                    apply: function () {
                ...
                ufa.bind(dialog.element.find('.k-action-buttons'), viewModel);
            },
        ...
        var BorderPalette = ufa.ui.Widget.extend({
            init: function (element, options) {
                ufa.ui.Widget.call(this, element, options);
                this.element = element;
                this.color = '#000';
            ...
            _borderTypePalette: function () {
                ...
                var buttons = BORDER_TYPES.map(function (type) {
                    return '<a title="' + messages[type] + '" href="#" data-border-type="' + type + '" class="k-button k-button-icon">' + '<span class="k-sprite k-font-icon k-icon k-i-' + ufa.toHyphens(type) + '"></span>' + '</a>';
                }).join('');
                var element = $('<div />', {
        ...
        ufa.spreadsheet.ColorChooser = ColorChooser;
        ufa.spreadsheet.BorderPalette = BorderPalette;
    }(window.ufa));
}, typeof define == 'function' && define.amd ? define : function (a1, a2, a3) {
    (a3 || a2)();
}));
(function (f, define) {
    ...
    (function (ufa) {
        ...
        var $ = ufa.jQuery;
        var ToolBar = ufa.ui.ToolBar;
        var MESSAGES = ufa.spreadsheet.messages.toolbar = {
        ...
        var SpreadsheetToolBar = ToolBar.extend({
            ...
            refresh: function (activeCell) {
                ...
                for (var i = 0; i < tools.length; i++) {
                    ...
                    var value = ufa.isFunction(range[property]) ? range[property]() : range;
                    if (tool.type === 'button') {
        ...
        ufa.spreadsheet.ToolBar = SpreadsheetToolBar;
        var DropDownTool = ufa.toolbar.Item.extend({
            init: function (options, toolbar) {
                var dropDownList = $('<select />').ufaDropDownList({ height: 'auto' }).data('ufaDropDownList');
            ...
            _open: function () {
                ...
                list.css('width', listWidth + ufa.support.scrollbar());
        ...
        var PopupTool = ufa.toolbar.Item.extend({
            init: function (options, toolbar) {
            ...
            _popup: function () {
                var element = this.element;
                this.popup = $('<div class=\'k-spreadsheet-popup\' />').appendTo(element).ufaPopup({ anchor: element }).data('ufaPopup');
            }
        });
        ufa.toolbar.registerComponent('dialog', ufa.toolbar.ToolBarButton.extend({
            init: function (options, toolbar) {
                ufa.toolbar.ToolBarButton.fn.init.call(this, options, toolbar);
        ...
        ufa.toolbar.registerComponent('exportAsDialog', ufa.toolbar.Item.extend({
            init: function (options, toolbar) {
                this._dialogName = options.dialogName;
                this.toolbar = toolbar;
        ...
        var OverflowDialogButton = ufa.toolbar.OverflowButton.extend({
            init: function (options, toolbar) {
                ufa.toolbar.OverflowButton.fn.init.call(this, options, toolbar);
        ...
        var ColorPicker = PopupTool.extend({
            init: function (options, toolbar) {
                ...
                this.colorChooser = new ufa.spreadsheet.ColorChooser(this.popup.element, { change: this._colorChange.bind(this) });
        ...
        ufa.toolbar.registerComponent('colorPicker', ColorPicker, ColorPickerButton);
        var FONT_SIZES = [
            8,
            9,
            10,
        ...
        var FontSize = ufa.toolbar.Item.extend({
            init: function (options, toolbar) {
                var comboBox = $('<input />').ufaComboBox({
            ...
            _valueChange: function (e) {
                this.toolbar.action({
                    ...
                    options: {
                        property: this.options.property,
                        value: ufa.parseInt(e.sender.value())
                    }
                });
            },
            update: function (value) {
                this.value(ufa.parseInt(value) || DEFAULT_FONT_SIZE);
            },
            value: function (value) {
                if (value !== undefined) {
        ...
        ufa.toolbar.registerComponent('fontSize', FontSize, FontSizeButton);
        var FONT_FAMILIES = [
            'Arial',
            'Courier New',
            'Georgia',
        ...
        ufa.toolbar.registerComponent('fontFamily', FontFamily, FontFamilyButton);
        var defaultFormats = ufa.spreadsheet.formats = {
            automatic: null,
            number: '#,0.00',
            percent: '0.00%',
        ...
        ufa.toolbar.registerComponent('format', Format, FormatButton);
        var BorderChangeTool = PopupTool.extend({
            ...
            _borderPalette: function () {
                ...
                this.borderPalette = new ufa.spreadsheet.BorderPalette(element, { change: this._action.bind(this) });
            },
            _action: function (e) {
                this.toolbar.action({
        ...
        ufa.toolbar.registerComponent('borders', BorderChangeTool, BorderChangeButton);
        ...
        ufa.toolbar.registerComponent('alignment', AlignmentTool, AlignmentButton);
        ...
        ufa.toolbar.registerComponent('merge', MergeTool, MergeButton);
        ...
        ufa.toolbar.registerComponent('freeze', FreezeTool, FreezeButton);
        ...
        ufa.toolbar.registerComponent('sort', Sort, SortButton);
        var Filter = ufa.toolbar.ToolBarButton.extend({
            init: function (options, toolbar) {
        ...
        ufa.toolbar.registerComponent('filter', Filter, FilterButton);
        var Open = ufa.toolbar.Item.extend({
            init: function (options, toolbar) {
                this.toolbar = toolbar;
        ...
        ufa.toolbar.registerComponent('open', Open);
        ufa.spreadsheet.TabStrip = ufa.ui.TabStrip.extend({
            init: function (element, options) {
                ufa.ui.TabStrip.fn.init.call(this, element, options);
            ...
            events: ufa.ui.TabStrip.fn.events.concat([
                'action',
                'dialog'
            ]),
            destroy: function () {
                this.quickAccessToolBar.off('click');
                ufa.ui.TabStrip.fn.destroy.call(this);
            ...
            _quickAccessButtons: function () {
                ...
                var buttonTemplate = ufa.template('<a href=\'\\#\' title=\'#= title #\' data-action=\'#= action #\' class=\'k-button k-button-icon\'><span class=\'k-icon k-font-icon k-i-#=iconClass#\'></span></a>');
                this.quickAccessToolBar = $('<div />', {
                    'class': 'k-spreadsheet-quick-access-toolbar',
                    'html': ufa.render(buttonTemplate, buttons)
            ...
            _toolbar: function (container, name, tools) {
                ...
                if (tools) {
                    ...
                    this.toolbars[name] = new ufa.spreadsheet.ToolBar(element, options);
                }
            }
        });
    }(window.ufa));
...
(function (f, define) {
    ...
    (function (ufa) {
        ...
        var $ = ufa.jQuery;
        var ObservableObject = ufa.data.ObservableObject;
        var MESSAGES = ufa.spreadsheet.messages.dialogs = {
            apply: 'Apply',
            save: 'Save',
            cancel: 'Cancel',
            remove: 'Remove',
        ...
        var SpreadsheetDialog = ufa.spreadsheet.SpreadsheetDialog = ufa.Observable.extend({
            init: function (options) {
                ufa.Observable.fn.init.call(this, options);
                this.options = $.extend(true, {}, this.options, options);
            ...
            dialog: function () {
                if (!this._dialog) {
                    this._dialog = $('<div class=\'k-spreadsheet-window k-action-window\' />').addClass(this.options.className || '').append(this.options.template).appendTo(document.body).ufaWindow({
                        scrollable: false,
                        resizable: false,
                        maximizable: false,
                        modal: true,
        ...
        function formattedValue(value, format) {
            return ufa.spreadsheet.formatting.text(value, format);
        }
        var FormatCellsViewModel = ufa.spreadsheet.FormatCellsViewModel = ObservableObject.extend({
            init: function (options) {
        ...
        var FormatCellsDialog = SpreadsheetDialog.extend({
            ...
            _generateFormats: function () {
                ...
                if (!options.dateFormats) {
                    var calendarPatterns = ufa.cultures.current.calendars.standard.patterns;
            ...
            open: function (range) {
                ...
                ufa.bind(element, this.viewModel);
                var currencyFilter = element.find('select.k-format-filter').data('ufaDropDownList');
                if (options.currencies.length > 10) {
                    currencyFilter.setOptions({ filter: 'contains' });
                }
                element.find(ufa.roleSelector('staticlist')).parent().addClass('k-list-wrapper');
            },
            apply: function () {
        ...
        FormatCellsDialog.currenciesFrom = function (cultures) {
            return uniqueBy('description', $.map(cultures, function (culture, name) {
                ...
                var description = ufa.format('{0} ({1}, {2})', currency.name, currency.abbr, currency.symbol);
                return {
        ...
        ufa.spreadsheet.dialogs.register('formatCells', FormatCellsDialog);
        ufa.spreadsheet.dialogs.FormatCellsDialog = FormatCellsDialog;
        ...
        ufa.spreadsheet.dialogs.register('message', MessageDialog);
        var FontFamilyDialog = SpreadsheetDialog.extend({
            ...
            _list: function () {
                ...
                this.list = new ufa.ui.StaticList(ul, {
                    dataSource: new ufa.data.DataSource({ data: fonts }),
                    template: '#:data#',
                    value: defaultFont,
        ...
        ufa.spreadsheet.dialogs.register('fontFamily', FontFamilyDialog);
        var FontSizeDialog = SpreadsheetDialog.extend({
            ...
            _list: function () {
                ...
                this.list = new ufa.ui.StaticList(ul, {
                    dataSource: new ufa.data.DataSource({ data: sizes }),
                    template: '#:data#',
                    value: defaultSize,
                    change: this.apply.bind(this)
                });
            ...
            apply: function (e) {
                ...
                this.trigger('action', {
                    ...
                    options: {
                        property: 'fontSize',
                        value: ufa.parseInt(e.sender.value()[0])
                    }
                });
            }
        });
        ufa.spreadsheet.dialogs.register('fontSize', FontSizeDialog);
        var BordersDialog = SpreadsheetDialog.extend({
            init: function (options) {
                SpreadsheetDialog.fn.init.call(this, options);
                this.element = this.dialog().element;
                this._borderPalette();
                this.viewModel = ufa.observable({
                    apply: this.apply.bind(this),
                    close: this.close.bind(this)
                });
                ufa.bind(this.element.find('.k-action-buttons'), this.viewModel);
            },
            options: {
            ...
            _borderPalette: function () {
                ...
                this.borderPalette = new ufa.spreadsheet.BorderPalette(element, { change: this.value.bind(this) });
            },
        ...
        ufa.spreadsheet.dialogs.register('borders', BordersDialog);
        ...
        var ColorPickerDialog = ColorChooser.extend({
            ...
            _colorPalette: function () {
                var element = this.dialog().element.find('div:first');
                this.colorPalette = element.ufaColorPalette({
                    palette: [
                        '#ffffff',
                        '#000000',
                        '#d6ecff',
        ...
        ufa.spreadsheet.dialogs.register('colorPicker', ColorPickerDialog);
        var CustomColorDialog = ColorChooser.extend({
            ...
            _colorPicker: function () {
                var element = this.dialog().element.find('div:first');
                this.colorPicker = element.ufaFlatColorPicker({ change: this.value.bind(this) }).data('ufaFlatColorPicker');
            }
        });
        ufa.spreadsheet.dialogs.register('customColor', CustomColorDialog);
        var AlignmentDialog = SpreadsheetDialog.extend({
            ...
            _list: function () {
                var ul = this.dialog().element.find('ul');
                this.list = new ufa.ui.StaticList(ul, {
                    dataSource: new ufa.data.DataSource({ data: this.options.buttons }),
        ...
        ufa.spreadsheet.dialogs.register('alignment', AlignmentDialog);
        ...
        ufa.spreadsheet.dialogs.register('merge', MergeDialog);
        ...
        ufa.spreadsheet.dialogs.register('freeze', FreezeDialog);
        var ValidationViewModel = ufa.spreadsheet.ValidationCellsViewModel = ObservableObject.extend({
            init: function (options) {
                ObservableObject.fn.init.call(this, options);
            ...
            buildMessages: function () {
                this._mute = true;
                this.set('hintTitle', this.hintTitleTemplate ? ufa.format(this.hintTitleTemplate, this.type) : '');
                this.set('hintMessage', this.hintMessageTemplate ? ufa.format(this.hintMessageTemplate, this.from, this.to) : '');
                this._mute = false;
            },
            reset: function () {
                this.setComparers();
            ...
            setHintMessageTemplate: function () {
                if (this.criterion !== 'custom' && this.criterion !== 'list') {
                    this.set('hintMessageTemplate', ufa.format(this.defaultHintMessage, this.criterion, this.comparerMessages[this.comparer]));
                } else {
        ...
        var ValidationDialog = SpreadsheetDialog.extend({
            ...
            open: function (range) {
                ...
                ufa.bind(element, this.viewModel);
                this.validatable = new ufa.ui.Validator(element.find('.k-edit-form-container'), {
        ...
        ufa.spreadsheet.dialogs.register('validation', ValidationDialog);
        ufa.spreadsheet.dialogs.ValidationDialog = ValidationDialog;
        var ExportAsDialog = SpreadsheetDialog.extend({
            init: function (options) {
                ...
                ufa.bind(this.dialog().element, this.viewModel);
            },
            options: {
        ...
        ufa.spreadsheet.dialogs.register('exportAs', ExportAsDialog);
        ...
        ufa.spreadsheet.dialogs.register('modifyMerged', ModifyMergedDialog);
        ...
        ufa.spreadsheet.dialogs.register('useKeyboard', UseKeyboardDialog);
        ...
        ufa.spreadsheet.dialogs.register('unsupportedSelection', UnsupportedSelectionDialog);
    }(window.ufa));
}, typeof define == 'function' && define.amd ? define : function (a1, a2, a3) {
    (a3 || a2)();
}));
(function (f, define) {
    ...
    (function (ufa) {
        ...
        var SheetDataSourceBinder = ufa.Class.extend({
            init: function (options) {
            ...
            _dataSource: function () {
                ...
                this.dataSource = ufa.data.DataSource.create(dataSource).bind('change', this._changeHandler);
            },
            _change: function () {
                if (this._skipRebind) {
                    return;
                ...
                var getters = columns.map(function (column) {
                    return ufa.getter(column.field);
                });
                this.sheet.batch(function () {
        ...
        ufa.spreadsheet.SheetDataSourceBinder = SheetDataSourceBinder;
    }(ufa));
}, typeof define == 'function' && define.amd ? define : function (a1, a2, a3) {
    (a3 || a2)();
}));
    ...
    (function (ufa) {
        ...
        var Details = Widget.extend({
            init: function (element, options) {
                ...
                this._container = ufa.wrap(this._summary.next(), true);
                if (!options.expanded) {
            ...
            toggle: function (show) {
                var animation = ufa.fx(this._container).expand('vertical');
        ...
        var FILTERMENU_MESSAGES = ufa.spreadsheet.messages.filterMenu = {
        ...
        ufa.data.binders.spreadsheetFilterValue = ufa.data.Binder.extend({
            init: function (element, bindings, options) {
                ufa.data.Binder.fn.init.call(this, element, bindings, options);
        ...
        ufa.data.binders.widget.spreadsheetFilterValue = ufa.data.Binder.extend({
            init: function (widget, bindings, options) {
                ufa.data.Binder.fn.init.call(this, widget.element[0], bindings, options);
                this.widget = widget;
                this._change = $.proxy(this.change, this);
        ...
        function filter(dataSource, query) {
            ...
            var data = dataSource instanceof ufa.data.HierarchicalDataSource && dataSource.data();
        ...
        var FilterMenuViewModel = ufa.spreadsheet.FilterMenuViewModel = ufa.data.ObservableObject.extend({
            valuesChange: function (e) {
                ...
                var value = function (item) {
                    return item.dataType === 'date' ? ufa.spreadsheet.dateToNumber(item.value) : item.value;
                };
        ...
        var FilterMenuController = ufa.spreadsheet.FilterMenuController = {
            ...
            values: function (range, column) {
                ...
                columnRange.forEachCell(function (row, col, cell) {
                    if (cell.value === undefined) {
                        ...
                        cell.dataType = ufa.spreadsheet.formatting.type(cell.value, cell.format);
                    } else {
                        cell.dataType = typeof cell.value;
                    }
                    if (cell.value !== null && cell.format) {
                        cell.text = ufa.spreadsheet.formatting.text(cell.value, cell.format);
                    } else {
                    ...
                    if (cell.dataType === 'date') {
                        cell.value = ufa.spreadsheet.numberToDate(cell.value);
                    }
        ...
        var FilterMenu = Widget.extend({
            ...
            _popup: function () {
                this.popup = this.element.ufaPopup({ copyAnchorStyles: false }).data('ufaPopup');
            },
            _sort: function () {
                var template = ufa.template(FilterMenu.templates.menuItem);
                var messages = FILTERMENU_MESSAGES;
                var items = [
                    {
                        command: 'sort',
                ...
                var ul = $('<ul />', { 'html': ufa.render(template, items) }).appendTo(this.element);
                this.menu = ul.ufaMenu({
            ...
            _appendTemplate: function (template, className, details, expanded) {
                var compiledTemplate = ufa.template(template);
                ...
                ufa.bind(wrapper, this.viewModel);
                return wrapper;
            },
            _detailToggle: function (e) {
                this.element.find('[data-role=details]').not(e.sender.element).data('ufaDetails').toggle(!e.show);
            },
            ...
            _filterByValue: function () {
                ...
                this.valuesTreeView = wrapper.find('[data-role=treeview]').data('ufaTreeView');
                var values = FilterMenuController.valuesTree(this.options.range, this.options.column);
                this.viewModel.set('valuesDataSource', new ufa.data.HierarchicalDataSource({ data: values }));
            },
            ...
            _active: function () {
                var activeContainer = this.element.find('[data-role=details]').filter(function (index, element) {
                    return $(element).data('ufaDetails').visible();
                });
        ...
        ufa.spreadsheet.FilterMenu = FilterMenu;
...
(function (f, define) {
    define('spreadsheet/editor', ['ufa.core'], f);
}(function () {
    (function (ufa) {
        if (ufa.support.browser.msie && ufa.support.browser.version < 9) {
            return;
        }
        var SheetEditor = ufa.Observable.extend({
            init: function (view) {
                ufa.Observable.fn.init.call(this);
                this.view = view;
            ...
            activeEditor: function () {
                var editor = null;
                var activeElement = ufa._activeElement();
        ...
        ufa.spreadsheet.SheetEditor = SheetEditor;
    }(ufa));
}, typeof define == 'function' && define.amd ? define : function (a1, a2, a3) {
    (a3 || a2)();
}));
...
(function (f, define) {
    ...
    var spreadsheet = ufa.spreadsheet;
    var CellRef = spreadsheet.CellRef;
    var drawing = ufa.drawing;
    var formatting = spreadsheet.formatting;
    var geo = ufa.geometry;
    ...
    function drawText(text, color, cell, group) {
        ...
        if (cell.wrap) {
            ...
            var box = ufa.util.measureText(text, style);
            lines.push({
                el: tmp,
                box: box
            });
    ...
    function lineBreak(text, style, width) {
        ...
        function split(min, eol, max) {
            ...
            var box = ufa.util.measureText(sub, style);
            if (box.width <= width) {
                if (eol < max - 1) {
    ...
    function draw(sheet, range, options, callback) {
        ...
        var paper = ufa.pdf.getPaperOptions(options);
...
(function (f, define) {
    ...
    (function (ufa, undefined) {
        if (ufa.support.browser.msie && ufa.support.browser.version < 9) {
            return;
        }
        var $ = ufa.jQuery;
        var Widget = ufa.ui.Widget;
        var Workbook = ufa.spreadsheet.Workbook;
        var Controller = ufa.spreadsheet.Controller;
        var View = ufa.spreadsheet.View;
        var NS = '.ufaSpreadsheet';
        var ALL_REASONS = {
            recalc: true,
            selection: true,
            activeCell: true,
        ...
        var Spreadsheet = ufa.ui.Widget.extend({
            init: function (element, options) {
                Widget.fn.init.call(this, element, options);
            ...
            _initProgress: function (deferred) {
                ...
                var pb = $('<div class=\'k-loading-progress\'>').appendTo(loading).ufaProgressBar({
                    type: 'chunk',
                    chunkCount: 10,
                    min: 0,
                    max: 1,
            ...
            destroy: function () {
                ufa.ui.Widget.fn.destroy.call(this);
                this._workbook.destroy();
            ...
            options: {
                ...
                pdf: {
                    ...
                    subject: null,
                    keywords: null,
                    creator: 'ufa UI PDF Generator v.' + ufa.version,
                    date: null
                }
            },
            events: [
        ...
        ufa.spreadsheet.ALL_REASONS = ALL_REASONS;
        ufa.ui.plugin(Spreadsheet);
