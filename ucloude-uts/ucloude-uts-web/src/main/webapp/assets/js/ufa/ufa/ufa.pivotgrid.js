(function (f, define) {
    ...
    (function ($, undefined) {
        ...
        function accumulateMembers(accumulator, rootTuple, tuple, level) {
            ...
            if (tuple === rootTuple) {
                accumulator[ufa.stringify([member.name])] = !!length;
            } else if (length) {
                accumulator[ufa.stringify(buildPath(tuple, level))] = true;
            }
            if (length) {
        ...
        function createAggregateGetter(m) {
            var measureGetter = ufa.getter(m.field, true);
        ...
        var PivotCubeBuilder = Class.extend({
            ...
            _addData: function (result, value, format, resultFunc) {
                ...
                if (value) {
                    ...
                    fmtValue = format ? ufa.format(format, value) : value;
                }
            ...
            _buildGetters: function (names) {
                ...
                for (var idx = 0; idx < names.length; idx++) {
                    ...
                    parts = name.split('&');
                    if (parts.length > 1) {
                        result[parts[0]] = ufa.getter(parts[0], true);
                    } else {
                        result[name] = ufa.getter(normalizeName(name), true);
                    }
                }
                return result;
            },
            ...
            _filter: function (data, filter) {
                ...
                return new ufa.data.Query(data).filter(filter).data;
            },
            ...
            _cubeSchema: function (cube) {
                return {
                    ...
                    members: $.proxy(function (response, restrictions) {
                        ...
                        getter = ufa.getter(normalizeName(name), true);
        ...
        PivotDataSource.create = function (options) {
            ...
            if (!(dataSource instanceof PivotDataSource) && dataSource instanceof ufa.data.DataSource) {
        ...
        function serializeExpression(expression) {
            ...
            if (operator == 'in') {
                ...
                command += ufa.format(filterFunctionFormats[operator], field, value);
                command += ')';
            }
            return command;
        }
        ...
        var XmlaTransport = ufa.data.RemoteTransport.extend({
            init: function (options) {
                var originalOptions = options;
                options = this.options = extend(true, {}, this.options, options);
                ufa.data.RemoteTransport.call(this, options);
            ...
            setup: function (options, type) {
                ...
                return ufa.data.RemoteTransport.fn.setup.call(this, options, type);
            },
            options: {
                read: {
                    dataType: 'text',
        ...
        function translateAxis(axis) {
            var result = { tuples: [] };
            var tuples = asArray(ufa.getter('Tuples.Tuple', true)(axis));
            var captionGetter = ufa.getter('Caption[\'#text\']');
            var unameGetter = ufa.getter('UName[\'#text\']');
            var levelNameGetter = ufa.getter('LName[\'#text\']');
            var levelNumGetter = ufa.getter('LNum[\'#text\']');
            var childrenGetter = ufa.getter('CHILDREN_CARDINALITY[\'#text\']', true);
            var hierarchyGetter = ufa.getter('[\'@Hierarchy\']');
            var parentNameGetter = ufa.getter('PARENT_UNIQUE_NAME[\'#text\']', true);
        ...
        var schemaDataReaderMap = {
            cubes: {
                name: ufa.getter('CUBE_NAME[\'#text\']', true),
                caption: ufa.getter('CUBE_CAPTION[\'#text\']', true),
                description: ufa.getter('DESCRIPTION[\'#text\']', true),
                type: ufa.getter('CUBE_TYPE[\'#text\']', true)
            },
            catalogs: {
                name: ufa.getter('CATALOG_NAME[\'#text\']', true),
                description: ufa.getter('DESCRIPTION[\'#text\']', true)
            },
            measures: {
                name: ufa.getter('MEASURE_NAME[\'#text\']', true),
                caption: ufa.getter('MEASURE_CAPTION[\'#text\']', true),
                uniqueName: ufa.getter('MEASURE_UNIQUE_NAME[\'#text\']', true),
                description: ufa.getter('DESCRIPTION[\'#text\']', true),
                aggregator: ufa.getter('MEASURE_AGGREGATOR[\'#text\']', true),
                groupName: ufa.getter('MEASUREGROUP_NAME[\'#text\']', true),
                displayFolder: ufa.getter('MEASURE_DISPLAY_FOLDER[\'#text\']', true),
                defaultFormat: ufa.getter('DEFAULT_FORMAT_STRING[\'#text\']', true)
            },
            kpis: {
                name: ufa.getter('KPI_NAME[\'#text\']', true),
                caption: ufa.getter('KPI_CAPTION[\'#text\']', true),
                value: ufa.getter('KPI_VALUE[\'#text\']', true),
                goal: ufa.getter('KPI_GOAL[\'#text\']', true),
                status: ufa.getter('KPI_STATUS[\'#text\']', true),
                trend: ufa.getter('KPI_TREND[\'#text\']', true),
                statusGraphic: ufa.getter('KPI_STATUS_GRAPHIC[\'#text\']', true),
                trendGraphic: ufa.getter('KPI_TREND_GRAPHIC[\'#text\']', true),
                description: ufa.getter('KPI_DESCRIPTION[\'#text\']', true),
                groupName: ufa.getter('MEASUREGROUP_NAME[\'#text\']', true)
            },
            dimensions: {
                name: ufa.getter('DIMENSION_NAME[\'#text\']', true),
                caption: ufa.getter('DIMENSION_CAPTION[\'#text\']', true),
                description: ufa.getter('DESCRIPTION[\'#text\']', true),
                uniqueName: ufa.getter('DIMENSION_UNIQUE_NAME[\'#text\']', true),
                defaultHierarchy: ufa.getter('DEFAULT_HIERARCHY[\'#text\']', true),
                type: ufa.getter('DIMENSION_TYPE[\'#text\']', true)
            },
            hierarchies: {
                name: ufa.getter('HIERARCHY_NAME[\'#text\']', true),
                caption: ufa.getter('HIERARCHY_CAPTION[\'#text\']', true),
                description: ufa.getter('DESCRIPTION[\'#text\']', true),
                uniqueName: ufa.getter('HIERARCHY_UNIQUE_NAME[\'#text\']', true),
                dimensionUniqueName: ufa.getter('DIMENSION_UNIQUE_NAME[\'#text\']', true),
                displayFolder: ufa.getter('HIERARCHY_DISPLAY_FOLDER[\'#text\']', true),
                origin: ufa.getter('HIERARCHY_ORIGIN[\'#text\']', true),
                defaultMember: ufa.getter('DEFAULT_MEMBER[\'#text\']', true)
            },
            levels: {
                name: ufa.getter('LEVEL_NAME[\'#text\']', true),
                caption: ufa.getter('LEVEL_CAPTION[\'#text\']', true),
                description: ufa.getter('DESCRIPTION[\'#text\']', true),
                uniqueName: ufa.getter('LEVEL_UNIQUE_NAME[\'#text\']', true),
                dimensionUniqueName: ufa.getter('DIMENSION_UNIQUE_NAME[\'#text\']', true),
                displayFolder: ufa.getter('LEVEL_DISPLAY_FOLDER[\'#text\']', true),
                orderingProperty: ufa.getter('LEVEL_ORDERING_PROPERTY[\'#text\']', true),
                origin: ufa.getter('LEVEL_ORIGIN[\'#text\']', true),
                hierarchyUniqueName: ufa.getter('HIERARCHY_UNIQUE_NAME[\'#text\']', true)
            },
            members: {
                name: ufa.getter('MEMBER_NAME[\'#text\']', true),
                caption: ufa.getter('MEMBER_CAPTION[\'#text\']', true),
                uniqueName: ufa.getter('MEMBER_UNIQUE_NAME[\'#text\']', true),
                dimensionUniqueName: ufa.getter('DIMENSION_UNIQUE_NAME[\'#text\']', true),
                hierarchyUniqueName: ufa.getter('HIERARCHY_UNIQUE_NAME[\'#text\']', true),
                levelUniqueName: ufa.getter('LEVEL_UNIQUE_NAME[\'#text\']', true),
                childrenCardinality: ufa.getter('CHILDREN_CARDINALITY[\'#text\']', true)
            }
        };
        ...
        var XmlaDataReader = ufa.data.XmlDataReader.extend({
            init: function (options) {
                ufa.data.XmlDataReader.call(this, options);
            ...
            parse: function (xml) {
                var result = ufa.data.XmlDataReader.fn.parse(xml.replace(/<(\/?)(\w|-)+:/g, '<$1'));
                return ufa.getter('[\'Envelope\'][\'Body\']', true)(result);
            },
            errors: function (root) {
                var fault = ufa.getter('[\'Fault\']', true)(root);
                if (fault) {
                    return [{
                            faultstring: ufa.getter('faultstring[\'#text\']', true)(fault),
                            faultcode: ufa.getter('faultcode[\'#text\']', true)(fault)
                        }];
                }
                return null;
            },
            axes: function (root) {
                root = ufa.getter('ExecuteResponse["return"].root', true)(root);
                var axes = asArray(ufa.getter('Axes.Axis', true)(root));
                var axis;
                var result = {
                    columns: {},
                    rows: {}
            ...
            data: function (root) {
                root = ufa.getter('ExecuteResponse["return"].root', true)(root);
                var cells = asArray(ufa.getter('CellData.Cell', true)(root));
                var result = [];
                var ordinalGetter = ufa.getter('[\'@CellOrdinal\']');
                var valueGetter = ufa.getter('Value[\'#text\']');
                var fmtValueGetter = ufa.getter('FmtValue[\'#text\']');
            ...
            _mapSchema: function (root, getters) {
                root = ufa.getter('DiscoverResponse["return"].root', true)(root);
                var rows = asArray(ufa.getter('row', true)(root));
                var result = [];
        ...
        ufa.ui.PivotSettingTarget = Widget.extend({
            init: function (element, options) {
                var that = this;
                Widget.fn.init.call(that, element, options);
                that.element.addClass('k-pivot-setting');
                that.dataSource = ufa.data.PivotDataSource.create(options.dataSource);
                that._refreshHandler = $.proxy(that.refresh, that);
                that.dataSource.first(CHANGE, that._refreshHandler);
                if (!options.template) {
                    that.options.template = '<div data-' + ufa.ns + 'name="${data.name || data}">${data.name || data}' + (that.options.enabled ? '<a class="k-button k-button-icon k-button-bare"><span class="k-icon k-setting-delete"></span></a>' : '') + '</div>';
                }
                that.template = ufa.template(that.options.template);
                that.emptyTemplate = ufa.template(that.options.emptyTemplate);
                that._sortable();
                that.element.on('click' + NS, '.k-button,.k-item', function (e) {
                    var target = $(e.target);
                    var name = target.closest('[' + ufa.attr('name') + ']').attr(ufa.attr('name'));
                    if (!name) {
                        return;
                    }
            ...
            _sortable: function () {
                ...
                if (that.options.enabled) {
                    this.sortable = this.element.ufaSortable({
                        ...
                        change: function (e) {
                            var name = e.item.attr(ufa.attr('name'));
                            if (e.action == 'receive') {
                                that.add(name);
        ...
        var PivotGrid = Widget.extend({
            init: function (element, options) {
                ...
                that.columnsHeader.add(that.rowsHeader).on('click', 'span.k-icon', function () {
                    ...
                    var eventName;
                    var path = button.attr(ufa.attr('path'));
                    var eventArgs = {
                        axis: 'columns',
                        path: $.parseJSON(path)
                    };
            ...
            _templates: function () {
                ...
                this._columnBuilder.template = ufa.template(columnTemplate || HEADER_TEMPLATE, { useWithBlock: !!columnTemplate });
                this._contentBuilder.dataTemplate = ufa.template(dataTemplate || DATACELL_TEMPLATE, { useWithBlock: !!dataTemplate });
                this._contentBuilder.kpiStatusTemplate = ufa.template(kpiStatusTemplate || KPISTATUS_TEMPLATE, { useWithBlock: !!kpiStatusTemplate });
                this._contentBuilder.kpiTrendTemplate = ufa.template(kpiTrendTemplate || KPITREND_TEMPLATE, { useWithBlock: !!kpiTrendTemplate });
                this._rowBuilder.template = ufa.template(rowTemplate || HEADER_TEMPLATE, { useWithBlock: !!rowTemplate });
            },
            _bindConfigurator: function () {
                var configurator = this.options.configurator;
                if (configurator) {
                    $(configurator).ufaPivotConfigurator('setDataSource', this.dataSource);
                }
            },
            ...
            _dataSource: function () {
                ...
                that.dataSource = ufa.data.PivotDataSource.create(dataSource).bind(CHANGE, that._refreshHandler).bind(PROGRESS, that._progressHandler).bind(STATERESET, that._stateResetHandler).bind(ERROR, that._errorHandler);
            },
            _error: function () {
            ...
            _createSettingTarget: function (element, options) {
                var template = '<span tabindex="0" class="k-button" data-' + ufa.ns + 'name="${data.name}">${data.name}';
                ...
                template += '</span>';
                return new ufa.ui.PivotSettingTarget(element, $.extend({
                    template: template,
            ...
            _createLayout: function () {
                ...
                that.columnsHeader.parent().css('padding-right', ufa.support.scrollbar());
                ...
                that.columnsHeaderTree = new ufa.dom.Tree(that.columnsHeader[0]);
                that.rowsHeaderTree = new ufa.dom.Tree(that.rowsHeader[0]);
                that.contentTree = new ufa.dom.Tree(that.content[0]);
                that._initSettingTargets();
            },
            _progress: function (toggle) {
                ufa.ui.progress(this.wrapper, toggle);
            },
            _resize: function () {
            ...
            _columnHeaderReflow: function () {
                ...
                if (!ufa.support.browser.mozilla) {
                    return;
                }
                clearTimeout(this._headerReflowTimeout);
            ...
            refresh: function () {
                ...
                if (that.touchScroller) {
                    ...
                    var touchScroller = ufa.touchScroller(that.content);
        ...
        var ColumnBuilder = Class.extend({
            ...
            _buildRows: function (tuple, memberIdx, parentMember) {
                ...
                path = ufa.stringify(buildPath(tuple, memberIdx));
        ...
        var RowBuilder = Class.extend({
            ...
            _buildRows: function (tuple, memberIdx) {
                ...
                if (member.hasChildren) {
                    ...
                    expandIconAttr[ufa.attr('path')] = path;
        ...
        ui.plugin(PivotGrid);
        ufa.PivotExcelExporter = ufa.Class.extend({
            init: function (options) {
        ...
        var PivotExcelMixin = {
            ...
            saveAsExcel: function () {
                ...
                var exporter = new ufa.PivotExcelExporter({ widget: this });
                exporter.workbook().then($.proxy(function (book) {
                    if (!this.trigger('excelExport', { workbook: book })) {
                        var workbook = new ufa.ooxml.Workbook(book);
                        ufa.saveAs({
        ...
        ufa.PivotExcelMixin = PivotExcelMixin;
        if (ufa.ooxml && ufa.ooxml.Workbook) {
            PivotExcelMixin.extend(PivotGrid.prototype);
        }
        if (ufa.PDFMixin) {
            ufa.PDFMixin.extend(PivotGrid.prototype);
