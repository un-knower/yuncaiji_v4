...
(function (f, define) {
    ...
    (function ($, undefined) {
        var ufa = window.ufa, ui = ufa.ui, DataSource = ufa.data.DataSource, tbodySupportsInnerHtml = ufa.support.tbodyInnerHtml, activeElement = ufa._activeElement, Widget = ui.Widget, keys = ufa.keys, isPlainObject = $.isPlainObject, extend = $.extend, map = $.map, grep = $.grep, isArray = $.isArray, inArray = $.inArray, push = Array.prototype.push, proxy = $.proxy, isFunction = ufa.isFunction, isEmptyObject = $.isEmptyObject, math = Math, PROGRESS = 'progress', ERROR = 'error', DATA_CELL = ':not(.k-group-cell):not(.k-hierarchy-cell):visible', SELECTION_CELL_SELECTOR = 'tbody>tr:not(.k-grouping-row):not(.k-detail-row):not(.k-group-footer) > td:not(.k-group-cell):not(.k-hierarchy-cell)', NAVROW = 'tr:not(.k-footer-template):visible', NAVCELL = ':not(.k-group-cell):not(.k-hierarchy-cell):visible', FIRSTNAVITEM = NAVROW + ':first>' + NAVCELL + ':first', HEADERCELLS = 'th.k-header:not(.k-group-cell):not(.k-hierarchy-cell)', NS = '.ufaGrid', EDIT = 'edit', SAVE = 'save', REMOVE = 'remove', DETAILINIT = 'detailInit', FILTERMENUINIT = 'filterMenuInit', COLUMNMENUINIT = 'columnMenuInit', CHANGE = 'change', COLUMNHIDE = 'columnHide', COLUMNSHOW = 'columnShow', SAVECHANGES = 'saveChanges', DATABOUND = 'dataBound', DETAILEXPAND = 'detailExpand', DETAILCOLLAPSE = 'detailCollapse', FOCUSED = 'k-state-focused', SELECTED = 'k-state-selected', NORECORDSCLASS = 'k-grid-norecords', COLUMNRESIZE = 'columnResize', COLUMNREORDER = 'columnReorder', COLUMNLOCK = 'columnLock', COLUMNUNLOCK = 'columnUnlock', NAVIGATE = 'navigate', CLICK = 'click', HEIGHT = 'height', TABINDEX = 'tabIndex', FUNCTION = 'function', STRING = 'string', DELETECONFIRM = 'Are you sure you want to delete this record?', NORECORDS = 'No records available.', CONFIRMDELETE = 'Delete', CANCELDELETE = 'Cancel', formatRegExp = /(\}|\#)/gi, templateHashRegExp = /#/gi, whitespaceRegExp = '[\\x20\\t\\r\\n\\f]', nonDataCellsRegExp = new RegExp('(^|' + whitespaceRegExp + ')' + '(k-group-cell|k-hierarchy-cell)' + '(' + whitespaceRegExp + '|$)'), filterRowRegExp = new RegExp('(^|' + whitespaceRegExp + ')' + '(k-filter-row)' + '(' + whitespaceRegExp + '|$)'), COMMANDBUTTONTMPL = '<a class="k-button k-button-icontext #=className#" #=attr# href="\\#"><span class="#=iconClass# #=imageClass#"></span>#=text#</a>', isRtl = false, browser = ufa.support.browser, isIE7 = browser.msie && browser.version == 7, isIE8 = browser.msie && browser.version == 8;
        var VirtualScrollable = Widget.extend({
            ...
            wrap: function () {
                var that = this, scrollbar = ufa.support.scrollbar() + 1, element = that.element, wrapper;
                element.css({
                    width: 'auto',
                    overflow: 'hidden'
                ...
                if (ufa.support.kineticScrollNeeded) {
                    that.drag = new ufa.UserEvents(that.wrapper, {
                        global: true,
                        start: function (e) {
                            e.sender.capture();
                        },
            ...
            _wheelScroll: function (e) {
                ...
                var scrollbar = this.verticalScrollbar, scrollTop = scrollbar.scrollTop(), delta = ufa.wheelDeltaY(e);
            ...
            _page: function (skip, take) {
                ...
                if (dataSource.inRange(skip, take)) {
                    ...
                    if (!delayLoading) {
                        ufa.ui.progress(that.wrapper.parent(), true);
                    }
            ...
            repaintScrollbar: function () {
                var that = this, html = '', maxHeight = 250000, dataSource = that.dataSource, scrollbar = !ufa.support.kineticScrollNeeded ? ufa.support.scrollbar() : 0, wrapperElement = that.wrapper[0], totalHeight, idx, itemHeight;
            ...
            refresh: function () {
                ...
                ufa.ui.progress(that.wrapper.parent(), false);
                clearTimeout(that._timeout);
        ...
        function updateCellIndex(thead, columns, offset) {
            ...
            for (var idx = 0, length = columns.length; idx < length; idx++) {
                ...
                cell.attr(ufa.attr('index'), offset + idx);
            }
        ...
        function childColumnsCells(cell) {
            ...
            var colSpanAttr = ufa.attr('colspan');
        ...
        function normalizeCols(table, visibleColumns, hasDetails, groups) {
            var colgroup = table.find('>colgroup'), width, cols = map(visibleColumns, function (column) {
                    ...
                    if (width && parseInt(width, 10) !== 0) {
                        return ufa.format('<col style="width:{0}"/>', typeof width === STRING ? width : width + 'px');
                    }
                    return '<col />';
                });
        ...
        function formatGroupValue(value, format, columnValues, encoded) {
            ...
            return format ? ufa.format(format, groupValue) : encoded === false ? groupValue : ufa.htmlEncode(groupValue);
        }
        ...
        var Grid = ufa.ui.DataBoundWidget.extend({
            init: function (element, options, events) {
                ...
                isRtl = ufa.support.isRtl(element);
                that._element();
                that._aria();
            ...
            destroy: function () {
                ...
                if (that.groupable && that.groupable.element) {
                    that.groupable.element.ufaGroupable('destroy');
                }
                that.groupable = null;
                if (that.options.reorderable) {
                    that.wrapper.data('ufaReorderable').destroy();
                }
            ...
            getOptions: function () {
                ...
                result.columns = ufa.deepExtend([], this.columns);
            ...
            setOptions: function (options) {
                ...
                ufa.deepExtend(currentOptions, options);
                ...
                this.options = null;
                if (this._isMobile) {
                    var mobileWrapper = wrapper.closest(ufa.roleSelector('pane')).parent();
            ...
            dataItems: function () {
                var dataItems = ufa.ui.DataBoundWidget.fn.dataItems.call(this);
                if (this.lockedContent) {
            ...
            _destroyColumnAttachments: function () {
                ...
                that.thead.add(that.lockedHeader).find('th').each(function () {
                    var th = $(this), filterMenu = th.data('ufaFilterMenu'), sortable = th.data('ufaColumnSorter'), columnMenu = th.data('ufaColumnMenu');
                    if (filterMenu) {
                        filterMenu.destroy();
                    }
                    if (sortable) {
            ...
            _positionColumnResizeHandleTouch: function () {
                ...
                that._resizeUserEvents = new ufa.UserEvents(lockedHead.add(that.thead), {
            ...
            _resizable: function () {
                var that = this, options = that.options, container, columnStart, columnWidth, gridWidth, isMobile = this._isMobile, scrollbar = !ufa.support.mobileOS ? ufa.support.scrollbar() : 0, isLocked, col, th;
                    ...
                    that.resizable = new ui.Resizable(container.add(that.lockedHeader), {
                        ...
                        resizeend: function () {
                            ...
                            if (columnWidth != newWidth) {
                                ...
                                var index = th.attr(ufa.attr('index'));
                                if (!index) {
            ...
            _draggable: function () {
                ...
                if (that.options.reorderable) {
                    ...
                    that._draggableInstance = that.wrapper.ufaDraggable({
                        group: ufa.guid(),
                        autoScroll: true,
                        ...
                        hint: function (target) {
                            ...
                            }).html(target.attr(ufa.attr('title')) || target.attr(ufa.attr('field')) || target.text()).prepend('<span class="k-icon k-drag-status k-denied" />');
                        }
                    }).data('ufaDraggable');
                }
            },
            _reorderable: function () {
                var that = this;
                if (that.options.reorderable) {
                    if (that.wrapper.data('ufaReorderable')) {
                        that.wrapper.data('ufaReorderable').destroy();
                    }
            ...
            _modelForContainer: function (container) {
                ...
                var id = container.attr(ufa.attr('uid'));
            ...
            editCell: function (cell) {
                ...
                if (model && (!model.editable || model.editable(column.field)) && !column.command && column.field) {
                    ...
                    that.editable = cell.addClass('k-edit-cell').ufaEditable({
                        fields: {
                            field: column.field,
            ...
            _adjustLockedHorizontalScrollBar: function () {
                ...
                var scrollbar = table[0].offsetWidth > content[0].clientWidth ? ufa.support.scrollbar() : 0;
            ...
            _destroyEditable: function () {
                ...
                if (that.editable) {
                    if (that._editMode() === 'popup' && !that._isMobile) {
                        that._editContainer.data('ufaWindow').bind('deactivate', destroy).close();
                    } else {
                        destroy();
                    }
                }
            ...
            closeCell: function (isCancel) {
                ...
                id = cell.closest('tr').attr(ufa.attr('uid'));
            ...
            _displayCell: function (cell, column, dataItem) {
                var that = this, state = {
                        storage: {},
                        count: 0
                    }, settings = extend({}, ufa.Template, that.options.templateSettings), tmpl = ufa.template(that._cellTmpl(column, state), settings);
                if (state.count > 0) {
            ...
            editRow: function (row) {
                var model;
                var that = this;
                if (row instanceof ufa.data.ObservableObject) {
                    model = row;
                } else {
                    row = $(row);
                    model = that._modelForContainer(row);
                ...
                that.cancelRow();
                if (model) {
                    row = that.tbody.children('[' + ufa.attr('uid') + '=' + model.uid + ']');
                    that._attachModelChange(model);
                    if (mode === 'popup') {
            ...
            _createPopupEditor: function (model) {
                var that = this, html = '<div ' + ufa.attr('uid') + '="' + model.uid + '" class="k-popup-edit-form' + (that._isMobile ? ' k-mobile-list' : '') + '"><div class="k-edit-form-container">', column, command, fields = [], idx, length, tmpl, updateText, cancelText, tempCommand, columns = leafColumns(that.columns), attr, editable = that.options.editable, template = editable.template, options = isPlainObject(editable) ? editable.window : {}, settings = extend({}, ufa.Template, that.options.templateSettings);
                options = options || {};
                if (template) {
                    ...
                    for (idx = 0, length = columns.length; idx < length; idx++) {
                        ...
                        if (!column.command) {
                            ...
                            if ((!model.editable || model.editable(column.field)) && column.field) {
                                ...
                                html += '<div ' + ufa.attr('container-for') + '="' + column.field + '" class="k-edit-field"></div>';
                            } else {
                                var state = {
                                    storage: {},
                                    count: 0
                                };
                                tmpl = ufa.template(that._cellTmpl(column, state), settings);
                                if (state.count > 0) {
                                    tmpl = proxy(tmpl, state.storage);
                                }
                ...
                if (!that._isMobile) {
                    ...
                    html += '</div></div></div>';
                    container = that._editContainer = $(html).appendTo(that.wrapper).eq(0).ufaWindow(extend({
                        modal: true,
                        resizable: false,
                        draggable: true,
                        title: 'Edit',
                    ...
                    html += '</div></div>';
                    that.editView = that.pane.append('<div data-' + ufa.ns + 'role="view" data-' + ufa.ns + 'use-native-scrolling="true" data-' + ufa.ns + 'init-widgets="false" class="k-grid-edit-form">' + '<div data-' + ufa.ns + 'role="header" class="k-header">' + that._createButton({
                        name: 'update',
                        text: updateText,
                        attr: attr
                ...
                that.editable = that._editContainer.ufaEditable({
                    fields: fields,
                    model: model,
            ...
            _openPopUpEditor: function () {
                if (!this._isMobile) {
                    this._editContainer.data('ufaWindow').center().open();
                } else {
            ...
            _createInlineEditor: function (row, model) {
                ...
                row.children(':not(.k-group-cell,.k-hierarchy-cell)').each(function () {
                    ...
                    if (!column.command && column.field && (!model.editable || model.editable(column.field))) {
                        ...
                        cell.attr(ufa.attr('container-for'), column.field);
                        cell.empty();
                    } else if (column.command) {
                ...
                that.editable = new ufa.ui.Editable(row.addClass('k-grid-edit-row'), {
                    target: that,
                    fields: fields,
                    model: model,
                    clearContainer: false
            ...
            cancelRow: function (notify) {
                ...
                if (container) {
                    ...
                    if (that._editMode() !== 'popup') {
                        ...
                        that._displayRow(that.tbody.find('[' + ufa.attr('uid') + '=' + model.uid + ']'));
                    }
                }
            },
            saveRow: function () {
            ...
            _showMessage: function (messages, row) {
                ...
                var template = ufa.template('<ul>' + '<li class="km-actionsheet-title">#:title#</li>' + '<li><a href="\\#" class="k-button k-grid-delete">#:confirmDelete#</a></li>' + '</ul>');
                var html = $(template(messages)).appendTo(that.view.element);
                var actionSheet = that._actionSheet = new ufa.mobile.ui.ActionSheet(html, {
            ...
            addRow: function () {
                ...
                if (that.editable && that.editable.end() || !that.editable) {
                    ...
                    var model = dataSource.insert(index, {}), id = model.uid, table = that.lockedContent ? that.lockedTable : that.table, row = table.find('tr[' + ufa.attr('uid') + '=' + id + ']'), cell = row.children('td:not(.k-group-cell,.k-hierarchy-cell)').eq(that._firstEditableColumnIndex(row));
                ...
                if (toolbar) {
                    ...
                    if (!container.length) {
                        if (!isFunction(toolbar)) {
                            ...
                            toolbar = proxy(ufa.template(toolbar), that);
                        }
            ...
            _attachGroupable: function () {
                var that = this, wrapper = that.wrapper, groupable = that.options.groupable, draggables = HEADERCELLS + '[' + ufa.attr('field') + ']', filter = that.content ? '.k-grid-header:first ' + draggables : 'table:first>.k-grid-header ' + draggables;
            ...
            _selectable: function () {
                ...
                if (selectable) {
                    ...
                    selectable = ufa.ui.Selectable.parseOptions(selectable);
                    ...
                    that.selectable = new ufa.ui.Selectable(elements, {
                        filter: filter,
                        aria: true,
                        multiple: multi,
                        change: function () {
            ...
            _scrollTo: function (element, container) {
                ...
                if (isRtl && isHorizontal) {
                    ...
                    if (browser.msie) {
                        ...
                        firefoxCorrection = table.offsetLeft - ufa.support.scrollbar();
                    }
                }
            ...
            _navigatable: function () {
                ...
                tables.on(ufa.support.touch ? 'touchstart' + NS : 'mousedown' + NS, NAVROW + '>' + NAVCELL, proxy(tableClick, that)).on('focus' + NS, proxy(that._tableFocus, that)).on('focusout' + NS, proxy(that._tableBlur, that)).on('keydown' + NS, proxy(that._tableKeyDown, that));
            },
            ...
            _handleEnterKey: function (current, currentTable, target) {
                ...
                var focusable = current.find(':ufaFocusable:first');
            ...
            _handleEditing: function (current, next, table) {
                ...
                if (that.editable) {
                    ...
                    if (that.editable.end()) {
                        ...
                        focusable = editContainer.find(':ufaFocusable:first')[0];
                        if (focusable) {
                            focusable.focus();
                        }
                        return;
            ...
            _initMobile: function () {
                var options = this.options;
                var that = this;
                this._isMobile = options.mobile === true && ufa.support.mobileOS || options.mobile === 'phone' || options.mobile === 'tablet';
                if (this._isMobile) {
                    var html = this.wrapper.addClass('k-grid-mobile').wrap('<div data-' + ufa.ns + 'stretch="true" data-' + ufa.ns + 'role="view" ' + 'data-' + ufa.ns + 'init-widgets="false"></div>').parent();
                    this.pane = ufa.mobile.ui.Pane.wrap(html);
            ...
            _scrollable: function () {
                var that = this, header, table, options = that.options, scrollable = options.scrollable, hasVirtualScroll = scrollable !== true && scrollable.virtual && !that.virtualScrollable, scrollbar = !ufa.support.kineticScrollNeeded || hasVirtualScroll ? ufa.support.scrollbar() : 0;
                if (scrollable) {
                    ...
                    if (scrollable.virtual) {
                        ...
                        var touchScroller = that.content.data('ufaTouchScroller');
                        if (touchScroller) {
                            touchScroller.destroy();
                        }
                        touchScroller = ufa.touchScroller(that.content);
            ...
            _applyLockedContainersWidth: function () {
                if (this.options.scrollable && this.lockedHeader) {
                    var headerTable = this.thead.parent(), headerWrap = headerTable.parent(), contentWidth = this.wrapper[0].clientWidth, groups = this._groups(), scrollbar = ufa.support.scrollbar(), cols = this.lockedHeader.find('>table>colgroup>col:not(.k-group-col, .k-hierarchy-col)'), nonLockedCols = headerTable.find('>colgroup>col:not(.k-group-col, .k-hierarchy-col)'), width = columnsWidth(cols), nonLockedColsWidth = columnsWidth(nonLockedCols), footerWrap;
                    if (groups > 0) {
            ...
            _setContentHeight: function () {
                var that = this, options = that.options, height = that.wrapper.innerHeight(), header = that.wrapper.children('.k-grid-header'), scrollbar = ufa.support.scrollbar();
            ...
            _modelChange: function (e) {
                var that = this, tbody = that.tbody, model = e.model, row = that.tbody.find('tr[' + ufa.attr('uid') + '=' + model.uid + ']'), relatedRow, cell, column, isAlt = row.hasClass('k-alt'), tmp, idx = that._items(tbody).index(row), isLocked = that.lockedContent, selectable, selectableRow, childCells, originalCells, length;
                if (isLocked) {
            ...
            _pageable: function () {
                ...
                if (pageable) {
                    ...
                    if (typeof pageable === 'object' && pageable instanceof ufa.ui.Pager) {
                        that.pager = pageable;
                    } else {
                        that.pager = new ufa.ui.Pager(wrapper, extend({}, pageable, { dataSource: that.dataSource }));
                    }
                }
            },
            _footer: function () {
            ...
            _wrapFooter: function (footerRow) {
                var that = this, html = '', scrollbar = !ufa.support.mobileOS ? ufa.support.scrollbar() : 0;
                if (that.options.scrollable) {
            ...
            _columnMenu: function () {
                ...
                if (columnMenu) {
                    ...
                    for (var idx = 0, length = cells.length; idx < length; idx++) {
                        ...
                        if (!column.command && (column.field || cell.attr('data-' + ufa.ns + 'field'))) {
                            menu = cell.data('ufaColumnMenu');
                            if (menu) {
                                menu.destroy();
                            }
                            ...
                            cell.ufaColumnMenu(menuOptions);
                        }
                    }
                }
            },
            ...
            _filterable: function () {
                ...
                if (filterable && !that.options.columnMenu) {
                    ...
                    for (var idx = 0, length = cells.length; idx < length; idx++) {
                        cell = cells.eq(idx);
                        if (columns[idx].filterable !== false && !columns[idx].command && (columns[idx].field || cell.attr('data-' + ufa.ns + 'field'))) {
                            filterMenu = cell.data('ufaFilterMenu');
                            if (filterMenu) {
                                filterMenu.destroy();
                            }
                            filterMenu = cell.data('ufaFilterMultiCheck');
                            if (filterMenu) {
                                filterMenu.destroy();
                            }
                            ...
                            if (columnFilterable && columnFilterable.multi) {
                                cell.ufaFilterMultiCheck(options);
                            } else {
                                cell.ufaFilterMenu(options);
                            }
                        }
                    }
                }
            ...
            _filterRow: function () {
                ...
                for (var i = 0; i < columns.length; i++) {
                    ...
                    if (field && col.filterable !== false) {
                        ...
                        $('<span/>').attr(ufa.attr('field'), field).appendTo(th).ufaFilterCell(settings);
                    } else {
                        th.html('&nbsp;');
                    }
                }
                ...
                if (sortable) {
                    ...
                    for (var idx = 0, length = cells.length; idx < length; idx++) {
                        ...
                        if (column.sortable !== false && !column.command && column.field) {
                            cell = cells.eq(idx);
                            sorterInstance = cell.data('ufaColumnSorter');
                            if (sorterInstance) {
                                sorterInstance.destroy();
                            }
                            cell.attr('data-' + ufa.ns + 'field', column.field).ufaColumnSorter(extend({}, sortable, column.sortable, {
                                dataSource: that.dataSource,
                                aria: true,
            ...
            _columns: function (columns) {
                ...
                columns = columns.length ? columns : map(table.find('th'), function (th, idx) {
                    th = $(th);
                    var sortable = th.attr(ufa.attr('sortable')), filterable = th.attr(ufa.attr('filterable')), type = th.attr(ufa.attr('type')), groupable = th.attr(ufa.attr('groupable')), field = th.attr(ufa.attr('field')), title = th.attr(ufa.attr('title')), menu = th.attr(ufa.attr('menu'));
                    if (!field) {
                    ...
                    return {
                        ...
                        menu: menu,
                        template: th.attr(ufa.attr('template')),
            ...
            _tmpl: function (rowTemplate, columns, alt, skipGroupCells) {
                var that = this, settings = extend({}, ufa.Template, that.options.templateSettings), idx, length = columns.length, template, state = {
                        storage: {},
                        count: 0
                ...
                if (!rowTemplate) {
                    ...
                    if (length) {
                        rowTemplate += ' ' + ufa.attr('uid') + '="#=' + ufa.expr('uid', settings.paramName) + '#"';
                    }
                ...
                rowTemplate = ufa.template(rowTemplate, settings);
                if (state.count > 0) {
            ...
            _headerCellText: function (column) {
                var that = this, settings = extend({}, ufa.Template, that.options.templateSettings), template = column.headerTemplate, type = typeof template, text = column.title || column.field || '';
                if (type === FUNCTION) {
                    text = ufa.template(template, settings)({});
                } else if (type === STRING) {
                    text = template;
                }
                return text;
            ...
            _cellTmpl: function (column, state) {
                var that = this, settings = extend({}, ufa.Template, that.options.templateSettings), template = column.template, paramName = settings.paramName, field = column.field, html = '', idx, length, format = column.format, type = typeof template, columnValues = column.values;
                if (column.command) {
                ...
                if (type === FUNCTION) {
                    ...
                    html += '#var v =' + ufa.stringify(convertToObject(columnValues)).replace(templateHashRegExp, '\\#') + '#';
                    html += '#var f = v[';
                    ...
                    html += column.encoded ? '#:' : '#=';
                    if (format) {
                        html += 'ufa.format("' + format.replace(formatRegExp, '\\$1') + '",';
                    }
                    if (field) {
                        field = ufa.expr(field, paramName);
            ...
            _noRecordsTmpl: function () {
                ...
                if (this.options.noRecords.template) {
                    ...
                    template = ufa.format(defaultTemplate, this.options.messages.noRecords, scrollableNoGridHeightStyles);
                }
                type = typeof template;
                if (type === 'function') {
                    state.storage['tmpl' + state.count] = template;
                ...
                tmpl = ufa.template(ufa.format(wrapper, NORECORDSCLASS, html), settings);
                if (state.count > 0) {
                    tmpl = $.proxy(tmpl, state.storage);
                }
                return tmpl;
            ...
            _footerTmpl: function (columns, aggregates, templateName, rowClass, skipGroupCells) {
                var that = this, settings = extend({}, ufa.Template, that.options.templateSettings), paramName = settings.paramName, html = '', idx, length, template, type, storage = {}, count = 0, scope = {}, groups = that._groups(), fieldsMap = that.dataSource._emptyAggregates(aggregates), column;
                ...
                for (idx = 0, length = columns.length; idx < length; idx++) {
                    ...
                    if (template) {
                        if (type !== FUNCTION) {
                            ...
                            template = ufa.template(template, scope);
                        }
                ...
                html += '</tr>';
                html = ufa.template(html, settings);
                if (count > 0) {
            ...
            _detailTmpl: function (template) {
                var that = this, html = '', settings = extend({}, ufa.Template, that.options.templateSettings), paramName = settings.paramName, templateFunctionStorage = {}, templateFunctionCount = 0, groups = that._groups(), colspan = visibleColumns(leafColumns(that.columns)).length, type = typeof template;
                ...
                for (idx = 0, length = columns.length; idx < length; idx++) {
                    ...
                    if (!th.command) {
                        ...
                        if (columns[idx].colSpan > 1) {
                            ...
                            html += ufa.attr('colspan') + '=\'' + columns[idx].colSpan + '\'';
                        }
                        if (th.title) {
                            html += ufa.attr('title') + '="' + th.title.replace('"', '&quot;').replace(/'/g, '\'') + '" ';
                        }
                        if (th.groupable !== undefined) {
                            html += ufa.attr('groupable') + '=\'' + th.groupable + '\' ';
                        }
                        if (th.aggregates && th.aggregates.length) {
                            html += ufa.attr('aggregates') + '=\'' + th.aggregates + '\'';
                        }
                        if (index > -1) {
                            html += ufa.attr('index') + '=\'' + index + '\'';
                        }
            ...
            _wheelScroll: function (e) {
                ...
                var scrollTop = content.scrollTop(), delta = ufa.wheelDeltaY(e);
                if (delta) {
                    e.preventDefault();
                    $(e.currentTarget).one('wheel' + NS, false);
            ...
            _groupRowHtml: function (group, colspan, level, groupHeaderBuilder, templates, skipColspan) {
                ...
                if (template) {
                    text = typeof template === FUNCTION ? template(data) : ufa.template(template)(data);
                }
            ...
            _setParentsVisibility: function (column, visible) {
                ...
                if (columnParents(column, columns, parents) && parents.length) {
                    for (idx = parents.length - 1; idx >= 0; idx--) {
                        ...
                        if (cell.filter('[' + ufa.attr('colspan') + ']').length) {
                            colSpan = parseInt(cell.attr(ufa.attr('colspan')), 10);
            ...
            refresh: function (e) {
                ...
                that._angularItems('cleanup');
                if (navigatable && (that._isActiveInTable() || that._editContainer && that._editContainer.data('ufaWindow'))) {
            ...
            _angularItems: function (cmd) {
                ufa.ui.DataBoundWidget.fn._angularItems.call(this, cmd);
                if (cmd === 'cleanup') {
        ...
        if (ufa.ExcelMixin) {
            ufa.ExcelMixin.extend(Grid.prototype);
        }
        if (ufa.PDFMixin) {
            ufa.PDFMixin.extend(Grid.prototype);
            ...
            Grid.prototype._initPDFProgress = function (deferred) {
                ...
                var pb = $('<div class=\'k-loading-pdf-progress\'>').appendTo(loading).ufaProgressBar({
                    type: 'chunk',
                    chunkCount: 10,
                    min: 0,
                    max: 1,
        ...
        function tableClick(e) {
            ...
            if (isInput && currentTarget.find(ufa.roleSelector('filtercell')).length) {
            ...
            if (isHeader || !isInput) {
                setTimeout(function () {
                    if (!(isIE8 && $(ufa._activeElement()).hasClass('k-widget'))) {
                        if (!isInputElement(ufa._activeElement())) {
                            focusTable(currentTable, true);
                        }
                    }
                });
