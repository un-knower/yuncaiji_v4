    ...
    (function ($, undefined) {
        ...
        var Chart = Widget.extend({
            ...
            _attachEvents: function () {
                ...
                chart._mousemove = ufa.throttle(proxy(chart._mousemove, chart), MOUSEMOVE_DELAY);
                if (chart._shouldAttachMouseMove()) {
                    element.on(MOUSEMOVE_NS, chart._mousemove);
                }
                if (ufa.UserEvents) {
                    chart._userEvents = new ufa.UserEvents(element, {
                        global: true,
        ...
        var BarChart = CategoricalChart.extend({
            ...
            createPoint: function (data, fields) {
                ...
                if (ufa.isFunction(series.color)) {
        ...
        var BaseTooltip = Observable.extend({
            init: function (chartElement, options) {
                ...
                var mobileScrollerSelector = ufa.format('[{0}=\'content\'],[{0}=\'scroller\']', ufa.attr('role'));
                tooltip._mobileScroller = chartElement.closest(mobileScrollerSelector).data('ufaMobileScroller');
            },
            destroy: function () {
            ...
            _offset: function () {
                var tooltip = this, size = tooltip._measure(), anchor = tooltip.anchor, chartPadding = tooltip._padding(), chartOffset = tooltip.chartElement.offset(), top = round(anchor.y + chartPadding.top + chartOffset.top), left = round(anchor.x + chartPadding.left + chartOffset.left), zoomLevel = ufa.support.zoomLevel(), viewport = $(window), scrollTop = window.pageYOffset || document.documentElement.scrollTop || 0, scrollLeft = window.pageXOffset || document.documentElement.scrollLeft || 0, movable = (this._mobileScroller || {}).movable;
        ...
        var SharedTooltip = BaseTooltip.extend({
            ...
            _content: function (points, category) {
                ...
                template = ufa.template(tooltip.options.sharedTemplate);
                content = template({
                    points: points,
                    category: category,
                ...
                for (i = 0; i < seriesFields.length; i++) {
                    ...
                    if (value !== null && typeof value === OBJECT && !defined(value.length) && !(value instanceof Date)) {
                        ...
                        if (defined(value)) {
                            ...
                            ufa.setter(field.name)(result, value);
                        }
                    }
                }
                return result;
        ...
        function toDate(value) {
            ...
            if (value instanceof Date) {
                ...
                result = ufa.parseDate(value) || new Date(value);
            } else if (value) {
                if (isArray(value)) {
                    result = [];
        ...
        function addDuration(date, value, unit, weekStartDay) {
            ...
            if (date) {
                ...
                if (unit === YEARS) {
                    ...
                    ufa.date.adjustDST(result, 0);
                } else if (unit === MONTHS) {
                    result = new Date(date.getFullYear(), date.getMonth() + value, 1);
                    ufa.date.adjustDST(result, hours);
                } else if (unit === WEEKS) {
        ...
        function groupSeries(series, data) {
            ...
            if (defined(legacyTemplate)) {
                ufa.logToConsole('\'groupNameTemplate\' is obsolete and will be removed in future versions. ' + 'Specify the group name template as \'series.name\'');
                if (legacyTemplate) {
        ...
        function ensureTree(fieldName, target) {
            if (fieldName.indexOf('.') > -1) {
                ...
                while (parts.length > 1) {
                    path += parts.shift();
                    val = ufa.getter(path)(target) || {};
                    ufa.setter(path)(target, val);
                    path += '.';
                }
            }
        }
