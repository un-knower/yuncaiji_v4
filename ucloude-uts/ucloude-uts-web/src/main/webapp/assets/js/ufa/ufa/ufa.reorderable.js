(function (f, define) {
    ...
    (function ($, undefined) {
        var ufa = window.ufa, getOffset = ufa.getOffset, Widget = ufa.ui.Widget, CHANGE = 'change', KREORDERABLE = 'k-reorderable';
        ...
        var Reorderable = Widget.extend({
            init: function (element, options) {
                var that = this, draggable, group = ufa.guid() + '-reorderable';
                Widget.fn.init.call(that, element, options);
                element = that.element.addClass(KREORDERABLE);
                options = that.options;
                that.draggable = draggable = options.draggable || new ufa.ui.Draggable(element, {
                    group: group,
                    autoScroll: true,
                ...
                element.find(draggable.options.filter).ufaDropTarget({
            ...
            destroy: function () {
                ...
                that.element.find(that.draggable.options.filter).each(function () {
                    var item = $(this);
                    if (item.data('ufaDropTarget')) {
                        item.data('ufaDropTarget').destroy();
                    }
                });
                if (that.draggable) {
