(function (f, define) {
    ...
    (function ($, undefined) {
        ...
        var extend = $.extend;
        var ufaDom = ufa.dom;
        var ufaDomElement = ufaDom.element;
        var ufaTextElement = ufaDom.text;
        var ufaHtmlElement = ufaDom.html;
        var ui = ufa.ui;
        ...
        function createPlaceholders(options) {
            ...
            for (var i = 0, level = options.level; i < level; i++) {
                spans.push(ufaDomElement('span', { className: className }));
            }
            return spans;
        }
        ...
        var Editor = ufa.Observable.extend({
            ...
            destroy: function () {
                ...
                this.editable.element.find('[' + ufa.attr('container-for') + ']').empty().end().removeAttr(ufa.attr('role'));
        ...
        var PopupEditor = Editor.extend({
            ...
            _initContainer: function () {
                var options = this.options;
                var formContent = [];
                this.wrapper = $('<div class="k-popup-edit-form"/>').attr(ufa.attr('uid'), this.model.uid).append('<div class="k-edit-form-container"/>');
                if (options.template) {
                ...
                new ufaDom.Tree(this.wrapper.children()[0]).render(formContent);
            ...
            _appendTemplate: function (form) {
                ...
                form.push(ufaHtmlElement(template));
            },
            _appendFields: function (form) {
                ...
                for (idx = 0, length = columns.length; idx < length; idx++) {
                    ...
                    form.push(ufaHtmlElement('<div class="k-edit-label"><label for="' + column.field + '">' + (column.title || column.field || '') + '</label></div>'));
                    if (this._isEditable(column)) {
                        form.push(ufaHtmlElement('<div ' + ufa.attr('container-for') + '="' + column.field + '" class="k-edit-field"></div>'));
                    } else {
                        form.push(ufaDomElement('div', { 'class': 'k-edit-field' }, [this.options.fieldRenderer(column, this.model)]));
                    }
                }
            },
            _appendButtons: function (form) {
                form.push(ufaDomElement('div', { 'class': 'k-edit-buttons k-state-default' }, this.options.commandRenderer()));
            },
        ...
        var TreeList = DataBoundWidget.extend({
            ...
            _draggable: function () {
                ...
                this._dragging = new ufa.ui.HierarchicalDragAndDrop(this.wrapper, {
            ...
            itemFor: function (model) {
                ...
                return this.tbody.find('[' + ufa.attr('uid') + '=' + model.uid + ']');
            },
            _scrollable: function () {
                if (this.options.scrollable) {
                    ...
                    var touchScroller = ufa.touchScroller(this.content);
            ...
            _progress: function () {
                ...
                if (!this.tbody.find('tr').length) {
                    this._showStatus(ufa.template('<span class=\'#= className #\' /> #: messages.loading #')({
            ...
            _columnTemplates: function () {
                ...
                for (idx = 0, length = columns.length; idx < length; idx++) {
                    column = columns[idx];
                    if (column.template) {
                        column.template = ufa.template(column.template);
                    }
                    if (column.headerTemplate) {
                        column.headerTemplate = ufa.template(column.headerTemplate);
                    }
                    if (column.footerTemplate) {
                        column.footerTemplate = ufa.template(column.footerTemplate);
                    }
                }
            },
            ...
            _layout: function () {
                ...
                element.append(ufa.template(layout)(classNames) + '<div class=\'k-status\' />');
                ...
                if (this.options.scrollable) {
                    var rtl = ufa.support.isRtl(element);
                    element.find('div.' + classNames.gridHeader).css(rtl ? 'padding-left' : 'padding-right', ufa.support.scrollbar());
                }
            ...
            _initVirtualTrees: function () {
                this._headerColsTree = new ufaDom.Tree(this.thead.prev()[0]);
                this._contentColsTree = new ufaDom.Tree(this.tbody.prev()[0]);
                this._headerTree = new ufaDom.Tree(this.thead[0]);
                this._contentTree = new ufaDom.Tree(this.tbody[0]);
                this._statusTree = new ufaDom.Tree(this.element.children('.k-status')[0]);
                if (this.lockedHeader) {
                    this._lockedHeaderColsTree = new ufaDom.Tree(this.lockedHeader.find('colgroup')[0]);
                    this._lockedContentColsTree = new ufaDom.Tree(this.lockedTable.find('>colgroup')[0]);
                    this._lockedHeaderTree = new ufaDom.Tree(this.lockedHeader.find('thead')[0]);
                    this._lockedContentTree = new ufaDom.Tree(this.lockedTable.find('>tbody')[0]);
                }
            },
            _toolbar: function () {
                ...
                if ($.isArray(options)) {
                    ...
                    new ufaDom.Tree(toolbar[0]).render(buttons);
                } else {
                    toolbar.append(ufa.template(options)({}));
                }
                this.angular('compile', function () {
            ...
            _render: function (options) {
                ...
                this._flushCache();
                if (options.error) {
                    this._showStatus(ufa.template('#: messages.requestFailed # ' + '<button class=\'#= buttonClass #\'>#: messages.retry #</button>')({
                        buttonClass: [
                            classNames.button,
                            classNames.retry
                        ].join(' '),
                    ...
                    this._showStatus(ufa.htmlEncode(messages.noRows));
                } else {
                    this._hideStatus();
            ...
            _ths: function (columns) {
                ...
                for (var i = 0, length = columns.length; i < length; i++) {
                    ...
                    if (column.headerTemplate) {
                        headerContent = ufaHtmlElement(title);
                    } else {
                        headerContent = ufaTextElement(title);
                    }
                    if (column.sortable) {
                        children.push(ufaDomElement('a', {
                            href: '#',
                    ...
                    ths.push(ufaDomElement('th', attr, children));
                }
                return ths;
            },
            _cols: function (columns) {
                ...
                for (var i = 0; i < columns.length; i++) {
                    ...
                    cols.push(ufaDomElement('col', attr));
                }
                return cols;
            },
            ...
            _renderHeader: function () {
                var columns = this._nonLockedColumns();
                this._headerTree.render([ufaDomElement('tr', { 'role': 'row' }, this._ths(columns))]);
                if (this._hasLockedColumns) {
                    columns = this._lockedColumns();
                    this._lockedHeaderTree.render([ufaDomElement('tr', { 'role': 'row' }, this._ths(columns))]);
                    this._applyLockedContainersWidth();
                }
            },
            ...
            _footerTd: function (options) {
                ...
                content.push(ufaHtmlElement(template(aggregates) || ''));
                return ufaDomElement('td', attr, content);
            },
            ...
            _tds: function (options, columns, renderer) {
                ...
                return ufaDomElement('tr', options.attr, children);
            },
            _td: function (options) {
                ...
                if (model._edit && column.field && model.editable(column.field)) {
                    attr[ufa.attr('container-for')] = column.field;
                } else {
                    if (column.expandable) {
                        ...
                        children.push(ufaDomElement('span', { className: iconClass.join(' ') }));
                ...
                return ufaDomElement('td', attr, children);
            },
            _cellContent: function (column, model) {
                ...
                if (column.template) {
                    ...
                    if (value !== null && column.format) {
                        value = ufa.format(column.format, value);
                    }
                }
                ...
                if (column.template || !column.encoded) {
                    return ufaHtmlElement(value);
                } else {
                    return ufaTextElement(value);
                }
            },
            ...
            _button: function (command) {
                ...
                }, icon.concat([ufaTextElement(command.text || command.name)]));
            },
            ...
            autoFitColumn: function (column) {
                var that = this, options = that.options, columns = that.columns, index, browser = ufa.support.browser, th, headerTable, isLocked, visibleLocked = that.lockedHeader ? leafDataCells(that.lockedHeader.find('>table>thead')).filter(isCellVisible).length : 0, col;
            ...
            _resizable: function () {
                ...
                this.resizable = new ufa.ui.Resizable(this.wrapper, {
            ...
            _sortable: function () {
                ...
                for (idx = 0, length = cells.length; idx < length; idx++) {
                    ...
                    if (column.sortable !== false && !column.command && column.field) {
                        cell = cells.eq(idx);
                        sortableInstance = cell.data('ufaColumnSorter');
                        if (sortableInstance) {
                            sortableInstance.destroy();
                        }
                        cell.attr(fieldAttr, column.field).ufaColumnSorter(extend({}, sortable, column.sortable, { dataSource: this.dataSource }));
                    }
                }
            },
            _filterable: function () {
                ...
                for (idx = 0, length = cells.length; idx < length; idx++) {
                    ...
                    cell = cells.eq(idx);
                    filterMenuInstance = cell.data('ufaFilterMenu');
                    ...
                    cell.ufaFilterMenu(extend(true, {}, filterable, column.filterable, {
            ...
            _selectable: function () {
                ...
                if (selectable) {
                    ...
                    this.selectable = new ufa.ui.Selectable(element, {
                        filter: filter,
                        aria: true,
            ...
            dataItem: function (element) {
                ...
                var model = this.dataSource.getByUid(row.attr(ufa.attr('uid')));
                return model;
            },
            ...
            _columnMenu: function () {
                ...
                for (var i = 0; i < ths.length; i++) {
                    ...
                    menu = ths.eq(i).data('ufaColumnMenu');
                    if (menu) {
                        menu.destroy();
                    }
                    sortable = false;
                    ...
                    ths.eq(i).ufaColumnMenu(menuOptions);
                }
            },
        ...
        if (ufa.ExcelMixin) {
            ufa.ExcelMixin.extend(TreeList.prototype);
        }
        if (ufa.PDFMixin) {
            ufa.PDFMixin.extend(TreeList.prototype);
        ...
        extend(true, ufa.ui, { TreeList: TreeList });
        ui.plugin(TreeList);
    }(window.ufa.jQuery));
    return window.ufa;
