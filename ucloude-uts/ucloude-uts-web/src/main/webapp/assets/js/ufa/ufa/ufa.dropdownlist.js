(function (f, define) {
    ...
    (function ($, undefined) {
        var ufa = window.ufa, ui = ufa.ui, Select = ui.Select, support = ufa.support, activeElement = ufa._activeElement, ObservableObject = ufa.data.ObservableObject, keys = ufa.keys, ns = '.ufaDropDownList', DISABLED = 'disabled', READONLY = 'readonly', CHANGE = 'change', FOCUSED = 'k-state-focused', DEFAULT = 'k-state-default', STATEDISABLED = 'k-state-disabled', ARIA_DISABLED = 'aria-disabled', ARIA_READONLY = 'aria-readonly', HOVEREVENTS = 'mouseenter' + ns + ' mouseleave' + ns, TABINDEX = 'tabindex', STATE_FILTER = 'filter', STATE_ACCEPT = 'accept', MSG_INVALID_OPTION_LABEL = 'The `optionLabel` option is not valid due to missing fields. Define a custom optionLabel as shown here http://docs.telerik.com/ufa-ui/api/javascript/ui/dropdownlist#configuration-optionLabel', proxy = $.proxy;
        var DropDownList = Select.extend({
            ...
            _optionLabel: function () {
                ...
                if (!template) {
                    ...
                    if (typeof optionLabel === 'string') {
                        ...
                        template += ufa.expr(options.dataTextField, 'data');
                    }
                    template += '#';
                }
            ...
            _keypress: function (e) {
                var that = this;
                if (e.which === 0 || e.keyCode === ufa.keys.ENTER) {
                    return;
                }
            ...
            _popupOpen: function () {
                ...
                popup.wrapper = ufa.wrap(popup.element);
            ...
            _inputTemplate: function () {
                ...
                if (!template) {
                    template = $.proxy(ufa.template('#:this._text(data)#', { useWithBlock: false }), that);
                } else {
                    template = ufa.template(template);
                }
