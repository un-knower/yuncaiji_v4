(function (f, define) {
    ...
    (function ($, undefined) {
        var ufa = window.ufa, Class = ufa.Class, Widget = ufa.ui.Widget, proxy = $.proxy, template = ufa.template, keys = ufa.keys, isFunction = $.isFunction, NS = 'ufaWindow', KDIALOG = '.k-dialog', KWINDOW = '.k-window', KICONCLOSE = '.k-dialog-close', KCONTENTCLASS = 'k-content', KCONTENT = '.k-content', KTITLELESS = 'k-dialog-titleless', KDIALOGTITLE = '.k-dialog-title', KDIALOGTITLEBAR = '.k-window-titlebar', KBUTTONGROUP = '.k-dialog-buttongroup', KBUTTON = '.k-button', KALERT = 'k-alert', KCONFIRM = 'k-confirm', KPROMPT = 'k-prompt', KTEXTBOX = '.k-textbox', KOVERLAY = '.k-overlay', VISIBLE = ':visible', ZINDEX = 'zIndex', BODY = 'body', INITOPEN = 'initOpen', OPEN = 'open', CLOSE = 'close', SHOW = 'show', HIDE = 'hide', WIDTH = 'width', HUNDREDPERCENT = 100, OK_CANCEL = {
                okText: 'OK',
                cancel: 'Cancel'
        ...
        var DialogBase = Widget.extend({
            ...
            _createDialog: function () {
                var that = this, content = that.element, options = that.options, isRtl = ufa.support.isRtl(content), titlebar = $(templates.titlebar(options)), titleId = (content.id || ufa.guid()) + '_title', wrapper = $(that.wrapperTemplate(options));
            ...
            open: function () {
                ...
                if (!that.trigger(OPEN)) {
                    ...
                    if (options.modal) {
                        ...
                        if (showOptions.duration && ufa.effects.Fade && !otherModalsVisible) {
            ...
            _centerOnResize: function () {
                ...
                ufa.onResize(this._centerCallback);
                this._trackResize = true;
            },
            _stopCenterOnResize: function () {
                ufa.unbindResize(this._centerCallback);
        ...
        var PopupBox = DialogBase.extend({
            ...
            _ensureContentId: function (element) {
                var node = $(element);
                if (!node.attr('id')) {
                    node.attr('id', ufa.guid() + '_k-popup');
                }
            },
        ...
        ufa.ui.plugin(Alert);
        var ufaAlert = function (title,text) {
            return $(templates.alert).ufaAlert({ content: text,title:title }).data('ufaAlert').open().result;
        };
        ...
        ufa.ui.plugin(Confirm);
        var ufaConfirm = function (options) {
            var confirmDialog = $(templates.confirm).ufaConfirm(options).data('ufaConfirm').open();
        ...
        ufa.ui.plugin(Prompt);
        var ufaPrompt = function (text, value) {
            var promptDialog = $(templates.prompt).ufaPrompt({
                content: text,
                value: value
            }).data('ufaPrompt').open();
        ...
        var tabKeyTrapNS = 'ufaTabKeyTrap';
        ...
        var TabKeyTrap = Class.extend({
            ...
            removeTrap: function () {
                this.element.ufaDestroy(tabKeyTrapNS);
            },
            destroy: function () {
                this.element.ufaDestroy(tabKeyTrapNS);
        ...
        ufa.alert = ufaAlert;
        ufa.confirm = ufaConfirm;
        ufa.prompt = ufaPrompt;
    }(window.ufa.jQuery));
    return window.ufa;
