(function (f, define) {
    define('ufa.scheduler.timelineview', ['ufa.scheduler.view'], f);
}(function () {
    var __meta__ = {
    ...
    (function ($, undefined) {
        var ufa = window.ufa, ui = ufa.ui, setTime = ufa.date.setTime, SchedulerView = ui.SchedulerView, extend = $.extend, proxy = $.proxy, getDate = ufa.date.getDate, getMilliseconds = ufa.date.getMilliseconds, MS_PER_DAY = ufa.date.MS_PER_DAY, MS_PER_MINUTE = ufa.date.MS_PER_MINUTE, NS = '.ufaTimelineView';
        var EVENT_TEMPLATE = ufa.template('<div>' + '<div class="k-event-template k-event-time">#:ufa.format("{0:t} - {1:t}", start, end)#</div>' + '<div class="k-event-template">${title}</div></div>'), DATA_HEADER_TEMPLATE = ufa.template('<span class=\'k-link k-nav-day\'>#=ufa.format(\'{0:m}\', date)#</span>'), EVENT_WRAPPER_STRING = '<div role="gridcell" aria-selected="false" ' + 'data-#=ns#uid="#=uid#"' + '#if (resources[0]) { #' + 'style="background-color:#=resources[0].color#; border-color: #=resources[0].color#"' + 'class="k-event#=inverseColor ? " k-event-inverse" : ""#" ' + '#} else {#' + 'class="k-event"' + '#}#' + '>' + '<span class="k-event-actions">' + '# if(data.tail) {#' + '<span class="k-icon k-i-arrow-w"></span>' + '#}#' + '# if(data.isException()) {#' + '<span class="k-icon k-i-exception"></span>' + '# } else if(data.isRecurring()) {#' + '<span class="k-icon k-i-refresh"></span>' + '# } #' + '</span>' + '{0}' + '<span class="k-event-actions">' + '#if (showDelete) {#' + '<a href="\\#" class="k-link k-event-delete"><span class="k-icon k-si-close"></span></a>' + '#}#' + '# if(data.head) {#' + '<span class="k-icon k-i-arrow-e"></span>' + '#}#' + '</span>' + '#if(resizable && !data.tail){#' + '<span class="k-resize-handle k-resize-w"></span>' + '#}#' + '#if(resizable && !data.head){#' + '<span class="k-resize-handle k-resize-e"></span>' + '#}#' + '</div>';
        ...
        var TimelineView = SchedulerView.extend({
            ...
            _calculateSlotRanges: function () {
                ...
                for (var i = 0; i < dates.length; i++) {
                    ...
                    slotRanges.push({
                        start: ufa.date.toUtcTime(rangeStart),
                        end: ufa.date.toUtcTime(rangeEnd)
                    });
                }
            ...
            _content: function (dates) {
                ...
                var appendRow = function (date) {
                    ...
                    if (ufa.date.getMilliseconds(date) < ufa.date.getMilliseconds(options.workDayStart) || ufa.date.getMilliseconds(date) >= ufa.date.getMilliseconds(options.workDayEnd) || !that._isWorkDay(dates[idx])) {
            ...
            _groups: function () {
                ...
                for (var idx = 0; idx < groupCount; idx++) {
                    ...
                    view.addTimeSlotCollection(start, ufa.date.addDays(end, 1));
                }
            ...
            _isInDateSlot: function (event) {
                ...
                var rangeEnd = ufa.date.addDays(getDate(this._endDate), 1);
            ...
            _isInTimeSlot: function (event) {
                var startTime = event._startTime || ufa.date.toUtcTime(event.start);
                var endTime = event._endTime || ufa.date.toUtcTime(event.end);
            ...
            _adjustEvent: function (event) {
                ...
                if (event.isAllDay) {
                    ...
                    if (endTime === getMilliseconds(getDate(this.endTime()))) {
                        adjustedEndDate = ufa.date.addDays(adjustedEndDate, 1);
                    } else {
                    ...
                    if (startTime > eventStartTime) {
                        ...
                        adjustedStartDate = getDate(start);
                        adjustedStartDate = ufa.date.addDays(adjustedStartDate, 1);
                        setTime(adjustedStartDate, startTime);
                        tail = true;
                    }
                ...
                occurrence = event.clone({
                    ...
                    _startTime: adjustedStartDate ? ufa.date.toUtcTime(adjustedStartDate) : event._startTime,
                    _endTime: adjustedEndDate ? ufa.date.toUtcTime(adjustedEndDate) : event._endTime,
                    isAllDay: false
                });
                return {
        ...
        extend(true, ui, {
            ...
            TimelineWorkWeekView: TimelineView.extend({
                ...
                nextDate: function () {
                    return ufa.date.dayOfWeek(ufa.date.nextDay(this.endDate()), this.options.workWeekStart, 1);
                },
                previousDate: function () {
                    return ufa.date.previousDay(this.startDate());
                },
                calculateDateRange: function () {
                    var selectedDate = this.options.date, start = ufa.date.dayOfWeek(selectedDate, this.options.workWeekStart, -1), end = ufa.date.dayOfWeek(start, this.options.workWeekEnd, 1), dates = [];
                    while (start <= end) {
                        dates.push(start);
                        start = ufa.date.nextDay(start);
                    }
            ...
            TimelineMonthView: TimelineView.extend({
                ...
                name: 'timelineMonth',
                calculateDateRange: function () {
                    var selectedDate = this.options.date, start = ufa.date.firstDayOfMonth(selectedDate), end = ufa.date.lastDayOfMonth(selectedDate), idx, length, dates = [];
