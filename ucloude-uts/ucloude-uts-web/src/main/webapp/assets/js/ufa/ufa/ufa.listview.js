(function (f, define) {
    ...
    (function ($, undefined) {
        var ufa = window.ufa, CHANGE = 'change', CANCEL = 'cancel', DATABOUND = 'dataBound', DATABINDING = 'dataBinding', Widget = ufa.ui.Widget, keys = ufa.keys, FOCUSSELECTOR = '>*:not(.k-loading-mask)', PROGRESS = 'progress', ERROR = 'error', FOCUSED = 'k-state-focused', SELECTED = 'k-state-selected', KEDITITEM = 'k-edit-item', EDIT = 'edit', REMOVE = 'remove', SAVE = 'save', CLICK = 'click', NS = '.ufaListView', proxy = $.proxy, activeElement = ufa._activeElement, progress = ufa.ui.progress, DataSource = ufa.data.DataSource;
        var ListView = ufa.ui.DataBoundWidget.extend({
            ...
            _templates: function () {
                ...
                this.template = ufa.template(options.template || '');
                this.altTemplate = ufa.template(options.altTemplate || options.template);
                this.editTemplate = ufa.template(options.editTemplate || '');
            },
            ...
            refresh: function (e) {
                ...
                if (e.action === 'itemchange') {
                    if (!that._hasBindingTarget() && !that.editable) {
                        data = e.items[0];
                        item = that.items().filter('[' + ufa.attr('uid') + '=' + data.uid + ']');
                        if (item.length > 0) {
                            idx = item.index();
                            ...
                            item.attr(ufa.attr('uid'), data.uid);
                ...
                items = that.items();
                for (idx = 0, length = view.length; idx < length; idx++) {
                    items.eq(idx).attr(ufa.attr('uid'), view[idx].uid).attr('role', 'option').attr('aria-selected', 'false');
                }
            ...
            _pageable: function () {
                ...
                if ($.isPlainObject(pageable)) {
                    ...
                    that.pager = new ufa.ui.Pager($('#' + pagerId), settings);
                }
            },
            _selectable: function () {
                var that = this, multi, current, selectable = that.options.selectable, navigatable = that.options.navigatable;
                if (selectable) {
                    multi = ufa.ui.Selectable.parseOptions(selectable).multiple;
                    that.selectable = new ufa.ui.Selectable(that.element, {
                        aria: true,
                        multiple: multi,
            ...
            _navigatable: function () {
                ...
                if (navigatable) {
                    ...
                    element.on('focus' + NS, function () {
                        ...
                        var key = e.keyCode, current = that.current(), target = $(e.target), canHandle = !target.is(':button,textarea,a,a>.t-icon,input'), isTextBox = target.is(':text,:password'), preventDefault = ufa.preventDefault, editItem = element.find('.' + KEDITITEM), active = activeElement(), idx;
            ...
            _modelFromElement: function (element) {
                var uid = element.attr(ufa.attr('uid'));
                return this.dataSource.getByUid(uid);
            },
            ...
            edit: function (item) {
                ...
                that.cancel();
                item = that.items().filter('[' + ufa.attr('uid') + '=' + uid + ']');
                index = item.index();
                item.replaceWith(that.editTemplate(data));
                container = that.items().eq(index).addClass(KEDITITEM).attr(ufa.attr('uid'), data.uid);
            ...
            _crudHandlers: function () {
                ...
                that.element.on(clickNS, '.k-edit-button', function (e) {
                    var item = $(this).closest('[' + ufa.attr('uid') + ']');
                    that.edit(item);
                    e.preventDefault();
                });
