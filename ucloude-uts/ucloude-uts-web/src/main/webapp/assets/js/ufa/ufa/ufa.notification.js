(function (f, define) {
    ...
    (function ($, undefined) {
        var ufa = window.ufa, Widget = ufa.ui.Widget, proxy = $.proxy, extend = $.extend, setTimeout = window.setTimeout, CLICK = 'click', SHOW = 'show', HIDE = 'hide', KNOTIFICATION = 'k-notification', KICLOSE = '.k-notification-wrap .k-i-close', INFO = 'info', SUCCESS = 'success', WARNING = 'warning', ERROR = 'error', TOP = 'top', LEFT = 'left', BOTTOM = 'bottom', RIGHT = 'right', UP = 'up', NS = '.ufaNotification', WRAPPER = '<div class="k-widget k-notification"></div>', TEMPLATE = '<div class="k-notification-wrap">' + '<span class="k-icon k-i-note">#=typeIcon#</span>' + '#=content#' + '<span class="k-icon k-i-close">Hide</span>' + '</div>';
        var Notification = Widget.extend({
            init: function (element, options) {
                ...
                that._guid = '_' + ufa.guid();
                that._isRtl = ufa.support.isRtl(element);
            ...
            _compileTemplates: function (templates) {
                var that = this;
                var ufaTemplate = ufa.template;
                that._compiled = {};
                $.each(templates, function (key, value) {
                    that._compiled[value.type] = ufaTemplate(value.template || $('#' + value.templateId).html());
                });
                that._defaultCompiled = ufaTemplate(TEMPLATE);
            },
            ...
            _attachPopupEvents: function (options, popup) {
                ...
                if (popup.options.anchor !== document.body && popup.options.origin.indexOf(RIGHT) > 0) {
                    popup.bind('open', function () {
                        var shadows = ufa.getShadows(popup.element);
                        setTimeout(function () {
            ...
            _showPopup: function (wrapper, options) {
                ...
                popup = new ufa.ui.Popup(wrapper, {
            ...
            _showStatic: function (wrapper, options) {
                ...
                wrapper.addClass(that._guid)[insertionMethod](options.appendTo).hide().ufaAnimate(animation.open || false);
            ...
            _hideStatic: function (wrapper) {
                wrapper.ufaAnimate(extend(this.options.animation.close || false, {
                    complete: function () {
            ...
            hide: function () {
                ...
                if (that.options.appendTo) {
                    ...
                    openedNotifications.each(function (idx, element) {
                        var popup = $(element).data('ufaPopup');
                        if (popup) {
                            that._hidePopup(popup);
                        }
                    });
