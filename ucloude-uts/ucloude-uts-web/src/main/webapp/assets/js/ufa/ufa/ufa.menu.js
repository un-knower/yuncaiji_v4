(function (f, define) {
    ...
    (function ($, undefined) {
        var ufa = window.ufa, ui = ufa.ui, activeElement = ufa._activeElement, touch = ufa.support.touch && ufa.support.mobileOS, MOUSEDOWN = 'mousedown', CLICK = 'click', extend = $.extend, proxy = $.proxy, each = $.each, template = ufa.template, keys = ufa.keys, Widget = ui.Widget, excludedNodesRegExp = /^(ul|a|div)$/i, NS = '.ufaMenu', IMG = 'img', OPEN = 'open', MENU = 'k-menu', LINK = 'k-link', LAST = 'k-last', CLOSE = 'close', TIMER = 'timer', FIRST = 'k-first', IMAGE = 'k-image', SELECT = 'select', ZINDEX = 'zIndex', ACTIVATE = 'activate', DEACTIVATE = 'deactivate', POINTERDOWN = 'touchstart' + NS + ' MSPointerDown' + NS + ' pointerdown' + NS, pointers = ufa.support.pointers, msPointers = ufa.support.msPointers, allPointers = msPointers || pointers, MOUSEENTER = pointers ? 'pointerover' : msPointers ? 'MSPointerOver' : 'mouseenter', MOUSELEAVE = pointers ? 'pointerout' : msPointers ? 'MSPointerOut' : 'mouseleave', mobile = touch || allPointers, DOCUMENT_ELEMENT = $(document.documentElement), ufaPOPUP = 'ufaPopup', DEFAULTSTATE = 'k-state-default', HOVERSTATE = 'k-state-hover', FOCUSEDSTATE = 'k-state-focused', DISABLEDSTATE = 'k-state-disabled', menuSelector = '.k-menu', groupSelector = '.k-menu-group', popupSelector = groupSelector + ',.k-animation-container', allItemsSelector = ':not(.k-list) > .k-item', disabledSelector = '.k-item.k-state-disabled', itemSelector = '.k-item:not(.k-state-disabled)', linkSelector = '.k-item:not(.k-state-disabled) > .k-link', exclusionSelector = ':not(.k-item.k-separator)', nextSelector = exclusionSelector + ':eq(0)', lastSelector = exclusionSelector + ':last', templateSelector = '> div:not(.k-animation-container,.k-list-container)', touchPointerTypes = {
                '2': 1,
                'touch': 1
            }, templates = {
                ...
                text: function (item) {
                    return item.encoded === false ? item.text : ufa.htmlEncode(item.text);
                },
                tag: function (item) {
        ...
        function parseDirection(direction, root, isRtl) {
            ...
            if (horizontal) {
                ...
                output.position[1] = ufa.directions[direction].reverse;
            } else {
                output.origin[0] = direction;
                output.position[0] = ufa.directions[direction].reverse;
            }
        ...
        var Menu = Widget.extend({
            init: function (element, options) {
                ...
                if (element[0].id) {
                    that._ariaId = ufa.format('{0}_mn_active', element[0].id);
                }
                ufa.notify(that);
            },
            events: [
            ...
            open: function (element) {
                var that = this, options = that.options, horizontal = options.orientation == 'horizontal', direction = options.direction, isRtl = ufa.support.isRtl(that.wrapper);
                        ...
                        if (ul[0] && that._triggerEvent({
                            ...
                            if (!ul.find('.k-menu-group')[0] && ul.children('.k-item').length > 1) {
                                var windowHeight = $(window).height(), setScrolling = function () {
                                        ul.css({
                                            maxHeight: windowHeight - (ul.outerHeight() - ul.height()) - ufa.getShadows(ul).bottom,
                                            overflow: 'auto'
                                        });
                                    };
                                if (ufa.support.browser.msie && ufa.support.browser.version <= 7) {
            ...
            close: function (items, dontClearClose) {
                ...
                items.each(function () {
                    ...
                    clearTimeout(li.data(TIMER));
                    li.data(TIMER, setTimeout(function () {
                        var popup = li.find('.k-menu-group:not(.k-list-container):not(.k-calendar-container):first:visible').data(ufaPOPUP);
                        if (popup) {
                            popup.close();
            ...
            _toggleHover: function (e) {
                var target = $(ufa.eventTarget(e) || e.target).closest(allItemsSelector), isEnter = e.type == MOUSEENTER || MOUSEDOWN.indexOf(e.type) !== -1;
            ...
            _checkActiveElement: function (e) {
                ...
                if (!this._closurePrevented) {
                    setTimeout(function () {
                        if (!document.hasFocus() || !contains(target, ufa._activeElement()) && e && !contains(target, e.currentTarget)) {
                            that.close(target);
                        }
                    }, 0);
                }
            ...
            _updateClasses: function () {
                ...
                element.find('li > ul').filter(function () {
                    return !ufa.support.matchesSelector.call(this, nonContentGroupsSelector);
            ...
            _mouseleave: function (e) {
                ...
                if (!that.options.openOnClick && !touch && !((pointers || msPointers) && e.originalEvent.pointerType in touchPointerTypes) && !contains(e.currentTarget, e.relatedTarget || e.target) && hasChildren && !contains(e.currentTarget, ufa._activeElement())) {
                    that.close(element);
                }
            },
            _click: function (e) {
                var that = this, openHandle, options = that.options, target = $(ufa.eventTarget(e)), nodeName = target[0] ? target[0].nodeName.toUpperCase() : '', formNode = nodeName == 'INPUT' || nodeName == 'SELECT' || nodeName == 'BUTTON' || nodeName == 'LABEL', link = target.closest('.' + LINK), element = target.closest(allItemsSelector), href = link.attr('href'), childGroup, childGroupVisible, targetHref = target.attr('href'), sampleHref = $('<a href=\'#\' />').attr('href'), isLink = !!href && href !== sampleHref, isLocalLink = isLink && !!href.match(/^#/), isTargetLink = !!targetHref && targetHref !== sampleHref, shouldCloseTheRootItem = options.openOnClick && childGroupVisible && that._isRootItem(element);
                ...
                if ((!that._isRootItem(element) || !options.openOnClick) && !ufa.support.touch && !((pointers || msPointers) && that._isRootItem(element.closest(allItemsSelector)))) {
                    return;
                }
            ...
            _focus: function (e) {
                ...
                if (target != that.wrapper[0] && !$(target).is(':ufaFocusable')) {
                    e.stopPropagation();
            ...
            _keydown: function (e) {
                var that = this, key = e.keyCode, hoverItem = that._oldHoverItem, target, belongsToVertical, hasChildren, isRtl = ufa.support.isRtl(that.wrapper);
            ...
            _focusHandler: function (e) {
                var that = this, item = $(ufa.eventTarget(e)).closest(allItemsSelector);
                setTimeout(function () {
        ...
        var ContextMenu = Menu.extend({
            ...
            destroy: function () {
                ...
                DOCUMENT_ELEMENT.off(ufa.support.mousedown + NS, that._closeProxy);
                if (that.userEvents) {
                    that.userEvents.destroy();
                }
                ...
                if (contains(that.element[0], $(x)[0])) {
                    ...
                    if (that._triggerEvent({
                        ...
                        if (that.popup.visible() && that.options.filter) {
                            ...
                            that.popup.element.ufaStop(true);
                        }
                        if (y !== undefined) {
                        ...
                        DOCUMENT_ELEMENT.on(ufa.support.mousedown + NS, that._closeProxy);
                    }
                }
                return that;
            },
            ...
            _showHandler: function (e) {
                ...
                if (options.filter && ufa.support.matchesSelector.call(ev.currentTarget, options.filter) || !options.filter) {
            ...
            _wire: function () {
                ...
                if (target[0]) {
                    if (ufa.support.mobileOS && options.showOn == 'contextmenu') {
                        that.userEvents = new ufa.UserEvents(target, {
            ...
            _popup: function () {
                ...
                that.popup = that.element.addClass('k-context-menu').ufaPopup({
