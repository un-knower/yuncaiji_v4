(function (f, define) {
    ...
    (function ($, undefined) {
        var ufa = window.ufa, ui = ufa.ui, Widget = ui.Widget, keys = ufa.keys, support = ufa.support, htmlEncode = ufa.htmlEncode, activeElement = ufa._activeElement, ObservableArray = ufa.data.ObservableArray, ID = 'id', CHANGE = 'change', FOCUSED = 'k-state-focused', HOVER = 'k-state-hover', LOADING = 'k-loading', OPEN = 'open', CLOSE = 'close', CASCADE = 'cascade', SELECT = 'select', SELECTED = 'selected', REQUESTSTART = 'requestStart', REQUESTEND = 'requestEnd', WIDTH = 'width', extend = $.extend, proxy = $.proxy, isArray = $.isArray, browser = support.browser, isIE8 = browser.msie && browser.version < 9, quotRegExp = /"/g, alternativeNames = {
                'ComboBox': 'DropDownList',
                'DropDownList': 'ComboBox'
            };
        var List = ufa.ui.DataBoundWidget.extend({
            ...
            _listOptions: function (options) {
                ...
                if (!options.template) {
                    options.template = '#:' + ufa.expr(options.dataTextField, 'data') + '#';
                }
                return options;
            },
            _initList: function () {
                ...
                if (!that.options.virtual) {
                    that.listView = new ufa.ui.StaticList(that.ul, listOptions);
                } else {
                    that.listView = new ufa.ui.VirtualList(that.ul, listOptions);
                }
                that._setListValue();
            },
            ...
            _accessors: function () {
                ...
                var options = that.options;
                var getter = ufa.getter;
                var textField = element.attr(ufa.attr('text-field'));
                var valueField = element.attr(ufa.attr('value-field'));
        ...
        ui.Select = List.extend({
            ...
            _dataSource: function () {
                ...
                that.dataSource = ufa.data.DataSource.create(dataSource).bind(REQUESTSTART, that._requestStartHandler).bind(REQUESTEND, that._requestEndHandler).bind('error', that._errorHandler);
            },
            _firstItem: function () {
            ...
            _parentWidget: function () {
                ...
                var parent = parentElement.data('ufa' + name);
                if (!parent) {
                    parent = parentElement.data('ufa' + alternativeNames[name]);
                }
                return parent;
            },
            _cascade: function () {
        ...
        var StaticList = ufa.ui.DataBoundWidget.extend({
            init: function (element, options) {
            ...
            _getter: function () {
                this._valueGetter = ufa.getter(this.options.dataValueField);
            },
            ...
            _template: function () {
                ...
                if (!template) {
                    template = ufa.template('<li tabindex="-1" role="option" unselectable="on" class="k-item">${' + ufa.expr(options.dataTextField, 'data') + '}</li>', { useWithBlock: false });
                } else {
            ...
            _templates: function () {
                ...
                for (var key in templates) {
                    ...
                    if (template && typeof template !== 'function') {
                        templates[key] = ufa.template(template);
                    }
                }
            ...
            _renderItem: function (context) {
                ...
                // 添加 title
			    var titval = this.templates.template(dataItem);
			    try {
				    $(titval);//包含半角 字符串异常
			        if($(titval).length > 0) {
				        if($(titval).attr('name') != null) {
					        item += '" title ="' + $(titval).attr('name') + '';
				        }
			        } else {
				        item += '" title ="' + titval + '';
			        }
            ...
            isFiltered: function () {
                ...
                return !ufa.data.Query.compareFilters(this.dataSource.filter(), this._lastDSFilter);
            },
            refresh: function (e) {
                var that = this;
