...
(function (f, define) {
    ...
    (function ($, undefined) {
        var ufa = window.ufa, Widget = ufa.ui.Widget, extend = $.extend, deepExtend = ufa.deepExtend, inArray = $.inArray, isPlainObject = $.isPlainObject, draw = ufa.drawing, geom = ufa.geometry, util = ufa.util, defined = util.defined, dataviz = ufa.dataviz, Box2D = dataviz.Box2D, TextBox = dataviz.TextBox, DEFAULT_WIDTH = 300, DEFAULT_HEIGHT = 100, DEFAULT_QUIETZONE_LENGTH = 10, numberRegex = /^\d+$/, alphanumericRegex = /^[a-z0-9]+$/i, InvalidCharacterErrorTemplate = 'Character \'{0}\' is not valid for symbology {1}';
        function getNext(value, index, count) {
            return value.substring(index, index + count);
        }
        var Encoding = ufa.Class.extend({
            init: function (options) {
            ...
            invalidCharacterError: function (character) {
                throw new Error(ufa.format(InvalidCharacterErrorTemplate, character, this.name));
            }
        });
        var encodings = {};
        ...
        states128.FNC1 = state128.extend({
            ...
            unsupportedAIError: function (id) {
                throw new Error(ufa.format('\'{0}\' is not a supported Application Identifier'), id);
            },
