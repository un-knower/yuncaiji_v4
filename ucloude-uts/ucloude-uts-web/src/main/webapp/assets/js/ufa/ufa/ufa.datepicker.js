(function (f, define) {
    ...
    (function ($, undefined) {
        var ufa = window.ufa, ui = ufa.ui, Widget = ui.Widget, parse = ufa.parseDate, keys = ufa.keys, template = ufa.template, activeElement = ufa._activeElement, DIV = '<div />', SPAN = '<span />', ns = '.ufaDatePicker', CLICK = 'click' + ns, OPEN = 'open', CLOSE = 'close', CHANGE = 'change', DISABLED = 'disabled', READONLY = 'readonly', DEFAULT = 'k-state-default', FOCUSED = 'k-state-focused', SELECTED = 'k-state-selected', STATEDISABLED = 'k-state-disabled', HOVER = 'k-state-hover', HOVEREVENTS = 'mouseenter' + ns + ' mouseleave' + ns, MOUSEDOWN = 'mousedown' + ns, ID = 'id', MIN = 'min', MAX = 'max', MONTH = 'month', ARIA_DISABLED = 'aria-disabled', ARIA_EXPANDED = 'aria-expanded', ARIA_HIDDEN = 'aria-hidden', ARIA_READONLY = 'aria-readonly', calendar = ufa.calendar, isInRange = calendar.isInRange, restrictValue = calendar.restrictValue, isEqualDatePart = calendar.isEqualDatePart, extend = $.extend, proxy = $.proxy, DATE = Date;
        ...
        var DateView = function (options) {
            ...
            that.popup = new ui.Popup(div, extend(options.popup, options, {
                name: 'Popup',
                isRtl: ufa.support.isRtl(options.anchor)
            }));
            that.div = div;
        ...
        DateView.prototype = {
            _calendar: function () {
                ...
                var div;
                if (!calendar) {
                    div = $(DIV).attr(ID, ufa.guid()).appendTo(that.popup.element).on(MOUSEDOWN, preventDefault).on(CLICK, 'td:has(.k-link)', proxy(that._click, that));
                    that.calendar = calendar = new ui.Calendar(div);
                    that._setOptions(options);
                    ufa.calendar.makeUnselectable(calendar.element);
        ...
        var DatePicker = Widget.extend({
            init: function (element, options) {
                ...
                options.disableDates = ufa.calendar.disabled(options.disableDates);
                ...
                if(options.start == "week")
                {
                	if(options.value)
                	{
                		var result = ufa.date.getFirstWeekBeginDay(options.value.getFullYear());
	                    var s1 = Math.ceil((options.value - result) / (24 * 60 * 60 * 1000));
	                    options.value = ufa.date.addDays(result, (Math.floor(s1 / 7) + 1) * 7);
                	}
                }
            ...
            options: {
                ...
                month: {},
                dates: [],
                ARIATemplate: 'Current focused date is #=ufa.toString(data.current, "D")#',
                ARIAWeekTemplate: 'Current focused date is #=ufa.toString(data.current, "w")#'
            },
            setOptions: function (options) {
                var that = this;
                ...
                if (value) {
                    that.element.val(ufa.toString(value, options.format, options.culture));
            ...
            _click: function () {
                ...
                if (!ufa.support.touch && element[0] !== activeElement()) {
                    element.focus();
                }
            },
            ...
            _update: function (value) {
                ...
                if (+date === +current && isSameType) {
                    formattedValue = ufa.toString(date, options.format, options.culture);
                ...
                that.element.val(date ? ufa.toString(date, options.format, options.culture) : value);
                that._updateARIA(date);
                return date;
            },
            _wrapper: function () {
            ...
            _template: function () {
            	if(this.options.start == "week" && this.options.depth == "week")
                	this._ariaTemplate = template(this.options.ARIAWeekTemplate);
                else 
