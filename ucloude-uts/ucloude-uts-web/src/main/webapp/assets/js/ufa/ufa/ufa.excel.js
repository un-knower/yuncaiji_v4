(function (f, define) {
    ...
    (function ($, ufa) {
        ufa.ExcelExporter = ufa.Class.extend({
            init: function (options) {
                ...
                if (dataSource instanceof ufa.data.DataSource) {
            ...
            _prepareColumn: function (column) {
                ...
                return $.extend({}, column, {
                    value: value,
                    values: values,
                    groupHeaderTemplate: ufa.template(column.groupHeaderTemplate || '#= title #: #= value #'),
                    groupFooterTemplate: column.groupFooterTemplate ? ufa.template(column.groupFooterTemplate) : null,
                    footerTemplate: column.footerTemplate ? ufa.template(column.footerTemplate) : null
                });
            },
            _filter: function () {
        ...
        ufa.ExcelMixin = {
            ...
            saveAsExcel: function (opt) {//opt.isExportAllPage 是否导出全部页，仅支持客户端分页
            	if(!opt)
            	{
            		opt = {};
            	}
                var excel = this.options.excel || {};
                var columnNames = opt.columnNames;//自定义导出列
        		var rowIndexs = opt.rowIndexs;//自定义导出行
        		if (opt&&opt.fileName) {
            		excel.fileName = opt.fileName;
        		}
        		//允许自定义导出列
		        var columns=[];
		        if (columnNames && columnNames.length > 0) {
		            var arrStr =(","+ columnNames.toString()+",").toLowerCase();
		            for (var i = 0; i < this.columns.length; i++) {
		                if (this.columns[i].field&&arrStr.indexOf(","+this.columns[i].field.toLowerCase()+",") > -1) {
                ...
                var exporter = new ufa.ExcelExporter({
                    columns: columns,
                    ...
                    isExportAllPage:opt.isExportAllPage
                });
