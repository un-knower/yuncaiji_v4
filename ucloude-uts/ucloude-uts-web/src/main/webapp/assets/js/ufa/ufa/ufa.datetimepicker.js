(function (f, define) {
    ...
    (function ($, undefined) {
        var ufa = window.ufa, TimeView = ufa.TimeView, parse = ufa.parseDate, activeElement = ufa._activeElement, extractFormat = ufa._extractFormat, calendar = ufa.calendar, isInRange = calendar.isInRange, restrictValue = calendar.restrictValue, isEqualDatePart = calendar.isEqualDatePart, getMilliseconds = TimeView.getMilliseconds, ui = ufa.ui, Widget = ui.Widget, OPEN = 'open', CLOSE = 'close', CHANGE = 'change', ns = '.ufaDateTimePicker', CLICK = 'click' + ns, DISABLED = 'disabled', READONLY = 'readonly', DEFAULT = 'k-state-default', FOCUSED = 'k-state-focused', HOVER = 'k-state-hover', STATEDISABLED = 'k-state-disabled', HOVEREVENTS = 'mouseenter' + ns + ' mouseleave' + ns, MOUSEDOWN = 'mousedown' + ns, MONTH = 'month', SPAN = '<span/>', ARIA_ACTIVEDESCENDANT = 'aria-activedescendant', ARIA_EXPANDED = 'aria-expanded', ARIA_HIDDEN = 'aria-hidden', ARIA_OWNS = 'aria-owns', ARIA_DISABLED = 'aria-disabled', ARIA_READONLY = 'aria-readonly', DATE = Date, MIN = new DATE(1800, 0, 1), MAX = new DATE(2099, 11, 31), dateViewParams = { view: 'date' }, timeViewParams = { view: 'time' }, extend = $.extend;
        var DateTimePicker = Widget.extend({
            ...
            _keydown: function (e) {
                ...
                if (e.altKey && e.keyCode === ufa.keys.DOWN) {
                    ...
                    timeView.move(e);
                } else if (e.keyCode === ufa.keys.ENTER && value !== that._oldText) {
                    that._change(value);
                } else {
                    that._typing = true;
                }
            ...
            _views: function () {
                ...
                that.dateView = dateView = new ufa.DateView(extend({}, options, {
                    id: id,
                    anchor: that.wrapper,
                    change: function () {
                        ...
                        if (that._value) {
                            value = ufa.date.setHours(new Date(value), that._value);
                        }
                        that._change(value);
                        that.close('date');
                    },
                ...
                that.timeView = timeView = new TimeView({
                    ...
                    change: function (value, trigger) {
                        ...
                        if (trigger) {
                            ...
                            element.val(ufa.toString(value, options.format, options.culture));
            ...
            _template: function () {
                this._ariaTemplate = ufa.template(this.options.ARIATemplate);
            },
        ...
        function normalize(options) {
            var patterns = ufa.getCulture(options.culture).calendars.standard.patterns, parseFormats = !options.parseFormats.length, timeFormat;
            options.format = extractFormat(options.format || patterns.g);
            options.timeFormat = timeFormat = extractFormat(options.timeFormat || patterns.t);
            ufa.DateView.normalize(options);
            if (parseFormats) {
