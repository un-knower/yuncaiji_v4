(function (f, define) {
    define('ufa.window', ['ufa.draganddrop'], f);
}(function () {
    var __meta__ = {
        id: 'window',
        name: 'Window',
    ...
    (function ($, undefined) {
        var ufa = window.ufa, Widget = ufa.ui.Widget, Draggable = ufa.ui.Draggable, isPlainObject = $.isPlainObject, activeElement = ufa._activeElement, proxy = $.proxy, extend = $.extend, each = $.each, template = ufa.template, BODY = 'body', templates, NS = '.ufaWindow', KWINDOW = '.k-window', KWINDOWTITLE = '.k-window-title', KWINDOWTITLEBAR = KWINDOWTITLE + 'bar', KWINDOWCONTENT = '.k-window-content', KWINDOWRESIZEHANDLES = '.k-resize-handle', KOVERLAY = '.k-overlay', KCONTENTFRAME = 'k-content-frame', LOADING = 'k-loading', KHOVERSTATE = 'k-state-hover', KFOCUSEDSTATE = 'k-state-focused', MAXIMIZEDSTATE = 'k-window-maximized', VISIBLE = ':visible', HIDDEN = 'hidden', CURSOR = 'cursor', OPEN = 'open', ACTIVATE = 'activate', DEACTIVATE = 'deactivate', CLOSE = 'close',CONFIRM='confirm', REFRESH = 'refresh', RESIZE = 'resize', RESIZEEND = 'resizeEnd', DRAGSTART = 'dragstart', DRAGEND = 'dragend', ERROR = 'error', OVERFLOW = 'overflow', ZINDEX = 'zIndex', MINIMIZE_MAXIMIZE = '.k-window-actions .k-i-minimize,.k-window-actions .k-i-maximize', KPIN = '.k-i-pin', KUNPIN = '.k-i-unpin', PIN_UNPIN = KPIN + ',' + KUNPIN, TITLEBAR_BUTTONS = '.k-window-titlebar .k-window-action', REFRESHICON = '.k-window-titlebar .k-i-refresh', isLocalUrl = ufa.isLocalUrl;
        function defined(x) {
        ...
        var Window = Widget.extend({
            init: function (element, options) {
                ...
                if (options.footer) {
	                ...
	                for (i in options.footer) {
	                    var start = options.footer[i];                    
	                    if (start) {
	                        var btn = $('<button class="k-button k-primary k-window-footer-' + i + '" style="margin-left:.5em;"><tton>');
	                        switch (i) {
	                            case "submit":
	                                btn.text("确定");
	                                btn.click("click" + NS, function(){ that.trigger(CONFIRM);});
	                                break;
	                            case "cancel":
	                                btn.text("取消");                                
	                                btn.click("click" + NS, proxy(that.close, that));
	                                break;
	                        };                        
	                        btn.appendTo(footer);
	                    }
                ...
                that.touchScroller = ufa.touchScroller(element);
                that._resizeHandler = proxy(that._onDocumentResize, that);
                that._marker = ufa.guid().substring(0, 8);
            ...
            _resize: function () {
                ufa.resize(this.element.children());
            },
            _resizable: function () {
            ...
            _actions: function () {
                ...
                container.html(ufa.render(templates.action, actions));
            },
            ...
            _keydown: function (e) {
                var that = this, options = that.options, keys = ufa.keys, keyCode = e.keyCode, wrapper = that.wrapper, offset, handled, distance = 10, isMaximized = that.options.isMaximized, newWidth, newHeight, w, h;
            ...
            _object: function (element) {
                ...
                var widget = ufa.widgetInstance(content);
            ...
            content: function (html, data) {
                ...
                ufa.destroy(this.element.children());
            ...
            open: function () {
                ...
                if (!that.trigger(OPEN)) {
                    ...
                    if (options.modal) {
                        ...
                        overlay.ufaStop(true, true);
                        if (showOptions.duration && ufa.effects.Fade) {
                            var overlayFx = ufa.fx(overlay).fadeIn();
                    ...
                    if (!wrapper.is(VISIBLE)) {
                        ...
                        wrapper.show().ufaStop().ufaAnimate({
            ...
            _removeOverlay: function (suppressAnimation) {
                ...
                if (hideOverlay) {
                    if (!suppressAnimation && hideOptions.duration && ufa.effects.Fade) {
                        var overlayFx = ufa.fx(overlay).fadeOut();
            ...
            _onDocumentResize: function () {
                var that = this, wrapper = that.wrapper, wnd = $(window), zoomLevel = ufa.support.zoomLevel(), w, h;
                if (!that.options.isMaximized) {
                    return;
                }
            ...
            _createWindow: function () {
                var contentHtml = this.element, options = this.options, iframeSrcAttributes, wrapper, isRtl = ufa.support.isRtl(contentHtml);
                ...
                contentHtml.find('[data-role=editor]').each(function () {
                    var editor = $(this).data('ufaEditor');
                    if (editor) {
                        editor.refresh();
                    }
                });
        ...
        templates = {
            ...
            footer: template("<div class='k-window-footer'></div>"),
            overlay: '<div class=\'k-overlay\' />',
        ...
        WindowResizing.prototype = {
            ...
            dragstart: function (e) {
                ...
                that.initialPosition = ufa.getOffset(wrapper, 'position');
                that.resizeDirection = e.currentTarget.prop('className').replace('k-resize-handle k-resize-', '');
                that.initialSize = {
                    width: wrapper.width(),
                    height: wrapper.height()
                };
                that.containerOffset = ufa.getOffset(wnd.appendTo, 'position');
        ...
        WindowDragging.prototype = {
            dragstart: function (e) {
                var wnd = this.owner, element = wnd.element, actions = element.find('.k-window-actions'), containerOffset = ufa.getOffset(wnd.appendTo);
                wnd.trigger(DRAGSTART);
                wnd.initialWindowPosition = ufa.getOffset(wnd.wrapper, 'position');
                wnd.startPosition = {
