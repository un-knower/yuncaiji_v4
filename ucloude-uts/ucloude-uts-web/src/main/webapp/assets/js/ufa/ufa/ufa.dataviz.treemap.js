...
(function (f, define) {
    ...
    var __meta__ = {
        ...
        name: 'TreeMap',
        category: 'dataviz',
        description: 'The ufa DataViz TreeMap',
        depends: [
            'data',
            'userevents',
            'dataviz.themes'
    ...
    (function ($, undefined) {
        var math = Math, proxy = $.proxy, isArray = $.isArray, ufa = window.ufa, Class = ufa.Class, Widget = ufa.ui.Widget, template = ufa.template, deepExtend = ufa.deepExtend, HierarchicalDataSource = ufa.data.HierarchicalDataSource, getter = ufa.getter, dataviz = ufa.dataviz;
        var NS = '.ufaTreeMap', CHANGE = 'change', DATA_BOUND = 'dataBound', ITEM_CREATED = 'itemCreated', MAX_VALUE = Number.MAX_VALUE, MOUSEOVER_NS = 'mouseover' + NS, MOUSELEAVE_NS = 'mouseleave' + NS, UNDEFINED = 'undefined';
        var TreeMap = Widget.extend({
            ...
            _attachEvents: function () {
                ...
                ufa.onResize(this._resizeHandler);
            },
            _setLayout: function () {
            ...
            _onDataChange: function (e) {
                ...
                if (!node) {
                    ...
                    if (items.length) {
                        ...
                        root.children = [];
                        items = new ufa.data.Query(items)._sortForGrouping(options.valueField, 'desc');
            ...
            dataItem: function (node) {
                var uid = $(node).attr(ufa.attr('uid')), dataSource = this.dataSource;
                return dataSource && dataSource.getByUid(uid);
            },
            findByUid: function (uid) {
                return this.element.find('.k-treemap-tile[' + ufa.attr('uid') + '=\'' + uid + '\']');
            },
            ...
            _resizeItems: function (root, element) {
                if (root.children && root.children.length) {
                    ...
                    for (var idx = 0; idx < root.children.length; idx++) {
                        ...
                        childElement = elements.filter('[' + ufa.attr('uid') + '=\'' + child.dataItem.uid + '\']');
        ...
        var SquarifiedView = Class.extend({
            ...
            _createTile: function (item) {
                ...
                if (defined(item.dataItem) && defined(item.dataItem.uid)) {
                    tile.attr(ufa.attr('uid'), item.dataItem.uid);
                }
                return tile;
            },
