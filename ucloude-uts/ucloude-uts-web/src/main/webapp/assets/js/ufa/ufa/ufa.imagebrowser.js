(function (f, define) {
    define('ufa.imagebrowser', ['ufa.filebrowser'], f);
}(function () {
    var __meta__ = {
        id: 'imagebrowser',
        name: 'ImageBrowser',
    ...
    (function ($, undefined) {
        var ufa = window.ufa, FileBrowser = ufa.ui.FileBrowser, isPlainObject = $.isPlainObject, proxy = $.proxy, extend = $.extend, browser = ufa.support.browser, isFunction = ufa.isFunction, trimSlashesRegExp = /(^\/|\/$)/g, ERROR = 'error', NS = '.ufaImageBrowser', NAMEFIELD = 'name', SIZEFIELD = 'size', TYPEFIELD = 'type', DEFAULTSORTORDER = {
                field: TYPEFIELD,
                dir: 'asc'
            }, EMPTYTILE = ufa.template('<li class="k-tile-empty"><strong>${text}</strong></li>');
        ...
        extend(true, ufa.data, {
            transports: {
                'imagebrowser': ufa.data.RemoteTransport.extend({
        ...
        var ImageBrowser = FileBrowser.extend({
            ...
            value: function () {
                ...
                if (selected && selected.get(TYPEFIELD) === 'f') {
                    ...
                    if (imageUrl) {
                        path = isFunction(imageUrl) ? imageUrl(path) : ufa.format(imageUrl, encodeURIComponent(path));
                    }
                    return path;
                }
            },
            ...
            _loadImage: function (li) {
                var that = this, element = $(li), dataItem = that.dataSource.getByUid(element.attr(ufa.attr('uid'))), name = dataItem.get(NAMEFIELD), thumbnailUrl = that.options.transport.thumbnailUrl, img = $('<img />', { alt: name }), urlJoin = '?';
            ...
            _itemTmpl: function () {
                var that = this, html = '<li class="k-tile" ' + ufa.attr('uid') + '="#=uid#" ';
