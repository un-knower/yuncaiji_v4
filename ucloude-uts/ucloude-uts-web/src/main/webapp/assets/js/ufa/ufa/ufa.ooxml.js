(function (f, define) {
    ...
    (function ($, ufa) {
        ...
        var Worksheet = ufa.Class.extend({
            ...
            _cell: function (data, rowIndex, cellIndex) {
                ...
                if (type === 'string') {
                    ...
                    type = null;
                    var offset = (value.getTimezoneOffset() - DATE_EPOCH.getTimezoneOffset()) * ufa.date.MS_PER_MINUTE;
                    value = (value - DATE_EPOCH - offset) / ufa.date.MS_PER_DAY + 1;
                    if (!style.format) {
        ...
        var Workbook = ufa.Class.extend({
            ...
            toDataURL: function () {
                if (typeof JSZip === 'undefined') {
                    throw new Error('JSZip not found. Check http://docs.telerik.com/ufa-ui/framework/excel/introduction#requirements for more details.');
                }
                var zip = new JSZip();
                var docProps = zip.folder('docProps');
                docProps.file('core.xml', CORE({
                    creator: this.options.creator || 'ufa UI',
                    lastModifiedBy: this.options.creator || 'ufa UI',
                ...
                for (var idx = 0; idx < sheetCount; idx++) {
                    worksheets.file(ufa.format('sheet{0}.xml', idx + 1), this._sheets[idx].toXML(idx));
                }
