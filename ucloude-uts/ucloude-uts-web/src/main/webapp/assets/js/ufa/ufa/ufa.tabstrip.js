(function (f, define) {
    define('ufa.tabstrip', ['ufa.data'], f);
}(function () {
    var __meta__ = {
        id: 'tabstrip',
        name: 'TabStrip',
    ...
    (function ($, undefined) {
        var ufa = window.ufa, ui = ufa.ui, keys = ufa.keys, map = $.map, each = $.each, trim = $.trim, extend = $.extend, template = ufa.template, Widget = ui.Widget, excludedNodesRegExp = /^(a|div)$/i, NS = '.ufaTabStrip', IMG = 'img', HREF = 'href', PREV = 'prev', SHOW = 'show', LINK = 'k-link', LAST = 'k-last', CLICK = 'click', ERROR = 'error', EMPTY = ':empty', IMAGE = 'k-image', FIRST = 'k-first', SELECT = 'select', ACTIVATE = 'activate', CONTENT = 'k-content', CONTENTURL = 'contentUrl', MOUSEENTER = 'mouseenter', MOUSELEAVE = 'mouseleave', CONTENTLOAD = 'contentLoad', DISABLEDSTATE = 'k-state-disabled', DEFAULTSTATE = 'k-state-default', ACTIVESTATE = 'k-state-active', FOCUSEDSTATE = 'k-state-focused', HOVERSTATE = 'k-state-hover', TABONTOP = 'k-tab-on-top', NAVIGATABLEITEMS = '.k-item:not(.' + DISABLEDSTATE + ')', HOVERABLEITEMS = '.k-tabstrip-items > ' + NAVIGATABLEITEMS + ':not(.' + ACTIVESTATE + ')', templates = {
                ...
                contentUrl: function (item) {
                    return item.contentUrl ? ufa.attr('content-url') + '="' + item.contentUrl + '"' : '';
                }
            };
        ...
        var TabStrip = Widget.extend({
            init: function (element, options) {
                ...
                that._wrapper();
                that._isRtl = ufa.support.isRtl(that.wrapper);
                that._tabindex();
                that._updateClasses();
                that._dataSource();
                ...
                that.wrapper.children('.k-tabstrip-items').on(CLICK + NS, '.k-state-disabled .k-link', false).on(CLICK + NS, ' > ' + NAVIGATABLEITEMS, function (e) {
                    ...
                    if (wr !== document.activeElement) {
                        var msie = ufa.support.browser.msie;
                        if (msie) {
                            try {
                                wr.setActive();
                            } catch (j) {
            ...
            _dataSource: function () {
                ...
                that.dataSource = ufa.data.DataSource.create(that.options.dataSource).bind('change', that._refreshHandler);
            },
            ...
            refresh: function (e) {
                var that = this, options = that.options, text = ufa.getter(options.dataTextField), content = ufa.getter(options.dataContentField), contentUrl = ufa.getter(options.dataContentUrlField), image = ufa.getter(options.dataImageUrlField), url = ufa.getter(options.dataUrlField), sprite = ufa.getter(options.dataSpriteCssClass), idx, tabs = [], tab, action, view = that.dataSource.view(), length;
                e = e || {};
                action = e.action;
                if (action) {
                    view = e.items;
            ...
            _updateContentElements: function () {
                var that = this, contentUrls = that.options.contentUrls || [], items = that.tabGroup.find('.k-item'), tabStripID = (that.element.attr('id') || ufa.guid()) + '-', contentElements = that.wrapper.children('div');
                ...
                if (ufa.kineticScrollNeeded && ufa.mobile.ui.Scroller) {
                    ufa.touchScroller(that.contentElements);
            ...
            deactivateTab: function (item) {
                ...
                if (ufa.size(animation.effects)) {
                    item.ufaAddClass(DEFAULTSTATE, { duration: animation.duration });
                    item.ufaRemoveClass(ACTIVESTATE, { duration: animation.duration });
                } else {
                    item.addClass(DEFAULTSTATE);
                    item.removeClass(ACTIVESTATE);
                }
                item.removeAttr('aria-selected');
                that.contentAnimators.filter('.' + ACTIVESTATE).ufaStop(true, true).ufaAnimate(close).removeClass(ACTIVESTATE).attr('aria-hidden', true);
            },
            activateTab: function (item) {
                ...
                close = extend(hasCloseAnimation ? close : extend({ reverse: true }, animation), { hide: true });
                if (ufa.size(animation.effects)) {
                    oldTab.ufaRemoveClass(ACTIVESTATE, { duration: close.duration });
                    item.ufaRemoveClass(HOVERSTATE, { duration: close.duration });
                } else {
                ...
                if (contentHolder.length === 0) {
                    visibleContents.removeClass(ACTIVESTATE).attr('aria-hidden', true).ufaStop(true, true).ufaAnimate(close);
                    return false;
                }
                ...
                var isAjaxContent = (item.children('.' + LINK).data(CONTENTURL) || false) && contentHolder.is(EMPTY), showContentElement = function () {
                        ...
                        item.addClass(TABONTOP).css('z-index');
                        if (ufa.size(animation.effects)) {
                            oldTab.ufaAddClass(DEFAULTSTATE, { duration: animation.duration });
                            item.ufaAddClass(ACTIVESTATE, { duration: animation.duration });
                        } else {
                        ...
                        contentElement.addClass(ACTIVESTATE).removeAttr('aria-hidden').ufaStop(true, true).attr('aria-expanded', true).ufaAnimate(extend({
                            init: function () {
                                that.trigger(SHOW, {
                                    item: item[0],
                                    contentElement: contentHolder[0]
                                });
                                ufa.resize(contentHolder);
                            }
                        }, animation, {
                            complete: function () {
                ...
                if (visibleContents.length) {
                    visibleContents.ufaStop(true, true).ufaAnimate(extend({ complete: showContent }, close));
                } else {
                    showContent();
                }
                return true;
            ...
            contentElement: function (itemIndex) {
                ...
                var contentElements = this.contentElements && this.contentElements[0] && !ufa.kineticScrollNeeded ? this.contentElements : this.contentAnimators;
            ...
            contentHolder: function (itemIndex) {
                ...
                return ufa.support.touch && scrollContainer[0] ? scrollContainer : contentElement;
            },
