...
(function (f, define) {
    ...
    (function ($, undefined) {
        var math = Math, ufa = window.ufa, util = ufa.util, Widget = ufa.ui.Widget, deepExtend = ufa.deepExtend, dataviz = ufa.dataviz, autoMajorUnit = dataviz.autoMajorUnit, ChartElement = dataviz.ChartElement, NumericAxis = dataviz.NumericAxis, Axis = dataviz.Axis, Box2D = dataviz.Box2D, Class = ufa.Class, defined = util.defined, isNumber = util.isNumber, interpolateValue = dataviz.interpolateValue, getSpacing = dataviz.getSpacing, round = dataviz.round, geo = dataviz.geometry, draw = dataviz.drawing, Point = geo.Point, Group = draw.Group, Path = draw.Path, Rect = geo.Rect, Text = draw.Text;
        ...
        var RadialGauge = Gauge.extend({
            init: function (element, options) {
                ...
                ufa.notify(radialGauge, dataviz.ui);
            },
            options: {
                name: 'RadialGauge',
                transitions: true,
        ...
        var LinearGauge = Gauge.extend({
            init: function (element, options) {
                ...
                ufa.notify(linearGauge, dataviz.ui);
            },
            options: {
                name: 'LinearGauge',
                transitions: true,
