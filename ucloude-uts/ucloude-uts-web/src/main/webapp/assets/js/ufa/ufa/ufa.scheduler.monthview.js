(function (f, define) {
    define('ufa.scheduler.monthview', ['ufa.scheduler.view'], f);
}(function () {
    var __meta__ = {
    ...
    (function ($) {
        var ufa = window.ufa, ui = ufa.ui, SchedulerView = ui.SchedulerView, NS = '.ufaMonthView', extend = $.extend, getDate = ufa.date.getDate, MS_PER_DAY = ufa.date.MS_PER_DAY, NUMBER_OF_ROWS = 6, NUMBER_OF_COLUMNS = 7, DAY_TEMPLATE = ufa.template('<span class="k-link k-nav-day">#:ufa.toString(date, "dd")#</span>'), EVENT_WRAPPER_STRING = '<div role="gridcell" aria-selected="false" data-#=ns#uid="#=uid#"' + '#if (resources[0]) { #' + 'style="background-color:#=resources[0].color #; border-color: #=resources[0].color#"' + 'class="k-event#=inverseColor ? " k-event-inverse" : ""#"' + '#} else {#' + 'class="k-event"' + '#}#' + '>' + '<span class="k-event-actions">' + '# if(data.tail || data.middle) {#' + '<span class="k-icon k-i-arrow-w"></span>' + '#}#' + '# if(data.isException()) {#' + '<span class="k-icon k-i-exception"></span>' + '# } else if(data.isRecurring()) {#' + '<span class="k-icon k-i-refresh"></span>' + '#}#' + '</span>' + '{0}' + '<span class="k-event-actions">' + '#if (showDelete) {#' + '<a href="\\#" class="k-link k-event-delete"><span class="k-icon k-si-close"></span></a>' + '#}#' + '# if(data.head || data.middle) {#' + '<span class="k-icon k-i-arrow-e"></span>' + '#}#' + '</span>' + '# if(resizable && !data.tail && !data.middle) {#' + '<span class="k-resize-handle k-resize-w"></span>' + '#}#' + '# if(resizable && !data.head && !data.middle) {#' + '<span class="k-resize-handle k-resize-e"></span>' + '#}#' + '</div>', EVENT_TEMPLATE = ufa.template('<div title="#=title.replace(/"/g,"&\\#34;")#">' + '<div class="k-event-template">#:title#</div>' + '</div>');
        var MORE_BUTTON_TEMPLATE = ufa.template('<div style="width:#=width#px;left:#=left#px;top:#=top#px" class="k-more-events k-button"><span>...</span></div>');
        ui.MonthView = SchedulerView.extend({
            ...
            _changeViewPeriod: function (selection, reverse, vertical) {
                ...
                selection.start = ufa.date.addDays(selection.start, pad);
                selection.end = ufa.date.addDays(selection.end, pad);
            ...
            _templates: function () {
                ...
                this.dayTemplate = ufa.template(options.dayTemplate, settings);
                this.groupHeaderTemplate = ufa.template(options.groupHeaderTemplate, settings);
            },
            dateForTitle: function () {
                return ufa.format(this.options.selectedDateFormat, this._firstDayOfMonth, this._lastDayOfMonth);
            },
            shortDateForTitle: function () {
                return ufa.format(this.options.selectedShortDateFormat, this._firstDayOfMonth, this._lastDayOfMonth);
            },
            nextDate: function () {
                return ufa.date.nextDay(this._lastDayOfMonth);
            },
            previousDate: function () {
                return ufa.date.previousDay(this._firstDayOfMonth);
            },
            startDate: function () {
            ...
            _renderLayout: function (date) {
                var that = this;
                this._firstDayOfMonth = ufa.date.firstDayOfMonth(date);
                this._lastDayOfMonth = ufa.date.lastDayOfMonth(date);
                    ...
                    that._editUserEvents = new ufa.UserEvents(that.element, {
                        ...
                        tap: function (e) {
                            if ($(e.event.target).closest('a:has(.k-si-close)').length === 0) {
                                that.trigger('edit', { uid: $(e.target).closest('.k-event').attr(ufa.attr('uid')) });
                                e.preventDefault();
                            }
                        }
                    });
            ...
            _createCalendar: function (verticalGroupIndex) {
                ...
                for (var rowIdx = 0, length = cellCount / cellsPerRow; rowIdx < length; rowIdx++) {
                    ...
                    start = ufa.date.addDays(start, cellsPerRow);
                    html += '</tr>';
                }
                this._weekStartDates = weekStartDates;
                this._endDate = ufa.date.previousDay(start);
                return html;
            },
            _createRow: function (startDate, startIdx, cellsPerRow, groupIndex) {
                ...
                for (var cellIdx = 0; cellIdx < cellsPerRow; cellIdx++) {
                    classes = '';
                    if (ufa.date.isToday(startDate)) {
                        classes += 'k-today';
                    }
                    if (!ufa.date.isInDateRange(startDate, min, max)) {
                    ...
                    startDate = ufa.date.nextDay(startDate);
                }
                return html;
            },
            _layout: function () {
            ...
            _positionMobileEvent: function (slotRange, element, group) {
                ...
                if (!container) {
                    container = $(ufa.format('<div class="k-events-container" style="top:{0};left:{1};width:{2}"/>', startSlot.offsetTop + startSlot.firstChildTop + startSlot.firstChildHeight - 3 + 'px', startSlot.offsetLeft + 'px', startSlot.offsetWidth + 'px'));
            ...
            _updateResizeHint: function (event, groupIndex, startTime, endTime) {
                ...
                this._resizeHint.first().addClass('k-first').find('.k-label-top').text(ufa.toString(ufa.timezone.toLocalDate(startTime), 'M/dd'));
                this._resizeHint.last().addClass('k-last').find('.k-label-bottom').text(ufa.toString(ufa.timezone.toLocalDate(endTime), 'M/dd'));
            },
            ...
            _groups: function () {
                ...
                for (var groupIndex = 0; groupIndex < groupCount; groupIndex++) {
                    ...
                    for (var rowIndex = rowMultiplier * rowCount; rowIndex < (rowMultiplier + 1) * rowCount; rowIndex++) {
                        var group = this.groups[groupIndex];
                        var collection = group.addDaySlotCollection(ufa.date.addDays(startDate, cellCount), ufa.date.addDays(this.startDate(), cellCount + columnCount));
                        ...
                        for (var cellIndex = cellMultiplier * columnCount; cellIndex < (cellMultiplier + 1) * columnCount; cellIndex++) {
                            ...
                            var start = ufa.date.addDays(startDate, cellCount);
                            var end = ufa.date.MS_PER_DAY;
                            if (startDate.getHours() !== start.getHours()) {
                                end += (startDate.getHours() - start.getHours()) * ufa.date.MS_PER_HOUR;
                            }
                            start = ufa.date.toUtcTime(start);
                            end += start;
                            cellCount++;
            ...
            _renderGroups: function (events, resources, offset, columnLevel) {
                ...
                if (resource) {
                    ...
                    for (var itemIdx = 0; itemIdx < view.length; itemIdx++) {
                        ...
                        var tmp = new ufa.data.Query(events).filter({
        ...
        function firstVisibleMonthDay(date, calendarInfo) {
            ...
            while (firstVisibleDay.getDay() != firstDay) {
                ufa.date.setTime(firstVisibleDay, -1 * MS_PER_DAY);
            }
