(function (f, define) {
    ...
    (function ($, undefined) {
        var ufa = window.ufa, Widget = ufa.ui.Widget, proxy = $.proxy, isRtl = false, NS = '.ufaGroupable', CHANGE = 'change', indicatorTmpl = ufa.template('<div class="k-group-indicator" data-#=data.ns#field="${data.field}" data-#=data.ns#title="${data.title || ""}" data-#=data.ns#dir="${data.dir || "asc"}">' + '<a href="\\#" class="k-link">' + '<span class="k-icon k-si-arrow-${(data.dir || "asc") == "asc" ? "n" : "s"}">(sorted ${(data.dir || "asc") == "asc" ? "ascending": "descending"})</span>' + '${data.title ? data.title: data.field}' + '</a>' + '<a class="k-button k-button-icon k-button-bare">' + '<span class="k-icon k-group-delete"></span>' + '</a>' + '</div>', { useWithBlock: false }), hint = function (target) {
                ...
                }).html(target.attr(ufa.attr('title')) || target.attr(ufa.attr('field'))).prepend('<span class="k-icon k-drag-status k-denied" />');
        ...
        var Groupable = Widget.extend({
            init: function (element, options) {
                var that = this, group = ufa.guid(), intializePositions = proxy(that._intializePositions, that), draggable, horizontalCuePosition, dropCuePositions = that._dropCuePositions = [];
                Widget.fn.init.call(that, element, options);
                isRtl = ufa.support.isRtl(element);
                horizontalCuePosition = isRtl ? 'right' : 'left';
                that.draggable = draggable = that.options.draggable || new ufa.ui.Draggable(that.element, {
                    filter: that.options.draggableElements,
                    hint: hint,
                    group: group
                });
                that.groupContainer = $(that.options.groupContainer, that.element).ufaDropTarget({
                    ...
                    drop: function (e) {
                        var targetElement = e.draggable.currentTarget, field = targetElement.attr(ufa.attr('field')), title = targetElement.attr(ufa.attr('title')), sourceIndicator = that.indicator(field), dropCuePositions = that._dropCuePositions, lastCuePosition = dropCuePositions[dropCuePositions.length - 1], position;
                        if (!targetElement.hasClass('k-group-indicator') && !that._canDrag(targetElement)) {
                            return;
                        }
                        if (lastCuePosition) {
                            position = that._dropCuePosition(ufa.getOffset(dropCue).left + parseInt(lastCuePosition.element.css('marginLeft'), 10) * (isRtl ? -1 : 1) + parseInt(lastCuePosition.element.css('marginRight'), 10));
                    ...
                    var current = $(this).parent(), newIndicator = that.buildIndicator(current.attr(ufa.attr('field')), current.attr(ufa.attr('title')), current.attr(ufa.attr('dir')) == 'asc' ? 'desc' : 'asc');
            ...
            refresh: function () {
                ...
                if (that.groupContainer) {
                    that.groupContainer.empty().append($.map(dataSource.group() || [], function (item) {
                        ...
                        var attr = ufa.attr('field');
                        var element = that.element.find(that.options.filter).filter(function () {
                            return $(this).attr(attr) === fieldName;
                        });
                        return that.buildIndicator(item.field, element.attr(ufa.attr('title')), item.dir);
                    }).join(''));
                }
            ...
            destroy: function () {
                ...
                if (that.groupContainer.data('ufaDropTarget')) {
                    that.groupContainer.data('ufaDropTarget').destroy();
                }
                if (that.groupContainer.data('ufaDraggable')) {
                    that.groupContainer.data('ufaDraggable').destroy();
                }
            ...
            indicator: function (field) {
                ...
                return $.grep(indicators, function (item) {
                    return $(item).attr(ufa.attr('field')) === field;
                })[0];
            },
            ...
            descriptors: function () {
                ...
                aggregates = that.element.find(that.options.filter).map(function () {
                    var cell = $(this), aggregate = cell.attr(ufa.attr('aggregates')), member = cell.attr(ufa.attr('field'));
                ...
                return $.map(indicators, function (item) {
                    item = $(item);
                    field = item.attr(ufa.attr('field'));
                    return {
                        field: field,
                        dir: item.attr(ufa.attr('dir')),
            ...
            _canDrag: function (element) {
                var field = element.attr(ufa.attr('field'));
                return element.attr(ufa.attr('groupable')) != 'false' && field && (element.hasClass('k-group-indicator') || !this.indicator(field));
            },
            ...
            _dragEnd: function (draggable) {
                var that = this, field = draggable.currentTarget.attr(ufa.attr('field')), sourceIndicator = that.indicator(field);
