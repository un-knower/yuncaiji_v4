(function (f, define) {
    define('ufa.slider', ['ufa.draganddrop'], f);
}(function () {
    var __meta__ = {
        id: 'slider',
        name: 'Slider',
    ...
    (function ($, undefined) {
        var ufa = window.ufa, Widget = ufa.ui.Widget, Draggable = ufa.ui.Draggable, extend = $.extend, format = ufa.format, parse = ufa.parseFloat, proxy = $.proxy, isArray = $.isArray, math = Math, support = ufa.support, pointers = support.pointers, msPointers = support.msPointers, CHANGE = 'change', SLIDE = 'slide', NS = '.slider', MOUSE_DOWN = 'touchstart' + NS + ' mousedown' + NS, TRACK_MOUSE_DOWN = pointers ? 'pointerdown' + NS : msPointers ? 'MSPointerDown' + NS : MOUSE_DOWN, MOUSE_UP = 'touchend' + NS + ' mouseup' + NS, TRACK_MOUSE_UP = pointers ? 'pointerup' : msPointers ? 'MSPointerUp' + NS : MOUSE_UP, MOVE_SELECTION = 'moveSelection', KEY_DOWN = 'keydown' + NS, CLICK = 'click' + NS, MOUSE_OVER = 'mouseover' + NS, FOCUS = 'focus' + NS, BLUR = 'blur' + NS, DRAG_HANDLE = '.k-draghandle', TRACK_SELECTOR = '.k-slider-track', TICK_SELECTOR = '.k-tick', STATE_SELECTED = 'k-state-selected', STATE_FOCUSED = 'k-state-focused', STATE_DEFAULT = 'k-state-default', STATE_DISABLED = 'k-state-disabled', DISABLED = 'disabled', UNDEFINED = 'undefined', TABINDEX = 'tabindex', getTouches = ufa.getTouches;
        var SliderBase = Widget.extend({
            init: function (element, options) {
                ...
                that._isRtl = that._isHorizontal && ufa.support.isRtl(element);
                that._position = that._isHorizontal ? 'left' : 'bottom';
                that._sizeFn = that._isHorizontal ? 'width' : 'height';
                that._outerSize = that._isHorizontal ? 'outerWidth' : 'outerHeight';
                options.tooltip.format = options.tooltip.enabled ? options.tooltip.format || '{0}' : '{0}';
                if (options.smallStep <= 0) {
                    throw new Error('ufa UI Slider smallStep must be a positive number.');
                }
                that._createHtml();
                ...
                var rtlDirectionSign = ufa.support.isRtl(that.wrapper) ? -1 : 1;
                that._keyMap = {
            ...
            getSize: function () {
                return ufa.dimensions(this.wrapper);
            },
            ...
            _getFormattedValue: function (val, drag) {
                ...
                if (!tooltipTemplate && tooltip.template) {
                    tooltipTemplate = ufa.template(tooltip.template);
                }
            ...
            _getDraggableArea: function () {
                var that = this, offset = ufa.getOffset(that._trackDiv);
                return {
        ...
        function formatValue(value) {
            return (value + '').replace('.', ufa.cultures.current.numberFormat['.']);
        }
        ...
        var Slider = SliderBase.extend({
            ...
            enable: function (enable) {
                ...
                that.wrapper.find(TICK_SELECTOR + ', ' + TRACK_SELECTOR).on(TRACK_MOUSE_DOWN, clickHandler).end().on(TRACK_MOUSE_DOWN, function () {
                    $(document.documentElement).one('selectstart', ufa.preventDefault);
            ...
            disable: function () {
                ...
                that.wrapper.find('.k-button').off(MOUSE_DOWN).on(MOUSE_DOWN, ufa.preventDefault).off(MOUSE_UP).on(MOUSE_UP, ufa.preventDefault).off('mouseleave' + NS).on('mouseleave' + NS, ufa.preventDefault).off(MOUSE_OVER).on(MOUSE_OVER, ufa.preventDefault);
        ...
        Slider.Drag.prototype = {
            ...
            _createTooltip: function () {
                ...
                if (tooltip.template) {
                    tooltipTemplate = that.tooltipTemplate = ufa.template(tooltip.template);
                }
            ...
            moveTooltip: function () {
                var that = this, owner = that.owner, top = 0, left = 0, element = that.element, offset = ufa.getOffset(element), margin = 8, viewport = $(window), callout = that.tooltipDiv.find('.k-callout'), width = that.tooltipDiv.outerWidth(), height = that.tooltipDiv.outerHeight(), dragHandles, sdhOffset, diff, anchorSize;
                if (that.type) {
                    dragHandles = owner.wrapper.find(DRAG_HANDLE);
                    offset = ufa.getOffset(dragHandles.eq(0));
                    sdhOffset = ufa.getOffset(dragHandles.eq(1));
                    if (owner._isHorizontal) {
                        top = sdhOffset.top;
        ...
        var RangeSlider = SliderBase.extend({
            ...
            enable: function (enable) {
                ...
                that.wrapper.find(DRAG_HANDLE).off(KEY_DOWN, ufa.preventDefault).eq(0).on(KEY_DOWN, proxy(function (e) {
