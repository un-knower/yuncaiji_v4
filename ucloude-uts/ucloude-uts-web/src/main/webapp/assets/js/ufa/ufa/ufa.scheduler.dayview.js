(function (f, define) {
    define('ufa.scheduler.dayview', ['ufa.scheduler.view'], f);
}(function () {
    var __meta__ = {
    ...
    (function ($, undefined) {
        ...
        var MultiDayView = SchedulerView.extend({
            ...
            _currentTimeMarkerUpdater: function () {
                ...
                if (options.currentTimeMarker.useLocalTimezone === false) {
                    ...
                    if (options.dataSource && timezone) {
                        var timezoneOffset = ufa.timezone.offset(currentTime, timezone);
                        currentTime = ufa.timezone.convert(currentTime, currentTime.getTimezoneOffset(), timezoneOffset);
                    }
                }
                ...
                for (var groupIndex = 0; groupIndex < groupsCount; groupIndex++) {
                    ...
                    var utcCurrentTime = ufa.date.toUtcTime(currentTime);
            ...
            _updateResizeHint: function (event, groupIndex, startTime, endTime) {
                ...
                this._resizeHint.first().addClass('k-first').find('.k-label-top').text(ufa.toString(ufa.timezone.toLocalDate(startTime), format));
                this._resizeHint.last().addClass('k-last').find('.k-label-bottom').text(ufa.toString(ufa.timezone.toLocalDate(endTime), format));
            },
            _updateMoveHint: function (event, groupIndex, distance) {
                var multiday = event.isMultiDay();
                var group = this.groups[groupIndex];
                var start = ufa.date.toUtcTime(event.start) + distance;
                var end = start + event.duration();
                var ranges = group.ranges(start, end, multiday, event.isAllDay);
                start = ufa.timezone.toLocalDate(start);
                end = ufa.timezone.toLocalDate(end);
            ...
            _daySlotGroups: function (groupCount, columnCount) {
                ...
                for (var groupIndex = 0; groupIndex < groupCount; groupIndex++) {
                    ...
                    for (var cellIndex = cellMultiplier * columnCount; cellIndex < (cellMultiplier + 1) * columnCount; cellIndex++) {
                        ...
                        cell.setAttribute('role', 'gridcell');
                        cell.setAttribute('aria-selected', false);
                        collection.addDaySlot(cell, currentTime, currentTime + ufa.date.MS_PER_DAY);
                    }
                }
            },
            _groups: function () {
                ...
                for (var idx = 0; idx < groupCount; idx++) {
                    ...
                    for (var columnIndex = 0; columnIndex < columnCount; columnIndex++) {
                        view.addTimeSlotCollection(this._dates[columnIndex], ufa.date.addDays(this._dates[columnIndex], 1));
                    }
                    if (this.options.allDaySlot) {
                        view.addDaySlotCollection(this._dates[0], ufa.date.addDays(this._dates[this._dates.length - 1], 1));
                    }
                }
            ...
            options: {
                ...
                showWorkHours: false,
                title: '',
                startTime: ufa.date.today(),
                endTime: ufa.date.today(),
                minorTickCount: 2,
                majorTick: 60,
                majorTimeHeaderTemplate: '#=ufa.toString(date, \'t\')#',
            ...
            _templates: function () {
                var options = this.options, settings = extend({}, ufa.Template, options.templateSettings);
                this.eventTemplate = this._eventTmpl(options.eventTemplate, EVENT_WRAPPER_STRING);
                this.allDayEventTemplate = this._eventTmpl(options.allDayEventTemplate, ALLDAY_EVENT_WRAPPER_STRING);
                this.majorTimeHeaderTemplate = ufa.template(options.majorTimeHeaderTemplate, settings);
                this.minorTimeHeaderTemplate = ufa.template(options.minorTimeHeaderTemplate, settings);
                this.dateHeaderTemplate = ufa.template(options.dateHeaderTemplate, settings);
                this.slotTemplate = ufa.template(options.slotTemplate, settings);
                this.allDaySlotTemplate = ufa.template(options.allDaySlotTemplate, settings);
                this.groupHeaderTemplate = ufa.template(options.groupHeaderTemplate, settings);
            },
            _editable: function () {
            ...
            _mouseEditable: function () {
                ...
                that.element.on('click' + NS, '.k-event a:has(.k-si-close)', function (e) {
                    that.trigger('remove', { uid: $(this).closest('.k-event').attr(ufa.attr('uid')) });
                    e.preventDefault();
                });
                    ...
                    that.element.on('dblclick' + NS, '.k-scheduler-content td', function (e) {
                        ...
                        if (slot) {
                            ...
                            that.trigger('add', {
                                eventInfo: extend({}, {
                                    isAllDay: true,
                                    start: ufa.date.getDate(slot.startDate()),
                                    end: ufa.date.getDate(slot.startDate())
                                }, resourceInfo)
                            });
                        }
                        e.preventDefault();
                ...
                if (that.options.editable.update !== false) {
                    that.element.on('dblclick' + NS, '.k-event', function (e) {
                        that.trigger('edit', { uid: $(this).closest('.k-event').attr(ufa.attr('uid')) });
                        e.preventDefault();
                    });
                }
            },
            _touchEditable: function () {
                var that = this;
                var threshold = 0;
                if (ufa.support.mobileOS.android) {
                    threshold = 5;
                }
                if (that.options.editable.create !== false) {
                    that._addUserEvents = new ufa.UserEvents(that.element, {
                        threshold: threshold,
                    ...
                    that._allDayUserEvents = new ufa.UserEvents(that.element, {
                        threshold: threshold,
                ...
                if (that.options.editable.update !== false) {
                    that._editUserEvents = new ufa.UserEvents(that.element, {
                        threshold: threshold,
                        filter: '.k-event',
                        tap: function (e) {
                            var eventElement = $(e.target).closest('.k-event');
                            if (!eventElement.hasClass('k-event-active')) {
                                that.trigger('edit', { uid: eventElement.attr(ufa.attr('uid')) });
                            }
                            e.preventDefault();
                        }
                    });
                ...
                var appendRow = function (date, majorTick) {
                    ...
                    for (; groupIdx < groupsCount; groupIdx++) {
                        for (idx = 0, length = columnCount; idx < length; idx++) {
                            ...
                            if (ufa.date.getMilliseconds(date) < ufa.date.getMilliseconds(that.options.workDayStart) || ufa.date.getMilliseconds(date) >= ufa.date.getMilliseconds(that.options.workDayEnd) || !that._isWorkDay(dates[idx])) {
                                classes += ' k-nonwork-hour';
                            }
                            content += '<td' + (classes !== '' ? ' class="' + classes + '"' : '') + '>';
                            tmplDate = ufa.date.getDate(dates[idx]);
                            ufa.date.setTime(tmplDate, ufa.date.getMilliseconds(date));
            ...
            nextDate: function () {
                return ufa.date.nextDay(this.endDate());
            },
            previousDate: function () {
                return ufa.date.previousDay(this.startDate());
            },
            ...
            inRange: function (options) {
                ...
                var endTime = getMilliseconds(this.endTime()) || ufa.date.MS_PER_DAY;
                var start = getMilliseconds(options.start);
                var end = getMilliseconds(options.end) || ufa.date.MS_PER_DAY;
            ...
            _dateSlotIndex: function (date, overlaps) {
                ...
                for (idx = 0, length = slots.length; idx < length; idx++) {
                    slotStart = ufa.date.getDate(slots[idx]);
                    slotEnd = new Date(ufa.date.getDate(slots[idx]).getTime() + MS_PER_DAY - (overlaps ? 0 : 1));
            ...
            _createEventElement: function (event, isOneDayEvent, head, tail) {
                ...
                if (event._startTime && eventStartTime !== ufa.date.getMilliseconds(event.start)) {
                    eventStartDate = new Date(eventStartTime);
                    eventStartDate = ufa.timezone.apply(eventStartDate, 'Etc/UTC');
                }
                if (event._endTime && eventEndTime !== ufa.date.getMilliseconds(event.end)) {
                    eventEndDate = new Date(eventEndTime);
                    eventEndDate = ufa.timezone.apply(eventEndDate, 'Etc/UTC');
                }
                var data = extend({}, {
                    ns: ufa.ns,
                    resizable: resizable,
            ...
            _isInTimeSlot: function (event) {
                ...
                if (getMilliseconds(slotEndTime) === getMilliseconds(ufa.date.getDate(slotEndTime))) {
                    slotEndTime = ufa.date.getDate(slotEndTime);
            ...
            _isInDateSlot: function (event) {
                ...
                var startTime = ufa.date.toUtcTime(event.start);
                var endTime = ufa.date.toUtcTime(event.end);
            ...
            render: function (events) {
                ...
                events = new ufa.data.Query(events).sort([
                    {
                        field: 'start',
                        dir: 'asc'
                    },
            ...
            _eventsByResource: function (events, resources, result) {
                ...
                if (resource) {
                    ...
                    for (var itemIdx = 0; itemIdx < view.length; itemIdx++) {
                        ...
                        var eventsFilteredByResource = new ufa.data.Query(events).filter({
        ...
        extend(true, ui, {
            ...
            WeekView: MultiDayView.extend({
                ...
                name: 'week',
                calculateDateRange: function () {
                    var selectedDate = this.options.date, start = ufa.date.dayOfWeek(selectedDate, this.calendarInfo().firstDay, -1), idx, length, dates = [];
                    for (idx = 0, length = 7; idx < length; idx++) {
                        dates.push(start);
                        start = ufa.date.nextDay(start);
                    }
                    this._render(dates);
                }
            }),
            WorkWeekView: MultiDayView.extend({
                ...
                name: 'workWeek',
                nextDate: function () {
                    return ufa.date.dayOfWeek(ufa.date.nextDay(this.startDate()), this.calendarInfo().firstDay, 1);
                },
                previousDate: function () {
                    var weekStart = ufa.date.dayOfWeek(this.startDate(), this.calendarInfo().firstDay, -1);
                    return ufa.date.previousDay(weekStart);
                },
                calculateDateRange: function () {
                    var selectedDate = this.options.date, dayOfWeek = ufa.date.dayOfWeek, weekStart = dayOfWeek(selectedDate, this.calendarInfo().firstDay, -1), start = dayOfWeek(weekStart, this.options.workWeekStart, 1), end = dayOfWeek(start, this.options.workWeekEnd, 1), dates = [];
                    while (start <= end) {
                        dates.push(start);
                        start = ufa.date.nextDay(start);
                    }
