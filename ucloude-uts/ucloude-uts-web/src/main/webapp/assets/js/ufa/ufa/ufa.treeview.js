(function (f, define) {
    ...
    (function ($, undefined) {
        ...
        function templateNoWith(code) {
            return ufa.template(code, { useWithBlock: false });
        }
        ...
        TreeView = ufa.ui.DataBoundWidget.extend({
            init: function (element, options) {
                ...
                if (that.element[0].id) {
                    that._ariaId = ufa.format('{0}_tv_active', that.element[0].id);
                }
                ufa.notify(that);
            },
            ...
            _syncHtmlAndDataSource: function (root, dataSource) {
                ...
                var uidAttr = ufa.attr('uid');
                var expandedAttr = ufa.attr('expanded');
            ...
            _templates: function () {
                ...
                that.templates = {
                    ...
                    item: templateNoWith('# var item = data.item, r = data.r; #' + '<li role=\'treeitem\' class=\'#= r.wrapperCssClass(data.group, item) #\' ' + ufa.attr('uid') + '=\'#= item.uid #\' ' + 'aria-selected=\'#= item.selected ? "true" : "false " #\' ' + '#=item.enabled === false ? "aria-disabled=\'true\'" : \'\'#' + '# if (item.expanded) { #' + 'data-expanded=\'true\' aria-expanded=\'true\'' + '# } #' + '>' + '#= r.itemElement(data) #' + '</li>'),
            ...
            _accessors: function () {
                ...
                for (i in bindings) {
                    ...
                    textField = element.attr(ufa.attr(i + '-field'));
                    if (!field && textField) {
                        field = textField;
                    }
                    if (!field) {
            ...
            _fieldAccessor: function (fieldName) {
                ...
                if (count === 0) {
                    ...
                    result += 'var levels = [' + $.map(fieldBindings, function (x) {
                        return 'function(d){ return ' + ufa.expr(x) + '}';
                    }).join(',') + '];';
            ...
            updateIndeterminate: function (node) {
                ...
                if (subnodes.length) {
                    ...
                    if (checkbox && checkbox.prop(CHECKED)&& this.dataItem(node)!=undefined) {
            ...
            _keydown: function (e) {
                var that = this, key = e.keyCode, target, focused = that.current(), expanded = that._expanded(focused), checkbox = focused.find('.k-checkbox-wrapper:first :checkbox'), rtl = ufa.support.isRtl(that.element);
            ...
            dataItem: function (node) {
                var uid = $(node).closest(NODE).attr(ufa.attr('uid')), dataSource = this.dataSource;
            ...
            _toggle: function (node, dataItem, expand) {
                ...
                if (!this._trigger(direction, node)) {
                    ...
                    if (expand && !loaded) {
                        ...
                        contents.ufaStop(true, true).ufaAnimate(extend({ reset: true }, options.animation[direction], {
                            complete: function () {
                                if (expand) {
                                    contents.css('height', '');
                                }
            ...
            _expanded: function (node, value) {
                var expandedAttr = ufa.attr('expanded'), dataItem = this.dataItem(node);
                if (arguments.length == 1) {
            ...
            _objectOrSelf: function (node) {
                return $(node).closest('[data-role=treeview]').data('ufaTreeView') || this;
            },
            ...
            _insert: function (data, model, index) {
                if (!(model instanceof ufa.data.ObservableArray)) {
                    if (!isArray(model)) {
                        model = [model];
                    }
                } else {
            ...
            expandTo: function (node) {
                if (!(node instanceof ufa.data.Node)) {
