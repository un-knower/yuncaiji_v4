(function (f, define) {
    define('ufa.calendar', ['ufa.core'], f);
}(function () {
    var __meta__ = {
        id: 'calendar',
        name: 'Calendar',
    ...
    (function ($, undefined) {
        var ufa = window.ufa, support = ufa.support,  WEEK = "week",ui = ufa.ui, Widget = ui.Widget, keys = ufa.keys, parse = ufa.parseDate, adjustDST = ufa.date.adjustDST, extractFormat = ufa._extractFormat, template = ufa.template, getCulture = ufa.getCulture, transitions = ufa.support.transitions, transitionOrigin = transitions ? transitions.css + 'transform-origin' : '', cellTemplate = template('<td#=data.cssClass# role="gridcell"><a tabindex="-1" class="k-link" href="\\#" data-#=data.ns#value="#=data.dateString#">#=data.value#</a></td>', { useWithBlock: false }), emptyCellTemplate = template('<td role="gridcell">&nbsp;</td>', { useWithBlock: false }), browser = ufa.support.browser, isIE8 = browser.msie && browser.version < 9, ns = '.ufaCalendar', CLICK = 'click' + ns, KEYDOWN_NS = 'keydown' + ns, ID = 'id', MIN = 'min', LEFT = 'left', SLIDE = 'slideIn', MONTH = 'month', CENTURY = 'century', CHANGE = 'change', NAVIGATE = 'navigate', VALUE = 'value', HOVER = 'k-state-hover', DISABLED = 'k-state-disabled', FOCUSED = 'k-state-focused', OTHERMONTH = 'k-other-month', OTHERMONTHCLASS = ' class="' + OTHERMONTH + '"', TODAY = 'k-nav-today', CELLSELECTOR = 'td:has(.k-link)', BLUR = 'blur' + ns, FOCUS = 'focus', FOCUS_WITH_NS = FOCUS + ns, MOUSEENTER = support.touch ? 'touchstart' : 'mouseenter', MOUSEENTER_WITH_NS = support.touch ? 'touchstart' + ns : 'mouseenter' + ns, MOUSELEAVE = support.touch ? 'touchend' + ns + ' touchmove' + ns : 'mouseleave' + ns, MS_PER_MINUTE = 60000, MS_PER_DAY = 86400000, PREVARROW = '_prevArrow', NEXTARROW = '_nextArrow', ARIA_DISABLED = 'aria-disabled', ARIA_SELECTED = 'aria-selected', proxy = $.proxy, extend = $.extend, DATE = Date, views = {
                month: 0,
                year: 1,
                decade: 2,
                century: 3,
        ...
        var Calendar = Widget.extend({
            ...
            navigate: function (value, view) {
                ...
                var that = this, options = that.options, culture = options.culture,depth = options.depth;
                if(view >4 && that._index == 4)
	            {
	                view = 2;
	            }
	            else if(view == 1 && that._index == 2 && depth == "week")
	            {
	                view = 4;
	            }

                var min = options.min, max = options.max, title = that._title, from = that._table, old = that._oldTable, selectedValue = that._value, currentValue = that._current, future = value && +value > +currentValue, vertical = view !== undefined && view !== that._index, to, currentView, compare, disabled;
                if (!value) {
                    value = currentValue;
                }
            ...
            _move: function (e) {
                var that = this, options = that.options, key = e.keyCode, view = that._view, index = that._index, min = that.options.min, max = that.options.max, currentValue = new DATE(+that._current), isRtl = ufa.support.isRtl(that.wrapper), isDisabled = that.options.disableDates, value, prevent, method, temp;
            ...
            _footer: function (template) {
                ...
                that._today = footer.show().find('.k-link').html(template(today)).attr('title', ufa.toString(today, 'D', that.options.culture));
                that._toggle();
            },
            _header: function () {
            ...
            _navigate: function (arrow, modifier) {
                ...
                if (!arrow.hasClass(DISABLED)) {
                    if(index == 5)
	                {
	                    calendar.views[index - 3].setDate(currentValue, modifier);
	                }
	                else if (index > 3) {
            ...
            _toDateObject: function (link) {
                var value = $(link).attr(ufa.attr(VALUE)).split('/');
            ...
            _templates: function () {
                ...
                that.month = {
                    content: template('<td#=data.cssClass# role="gridcell"><a tabindex="-1" class="k-link#=data.linkClass#" href="#=data.url#" ' + ufa.attr('value') + '="#=data.dateString#" title="#=data.title#">' + (content || '#=data.value#') + '</a></td>', { useWithBlock: !!content }),
                    empty: template('<td role="gridcell">' + (empty || '&nbsp;') + '</td>', { useWithBlock: !!empty })
                };
                that.footer = footer !== false ? template(footer || '#= ufa.toString(data,"D","' + options.culture + '") #', { useWithBlock: false }) : null;
            }
        });
        ui.plugin(Calendar);
        var calendar = {
            ...
            firstVisibleDay: function (date, calendarInfo) {
                calendarInfo = calendarInfo || ufa.culture().calendar;
            ...
            views: [
                        ...
                        return view({
                            ...
                            build: function (date, idx, disableDates) {
                                ...
                                if (hasUrl && inArray(+date, dates)) {
                                    url = navigateUrl.replace('{0}', ufa.toString(date, format, culture));
                                ...
                                return {
                                    ...
                                    dates: dates,
                                    ns: ufa.ns,
                                    title: ufa.toString(date, 'D', culture),
                                    value: date.getDate(),
                ...
                },
		        {
		            ...
		            content: function(options) {
		                var namesAbbr = this.getWeekInfo,
		                toDateString = this.toDateString1,
		                toDateString2=this.toDateString3,
		                first = this.first,
		                last = this.last,
		                min = options.min,
		                max = options.max;
		                ...
		                return view({
		                    ...
		                    max: new DATE(max.getFullYear(), max.getMonth(), 1),
		                    start:  new DATE(options.date.getFullYear(), 0, 1),
		                    content:  template('<td#=data.cssClass# role="gridcell"><a tabindex="-1" class="k-link" href="\\#" data-#=data.ns#value="#=data.dateString#" title="#=data.title#">#=data.value#</a></td>', { useWithBlock: false }),
		                    setter: this.setDate,
		                    build: function(date,idx) {
		                        var start = ufa.date.getFirstWeekBeginDay(date.getFullYear()),
		                            date = ufa.date.addDays(start,idx*7),
		                            cssClass = [];
		                        if (date > new Date(start.getFullYear()+1,0,1)) {
		                            cssClass.push(OTHERMONTH);
		                        }
		                        return {
		                            value: namesAbbr(date),
		                            ns: ufa.ns,
		                            title: ufa.toString(date, "w"),
		                            dateString: toDateString(date),
		                            dateTitle:toDateString2(date),
		                            cssClass: cssClass[0] ? ' class="' + cssClass.join(" ") + '"' : "",
		                        };
		                    }
		                });
		            ...
		            first: function(date) {
		                var firstDay = ufa.date.getFirstWeekBeginDay(date.getFullYear());
		                if(date < firstDay)
		                    firstDay = ufa.date.getFirstWeekBeginDay(date.getFullYear() -1);
		                return new DATE(firstDay.getFullYear(),0,firstDay.getDate());       
		            },
		            
		            last: function(date) {
		                var that = this;
		                var firstday = that.first(date);
		                return new DATE(firstDay.getFullYear(),11,31)
		            },
		            compare: function(date1, date2){
		                return compare(date1, date2);
		            },
		            ...
		            getWeekInfo:function(date){
		                 var result = ufa.date.getFirstWeekBeginDay(date.getFullYear());
		                var s1 = Math.ceil((date - result)/(24*60*60*1000));
		                return  (Math.floor(s1/7)+1)+'周';
		            },
		            toDateString:function(date)
		            {
		                var result = ufa.date.getFirstWeekBeginDay(date.getFullYear());
		                var s1 = Math.ceil((date - result)/(24*60*60*1000));
		                date = ufa.date.addDays(result,(Math.floor(s1/7))*7);
		                 return date.getFullYear() + "/" + date.getMonth() + "/" + date.getDate();
		            },
		            toDateString1: function(date) {
		               return date.getFullYear() + "/" + date.getMonth() + "/" + date.getDate();
		                
		            },
		            toDateString3: function(date) {
		               return ufa.toString(new Date(date.setDate(date.getDate())), 'yyyy/MM/dd')+"-"+ufa.toString(new Date(date.setDate(date.getDate() + 6)), 'yyyy/MM/dd');
