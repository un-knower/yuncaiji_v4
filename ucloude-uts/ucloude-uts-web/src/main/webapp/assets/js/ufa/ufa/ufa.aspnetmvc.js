(function (f, define) {
    define('aspnetmvc/ufa.data.aspnetmvc', [
        'ufa.data',
        'ufa.combobox',
        'ufa.multiselect',
        'ufa.validator'
    ...
    (function ($, undefined) {
        var ufa = window.ufa, escapeQuoteRegExp = /'/gi, extend = $.extend, isArray = $.isArray, isPlainObject = $.isPlainObject, POINT = '.';
        ...
        var Serializer = function (options) {
            options = options || {};
            this.culture = options.culture || ufa.culture();
            ...
            serializeValue: function (value) {
                if (value instanceof Date) {
                    if (this.stringifyDates) {
                        value = ufa.stringify(value).replace(/"/g, '');
                    } else {
                        value = ufa.toString(value, 'G', this.culture.name);
                    }
        ...
        function encodeFilterValue(value, encode) {
            ...
            if (value && value.getTime) {
                return 'datetime\'' + ufa.format('{0:yyyy-MM-ddTHH-mm-ss}', value) + '\'';
            }
            return value;
        }
        ...
        extend(true, ufa.data, {
            transports: {
                'aspnetmvc-ajax': ufa.data.RemoteTransport.extend({
                    ...
                    read: function (options) {
                        ...
                        if (isPlainObject(data)) {
                            ...
                            if (!data.Data.length && url) {
                                ufa.data.RemoteTransport.fn.read.call(this, options);
                            } else {
        ...
        extend(true, ufa.data, { schemas: { 'webapi': ufa.data.schemas['aspnetmvc-ajax'] } });
        extend(true, ufa.data, {
            transports: {
                'webapi': ufa.data.RemoteTransport.extend({
                    init: function (options) {
                        var that = this;
                        ...
                        if (options.update) {
                            ...
                            options.update = extend(options.update, {
                                url: function (data) {
                                    return ufa.format(updateUrl, data[options.idField]);
                                }
                            });
                        }
                        if (options.destroy) {
                            ...
                            options.destroy = extend(options.destroy, {
                                url: function (data) {
                                    return ufa.format(destroyUrl, data[options.idField]);
                                }
                            });
                        }
        ...
        extend(true, ufa.data, {
            transports: {
                'aspnetmvc-server': ufa.data.RemoteTransport.extend({
                    init: function (options) {
                        var that = this;
                        ufa.data.RemoteTransport.fn.init.call(this, extend(options, {
                            parameterMap: function (options, operation) {
...
(function (f, define) {
    define('aspnetmvc/ufa.combobox.aspnetmvc', ['aspnetmvc/ufa.data.aspnetmvc'], f);
}(function () {
    (function ($, undefined) {
        var ufa = window.ufa, ui = ufa.ui;
        if (ui && ui.ComboBox) {
            ui.ComboBox.requestData = function (selector) {
                var combobox = $(selector).data('ufaComboBox');
                if (!combobox) {
                    return;
                }
...
(function (f, define) {
    define('aspnetmvc/ufa.dropdownlist.aspnetmvc', ['aspnetmvc/ufa.data.aspnetmvc'], f);
}(function () {
    (function ($, undefined) {
        var ufa = window.ufa, ui = ufa.ui;
        if (ui && ui.DropDownList) {
            ui.DropDownList.requestData = function (selector) {
                var dropdownlist = $(selector).data('ufaDropDownList');
                if (!dropdownlist) {
                    return;
                }
...
(function (f, define) {
    define('aspnetmvc/ufa.multiselect.aspnetmvc', ['aspnetmvc/ufa.combobox.aspnetmvc'], f);
}(function () {
    (function ($, undefined) {
        var ufa = window.ufa, ui = ufa.ui;
        if (ui && ui.MultiSelect) {
            ui.MultiSelect.requestData = function (selector) {
                var multiselect = $(selector).data('ufaMultiSelect');
                if (!multiselect) {
                    return;
                }
...
(function (f, define) {
    define('aspnetmvc/ufa.imagebrowser.aspnetmvc', ['aspnetmvc/ufa.multiselect.aspnetmvc'], f);
}(function () {
    (function ($, undefined) {
        var ufa = window.ufa, extend = $.extend, isFunction = $.isFunction;
        extend(true, ufa.data, {
            schemas: {
        ...
        extend(true, ufa.data, { schemas: { 'filebrowser-aspnetmvc': ufa.data.schemas['imagebrowser-aspnetmvc'] } });
        extend(true, ufa.data, {
            transports: {
                'imagebrowser-aspnetmvc': ufa.data.RemoteTransport.extend({
                    init: function (options) {
                        ufa.data.RemoteTransport.fn.init.call(this, $.extend(true, {}, this.options, options));
                    },
        ...
        extend(true, ufa.data, { transports: { 'filebrowser-aspnetmvc': ufa.data.transports['imagebrowser-aspnetmvc'] } });
    }(window.ufa.jQuery));
}, typeof define == 'function' && define.amd ? define : function (a1, a2, a3) {
    (a3 || a2)();
}));
(function (f, define) {
    define('aspnetmvc/ufa.validator.aspnetmvc', ['aspnetmvc/ufa.imagebrowser.aspnetmvc'], f);
}(function () {
    (function ($, undefined) {
        ...
        var validationRules = {
            ...
            number: function (input) {
                return input.val() === '' || input.val() == null || ufa.parseFloat(input.val()) !== null;
            },
            ...
            min: function (input, params) {
                var min = parseFloat(params.min) || 0, val = ufa.parseFloat(input.val());
                return min <= val;
            },
            max: function (input, params) {
                var max = parseFloat(params.max) || 0, val = ufa.parseFloat(input.val());
                return val <= max;
            },
            date: function (input) {
                return input.val() === '' || ufa.parseDate(input.val()) !== null;
            },
...
(function (f, define) {
    define('ufa.aspnetmvc', [
        'ufa.data',
        'ufa.combobox',
        'ufa.dropdownlist',
        'ufa.multiselect',
        'ufa.validator',
        'aspnetmvc/ufa.data.aspnetmvc',
        'aspnetmvc/ufa.combobox.aspnetmvc',
        'aspnetmvc/ufa.dropdownlist.aspnetmvc',
        'aspnetmvc/ufa.multiselect.aspnetmvc',
        'aspnetmvc/ufa.imagebrowser.aspnetmvc',
        'aspnetmvc/ufa.validator.aspnetmvc'
    ], f);
}(function () {
    var __meta__ = {
        id: 'aspnetmvc',
