(function (f, define) {
    ...
    (function ($, undefined) {
        var ufa = window.ufa, Widget = ufa.ui.Widget, isPlainObject = $.isPlainObject, proxy = $.proxy, extend = $.extend, placeholderSupported = ufa.support.placeholder, browser = ufa.support.browser, isFunction = ufa.isFunction, trimSlashesRegExp = /(^\/|\/$)/g, CHANGE = 'change', APPLY = 'apply', ERROR = 'error', CLICK = 'click', NS = '.ufaFileBrowser', BREADCRUBMSNS = '.ufaBreadcrumbs', SEARCHBOXNS = '.ufaSearchBox', NAMEFIELD = 'name', SIZEFIELD = 'size', TYPEFIELD = 'type', DEFAULTSORTORDER = {
                field: TYPEFIELD,
                dir: 'asc'
            }, EMPTYTILE = ufa.template('<li class="k-tile-empty"><strong>${text}</strong></li>'), TOOLBARTMPL = '<div class="k-widget k-filebrowser-toolbar k-header k-floatwrap">' + '<div class="k-toolbar-wrap">' + '# if (showUpload) { # ' + '<div class="k-widget k-upload"><div class="k-button k-button-icontext k-upload-button">' + '<span class="k-icon k-add"></span>#=messages.uploadFile#<input type="file" name="file" /></div></div>' + '# } #' + '# if (showCreate) { #' + '<button type="button" class="k-button k-button-icon"><span class="k-icon k-addfolder" /></button>' + '# } #' + '# if (showDelete) { #' + '<button type="button" class="k-button k-button-icon k-state-disabled"><span class="k-icon k-delete" /></button>&nbsp;' + '# } #' + '</div>' + '<div class="k-tiles-arrange">' + '<label>#=messages.orderBy#: <select /></label></a>' + '</div>' + '</div>';
        ...
        extend(true, ufa.data, {
            transports: {
                'filebrowser': ufa.data.RemoteTransport.extend({
                    init: function (options) {
                        ufa.data.RemoteTransport.fn.init.call(this, $.extend(true, {}, this.options, options));
                    },
                    _call: function (type, options) {
                        ...
                        if (isFunction(this.options[type])) {
                            ...
                            ufa.data.RemoteTransport.fn[type].call(this, options);
                        }
                    },
        ...
        var FileBrowser = Widget.extend({
            ...
            value: function () {
                ...
                if (selected && selected.get(TYPEFIELD) === 'f') {
                    ...
                    if (fileUrl) {
                        path = isFunction(fileUrl) ? fileUrl(path) : ufa.format(fileUrl, encodeURIComponent(path));
                    }
                    return path;
                }
            },
            _selectedItem: function () {
                ...
                if (selected.length) {
                    return this.dataSource.getByUid(selected.attr(ufa.attr('uid')));
                }
            },
            _toolbar: function () {
                var that = this, template = ufa.template(TOOLBARTMPL), messages = that.options.messages, arrangeBy = [
                        {
                ...
                })).appendTo(that.element).find('.k-upload input').ufaUpload({
                    multiple: false,
                ...
                that.upload = that.toolbar.find('.k-upload input').data('ufaUpload');
                that.arrangeBy = that.toolbar.find('.k-tiles-arrange select').ufaDropDownList({
            ...
            _deleteClick: function () {
                var that = this, item = that.listView.select(), message = ufa.format(that.options.messages.deleteFile, item.find('strong').text());
                ...
                if (filterRegExp.test(fileName)) {
                    ...
                    if (!model) {
                        ...
                        that.upload.one('success', function (e) {
                            ...
                            that._tiles = that.listView.items().filter('[' + ufa.attr('type') + '=f]');
                        });
                    }
                } else {
                    e.preventDefault();
                    that._showMessage(ufa.format(options.messages.invalidFileType, fileName, fileTypes));
                }
            },
            ...
            _createFile: function (fileName) {
                ...
                if (file) {
                    if (!that._showMessage(ufa.format(that.options.messages.overwriteFile, fileName), 'confirm')) {
                        return null;
                    } else {
            ...
            createDirectory: function () {
                ...
                that.listView.one('dataBound', function () {
                    var selected = that.listView.items().filter('[' + ufa.attr('uid') + '=' + model.uid + ']'), input = selected.find('input');
                    if (selected.length) {
                        this.edit(selected);
                    }
            ...
            _content: function () {
                ...
                that.listView = new ufa.ui.ListView(that.list, {
                    ...
                    dataBound: function () {
                        if (that.dataSource.view().length) {
                            that._tiles = this.items().filter('[' + ufa.attr('type') + '=f]');
                        } else {
            ...
            _dblClick: function (e) {
                ...
                if (li.filter('[' + ufa.attr('type') + '=d]').length) {
                    var folder = that.dataSource.getByUid(li.attr(ufa.attr('uid')));
                    if (folder) {
                        that.path(concatPaths(that.path(), folder.get(NAMEFIELD)));
                        that.breadcrumbs.value(that.path());
                    }
                } else if (li.filter('[' + ufa.attr('type') + '=f]').length) {
                    that.trigger(APPLY);
                }
            },
            ...
            _dataSource: function () {
                ...
                } else if (transport.type && isPlainObject(ufa.data.schemas[transport.type])) {
                    schema = ufa.data.schemas[transport.type];
                }
                ...
                that.dataSource = ufa.data.DataSource.create(dataSource).bind(ERROR, that._errorHandler);
            },
            _navigation: function () {
                var that = this, navigation = $('<div class="k-floatwrap"><input/><input/></div>').appendTo(this.element);
                that.breadcrumbs = navigation.find('input:first').ufaBreadcrumbs({
                    value: that.options.path,
                    change: function () {
                        that.path(this.value());
                    }
                }).data('ufaBreadcrumbs');
                that.searchBox = navigation.parent().find('input:last').ufaSearchBox({
            ...
            _editTmpl: function () {
                var html = '<li class="k-tile k-state-selected" ' + ufa.attr('uid') + '="#=uid#" ';
                html += ufa.attr('type') + '="${' + TYPEFIELD + '}">';
                html += '#if(' + TYPEFIELD + ' == "d") { #';
                ...
                html += '#}#';
                html += '#if(' + TYPEFIELD + ' == "d") { #';
                html += '<input class="k-input" ' + ufa.attr('bind') + '="value:' + NAMEFIELD + '"/>';
                html += '#}#';
                html += '</li>';
                return proxy(ufa.template(html), { sizeFormatter: sizeFormatter });
            },
            _itemTmpl: function () {
                var html = '<li class="k-tile" ' + ufa.attr('uid') + '="#=uid#" ';
                html += ufa.attr('type') + '="${' + TYPEFIELD + '}">';
                html += '#if(' + TYPEFIELD + ' == "d") { #';
                html += '<div class="k-thumb"><span class="k-icon k-folder"></span></div>';
                html += '#}else{#';
        ...
        ufa.ui.plugin(FileBrowser);
        ufa.ui.plugin(Breadcrumbs);
        ufa.ui.plugin(SearchBox);
    }(window.ufa.jQuery));
    return window.ufa;
