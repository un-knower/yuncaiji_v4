...
(function (f, define) {
    define('dataviz/diagram/utils', ['ufa.core'], f);
}(function () {
    (function ($, undefined) {
        var ufa = window.ufa, diagram = ufa.dataviz.diagram = {}, deepExtend = ufa.deepExtend, isArray = $.isArray, EPSILON = 0.000001;
        var Utils = {};
        deepExtend(Utils, {
        ...
        ufa.deepExtend(diagram, {
            init: function (element) {
                ufa.init(element, diagram.ui);
            },
            Utils: Utils,
            Range: Range,
            Ticker: Ticker
...
(function (f, define) {
    ...
    (function ($, undefined) {
        var ufa = window.ufa, diagram = ufa.dataviz.diagram, Class = ufa.Class, deepExtend = ufa.deepExtend, dataviz = ufa.dataviz, Utils = diagram.Utils, Point = dataviz.Point2D, isFunction = ufa.isFunction, contains = Utils.contains, map = $.map;
        ...
        var HashTable = ufa.Class.extend({
            init: function () {
                this._buckets = [];
                this.length = 0;
            },
        ...
        var Dictionary = ufa.Observable.extend({
            init: function (dictionary) {
                var that = this;
                ufa.Observable.fn.init.call(that);
        ...
        Graph.Utils = {
            parse: function (graphString) {
                ...
                for (var i = 0, len = parts.length; i < len; i++) {
                    ...
                    if (Utils.isObject(part)) {
                        ...
                        ufa.deepExtend(previousLink, part);
                    }
                }
                return graph;
            },
            ...
            _addShape: function (ufaDiagram, p, id, shapeDefaults) {
                ...
                return ufaDiagram.addShape(shapeDefaults);
            },
            ...
            randomDiagram: function (diagram, shapeCount, maxIncidence, isTree, randomSize) {
                var g = ufa.dataviz.diagram.Graph.Utils.createRandomConnectedGraph(shapeCount, maxIncidence, isTree);
...
(function (f, define) {
    ...
    (function ($, undefined) {
        var ufa = window.ufa, diagram = ufa.dataviz.diagram, Class = ufa.Class, deepExtend = ufa.deepExtend, Point = diagram.Point, Rect = diagram.Rect, Matrix = diagram.Matrix, Utils = diagram.Utils, isNumber = Utils.isNumber, isString = Utils.isString, MatrixVector = diagram.MatrixVector, g = ufa.geometry, d = ufa.drawing, defined = ufa.util.defined, inArray = $.inArray;
        ...
        var Scale = Class.extend({
            ...
            toString: function () {
                return ufa.format('scale({0},{1})', this.x, this.y);
            },
            invert: function () {
        ...
        var Translation = Class.extend({
            ...
            toString: function () {
                return ufa.format('translate({0},{1})', this.x, this.y);
            },
            plus: function (delta) {
                this.x += delta.x;
                this.y += delta.y;
        ...
        var Rotation = Class.extend({
            ...
            toString: function () {
                if (this.x && this.y) {
                    return ufa.format('rotate({0},{1},{2})', this.angle, this.x, this.y);
                } else {
                    return ufa.format('rotate({0})', this.angle);
                }
            },
            toMatrix: function () {
        ...
        var Layout = Group.extend({
            ...
            redraw: function (options) {
                ufa.deepExtend(this.drawingElement.options, options);
        ...
        var Canvas = Class.extend({
            init: function (element, options) {
                ...
                if (ufa.isFunction(this.surface.translate)) {
        ...
        function lineAngle(p1, p2) {
            var xDiff = p2.x - p1.x;
            var yDiff = p2.y - p1.y;
            var angle = ufa.util.deg(Math.atan2(yDiff, xDiff));
            return angle;
        }
...
(function (f, define) {
    ...
    (function ($, undefined) {
        var ufa = window.ufa, dataviz = ufa.dataviz, diagram = dataviz.diagram, Class = ufa.Class, Group = diagram.Group, Rect = diagram.Rect, Rectangle = diagram.Rectangle, Utils = diagram.Utils, isUndefined = Utils.isUndefined, Point = diagram.Point, Circle = diagram.Circle, Ticker = diagram.Ticker, deepExtend = ufa.deepExtend, Movable = ufa.ui.Movable, browser = ufa.support.browser, defined = ufa.util.defined, inArray = $.inArray, proxy = $.proxy;
        var Cursors = {
                arrow: 'default',
                grip: 'pointer',
                cross: 'pointer',
        ...
        var PositionAdapter = ufa.Class.extend({
        ...
        var UndoRedoService = ufa.Observable.extend({
            init: function (options) {
        ...
        var ScrollerTool = EmptyTool.extend({
            init: function (toolService) {
                var tool = this;
                var friction = ufa.support.mobileOS ? FRICTION_MOBILE : FRICTION;
                EmptyTool.fn.init.call(tool, toolService);
                var diagram = tool.toolService.diagram, canvas = diagram.canvas;
                var scroller = diagram.scroller = tool.scroller = $(diagram.scrollable).ufaMobileScroller({
                    friction: friction,
                    velocityMultiplier: VELOCITY_MULTIPLIER,
                    mousewheelScrolling: false,
                    zoom: false,
                    scroll: proxy(tool._move, tool)
                }).data('ufaMobileScroller');
                if (canvas.translate) {
        ...
        var ToolService = Class.extend({
            ...
            wheel: function (p, meta) {
                ...
                z = ufa.dataviz.round(Math.max(options.zoomMin, Math.min(options.zoomMax, z)), 2);
                zoomOptions.zoom = z;
        ...
        var ConnectionRouterBase = ufa.Class.extend({
            init: function () {
            }
        });
...
(function (f, define) {
    ...
    (function ($, undefined) {
        var ufa = window.ufa, diagram = ufa.dataviz.diagram, Graph = diagram.Graph, Node = diagram.Node, Link = diagram.Link, deepExtend = ufa.deepExtend, Size = diagram.Size, Rect = diagram.Rect, Dictionary = diagram.Dictionary, Set = diagram.Set, HyperTree = diagram.Graph, Utils = diagram.Utils, Point = diagram.Point, EPSILON = 0.000001, DEG_TO_RAD = Math.PI / 180, contains = Utils.contains, grep = $.grep;
        var LayoutBase = ufa.Class.extend({
            defaultOptions: {
                type: 'Tree',
                subtype: 'Down',
                roots: null,
            ...
            transferOptions: function (options) {
                this.options = ufa.deepExtend({}, this.defaultOptions);
                if (Utils.isUndefined(options)) {
                    return;
                }
                this.options = ufa.deepExtend(this.options, options || {});
            }
        });
        var DiagramToHyperTreeAdapter = ufa.Class.extend({
            init: function (diagram) {
        ...
        var TreeLayoutProcessor = ufa.Class.extend({
            init: function (options) {
                this.center = null;
        ...
        var LayoutState = ufa.Class.extend({
...
(function (f, define) {
    ...
    (function ($, undefined) {
        var dataviz = ufa.dataviz, draw = ufa.drawing, geom = ufa.geometry, diagram = dataviz.diagram, Widget = ufa.ui.Widget, Class = ufa.Class, proxy = $.proxy, deepExtend = ufa.deepExtend, extend = $.extend, HierarchicalDataSource = ufa.data.HierarchicalDataSource, Canvas = diagram.Canvas, Group = diagram.Group, Rectangle = diagram.Rectangle, Circle = diagram.Circle, CompositeTransform = diagram.CompositeTransform, Rect = diagram.Rect, Path = diagram.Path, DeleteShapeUnit = diagram.DeleteShapeUnit, DeleteConnectionUnit = diagram.DeleteConnectionUnit, TextBlock = diagram.TextBlock, Image = diagram.Image, Point = diagram.Point, Intersect = diagram.Intersect, ConnectionEditAdorner = diagram.ConnectionEditAdorner, UndoRedoService = diagram.UndoRedoService, ToolService = diagram.ToolService, Selector = diagram.Selector, ResizingAdorner = diagram.ResizingAdorner, ConnectorsAdorner = diagram.ConnectorsAdorner, Cursors = diagram.Cursors, Utils = diagram.Utils, Observable = ufa.Observable, ToBackUnit = diagram.ToBackUnit, ToFrontUnit = diagram.ToFrontUnit, PolylineRouter = diagram.PolylineRouter, CascadingRouter = diagram.CascadingRouter, isUndefined = Utils.isUndefined, isDefined = Utils.isDefined, defined = ufa.util.defined, isArray = $.isArray, isFunction = ufa.isFunction, isString = Utils.isString, isPlainObject = $.isPlainObject, math = Math;
        var NS = '.ufaDiagram', CASCADING = 'cascading', ITEMBOUNDSCHANGE = 'itemBoundsChange', CHANGE = 'change', CLICK = 'click', DRAG = 'drag', DRAG_END = 'dragEnd', DRAG_START = 'dragStart', MOUSE_ENTER = 'mouseEnter', MOUSE_LEAVE = 'mouseLeave', ERROR = 'error', AUTO = 'Auto', TOP = 'Top', RIGHT = 'Right', LEFT = 'Left', BOTTOM = 'Bottom', MAXINT = 9007199254740992, SELECT = 'select', ITEMROTATE = 'itemRotate', PAN = 'pan', ZOOM_START = 'zoomStart', ZOOM_END = 'zoomEnd', NONE = 'none', DEFAULT_CANVAS_WIDTH = 600, DEFAULT_CANVAS_HEIGHT = 600, DEFAULT_SHAPE_TYPE = 'rectangle', DEFAULT_SHAPE_WIDTH = 100, DEFAULT_SHAPE_HEIGHT = 100, DEFAULT_SHAPE_MINWIDTH = 20, DEFAULT_SHAPE_MINHEIGHT = 20, DEFAULT_SHAPE_POSITION = 0, DEFAULT_CONNECTION_BACKGROUND = 'Yellow', MAX_VALUE = Number.MAX_VALUE, MIN_VALUE = -Number.MAX_VALUE, ABSOLUTE = 'absolute', TRANSFORMED = 'transformed', ROTATED = 'rotated', TRANSPARENT = 'transparent', WIDTH = 'width', HEIGHT = 'height', X = 'x', Y = 'y', MOUSEWHEEL_NS = 'DOMMouseScroll' + NS + ' mousewheel' + NS, MOBILE_ZOOM_RATE = 0.05, MOBILE_PAN_DISTANCE = 5, BUTTON_TEMPLATE = '<a class="k-button k-button-icontext #=className#" href="\\#"><span class="#=iconClass# #=imageClass#"></span>#=text#</a>', CONNECTION_CONTENT_OFFSET = 5;
        ...
        var DiagramElement = Observable.extend({
            ...
            _template: function () {
                ...
                if (that.options.content.template) {
                    var data = that.dataItem || {}, elementTemplate = ufa.template(that.options.content.template, { paramName: 'dataItem' });
        ...
        var Connection = DiagramElement.extend({
            ...
            _alignContent: function () {
                if (this._contentVisual) {
                    ...
                    var point;
                    if (alignToPath) {
                        var angle = ufa.util.deg(math.atan2(endPoint.y - startPoint.y, endPoint.x - startPoint.x));
        ...
        var Diagram = Widget.extend({
            ...
            _createHandlers: function () {
                ...
                if (!ufa.support.touch && !ufa.support.mobileOS) {
                    ...
                    that._userEvents = new ufa.UserEvents(element, {
                        multiTouch: true,
                ...
                ufa.onResize(that._resizeHandler);
            ...
            destroy: function () {
                ...
                ufa.unbindResize(that._resizeHandler);
                that.clear();
                that.element.off(NS);
            ...
            focus: function () {
                if (!this.element.is(ufa._activeElement())) {
            ...
            addShape: function (item, undoable) {
                ...
                if (item instanceof Shape) {
                    shape = item;
                } else if (!(item instanceof ufa.Class)) {
            ...
            _dataSource: function () {
                if (defined(this.options.connectionsDataSource)) {
                    ...
                    this.dataSource = ufa.data.DataSource.create(ds).bind('change', this._shapesRefreshHandler).bind('requestStart', this._shapesRequestStartHandler).bind('error', this._shapesErrorHandler);
                } else {
            ...
            _connectionDataSource: function () {
                ...
                if (dsOptions) {
                    ...
                    this.connectionsDataSource = ufa.data.DataSource.create(ds).bind('change', this._connectionsRefreshHandler).bind('requestStart', this._connectionsRequestStartHandler).bind('error', this._connectionsErrorHandler);
                }
            },
        ...
        if (ufa.PDFMixin) {
            ufa.PDFMixin.extend(Diagram.fn);
        }
        ...
        var DiagramToolBar = ufa.Observable.extend({
            init: function (diagram, options) {
            ...
            createPopup: function () {
                ...
                this._popup = this.container.ufaPopup({}).getufaPopup();
            },
            ...
            createToolBar: function () {
                this.element = $('<div/>');
                this._toolBar = this.element.ufaToolBar({
            ...
            _setAttributes: function (attributes) {
                ...
                if (attributes.action) {
                    attr[ufa.attr('action')] = attributes.action;
                }
                if (attributes.step) {
                    attr[ufa.attr('step')] = attributes.step;
                }
                return attr;
            },
            _getAttributes: function (element) {
                var attr = {};
                var action = element.attr(ufa.attr('action'));
                if (action) {
                    attr.action = action;
                }
                var step = element.attr(ufa.attr('step'));
                if (step) {
                    attr.step = step;
                }
                return attr;
        ...
        var Editor = ufa.Observable.extend({
            ...
            createEditable: function () {
                var options = this.options;
                this.editable = new ufa.ui.Editable(this.wrapper, {
                    fields: this.fields,
            ...
            destroy: function () {
                ...
                this.editable.element.find('[' + ufa.attr('container-for') + ']').empty();
        ...
        var PopupEditor = Editor.extend({
            ...
            _initContainer: function () {
                var that = this;
                this.wrapper = $('<div class="k-popup-edit-form"/>').attr(ufa.attr('uid'), this.model.uid);
                var formContent = '';
                ...
                this.window = new ufa.ui.Window(this.wrapper.appendTo(this.element), this.options.window);
            ...
            _renderTemplate: function () {
                ...
                template = ufa.template(template)(this.model);
                return template;
            },
            _renderFields: function () {
                ...
                for (var i = 0; i < this.fields.length; i++) {
                    ...
                    if (this._isEditable(field.field)) {
                        form += '<div ' + ufa.attr('container-for') + '="' + field.field + '" class="k-edit-field"></div>';
                    }
                }
                return form;
            },
            ...
            _createButton: function (name) {
                return ufa.template(BUTTON_TEMPLATE)(defaultButtons[name]);
            },
        ...
        function connectionSelector(container, options) {
            ...
            if (model) {
                ...
                $('<input name=\'' + options.field + '\' />').appendTo(container).ufaDropDownList({
        ...
        function cloneDataItem(dataItem) {
            var result = dataItem;
            if (dataItem instanceof ufa.data.Model) {
        ...
        function createModel(dataSource, model) {
            ...
            return new ufa.data.ObservableObject(model);
        }
...
(function (f, define) {
    ...
    var __meta__ = {
        ...
        name: 'Diagram',
        category: 'dataviz',
        description: 'The ufa DataViz Diagram ',
        depends: [
            'data',
            'userevents',
            'mobile.scroller',
