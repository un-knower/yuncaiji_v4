(function (f, define) {
    ...
    (function ($, undefined) {
        var ufa = window.ufa, ui = ufa.ui, proxy = $.proxy, POPUP = 'ufaPopup', INIT = 'init', REFRESH = 'refresh', CHANGE = 'change', NS = '.ufaFilterMenu', EQ = 'Is equal to', NEQ = 'Is not equal to', roles = {
                ...
                'number': 'number',
                'date': 'date'
            }, isFunction = ufa.isFunction, Widget = ui.Widget;
        ...
        var defaultTemplate = '<div>' + '<div class="k-filter-help-text">#=messages.info#</div>' + '<select data-#=ns#bind="value: filters[0].operator" data-#=ns#role="dropdownlist">' + '#for(var op in operators){#' + '<option value="#=op#">#=operators[op]#</option>' + '#}#' + '</select>' + '#if(values){#' + '<select data-#=ns#bind="value:filters[0].value" data-#=ns#text-field="text" data-#=ns#value-field="value" data-#=ns#source=\'#=ufa.stringify(values).replace(/\'/g,"&\\#39;")#\' data-#=ns#role="dropdownlist" data-#=ns#option-label="#=messages.selectValue#" data-#=ns#value-primitive="true">' + '</select>' + '#}else{#' + '<input data-#=ns#bind="value:filters[0].value" class="k-textbox" type="text" #=role ? "data-" + ns + "role=\'" + role + "\'" : ""# />' + '#}#' + '#if(extra){#' + '<select class="k-filter-and" data-#=ns#bind="value: logic" data-#=ns#role="dropdownlist">' + '<option value="and">#=messages.and#</option>' + '<option value="or">#=messages.or#</option>' + '</select>' + '<select data-#=ns#bind="value: filters[1].operator" data-#=ns#role="dropdownlist">' + '#for(var op in operators){#' + '<option value="#=op#">#=operators[op]#</option>' + '#}#' + '</select>' + '#if(values){#' + '<select data-#=ns#bind="value:filters[1].value" data-#=ns#text-field="text" data-#=ns#value-field="value" data-#=ns#source=\'#=ufa.stringify(values).replace(/\'/g,"&\\#39;")#\' data-#=ns#role="dropdownlist" data-#=ns#option-label="#=messages.selectValue#" data-#=ns#value-primitive="true">' + '</select>' + '#}else{#' + '<input data-#=ns#bind="value: filters[1].value" class="k-textbox" type="text" #=role ? "data-" + ns + "role=\'" + role + "\'" : ""#/>' + '#}#' + '#}#' + '<div>' + '<button type="submit" class="k-button k-primary">#=messages.filter#</button>' + '<button type="reset" class="k-button">#=messages.clear#</button>' + '</div>' + '</div>';
        ...
        var FilterMenu = Widget.extend({
            init: function (element, options) {
                ...
                that.field = options.field || element.attr(ufa.attr('field'));
            ...
            _init: function () {
                ...
                that.form.find('[' + ufa.attr('role') + '=numerictextbox]').removeClass('k-textbox').end().find('[' + ufa.attr('role') + '=datetimepicker]').removeClass('k-textbox').end().find('[' + ufa.attr('role') + '=timepicker]').removeClass('k-textbox').end().find('[' + ufa.attr('role') + '=datepicker]').removeClass('k-textbox');
                that.refresh();
                that.trigger(INIT, {
                    field: that.field,
                    container: that.form
                });
                ufa.cycleForm(that.form);
            },
            _createForm: function (role) {
                var that = this, options = that.options, operators = that.operators || {}, type = that.type;
                operators = operators[type] || options.operators[type];
                that.form = $('<form class="k-filter-menu"/>').html(ufa.template(type === 'boolean' ? booleanTemplate : defaultTemplate)({
                    field: that.field,
            ...
            _createMobileForm: function (role) {
                var that = this, options = that.options, operators = that.operators || {}, type = that.type;
                operators = operators[type] || options.operators[type];
                that.form = $('<div />').html(ufa.template(type === 'boolean' ? booleanMobileTemplate : defaultMobileTemplate)({
                    field: that.field,
                    ...
                    type: type,
                    role: role,
                    useRole: !ufa.support.input.date && type === 'date' || type === 'number',
            ...
            refresh: function () {
                ...
                that.filterModel = ufa.observable({
                    logic: 'and',
                    filters: [
                        that._defaultFilter(),
                        that._defaultFilter()
                ...
                if (that.form) {
                    ufa.bind(that.form.children().first(), that.filterModel);
                }
            ...
            destroy: function () {
                ...
                if (that.form) {
                    ufa.unbind(that.form);
                    ufa.destroy(that.form);
                    that.form.unbind(NS);
                    if (that.popup) {
                        that.popup.destroy();
                        that.popup = null;
            ...
            _activate: function () {
                this.form.find(':ufaFocusable:first').focus();
            },
            _keydown: function (e) {
        ...
        var multiCheckNS = '.ufaFilterMultiCheck';
        ...
        function distinct(items, field) {
            var getter = ufa.getter(field, true), result = [], index = 0, seen = {};
        ...
        var FilterMultiCheck = Widget.extend({
            init: function (element, options) {
                ...
                var field = this.field = this.options.field || this.element.attr(ufa.attr('field'));
            ...
            _createForm: function () {
                ...
                if (this._isMobile) {
                    ...
                    if (!options.appendToElement) {
                        this.popup = this.form.ufaPopup({ anchor: this._link }).data(POPUP);
                    } else {
            ...
            createCheckAllItem: function () {
                var options = this.options;
                var template = ufa.template(options.itemTemplate({
                    field: 'all',
            ...
            createCheckBoxes: function () {
                ...
                var template = ufa.template(options.itemTemplate(templateOptions));
                var itemsHtml = ufa.render(template, data);
                if (options.checkAll) {
            ...
            options: {
                ...
                itemTemplate: function (options) {
                    ...
                    return '<li class=\'k-item\'>' + '<label class=\'k-label\'>' + '<input type=\'checkbox\' class=\'' + (mobile ? 'k-check' : '') + '\'  value=\'#:ufa.format(\'{0' + valueFormat + '}\',' + valueField + ')#\'/>' + '#:ufa.format(\'' + (format ? format : '{0}') + '\', ' + field + ')#' + '</label>' + '</li>';
                },
                checkAll: true,
                search: false,
                ignoreCase: true,
