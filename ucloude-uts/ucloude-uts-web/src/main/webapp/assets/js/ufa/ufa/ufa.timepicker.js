(function (f, define) {
    define('ufa.timepicker', ['ufa.popup'], f);
}(function () {
    var __meta__ = {
        id: 'timepicker',
        name: 'TimePicker',
    ...
    (function ($, undefined) {
        var ufa = window.ufa, keys = ufa.keys, parse = ufa.parseDate, activeElement = ufa._activeElement, extractFormat = ufa._extractFormat, support = ufa.support, browser = support.browser, ui = ufa.ui, Widget = ui.Widget, OPEN = 'open', CLOSE = 'close', CHANGE = 'change', ns = '.ufaTimePicker', CLICK = 'click' + ns, DEFAULT = 'k-state-default', DISABLED = 'disabled', READONLY = 'readonly', LI = 'li', SPAN = '<span/>', FOCUSED = 'k-state-focused', HOVER = 'k-state-hover', HOVEREVENTS = 'mouseenter' + ns + ' mouseleave' + ns, MOUSEDOWN = 'mousedown' + ns, MS_PER_MINUTE = 60000, MS_PER_DAY = 86400000, SELECTED = 'k-state-selected', STATEDISABLED = 'k-state-disabled', ARIA_SELECTED = 'aria-selected', ARIA_EXPANDED = 'aria-expanded', ARIA_HIDDEN = 'aria-hidden', ARIA_DISABLED = 'aria-disabled', ARIA_READONLY = 'aria-readonly', ARIA_ACTIVEDESCENDANT = 'aria-activedescendant', ID = 'id', isArray = $.isArray, extend = $.extend, proxy = $.proxy, DATE = Date, TODAY = new DATE();
        ...
        var TimeView = function (options) {
            ...
            that.template = ufa.template('<li tabindex="-1" role="option" class="k-item" unselectable="on">#=data#</li>', { useWithBlock: false });
        };
        TimeView.prototype = {
            ...
            dataBind: function (dates) {
                var that = this, options = that.options, format = options.format, toString = ufa.toString, template = that.template, length = dates.length, idx = 0, date, html = '';
                for (; idx < length; idx++) {
                    date = dates[idx];
            ...
            select: function (li) {
                ...
                if (li instanceof Date) {
                    li = ufa.toString(li, options.format, options.culture);
                }
                if (typeof li === 'string') {
        ...
        function normalize(options) {
            ...
            options.format = extractFormat(options.format || ufa.getCulture(options.culture).calendars.standard.patterns.t);
