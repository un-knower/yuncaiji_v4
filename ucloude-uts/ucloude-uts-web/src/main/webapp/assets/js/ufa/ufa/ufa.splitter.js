(function (f, define) {
    define('ufa.splitter', ['ufa.resizable'], f);
}(function () {
    var __meta__ = {
        id: 'splitter',
        name: 'Splitter',
    ...
    (function ($, undefined) {
        var ufa = window.ufa, ui = ufa.ui, keys = ufa.keys, extend = $.extend, proxy = $.proxy, Widget = ui.Widget, pxUnitsRegex = /^\d+(\.\d+)?px$/i, percentageUnitsRegex = /^\d+(\.\d+)?%$/i, NS = '.ufaSplitter', EXPAND = 'expand', COLLAPSE = 'collapse', CONTENTLOAD = 'contentLoad', ERROR = 'error', RESIZE = 'resize', LAYOUTCHANGE = 'layoutChange', HORIZONTAL = 'horizontal', VERTICAL = 'vertical', MOUSEENTER = 'mouseenter', CLICK = 'click', PANE = 'pane', MOUSELEAVE = 'mouseleave', FOCUSED = 'k-state-focused', KPANE = 'k-' + PANE, PANECLASS = '.' + KPANE;
        ...
        function panePropertyAccessor(propertyName, triggersResize) {
            return function (pane, value) {
                ...
                if (triggersResize) {
                    var splitter = this.element.data('ufa' + this.options.name);
        ...
        function PaneResizing(splitter) {
            ...
            that._resizable = new ufa.ui.Resizable(splitter.element, {
