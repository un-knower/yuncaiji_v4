(function (f, define) {
    ...
    (function ($, undefined) {
        var ufa = window.ufa, Widget = ufa.ui.Widget, antiForgeryTokens = ufa.antiForgeryTokens, logToConsole = ufa.logToConsole, rFileExtension = /\.([^\.]+)$/, NS = '.ufaUpload', SELECT = 'select', UPLOAD = 'upload', SUCCESS = 'success', ERROR = 'error', COMPLETE = 'complete', CANCEL = 'cancel', PROGRESS = 'progress', REMOVE = 'remove';
        var Upload = Widget.extend({
            ...
            _addInput: function (sourceInput) {
                ...
                input.insertAfter(that.element).data('ufaUpload', that);
            ...
            _onInputKeyDown: function (e) {
                ...
                if (e.keyCode === ufa.keys.TAB && firstButton.length > 0) {
                    e.preventDefault();
                    firstButton.focus();
                }
            },
            ...
            _enqueueFile: function (name, data) {
                ...
                fileEntry.attr(ufa.attr('uid'), fileUid).appendTo(fileList).data(data);
                if (!that._async) {
                    $('.k-progress', fileEntry).width('100%');
                }
            ...
            _supportsMultiple: function () {
                ...
                return !ufa.support.browser.opera && !(ufa.support.browser.safari && windows);
            },
