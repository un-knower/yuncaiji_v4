...
(function (f, define) {
    ...
    (function ($, undefined) {
        var ufa = window.ufa, util = ufa.util, append = util.append, defined = util.defined, last = util.last, valueOrDefault = util.valueOrDefault, dataviz = ufa.dataviz, geom = dataviz.geometry, draw = dataviz.drawing, measureText = draw.util.measureText, Class = ufa.Class, template = ufa.template, noop = $.noop, indexOf = $.inArray, isPlainObject = $.isPlainObject, trim = $.trim, math = Math, deepExtend = ufa.deepExtend;
            ...
            axisLabelText: function (value, dataItem, options) {
                ...
                if (options.template) {
                    ...
                    if (options.format.match(FORMAT_REGEX)) {
                        text = ufa.format(options.format, value);
                    } else {
                        text = ufa.toString(value, options.format, options.culture);
                    }
                }
                return text;
            },
        ...
        var ExportMixin = {
            ...
            svg: function () {
                if (draw.svg.Surface) {
                    ...
                    throw new Error('SVG Export failed. Unable to export instantiate ufa.drawing.svg.Surface');
                }
            },
            imageDataURL: function () {
                ...
                if (draw.canvas.Surface) {
                    ...
                    throw new Error('Image Export failed. Unable to export instantiate ufa.drawing.canvas.Surface');
                }
            }
        };
        ...
        function autoFormat(format, value) {
            if (format.match(FORMAT_REGEX)) {
                return ufa.format.apply(this, arguments);
            }
            return ufa.toString(value, format);
        }
