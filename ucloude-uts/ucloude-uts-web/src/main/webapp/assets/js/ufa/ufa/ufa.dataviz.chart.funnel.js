...
(function (f, define) {
    define('ufa.dataviz.chart.funnel', [
        'ufa.dataviz.chart',
        'ufa.drawing'
    ], f);
}(function () {
    ...
    (function ($, undefined) {
        var ufa = window.ufa, deepExtend = ufa.deepExtend, extend = $.extend, isFn = ufa.isFunction, template = ufa.template, util = ufa.util, append = util.append, draw = ufa.drawing, geom = ufa.geometry, dataviz = ufa.dataviz, Color = ufa.drawing.Color, ChartElement = dataviz.ChartElement, PieChartMixin = dataviz.PieChartMixin, PlotAreaBase = dataviz.PlotAreaBase, PlotAreaFactory = dataviz.PlotAreaFactory, Point2D = dataviz.Point2D, Box2D = dataviz.Box2D, SeriesBinder = dataviz.SeriesBinder, TextBox = dataviz.TextBox, autoFormat = dataviz.autoFormat, evalOptions = dataviz.evalOptions, limitValue = util.limitValue, seriesTotal = dataviz.seriesTotal;
