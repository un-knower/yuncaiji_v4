(function (f, define) {
    ...
    (function ($, undefined) {
        var ufa = window.ufa, ui = ufa.ui, Widget = ui.Widget, proxy = $.proxy, FIRST = '.k-i-seek-w', LAST = '.k-i-seek-e', PREV = '.k-i-arrow-w', NEXT = '.k-i-arrow-e', CHANGE = 'change', NS = '.ufaPager', CLICK = 'click', KEYDOWN = 'keydown', DISABLED = 'disabled', iconTemplate = ufa.template('<a href="\\#" title="#=text#" class="k-link k-pager-nav #= wrapClassName #"><span class="k-icon #= className #">#=text#</span></a>');
        ...
        function update(element, selector, page, disabled) {
            element.find(selector).parent().attr(ufa.attr('page'), page).attr('tabindex', -1).toggleClass('k-state-disabled', disabled);
        }
        ...
        var Pager = Widget.extend({
            init: function (element, options) {
                ...
                that.dataSource = ufa.data.DataSource.create(options.dataSource);
                that.linkTemplate = ufa.template(that.options.linkTemplate);
                that.selectTemplate = ufa.template(that.options.selectTemplate);
                that.currentPageTemplate = ufa.template(that.options.currentPageTemplate);
                page = that.page();
                ...
                if (options.input) {
                    if (!that.element.find('.k-pager-input').length) {
                        that.element.append('<span class="k-pager-input k-label">' + options.messages.page + '<input class="k-textbox">' + ufa.format(options.messages.of, totalPages) + '</span>');
                    }
                ...
                if (options.pageSizes) {
                    ...
                    if (ufa.ui.DropDownList) {
                        that.element.find('.k-pager-sizes select').show().ufaDropDownList();
                    }
            ...
            refresh: function (e) {
                ...
                if (options.info) {
                    if (total > 0) {
                        html = ufa.format(options.messages.display, (page - 1) * pageSize + 1, Math.min(page * pageSize, total), total);
                    } else {
                ...
                if (options.input) {
                    that.element.find('.k-pager-input').html(that.options.messages.page + '<input class="k-textbox">' + ufa.format(options.messages.of, totalPages)).find('input').val(page).attr(DISABLED, total < 1).toggleClass('k-state-disabled', total < 1);
                }
                if (options.previousNext) {
                    first(that.element, page, totalPages);
                    prev(that.element, page, totalPages);
                ...
                if (options.pageSizes) {
                    ...
                    that.element.find('.k-pager-sizes select').val(pageSize).filter('[' + ufa.attr('role') + '=dropdownlist]').ufaDropDownList('value', pageSize).ufaDropDownList('text', text);
                }
            },
            _keydown: function (e) {
                if (e.keyCode === ufa.keys.ENTER) {
            ...
            _click: function (e) {
                ...
                if (!target.is('.k-state-disabled')) {
                    this.page(target.attr(ufa.attr('page')));
                }
            },
            totalPages: function () {
