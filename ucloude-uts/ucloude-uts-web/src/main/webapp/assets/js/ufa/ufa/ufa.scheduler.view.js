(function (f, define) {
    define('ufa.scheduler.view', ['ufa.core'], f);
}(function () {
    var __meta__ = {
        id: 'scheduler.view',
        name: 'Scheduler View',
    ...
    (function ($) {
        var ufa = window.ufa, ui = ufa.ui, Widget = ui.Widget, keys = ufa.keys, NS = '.ufaSchedulerView', math = Math;
        ...
        function cellspacing() {
            if (ufa.support.cssBorderSpacing) {
                return '';
            }
        ...
        ufa.ui.scheduler = {};
        var ResourceView = ufa.Class.extend({
            ...
            ranges: function (startTime, endTime, isDay, isAllDay) {
                if (typeof startTime != 'number') {
                    startTime = ufa.date.toUtcTime(startTime);
                }
                if (typeof endTime != 'number') {
                    endTime = ufa.date.toUtcTime(endTime);
                }
                if (isDay) {
        ...
        var SlotRange = ufa.Class.extend({
        ...
        var TimeSlotRange = SlotRange.extend({
            ...
            _rect: function (property, start, end, snap) {
                ...
                var isRtl = ufa.support.isRtl(startSlot.element);
                if (typeof start != 'number') {
                    start = ufa.date.toUtcTime(start);
                }
                if (typeof end != 'number') {
                    end = ufa.date.toUtcTime(end);
                }
                if (snap) {
        ...
        var SlotCollection = ufa.Class.extend({
            init: function (startDate, endDate, groupIndex, collectionIndex) {
                this._slots = [];
                this._events = [];
                this._start = ufa.date.toUtcTime(startDate);
                this._end = ufa.date.toUtcTime(endDate);
        ...
        var Slot = ufa.Class.extend({
            ...
            startDate: function () {
                return ufa.timezone.toLocalDate(this.start);
            },
            endDate: function () {
                return ufa.timezone.toLocalDate(this.end);
            },
            startInRange: function (date) {
        ...
        var DaySlot = Slot.extend({
            ...
            startDate: function () {
                ...
                return ufa.timezone.apply(date, 'Etc/UTC');
            },
            endDate: function () {
        ...
        var scrollbarWidth;
        function scrollbar() {
            scrollbarWidth = scrollbarWidth ? scrollbarWidth : ufa.support.scrollbar();
            return scrollbarWidth;
        }
        ufa.ui.SchedulerView = Widget.extend({
            ...
            dateForTitle: function () {
                return ufa.format(this.options.selectedDateFormat, this.startDate(), this.endDate());
            },
            shortDateForTitle: function () {
                return ufa.format(this.options.selectedShortDateFormat, this.startDate(), this.endDate());
            },
            ...
            inRange: function (options) {
                ...
                var endDate = ufa.date.addDays(this.endDate(), 1);
            ...
            _resourceValue: function (resource, item) {
                if (resource.valuePrimitive) {
                    item = ufa.getter(resource.dataValueField)(item);
                }
                return item;
            },
            _resourceBySlot: function (slot) {
                ...
                if (resources.length) {
                    ...
                    for (var idx = resources.length - 1; idx >= 0; idx--) {
                        ...
                        var setter = ufa.setter(resource.field);
                        setter(result, value);
            ...
            _eventTmpl: function (template, wrapper) {
                var options = this.options, settings = $.extend({}, ufa.Template, options.templateSettings), paramName = settings.paramName, html = '', type = typeof template, state = {
                        storage: {},
                        count: 0
                    };
                if (type === 'function') {
                ...
                var tmpl = ufa.template(ufa.format(wrapper, html), settings);
                if (state.count > 0) {
            ...
            eventResources: function (event) {
                ...
                for (var idx = 0; idx < options.resources.length; idx++) {
                    ...
                    var eventResources = ufa.getter(field)(event);
                    ...
                    for (var resourceIndex = 0; resourceIndex < eventResources.length; resourceIndex++) {
                        ...
                        if (!resource.valuePrimitive) {
                            value = ufa.getter(resource.dataValueField)(value);
                        }
                        ...
                        if (eventResource !== null) {
                            var resourceColor = ufa.getter(resource.dataColorField)(eventResource);
                            resources.push({
                                field: resource.field,
                                title: resource.title,
                                name: resource.name,
                                text: ufa.getter(resource.dataTextField)(eventResource),
                                value: value,
                                color: resourceColor
                            });
                        }
            ...
            refreshLayout: function () {
                ...
                var contentDiv = that.content[0], scrollbarWidth = !ufa.support.kineticScrollNeeded ? scrollbar : 0;
        ...
        function createLayoutConfiguration(name, resources, inner, template) {
            ...
            if (resource) {
                ...
                for (var dataIndex = 0; dataIndex < data.length; dataIndex++) {
                    var obj = {
                        text: template({
                            text: ufa.htmlEncode(ufa.getter(resource.dataTextField)(data[dataIndex])),
                            color: ufa.getter(resource.dataColorField)(data[dataIndex]),
                            field: resource.field,
                            title: resource.title,
                            name: resource.name,
                            value: ufa.getter(resource.dataValueField)(data[dataIndex])
                        }),
        ...
        function groupEqFilter(value) {
            return function (item) {
                if ($.isArray(item) || item instanceof ufa.data.ObservableArray) {
