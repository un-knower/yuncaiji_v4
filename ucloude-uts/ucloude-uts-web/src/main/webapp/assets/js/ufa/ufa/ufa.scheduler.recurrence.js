(function (f, define) {
    define('ufa.scheduler.recurrence', [
        'ufa.dropdownlist',
        'ufa.datepicker',
        'ufa.numerictextbox'
    ], f);
    ...
    (function ($, undefined) {
        var ufa = window.ufa, timezone = ufa.timezone, Class = ufa.Class, ui = ufa.ui, Widget = ui.Widget, DropDownList = ui.DropDownList, ufaDate = ufa.date, setTime = ufaDate.setTime, setDayOfWeek = ufaDate.setDayOfWeek, adjustDST = ufaDate.adjustDST, firstDayOfMonth = ufaDate.firstDayOfMonth, getMilliseconds = ufaDate.getMilliseconds, DAYS_IN_LEAPYEAR = [
                0,
                31,
                60,
                91,
                ...
                interval: function (rule, current) {
                    ...
                    if (frequency === 'hourly') {
                        ...
                        if (hours !== startTimeHours) {
                            date += (startTimeHours - hours) * ufaDate.MS_PER_HOUR;
                        }
                        date -= start;
                        if (diff) {
                            date -= diff * ufaDate.MS_PER_MINUTE;
                        }
                        diff = Math.floor(date / ufaDate.MS_PER_HOUR);
                        ...
                        ufaDate.setTime(date, -start);
                        diff = Math.floor(date / ufaDate.MS_PER_DAY);
                        excess = intervalExcess(diff, interval);
                        if (excess !== 0) {
                        ...
                        if (excess !== 0) {
                            ufaDate.setDayOfWeek(current, rule.weekStart, -1);
        ...
        function toExceptionString(dates, zone) {
            ...
            for (length = result.length; idx < length; idx++) {
                date = result[idx];
                date = ufa.timezone.convert(date, zone || date.getTimezoneOffset(), 'Etc/UTC');
                result[idx] = ufa.toString(date, RECURRENCE_DATE_FORMAT);
            }
            return result.join(';') + ';';
        }
        ...
        function expand(event, start, end, zone) {
            ...
            rule._startTime = startTime = ufaDate.toInvariantTime(start);
            if (freq.setup) {
            ...
            while (start <= end) {
                ...
                if (inPeriod && !isException(exceptionDates, start, zone) || positions) {
                    startTime = ufaDate.toUtcTime(ufaDate.getDate(start)) + getMilliseconds(rule._startTime);
        ...
        function parseUTCDate(value, zone) {
            value = ufa.parseDate(value, DATE_FORMATS);
            if (value && zone) {
        ...
        function parseRule(recur, zone) {
            ...
            if (weekStart === undefined) {
                instance.weekStart = weekStart = ufa.culture().calendar.firstDay;
            }
            if (weekDays) {
        ...
        function serializeDateRule(dateRule, zone) {
            ...
            for (; idx < length; idx++) {
                ...
                value[idx] = ufa.toString(val, 'yyyyMMddTHHmmssZ');
            }
            if (tzid) {
                tzid = ';TZID=' + tzid;
            }
        ...
        function serialize(rule, zone) {
            ...
            if (until) {
                ...
                ruleString += ';UNTIL=' + ufa.toString(until, 'yyyyMMddTHHmmssZ');
            }
            if (rule.months) {
        ...
        var weekDayCheckBoxes = function (firstDay) {
            var shortNames = ufa.culture().calendar.days.namesShort, length = shortNames.length, result = '', idx = 0, values = [];
            for (; idx < length; idx++) {
                values.push(idx);
            }
        ...
        var BaseRecurrenceEditor = Widget.extend({
            init: function (element, options) {
                ...
                options.start = start = options.start || ufaDate.today();
                if (frequencies) {
                    options.frequencies = frequencies;
                }
                if (typeof start === 'string') {
                    options.start = ufa.parseDate(start, 'yyyyMMddTHHmmss');
                }
                if (options.firstWeekDay === null) {
                    options.firstWeekDay = ufa.culture().calendar.firstDay;
                }
            ...
            _initInterval: function () {
                ...
                that._container.find('.k-recur-interval').ufaNumericTextBox({
            ...
            _initWeekDay: function () {
                ...
                if (weekDayInput[0]) {
                    ...
                    that._weekDay = new DropDownList(weekDayInput, {
                        ...
                        dataTextField: 'text',
                        dataValueField: 'value',
                        dataSource: data.concat($.map(ufa.culture().calendar.days.names, function (dayName, idx) {
                            return {
                                text: dayName,
                                value: idx
                            };
            ...
            _initMonthDay: function () {
                ...
                if (monthDayInput[0]) {
                    that._monthDay = new ufa.ui.NumericTextBox(monthDayInput, {
                        spinners: that.options.spinners,
                        min: 1,
                        max: 31,
                        decimals: 0,
            ...
            _initCount: function () {
                ...
                that._count = input.ufaNumericTextBox({
                    spinners: that.options.spinners,
                    value: rule.count || 1,
                    decimals: 0,
                    format: '#',
            ...
            _initMonth: function () {
                ...
                if (monthInputs[0]) {
                    options = {
                        ...
                        dataTextField: 'text',
                        dataValueField: 'value',
                        dataSource: $.map(ufa.culture().calendar.months.names, function (monthName, idx) {
                            return {
                                text: monthName,
                                value: idx + 1
                            };
        ...
        var MobileRecurrenceEditor = BaseRecurrenceEditor.extend({
            ...
            _endText: function () {
                ...
                if (rule.count) {
                    text = ufa.format('{0} {1}', messages.after, rule.count);
                } else if (rule.until) {
                    text = ufa.format('{0} {1:d}', messages.on, rule.until);
                }
                return text;
            },
            _initFrequency: function () {
                ...
                that._view.element.find('.k-recur-pattern').append(html).on(CLICK + that._namespace, '.k-scheduler-navigation li', function (e) {
                    ...
                    that._value = { freq: li.children('a').attr(ufa.attr('value')) };
            ...
            _initEndNavigation: function () {
                ...
                that._view.element.find('.k-recur-pattern').append(html).on(CLICK + that._namespace, '.k-scheduler-navigation li', function (e) {
                    ...
                    e.preventDefault();
                    li.addClass('k-state-selected').siblings().removeClass('k-state-selected');
                    that._initEndView(li.children('a').attr(ufa.attr('value')));
                    if (that._count) {
                        count = that._count.value();
                        until = null;
                    } else if (that._until) {
                        count = null;
                        until = that._until.val ? ufa.parseDate(that._until.val(), 'yyyy-MM-dd') : that._until.value();
                    }
                    rule.count = count;
                    rule.until = until;
                });
            ...
            _initWeekDay: function () {
                ...
                if (weekDaySelect[0]) {
                    ...
                    data = data.concat($.map(ufa.culture().calendar.days.names, function (dayName, idx) {
                        return {
                            text: dayName,
                            value: idx
                        };
            ...
            _initMonth: function () {
                ...
                var monthNames = ufa.culture().calendar.months.names;
                if (monthSelect[0]) {
            ...
            _period: function () {
                ...
                if (repeatRuleGroupButton[0]) {
                    ...
                    repeatRuleGroupButton.append(html).on(CLICK + that._namespace, '.k-scheduler-navigation li', function (e) {
                        ...
                        var value = li.children('a').attr(ufa.attr('value'));
                        init(value);
                    });
                    init(currentValue);
                }
            ...
            _initUntil: function () {
                ...
                if (ufa.support.input.date) {
                    that._until = input.attr('min', ufa.toString(min, 'yyyy-MM-dd')).val(ufa.toString(until || start, 'yyyy-MM-dd')).on('change', function () {
                        rule.until = ufa.parseDate(this.value, 'yyyy-MM-dd');
                    });
                } else {
                    that._until = input.ufaDatePicker({
                        min: min,
