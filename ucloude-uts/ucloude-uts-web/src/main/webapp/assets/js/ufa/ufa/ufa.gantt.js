...
(function (f, define) {
    ...
    (function ($, undefined) {
        ...
        var mobileOS = ufa.support.mobileOS;
        var Observable = ufa.Observable;
        var Widget = ufa.ui.Widget;
        var DataSource = ufa.data.DataSource;
        var ObservableObject = ufa.data.ObservableObject;
        var ObservableArray = ufa.data.ObservableArray;
        var Query = ufa.data.Query;
        var isArray = $.isArray;
        var inArray = $.inArray;
        var isFunction = ufa.isFunction;
        var proxy = $.proxy;
        var extend = $.extend;
        ...
        var TOGGLE_BUTTON_TEMPLATE = ufa.template('<button class="#=styles.buttonToggle#"><span class="#=styles.iconToggle#">&nbps;</span></button>');
        ...
        var VIEWBUTTONTEMPLATE = ufa.template('<li class="#=styles.currentView# #=styles.viewButtonDefault#"><a href="\\#" class="#=styles.link#">&nbps;</a></li>');
        var HEADER_VIEWS_TEMPLATE = ufa.template('<ul class="#=styles.viewsWrapper#">' + '#for(var view in views){#' + '<li class="#=styles.viewButtonDefault# #=styles.viewButton#-#= view.toLowerCase() #" data-#=ns#name="#=view#"><a href="\\#" class="#=styles.link#">#=views[view].title#</a></li>' + '#}#' + '</ul>');
        var TASK_DROPDOWN_TEMPLATE = ufa.template('<div class="#=styles.popupWrapper#">' + '<ul class="#=styles.popupList#" role="listbox">' + '#for(var i = 0, l = actions.length; i < l; i++){#' + '<li class="#=styles.item#" data-action="#=actions[i].data#" role="option">#=actions[i].text#</span>' + '#}#' + '</ul>' + '</div>');
        var DATERANGEEDITOR = function (container, options) {
            ...
            if (validationRules && isPlainObject(validationRules) && validationRules.message) {
                attr[ufa.attr('dateCompare-msg')] = validationRules.message;
            }
            $('<input type="text" required ' + ufa.attr('type') + '="date" ' + ufa.attr('role') + '="datetimepicker" ' + ufa.attr('bind') + '="value:' + options.field + '" ' + ufa.attr('validate') + '=\'true\' />').attr(attr).appendTo(container);
        ...
        function dateCompareValidator(input) {
            if (input.filter('[name=end], [name=start]').length) {
                ...
                var picker = ufa.widgetInstance(input, ufa.ui);
                var dates = {};
                var container = input;
                var editable;
                var model;
                while (container !== window && !editable) {
                    ...
                    editable = container.data('ufaEditable');
                }
                ...
                dates.end = model.end;
                dates[field] = picker ? picker.value() : ufa.parseDate(input.value());
                return dates.start <= dates.end;
            }
            return true;
        }
        function focusTable(table, direct) {
            var wrapper = table.parents('[' + ufa.attr('role') + '="gantt"]');
            var scrollPositions = [];
        ...
        function scrollableParents(element) {
            return $(element).parentsUntil('body').filter(function (index, element) {
                var computedStyle = ufa.getComputedStyles(element, ['overflow']);
        ...
        var TaskDropDown = Observable.extend({
            ...
            _popup: function () {
                ...
                this.popup = new ufa.ui.Popup(this.list, extend({
                ...
                this.element.on(CLICK + NS, appendButtonSelector, function (e) {
                    var target = $(this);
                    var action = target.attr(ufa.attr('action'));
                    e.preventDefault();
                    if (action) {
                ...
                this.list.find(itemSelector).hover(function () {
                    ...
                    that.trigger('command', { type: $(this).attr(ufa.attr('action')) });
                    that.popup.close();
                });
                if (navigatable) {
                    this.popup.bind('close', function () {
                        that.list.find(itemSelector).removeClass(ganttStyles.focused).end().find('ul').attr(TABINDEX, 0);
                        that.element.parents('[' + ufa.attr('role') + '="gantt"]').find(DOT + ganttStyles.gridContent + ' > table:first').focus();
                    });
        ...
        var GanttDependency = ufa.data.Model.define({
            id: 'id',
            fields: {
                id: { type: 'number' },
        ...
        var GanttTask = ufa.data.Model.define({
            duration: function () {
                var end = this.end;
                ...
                if (template) {
                    ...
                    for (var i = 0, length = fields.length; i < length; i++) {
                        ...
                        if (!model.editable || model.editable(field.field)) {
                            ...
                            html += '<div ' + ufa.attr('container-for') + '="' + field.field + '" class="' + popupStyles.editField + '"></div>';
                        } else {
                            var tmpl = '#:';
                            if (field.field) {
        ...
        var PopupEditor = Editor.extend({
            ...
            close: function () {
                ...
                if (this.editable && this.container.is(':visible')) {
                    this.container.data('ufaWindow').bind('deactivate', destroy).close();
                } else {
                    destroy();
                }
            },
            showDialog: function (options) {
                ...
                var popupStyles = Gantt.styles.popup;
                var html = ufa.format('<div class="{0}"><div class="{1}"><p class="{2}">{3}</p><div class="{4}">', popupStyles.form, popupStyles.formContainer, popupStyles.message, options.text, popupStyles.buttonsContainer);
            ...
            _createPopupEditor: function (task) {
                ...
                var html = ufa.format('<div {0}="{1}" class="{2} {3}"><div class="{4}">', ufa.attr('uid'), task.uid, popupStyles.form, popupStyles.editForm, popupStyles.formContainer);
                ...
                html += '</div></div></div>';
                var container = this.container = $(html).appendTo(this.element).eq(0).ufaWindow(extend({
                    modal: true,
                    resizable: false,
                    draggable: true,
                ...
                var editableWidget = container.ufaEditable({
        ...
        var ResourceEditor = Widget.extend({
            ...
            _attachHandlers: function () {
                ...
                grid.wrapper.on(CLICK + NS, 'input[type=\'checkbox\']', function () {
                    ...
                    var model = grid.dataSource.getByUid(row.attr(ufa.attr('uid')));
            ...
            _initContainer: function () {
                var popupStyles = Gantt.styles.popup;
                var dom = ufa.format('<div class="{0} {1}"><div class="{2} {3}"/></div>"', popupStyles.form, popupStyles.editForm, popupStyles.formContainer, popupStyles.resourcesFormContainer);
                dom = $(dom);
                this.container = dom.find(DOT + popupStyles.resourcesFormContainer);
                this.window = dom.ufaWindow({
                    modal: true,
                    resizable: false,
                    draggable: true,
                    visible: false,
            ...
            _resourceGrid: function () {
                ...
                this.grid = new ufa.ui.Grid(element, {
                    columns: [
                        {
                            field: 'name',
                        ...
                        },
                        {
                            ...
                            template: function (dataItem) {
                                ...
                                return valueFormat ? ufa.toString(value, valueFormat) : value;
                            }
                        }
                    ],
                    height: 280,
        ...
        var Gantt = Widget.extend({
            ...
            setOptions: function (options) {
                var newOptions = ufa.deepExtend({}, this.options, options);
            ...
            _toolbar: function () {
                ...
                if (!isFunction(actions)) {
                    ...
                    actions = proxy(ufa.template(actions), this);
                }
            ...
            _footer: function () {
                ...
                var button = $(ufa.template(BUTTON_TEMPLATE)(extend(true, { styles: ganttStyles }, defaultCommands.append, { text: messages.append })));
            ...
            _list: function () {
                ...
                this.list = new ufa.ui.GanttList(element, options);
            ...
            _timeline: function () {
                ...
                this.timeline = new ufa.ui.GanttTimeline(element, options);
            ...
            _dataSource: function () {
                ...
                this.dataSource = ufa.data.GanttDataSource.create(dataSource).bind('change', this._refreshHandler).bind('progress', this._progressHandler).bind('error', this._errorHandler);
            },
            _dependencies: function () {
                ...
                this.dependencies = ufa.data.GanttDependencyDataSource.create(dataSource).bind('change', this._dependencyRefreshHandler).bind('error', this._dependencyErrorHandler);
            },
            _resources: function () {
                var resources = this.options.resources;
                ...
                this.resources.dataSource = ufa.data.DataSource.create(dataSource);
            },
            _assignments: function () {
                ...
                this.assignments.dataSource = ufa.data.DataSource.create(dataSource);
            ...
            _createPopupButton: function (command) {
                ...
                return ufa.template(COMMAND_BUTTON_TEMPLATE)(options);
            },
            view: function (type) {
            ...
            refresh: function () {
                ...
                var cachedIndex = -1;
                if (this.current) {
                    cachedUid = this.current.closest('tr').attr(ufa.attr('uid'));
            ...
            _assignResources: function (taskTree) {
                ...
                var applyTaskResource = function (task, action) {
                    ...
                    ufa.setter(resources.field)(task, new ObservableArray([]));
                ...
                var wrapTask = function (task, items) {
                    for (var j = 0, length = items.length; j < length; j++) {
                        ...
                        var formatedValue = ufa.toString(resourceValue, valueFormat);
            ...
            _progress: function (toggle) {
                ufa.ui.progress(this.element, toggle);
            },
            _resizable: function () {
                var that = this;
            ...
            _navigatable: function () {
                ...
                var ganttStyles = Gantt.styles;
                var isRtl = ufa.support.isRtl(this.wrapper);
                ...
                $(this.wrapper).on('mousedown' + NS, 'tr' + attr + ', div' + attr + ':not(' + DOT + ganttStyles.line + ')', function (e) {
                    ...
                    if (navigatable) {
                        if (currentTarget.is('tr')) {
                            ...
                            current = that.list.content.find('tr' + selector(currentTarget.attr(ufa.attr('uid'))) + ' > td:first');
                        }
            ...
            _current: function (element) {
                ...
                if (element && element.length) {
                    ...
                    activeElement = $(ufa._activeElement());
