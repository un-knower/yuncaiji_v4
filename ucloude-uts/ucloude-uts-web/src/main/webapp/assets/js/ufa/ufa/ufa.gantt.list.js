(function (f, define) {
    ...
    (function ($) {
        ...
        var GanttList = ui.GanttList = Widget.extend({
            ...
            _domTrees: function () {
                this.headerTree = new ufaDom.Tree(this.header[0]);
                this.contentTree = new ufaDom.Tree(this.content[0]);
            },
            _columns: function () {
            ...
            _header: function () {
                ...
                var thead;
                var table;
                colgroup = ufaDomElement('colgroup', null, this._cols());
                thead = ufaDomElement('thead', { 'role': 'rowgroup' }, [ufaDomElement('tr', { 'role': 'row' }, this._ths())]);
                table = ufaDomElement('table', {
            ...
            _render: function (tasks) {
                ...
                colgroup = ufaDomElement('colgroup', null, this._cols());
                tbody = ufaDomElement('tbody', { 'role': 'rowgroup' }, this._trs(tasks));
                table = ufaDomElement('table', tableAttr, [
                    colgroup,
                    tbody
                ]);
            ...
            _ths: function () {
                ...
                for (var i = 0, length = columns.length; i < length; i++) {
                    ...
                    ths.push(ufaDomElement('th', attr, [ufaTextElement(column.title)]));
                }
                if (this.options.resizable) {
                    ths.push(ufaDomElement('th', {
                        className: GanttList.styles.header,
                        'role': 'columnheader'
                    }));
                }
            ...
            _cols: function () {
                ...
                for (var i = 0, length = columns.length; i < length; i++) {
                    ...
                    cols.push(ufaDomElement('col', style, []));
                }
                if (this.options.resizable) {
                    cols.push(ufaDomElement('col', { style: { width: '1px' } }));
                }
                return cols;
            },
            _trs: function (tasks) {
            ...
            _tds: function (options) {
                ...
                if (this.options.resizable) {
                    children.push(ufaDomElement('td', { 'role': 'gridcell' }));
                }
            ...
            _td: function (options) {
                ...
                if (column.field == resourcesField) {
                    ...
                    for (var i = 0; i < value.length; i++) {
                        formatedValue.push(ufa.format('{0} [{1}]', value[i].get('name'), value[i].get('formatedValue')));
                    }
                    formatedValue = formatedValue.join(', ');
                } else {
                    formatedValue = column.format ? ufa.format(column.format, value) : value;
                }
                if (column.field === 'title') {
                    ...
                    children.push(ufaDomElement('span', { className: listStyles.icon + ' ' + (task.summary ? task.expanded ? listStyles.iconCollapse : listStyles.iconExpand : listStyles.iconHidden) }));
                    label = ufa.format('{0}, {1:P0}', formatedValue, task.percentComplete);
                }
                children.push(ufaDomElement('span', { 'aria-label': label }, [ufaTextElement(formatedValue)]));
                return ufaDomElement('td', { 'role': 'gridcell' }, children);
            },
            ...
            _sortable: function () {
                ...
                var cells = this.header.find('th[' + ufa.attr('field') + ']');
                ...
                for (var idx = 0, length = cells.length; idx < length; idx++) {
                    ...
                    if (column.sortable && column.field !== resourcesField) {
                        ...
                        cell.attr('data-' + ufa.ns + 'field', column.field).ufaColumnSorter({ dataSource: this.dataSource }).find(DOT + GanttList.link).on('click' + NS, handler);
                    }
                }
                cells = null;
            },
            ...
            _editable: function () {
                ...
                if (!mobileOS) {
                    ...
                    that.touch = that.content.ufaTouch({
                        filter: 'td',
            ...
            _editCell: function (options) {
                ...
                var BINDING = ufa.attr('bind');
                var FORMAT = ufa.attr('format');
                var attr = {
                    'name': column.field,
                ...
                if ((field.type === 'date' || $.type(field) === 'date') && (!column.format || /H|m|s|F|g|u/.test(column.format))) {
                    ...
                    attr[DATATYPE] = 'date';
                    if (column.format) {
                        attr[FORMAT] = ufa._extractFormat(column.format);
                    }
                    editor = function (container, options) {
                        $('<input type="text"/>').attr(attr).appendTo(container).ufaDateTimePicker({ format: options.format });
                    };
                }
                this.editable = cell.addClass(listStyles.editCell).ufaEditable({
                    fields: {
                        field: column.field,
                ...
                if (validation && validation.dateCompare && isFunction(validation.dateCompare) && validation.message) {
                    $('<span ' + ufa.attr('for') + '="' + column.field + '" class="k-invalid-msg"/>').hide().appendTo(cell);
                    cell.find('[name=' + column.field + ']').attr(ufa.attr('dateCompare-msg'), validation.message);
                }
                ...
                this.editable.bind('validate', function (e) {
                    var focusable = this.element.find(':ufaFocusable:first').focus();
                    if (oldIE) {
                        focusable.focus();
                    }
                    e.preventDefault();
            ...
            _draggable: function () {
                ...
                var dropTarget;
                var listStyles = GanttList.styles;
                var isRtl = ufa.support.isRtl(this.element);
                var selector = 'tr[' + ufa.attr('level') + ' = 0]:last';
                var action = {};
                var clear = function () {
                    draggedTask = null;
                    dropTarget = null;
                ...
                var defineLimits = function () {
                    ...
                    var offsetTop = ufa.getOffset(dropTarget).top;
                    extend(dropTarget, {
                ...
                var defineAction = function (coordinate) {
                    ...
                    var command = 'add';
                    var level = parseInt(dropTarget.attr(ufa.attr('level')), 10);
                    var sibling;
                    ...
                    if (sibling && parseInt(sibling.attr(ufa.attr('level')), 10) === level) {
                ...
                this._reorderDraggable = this.content.ufaDraggable({
                    distance: 10,
                    holdToDrag: mobileOS,
                    group: 'listGroup',
                    filter: 'tr[data-uid]',
                ...
                this._tableDropArea = this.content.ufaDropTargetArea({
                    distance: 0,
                    group: 'listGroup',
                    filter: 'tr[data-uid]',
                ...
                }).data('ufaDropTargetArea');
                this._contentDropArea = this.element.ufaDropTargetArea({
                    distance: 0,
                    group: 'listGroup',
            ...
            _resizable: function () {
                ...
                this._columnResizable = this.header.ufaResizable({
