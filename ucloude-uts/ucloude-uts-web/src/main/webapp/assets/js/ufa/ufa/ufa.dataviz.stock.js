...
(function (f, define) {
    define('ufa.dataviz.stock', ['ufa.dataviz.chart'], f);
}(function () {
    var __meta__ = {
    ...
    (function ($, undefined) {
        var ufa = window.ufa, Class = ufa.Class, Observable = ufa.Observable, deepExtend = ufa.deepExtend, math = Math, proxy = $.proxy, util = ufa.util, last = util.last, renderTemplate = util.renderTemplate, dataviz = ufa.dataviz, defined = util.defined, filterSeriesByType = dataviz.filterSeriesByType, template = ufa.template, Chart = dataviz.ui.Chart, Selection = dataviz.Selection, addDuration = dataviz.addDuration, limitValue = util.limitValue, lteDateIndex = dataviz.lteDateIndex, toDate = dataviz.toDate, toTime = dataviz.toTime;
            ...
            options: {
                ...
                navigator: {
                    ...
                    seriesDefaults: {
                        ...
                        tooltip: {
                            visible: true,
                            template: '#= ufa.toString(category, \'d\') #'
                        },
                        line: { width: 2 }
                    },
                    hint: {},
        ...
        var Navigator = Observable.extend({
            ...
            _initDataSource: function () {
                ...
                if (dsOptions) {
                    navi.dataSource = ufa.data.DataSource.create(dsOptions).bind(CHANGE, navi._dataChangedHandler);
                    if (autoBind) {
            ...
            _liveDrag: function () {
                var support = ufa.support, isTouch = support.touch, browser = support.browser, isFirefox = browser.mozilla, isOldIE = browser.msie && browser.version < 9;
