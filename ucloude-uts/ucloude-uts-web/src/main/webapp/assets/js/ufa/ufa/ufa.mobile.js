(function (f, define) {
    define('ufa.core', ['jquery'], f);
}(function () {
    var __meta__ = {
        id: 'core',
        name: 'Core',
        category: 'framework',
        description: 'The core of the ufa framework.'
    };
    (function ($, window, undefined) {
        var ufa = window.ufa = window.ufa || { cultures: {} }, extend = $.extend, each = $.each, isArray = $.isArray, proxy = $.proxy, noop = $.noop, math = Math, Template, JSON = window.JSON || {}, support = {}, percentRegExp = /%/, formatRegExp = /\{(\d+)(:[^\}]+)?\}/g, boxShadowRegExp = /(\d+(?:\.?)\d*)px\s*(\d+(?:\.?)\d*)px\s*(\d+(?:\.?)\d*)px\s*(\d+)?/i, numberRegExp = /^(\+|-?)\d+(\.?)\d*$/, FUNCTION = 'function', STRING = 'string', NUMBER = 'number', OBJECT = 'object', NULL = 'null', BOOLEAN = 'boolean', UNDEFINED = 'undefined', getterCache = {}, setterCache = {}, slice = [].slice;
        ufa.version = '2016.1.112'.replace(/^\s+|\s+$/g, '');
        function Class() {
        }
        ...
        var isFunction = ufa.isFunction = function (fn) {
        ...
        function compilePart(part, stringPart) {
            if (stringPart) {
                ...
                if (first === '=') {
                    ...
                    return '+$ufaHtmlEncode(' + rest + ')+';
                } else {
                    return ';' + part + ';$ufaOutput+=';
                }
            }
        }
        ...
        Template = {
            ...
            compile: function (template, options) {
                var settings = extend({}, this, options), paramName = settings.paramName, argumentName = paramName.match(argumentNameRegExp)[0], useWithBlock = settings.useWithBlock, functionBody = 'var $ufaOutput, $ufaHtmlEncode = ufa.htmlEncode;', fn, parts, idx;
                if (isFunction(template)) {
                    return template;
                }
                functionBody += useWithBlock ? 'with(' + paramName + '){' : '';
                functionBody += '$ufaOutput=';
                parts = template.replace(escapedCurlyRegExp, '__CURLY__').replace(encodeRegExp, '#=$ufaHtmlEncode($1)#').replace(curlyRegExp, '}').replace(escapedSharpRegExp, '__SHARP__').split('#');
                for (idx = 0; idx < parts.length; idx++) {
                    functionBody += compilePart(parts[idx], idx % 2 === 0);
                }
                functionBody += useWithBlock ? ';}' : ';';
                functionBody += 'return $ufaOutput;';
                ...
                try {
                    ...
                    throw new Error(ufa.format('Invalid template:\'{0}\' Generated code:\'{1}\'', template, functionBody));
                }
            }
        };
        ...
        (function () {
            ...
            function getCulture(culture) {
                ...
                return culture || ufa.cultures.current;
            }
            ufa.culture = function (cultureName) {
                var cultures = ufa.cultures, culture;
            ...
            ufa.findCulture = findCulture;
            ufa.getCulture = getCulture;
            ufa.culture(EN);
            ...
            ufa._extractFormat = function (format) {
                if (format.slice(0, 3) === '{0:') {
                    format = format.slice(3, format.length - 1);
                }
                return format;
            };
            ufa._activeElement = function () {
                try {
        ...
        (function () {
                    ...
                    if (literal) {
                        ...
                        if (ch === 'd') {
                            ...
                            if (milliseconds !== null) {
                                ...
                                milliseconds = ufa._round(milliseconds, 3);
                                milliseconds *= 1000;
                            }
            ...
            ufa.parseDate = function (value, formats, culture) {
                if (objectToString.call(value) === '[object Date]') {
                    return value;
                }
                var idx = 0;
                ...
                if (value && value.indexOf('/D') === 0) {
                    ...
                    if (date) {
                        ...
                        if (tzoffset) {
                            ...
                            date = ufa.timezone.apply(date, 0);
                            date = ufa.timezone.convert(date, 0, -1 * tzoffset);
                        }
                        return date;
                    }
                }
                culture = ufa.getCulture(culture);
                if (!formats) {
                    formats = [];
            ...
            ufa.parseInt = function (value, culture) {
                var result = ufa.parseFloat(value, culture);
                if (result) {
                    result = result | 0;
                }
                return result;
            };
            ufa.parseFloat = function (value, culture, format) {
                if (!value && value !== 0) {
                    return null;
                }
        ...
        function getShadows(element) {
            var shadow = element.css(ufa.support.transitions.css + 'box-shadow') || element.css('box-shadow'), radius = shadow ? shadow.match(boxShadowRegExp) || [
                    0,
                    0,
                    0,
                    0,
        ...
        function deepExtendOne(destination, source) {
            var ObservableArray = ufa.data.ObservableArray, LazyObservableArray = ufa.data.LazyObservableArray, DataSource = ufa.data.DataSource, HierarchicalDataSource = ufa.data.HierarchicalDataSource, property, propValue, propType, propInit, destProp;
        ...
        function fx(element) {
            return new ufa.effects.Element(element);
        }
        var effects = {};
        $.extend(effects, {
            enabled: true,
        ...
        if (!('ufaAnimate' in $.fn)) {
            extend($.fn, {
                ufaStop: function (clearQueue, gotoEnd) {
                    return this.stop(clearQueue, gotoEnd);
                },
                ufaAnimate: function (options, duration, reverse, complete) {
                    return animate(this, options, duration, reverse, complete);
                },
                ufaAddClass: function (classes, options) {
                    return ufa.toggleClass(this, classes, options, true);
                },
                ufaRemoveClass: function (classes, options) {
                    return ufa.toggleClass(this, classes, options, false);
                },
                ufaToggleClass: function (classes, options, toggle) {
                    return ufa.toggleClass(this, classes, options, toggle);
                }
            });
        }
        ...
        extend(ufa, {
            ...
            support: ufa.support || support,
            animate: ufa.animate || animate,
            ns: '',
            attr: function (value) {
                return 'data-' + ufa.ns + value;
            },
            getShadows: getShadows,
            wrap: wrap,
            deepExtend: deepExtend,
            getComputedStyles: getComputedStyles,
            webComponents: ufa.webComponents || [],
            isScrollable: isScrollable,
            scrollLeft: scrollLeft,
            size: size,
            toCamelCase: toCamelCase,
            toHyphens: toHyphens,
            getOffset: ufa.getOffset || getOffset,
            parseEffects: ufa.parseEffects || parseEffects,
            toggleClass: ufa.toggleClass || toggleClass,
            directions: ufa.directions || directions,
            ...
            getter: function (expression, safe) {
                ...
                return getterCache[key] = getterCache[key] || new Function('d', 'return ' + ufa.expr(expression, safe));
            },
            setter: function (expression) {
                return setterCache[expression] = setterCache[expression] || new Function('d,value', ufa.expr(expression) + '=value');
            },
            accessor: function (expression) {
                return {
                    get: ufa.getter(expression),
                    set: ufa.setter(expression)
                };
            },
            guid: function () {
                var id = '', i, random;
            ...
            roleSelector: function (role) {
                return role.replace(/(\S+)/g, '[' + ufa.attr('role') + '=$1],').slice(0, -1);
            },
            directiveSelector: function (directives) {
                ...
                return selectors.join(' ').replace(/(\S+)/g, 'ufa-mobile-$1,').slice(0, -1);
            },
            triggeredByInput: function (e) {
                return /^(label|input|textarea|select)$/i.test(e.target.tagName);
            },
            onWidgetRegistered: function (callback) {
                for (var i = 0, len = ufa.widgets.length; i < len; i++) {
                    callback(ufa.widgets[i]);
                }
                ufa._widgetRegisteredCallbacks.push(callback);
            },
            logToConsole: function (message) {
                var console = window.console;
                if (!ufa.suppressLog && typeof console != 'undefined' && console.log) {
                    console.log(message);
                }
            }
        });
        var Widget = Observable.extend({
            init: function (element, options) {
                var that = this;
                that.element = ufa.jQuery(element).handler(that);
                ...
                if (!that.element.attr(ufa.attr('role'))) {
                    that.element.attr(ufa.attr('role'), (options.name || '').toLowerCase());
                }
                that.element.data('ufa' + options.prefix + options.name, that);
                that.bind(that.events, options);
            },
            events: [],
            options: { prefix: '' },
            _hasBindingTarget: function () {
                return !!this.element[0].ufaBindingTarget;
            },
            ...
            getSize: function () {
                return ufa.dimensions(this.element);
            },
            size: function (size) {
                if (!size) {
                    return this.getSize();
            ...
            destroy: function () {
                var that = this;
                that.element.removeData('ufa' + that.options.prefix + that.options.name);
        ...
        ufa.dimensions = function (element, dimensions) {
        ...
        function parseOption(element, option) {
            ...
            value = element.getAttribute('data-' + ufa.ns + option);
            if (value === null) {
                value = undefined;
            } else if (value === 'null') {
                value = null;
        ...
        ufa.initWidget = function (element, options, roles) {
            ...
            role = element.getAttribute('data-' + ufa.ns + 'role');
            if (!role) {
                return;
            }
            ...
            if (fullPath) {
                ...
                widget = ufa.getter(role)(window);
            }
            var data = $(element).data(), widgetKey = widget ? 'ufa' + widget.fn.options.prefix + widget.fn.options.name : '';
            if (fullPath) {
                widgetKeyRegExp = new RegExp('^ufa.*' + role + '$', 'i');
            } else {
            ...
            if (dataSource) {
                if (typeof dataSource === STRING) {
                    options.dataSource = ufa.getter(dataSource)(window);
                } else {
                    options.dataSource = dataSource;
                }
            }
            for (idx = 0, length = widget.fn.events.length; idx < length; idx++) {
                ...
                value = parseOption(element, option);
                if (value !== undefined) {
                    options[option] = ufa.getter(value)(window);
                }
            }
            if (!result) {
        ...
        ufa.rolesFromNamespaces = function (namespaces) {
        ...
        ufa.init = function (element) {
            var roles = ufa.rolesFromNamespaces(slice.call(arguments, 1));
            $(element).find('[data-' + ufa.ns + 'role]').addBack().each(function () {
                ufa.initWidget(this, {}, roles);
            });
        };
        ufa.destroy = function (element) {
            $(element).find('[data-' + ufa.ns + 'role]').addBack().each(function () {
                var data = $(this).data();
                for (var key in data) {
                    if (key.indexOf('ufa') === 0 && typeof data[key].destroy === FUNCTION) {
                        data[key].destroy();
                    }
                }
            });
        ...
        function resizableWidget() {
            var widget = $(this);
            return $.inArray(widget.attr('data-' + ufa.ns + 'role'), [
                'slider',
                'rangeslider'
            ]) > -1 || widget.is(':visible');
        }
        ufa.resize = function (element, force) {
            var widgets = $(element).find('[data-' + ufa.ns + 'role]').addBack().filter(resizableWidget);
            if (!widgets.length) {
                return;
            }
            ...
            $.each(widgetsArray, function () {
                var widget = ufa.widgetInstance($(this));
                if (widget) {
                    widget.resize(force);
                }
            });
        };
        ufa.parseOptions = parseOptions;
        extend(ufa.ui, {
            Widget: Widget,
            DataBoundWidget: DataBoundWidget,
            roles: {},
            progress: function (container, toggle) {
                var mask = container.find('.k-loading-mask'), support = ufa.support, browser = support.browser, isRtl, leftRight, webkitCorrection, containerScrollLeft;
                if (toggle) {
                    if (!mask.length) {
            ...
            plugin: function (widget, register, prefix) {
                ...
                register = register || ufa.ui;
                prefix = prefix || '';
                register[name] = widget;
                register.roles[name.toLowerCase()] = widget;
                getter = 'getufa' + prefix + name;
                name = 'ufa' + prefix + name;
                var widgetEntry = {
                    name: name,
                    widget: widget,
                    prefix: prefix || ''
                };
                ufa.widgets.push(widgetEntry);
                for (var i = 0, len = ufa._widgetRegisteredCallbacks.length; i < len; i++) {
                    ufa._widgetRegisteredCallbacks[i](widgetEntry);
                }
                $.fn[name] = function (options) {
                    ...
                    if (typeof options === STRING) {
                        ...
                        this.each(function () {
                            ...
                            if (!widget) {
                                throw new Error(ufa.format('Cannot call method \'{0}\' of {1} before it is initialized', options, name));
                            }
                            method = widget[options];
                            if (typeof method !== FUNCTION) {
                                throw new Error(ufa.format('Cannot find method \'{0}\' of {1}', options, name));
                            }
        ...
        var MobileWidget = Widget.extend({
            ...
            destroy: function () {
                ...
                this.element.ufaDestroy();
            },
            options: { prefix: 'Mobile' },
            events: [],
            view: function () {
                var viewElement = this.element.closest(ufa.roleSelector('view splitview modalview drawer'));
                return ufa.widgetInstance(viewElement, ufa.mobile.ui) || ContainerNullObject;
            },
            ...
            container: function () {
                var element = this.element.closest(ufa.roleSelector('view layout modalview drawer splitview'));
                return ufa.widgetInstance(element.eq(0), ufa.mobile.ui) || ContainerNullObject;
            }
        });
        extend(ufa.mobile, {
            init: function (element) {
                ufa.init(element, ufa.mobile.ui, ufa.ui, ufa.dataviz.ui);
            },
            appLevelNativeScrolling: function () {
                return ufa.mobile.application && ufa.mobile.application.options && ufa.mobile.application.options.useNativeScrolling;
            },
            roles: {},
            ui: {
                Widget: MobileWidget,
                ...
                plugin: function (widget) {
                    ufa.ui.plugin(widget, ufa.mobile.ui, 'Mobile');
                }
            }
        });
        deepExtend(ufa.dataviz, {
            init: function (element) {
                ufa.init(element, ufa.dataviz.ui);
            },
            ui: {
                roles: {},
                themes: {},
                views: [],
                plugin: function (widget) {
                    ufa.ui.plugin(widget, ufa.dataviz.ui);
                }
            },
            roles: {}
        });
        ufa.touchScroller = function (elements, options) {
            if (!options) {
                options = {};
            }
            ...
            return $(elements).map(function (idx, element) {
                element = $(element);
                if (support.kineticScrollNeeded && ufa.mobile.ui.Scroller && !element.data('ufaMobileScroller')) {
                    element.ufaMobileScroller(options);
                    return element.data('ufaMobileScroller');
                } else {
                    return false;
                }
            })[0];
        };
        ufa.preventDefault = function (e) {
            e.preventDefault();
        };
        ufa.widgetInstance = function (element, suites) {
            var role = element.data(ufa.ns + 'role'), widgets = [], i, length;
            if (role) {
                if (role === 'content') {
                    role = 'scroller';
                }
                if (suites) {
                    ...
                    widgets = [
                        ufa.ui.roles[role],
                        ufa.dataviz.ui.roles[role],
                        ufa.mobile.ui.roles[role]
                    ];
                }
                if (role.indexOf('.') >= 0) {
                    widgets = [ufa.getter(role)(window)];
                }
                for (i = 0, length = widgets.length; i < length; i++) {
                    var widget = widgets[i];
                    if (widget) {
                        var instance = element.data('ufa' + widget.fn.options.prefix + widget.fn.options.name);
                        if (instance) {
                            return instance;
                        }
                    }
        ...
        ufa.onResize = function (callback) {
        ...
        ufa.unbindResize = function (callback) {
            $(window).off(support.resize, callback);
        };
        ufa.attrValue = function (element, key) {
            return element.data(ufa.ns + key);
        };
        ufa.days = {
            Sunday: 0,
            Monday: 1,
        ...
        ufa.applyEventMap = function (events, ns) {
        ...
        var on = $.fn.on;
        function ufaJQuery(selector, context) {
            return new ufaJQuery.fn.init(selector, context);
        }
        extend(true, ufaJQuery, $);
        ufaJQuery.fn = ufaJQuery.prototype = new $();
        ufaJQuery.fn.constructor = ufaJQuery;
        ufaJQuery.fn.init = function (selector, context) {
            if (context && context instanceof $ && !(context instanceof ufaJQuery)) {
                context = ufaJQuery(context);
            }
            return $.fn.init.call(this, selector, context, rootjQuery);
        };
        ufaJQuery.fn.init.prototype = ufaJQuery.fn;
        var rootjQuery = ufaJQuery(document);
        extend(ufaJQuery.fn, {
            ...
            autoApplyNS: function (ns) {
                this.data('ufaNS', ns || ufa.guid());
                return this;
            },
            on: function () {
                var that = this, ns = that.data('ufaNS');
                ...
                var callback = args[args.length - 1], events = ufa.applyEventMap(args[0], ns);
            ...
            ufaDestroy: function (ns) {
                ns = ns || this.data('ufaNS');
                if (ns) {
                    this.off('.' + ns);
                }
                return this;
        ...
        ufa.jQuery = ufaJQuery;
        ufa.eventMap = eventMap;
        ufa.timezone = function () {
            var months = {
                Jan: 0,
                Feb: 1,
                Mar: 2,
            ...
            function findZone(utcTime, zones, timezone) {
                ...
                if (!zoneRules) {
                    throw new Error('Timezone "' + timezone + '" is either incorrect, or ufa.timezones.min.js is not included.');
                }
            ...
            function offset(utcTime, timezone) {
                ...
                var zone = info.zone;
                var rule = info.rule;
                return ufa.parseFloat(rule ? zone[0] - rule[6] : zone[0]);
            }
        ...
        ufa.date = function () {
            ...
            function setHours(date, time) {
                date = new Date(ufa.date.getDate(date).getTime() + ufa.date.getMilliseconds(time));
        ...
        ufa.stripWhitespace = function (element) {
        ...
        ufa.animationFrame = function (callback) {
            animationFrame.call(window, callback);
        };
        var animationQueue = [];
        ufa.queueAnimation = function (callback) {
            animationQueue[animationQueue.length] = callback;
            if (animationQueue.length === 1) {
                ufa.runNextAnimation();
            }
        };
        ufa.runNextAnimation = function () {
            ufa.animationFrame(function () {
                if (animationQueue[0]) {
        ...
        ufa.parseQueryStringParams = function (url) {
        ...
        ufa.elementUnderCursor = function (e) {
            if (typeof e.x.client != 'undefined') {
                return document.elementFromPoint(e.x.client, e.y.client);
            }
        };
        ufa.wheelDeltaY = function (jQueryEvent) {
        ...
        ufa.throttle = function (fn, delay) {
            var timeout;
            var lastExecTime = 0;
            if (!delay || delay <= 0) {
                return fn;
        ...
        ufa.caret = function (element, start, end) {
            var rangeElement;
        ...
        ufa.compileMobileDirective = function (element, scope) {
            var angular = window.angular;
            element.attr('data-' + ufa.ns + 'role', element[0].tagName.toLowerCase().replace('ufa-mobile-', '').replace('-', ''));
            ...
            return ufa.widgetInstance(element, ufa.mobile.ui);
        };
        ufa.antiForgeryTokens = function () {
        ...
        ufa.cycleForm = function (form) {
            var firstElement = form.find('input, .k-widget').first();
            var lastElement = form.find('button, .k-button').last();
            function focus(el) {
                var widget = ufa.widgetInstance(el);
            ...
            lastElement.on('keydown', function (e) {
                if (e.keyCode == ufa.keys.TAB && !e.shiftKey) {
                    e.preventDefault();
                    focus(firstElement);
                }
            });
        ...
        (function () {
            function postToProxy(dataURI, fileName, proxyURL, proxyTarget) {
                ...
                var data = ufa.antiForgeryTokens();
            ...
            var downloadAttribute = 'download' in fileSaver && !ufa.support.browser.edge;
        ...
        ufa.proxyModelSetters = function proxyModelSetters(data) {
            var observable = {};
...
(function (f, define) {
    define('ufa.fx', ['ufa.core'], f);
}(function () {
    var __meta__ = {
        id: 'fx',
        name: 'Effects',
        category: 'framework',
        description: 'Required for animation effects in all ufa UI widgets.',
        depends: ['core']
    };
    (function ($, undefined) {
        var ufa = window.ufa, fx = ufa.effects, each = $.each, extend = $.extend, proxy = $.proxy, support = ufa.support, browser = support.browser, transforms = support.transforms, transitions = support.transitions, scaleProperties = {
                scale: 0,
                scalex: 0,
                scaley: 0,
                scale3d: 0
        ...
        ufa.toggleClass = function (element, classes, options, add) {
            if (classes) {
                classes = classes.split(' ');
                if (transitions) {
                    options = extend({
        ...
        ufa.parseEffects = function (input, mirror) {
            var effects = {};
        ...
        if (transitions) {
            extend(fx, {
                ...
                stopQueue: function (element, clearQueue, gotoEnd) {
                    ...
                    if (retainPosition) {
                        cssValues = ufa.getComputedStyles(element[0], taskKeys);
                    }
                    if (completeCallback) {
                        completeCallback();
                    }
        ...
        var EffectSet = ufa.Class.extend({
            init: function (element, options) {
                var that = this;
            ...
            stop: function () {
                $(this.element).ufaStop(true, true);
            },
        ...
        fx.promise = function (element, options) {
            var effects = [], effectClass, effectSet = new EffectSet(element, options), parsedEffects = ufa.parseEffects(options.effects), effect;
        ...
        createEffect('tile', {
            ...
            children: function () {
                ...
                var children = [ufa.fx(that.element).slideIn(dir).setReverse(reverse)];
                if (previous) {
                    children.push(ufa.fx(previous).slideIn(directions[dir].reverse).divisor(divisor).setReverse(!reverse));
                }
                return children;
            }
        });
        ...
        function clipInHalf(container, direction) {
            var vertical = ufa.directions[direction].vertical, size = container[vertical ? HEIGHT : WIDTH]() / 2 + 'px';
        ...
        createEffect('turningPage', {
            ...
            prepare: function (start, end) {
                ...
                start.zIndex = 1;
                if (that._clipInHalf) {
                    start.clip = clipInHalf(that._container, ufa.directions[direction].reverse);
                }
        ...
        createEffect('pageturn', {
            ...
            children: function () {
                var that = this, options = that.options, direction = that._direction === 'horizontal' ? 'left' : 'top', reverseDirection = ufa.directions[direction].reverse, reverse = that._reverse, temp, faceClone = options.face.clone(true).removeAttr('id'), backClone = options.back.clone(true).removeAttr('id'), element = that.element;
                if (reverse) {
                    temp = direction;
                ...
                return [
                    ufa.fx(options.face).staticPage(direction, element).face(true).setReverse(reverse),
                    ufa.fx(options.back).staticPage(reverseDirection, element).setReverse(reverse),
                    ufa.fx(faceClone).turningPage(direction, element).face(true).clipInHalf(true).temporary().setReverse(reverse),
                    ufa.fx(backClone).turningPage(reverseDirection, element).clipInHalf(true).temporary().setReverse(reverse)
                ];
            },
            prepare: function (start, end) {
        ...
        createEffect('flip', {
            ...
            children: function () {
                var that = this, options = that.options, direction = that._direction === 'horizontal' ? 'left' : 'top', reverseDirection = ufa.directions[direction].reverse, reverse = that._reverse, temp, element = that.element;
                if (reverse) {
                    temp = direction;
                    direction = reverseDirection;
                    reverseDirection = temp;
                ...
                return [
                    ufa.fx(options.face).turningPage(direction, element).face(true).setReverse(reverse),
                    ufa.fx(options.back).turningPage(reverseDirection, element).setReverse(reverse)
                ];
            },
        ...
        var Animation = ufa.Class.extend({
            init: function () {
                var that = this;
            ...
            start: function () {
                ...
                if (!this.done()) {
                    this._started = true;
                    ufa.animationFrame(this._tickProxy);
                } else {
                    this.onEnd();
                }
            },
            ...
            _tick: function () {
                ...
                that.tick();
                if (!that.done()) {
                    ufa.animationFrame(that._tickProxy);
                } else {
                    that._started = false;
                    that.onEnd();
                }
...
(function (f, define) {
    define('ufa.data.odata', ['ufa.core'], f);
}(function () {
    var __meta__ = {
        id: 'data.odata',
        name: 'OData',
    ...
    (function ($, undefined) {
        var ufa = window.ufa, extend = $.extend, odataFilters = {
                eq: 'eq',
                neq: 'ne',
                gt: 'gt',
                gte: 'ge',
            ...
            for (idx = 0, length = filters.length; idx < length; idx++) {
                ...
                if (filter.filters) {
                    ...
                    if (operator === 'isnull' || operator === 'isnotnull') {
                        filter = ufa.format('{0} {1} null', field, filter);
                    } else if (operator === 'isempty' || operator === 'isnotempty') {
                        filter = ufa.format('{0} {1} \'\'', field, filter);
                        ...
                        filter = ufa.format(format, filter, value, field);
                    }
                }
                result.push(filter);
            }
            ...
            transports: {
                'odata-v4': {
                    ...
                    parameterMap: function (options, type) {
                        var result = ufa.data.transports.odata.parameterMap(options, type, true);
                        if (type == 'read') {
                            result.$count = true;
...
(function (f, define) {
    define('ufa.data.xml', ['ufa.core'], f);
}(function () {
    var __meta__ = {
        id: 'data.xml',
        name: 'XML',
    ...
    (function ($, undefined) {
        var ufa = window.ufa, isArray = $.isArray, isPlainObject = $.isPlainObject, map = $.map, each = $.each, extend = $.extend, getter = ufa.getter, Class = ufa.Class;
            ...
            init: function (options) {
                ...
                if (model) {
                    if (isPlainObject(model)) {
                        var base = options.modelBase || ufa.data.Model;
                        if (model.fields) {
...
(function (f, define) {
    ...
    (function ($, undefined) {
        var extend = $.extend, proxy = $.proxy, isPlainObject = $.isPlainObject, isEmptyObject = $.isEmptyObject, isArray = $.isArray, grep = $.grep, ajax = $.ajax, map, each = $.each, noop = $.noop, ufa = window.ufa, isFunction = ufa.isFunction, Observable = ufa.Observable, Class = ufa.Class, STRING = 'string', FUNCTION = 'function', CREATE = 'create', READ = 'read', UPDATE = 'update', DESTROY = 'destroy', CHANGE = 'change', SYNC = 'sync', GET = 'get', ERROR = 'error', REQUESTSTART = 'requestStart', PROGRESS = 'progress', REQUESTEND = 'requestEnd', crud = [
                CREATE,
                READ,
                UPDATE,
                DESTROY
                ...
                return o;
            }, getter = ufa.getter, stringify = ufa.stringify, math = Math, push = [].push, join = [].join, pop = [].pop, splice = [].splice, shift = [].shift, slice = [].slice, unshift = [].unshift, toString = {}.toString, stableSort = ufa.support.stableSort, dateRegExp = /^\/Date\((.*?)\)\/$/, newLineRegExp = /(\r+|\n+)/g, quoteRegExp = /(?=['\\])/g;
        ...
        var ObservableObject = Observable.extend({
            ...
            get: function (field) {
                ...
                if (field === 'this') {
                    ...
                    result = ufa.getter(field, true)(that);
                }
                return result;
            },
            _set: function (field, value) {
                ...
                if (composite) {
                    ...
                    while (paths.length > 1) {
                        path += paths.shift();
                        var obj = ufa.getter(path, true)(that);
                ...
                ufa.setter(field)(that, value);
                return composite;
            },
            set: function (field, value) {
                var that = this, composite = field.indexOf('.') >= 0, current = ufa.getter(field, true)(that);
                if (current !== value) {
        ...
        var parsers = {
            'number': function (value) {
                return ufa.parseFloat(value);
            },
            'date': function (value) {
                return ufa.parseDate(value);
            },
        ...
        Query.prototype = {
            ...
            groupBy: function (descriptor) {
                ...
                var field = descriptor.field, sorted = this._sortForGrouping(field, descriptor.dir || 'asc'), accessor = ufa.accessor(field), item, groupValue = accessor.get(sorted[0], field), group = {
                        field: field,
                        value: groupValue,
                        items: []
        ...
        function calculateAggregate(accumulator, aggregates, item, index, length, state) {
            ...
            for (idx = 0; idx < len; idx++) {
                ...
                accumulator[field][functionName] = functions[functionName.toLowerCase()](accumulator[field][functionName], item, ufa.accessor(field), index, length, state[field][functionName]);
            }
        }
        var functions = {
                ...
                for (getter in getters) {
                    ...
                    if (originalName && originalName !== getter) {
                        if (!setters[originalName]) {
                            setters[originalName] = ufa.setter(originalName);
                        }
        ...
        var DataSource = Observable.extend({
            init: function (options) {
                ...
                that.reader = new ufa.data.readers[options.schema.type || 'json'](options.schema);
            ...
            cancelChanges: function (model) {
                var that = this;
                if (model instanceof ufa.data.Model) {
            ...
            _updatePristineForModel: function (model, values) {
                this._executeOnPristineForModel(model, function (index, items) {
                    ufa.deepExtend(items[index], values);
                });
            },
        ...
        Transport.create = function (options, data, dataSource) {
            ...
            if (transportOptions) {
                ...
                if (options.type) {
                    ufa.data.transports = ufa.data.transports || {};
                    ufa.data.schemas = ufa.data.schemas || {};
                    if (ufa.data.transports[options.type] && !isPlainObject(ufa.data.transports[options.type])) {
                        transport = new ufa.data.transports[options.type](extend(transportOptions, { data: data }));
                    } else {
                        transportOptions = extend(true, {}, ufa.data.transports[options.type], transportOptions);
                    }
                    options.schema = extend(true, {}, ufa.data.schemas[options.type], options.schema);
                }
                if (!transport) {
        ...
        DataSource.create = function (options) {
            ...
            if (ufa.data.Model && fields && (!dataSource.schema || !dataSource.schema.model)) {
        ...
        var Node = Model.define({
            ...
            init: function (value) {
                ...
                ufa.data.Model.fn.init.call(that, value);
                ...
                if (typeof hasChildren === STRING) {
                    hasChildren = ufa.getter(hasChildren);
                }
        ...
        var Buffer = ufa.Observable.extend({
            init: function (dataSource, viewSize, disablePrefetch) {
                ufa.Observable.fn.init.call(this);
        ...
        var BatchBuffer = ufa.Observable.extend({
            init: function (dataSource, batchSize) {
                var batchBuffer = this;
                ufa.Observable.fn.init.call(batchBuffer);
                this.dataSource = dataSource;
...
(function (f, define) {
    define('ufa.data.signalr', ['ufa.data'], f);
}(function () {
    var __meta__ = {
        id: 'data.signalr',
        name: 'SignalR',
    ...
    (function ($) {
        var transport = ufa.data.RemoteTransport.extend({
            init: function (options) {
                ...
                this.hub = hub;
                ufa.data.RemoteTransport.fn.init.call(this, options);
            },
            ...
            _crud: function (options, type) {
                ...
                if (!server || !server[type]) {
                    throw new Error(ufa.format('The "server.{0}" option must be set.', type));
                }
        ...
        $.extend(true, ufa.data, { transports: { signalr: transport } });
    }(window.ufa.jQuery));
    return window.ufa;
}, typeof define == 'function' && define.amd ? define : function (a1, a2, a3) {
    (a3 || a2)();
...
(function (f, define) {
    ...
    (function ($, undefined) {
        var ufa = window.ufa, Observable = ufa.Observable, ObservableObject = ufa.data.ObservableObject, ObservableArray = ufa.data.ObservableArray, toString = {}.toString, binders = {}, Class = ufa.Class, proxy = $.proxy, VALUE = 'value', SOURCE = 'source', EVENTS = 'events', CHECKED = 'checked', CSS = 'css', deleteExpando = true, FUNCTION = 'function', CHANGE = 'change';
        (function () {
        ...
        var Binding = Observable.extend({
            ...
            set: function (value) {
                ...
                var field = ufa.getter(this.path)(source);
                if (typeof field === 'function') {
        ...
        var TemplateBinding = Binding.extend({
            ...
            render: function (value) {
                ...
                html = ufa.render(this.template, value);
        ...
        var TypedBinder = Binder.extend({
            ...
            _parseValue: function (value, dataType) {
                if (dataType == 'date') {
                    value = ufa.parseDate(value, 'yyyy-MM-dd');
                } else if (dataType == 'datetime-local') {
                    value = ufa.parseDate(value, [
                        'yyyy-MM-ddTHH:mm:ss',
                        'yyyy-MM-ddTHH:mm'
                    ]);
                } else if (dataType == 'number') {
                    value = ufa.parseFloat(value);
                } else if (dataType == 'boolean') {
                    value = value.toLowerCase();
                    if (ufa.parseFloat(value) !== null) {
                        value = Boolean(ufa.parseFloat(value));
                    } else {
        ...
        binders.text = Binder.extend({
            refresh: function () {
                ...
                $(this.element).text(ufa.toString(text, dataFormat));
            }
        });
        ...
        binders.value = TypedBinder.extend({
            ...
            refresh: function () {
                if (!this._initChange) {
                    ...
                    if (type == 'date') {
                        value = ufa.toString(value, 'yyyy-MM-dd');
                    } else if (type == 'datetime-local') {
                        value = ufa.toString(value, 'yyyy-MM-ddTHH:mm:ss');
                    }
        ...
        binders.source = Binder.extend({
            init: function (element, bindings, options) {
                ...
                if (source instanceof ufa.data.DataSource && options.autoBind !== false) {
                    source.fetch();
                }
            },
            refresh: function (e) {
                var that = this, source = that.bindings.source.get();
                if (source instanceof ObservableArray || source instanceof ufa.data.DataSource) {
                    e = e || {};
                    if (e.action == 'add') {
                ...
                if (!template) {
                    if (nodeName == 'select') {
                        if (options.valueField || options.textField) {
                            template = ufa.format('<option value="#:{0}#">#:{1}#</option>', options.valueField || options.textField, options.textField || options.valueField);
                        } else {
            ...
            render: function () {
                ...
                if (source instanceof ufa.data.DataSource) {
                    source = source.view();
                }
                ...
                if (this.bindings.template) {
                    ...
                    $(element).html(ufa.render(template, source));
                }
            }
        });
        binders.input = {
            ...
            value: TypedBinder.extend({
                ...
                refresh: function () {
                    ...
                    for (var valueIndex = 0; valueIndex < values.length; valueIndex++) {
                        ...
                        if (type == 'date') {
                            value = ufa.toString(values[valueIndex], 'yyyy-MM-dd');
                        } else if (type == 'datetime-local') {
                            value = ufa.toString(values[valueIndex], 'yyyy-MM-ddTHH:mm:ss');
                        }
        ...
        function dataSourceBinding(bindingName, fieldName, setter) {
            return Binder.extend({
                ...
                _ns: function (ns) {
                    ...
                    all.unshift(ns);
                    return ufa.rolesFromNamespaces(all);
                },
                dataBound: function (e) {
                    var idx, length, widget = this.widget, items = e.addedItems || widget.items(), dataSource = widget[fieldName], view, parents, hds = ufa.data.HierarchicalDataSource;
                ...
                refresh: function (e) {
                    ...
                    if (!e.action) {
                        ...
                        if (widget[fieldName] instanceof ufa.data.DataSource && widget[fieldName] != source) {
                            if (source instanceof ufa.data.DataSource) {
                                ...
                                select = ufa.ui.Select && widget instanceof ufa.ui.Select;
                                multiselect = ufa.ui.MultiSelect && widget instanceof ufa.ui.MultiSelect;
        ...
        binders.widget = {
            ...
            value: Binder.extend({
                ...
                _source: function () {
                    ...
                    if (!source || source instanceof ufa.data.DataSource) {
        ...
        function bindingTargetForRole(element, roles) {
            var widget = ufa.initWidget(element, {}, roles);
            if (widget) {
        ...
        function bindElement(element, source, roles, parents) {
            var role = element.getAttribute('data-' + ufa.ns + 'role'), idx, bind = element.getAttribute('data-' + ufa.ns + 'bind'), children = element.children, childrenCopy = [], deep = true, bindings, options = {}, target;
            ...
            if (bind) {
                ...
                if (!target) {
                    options = ufa.parseOptions(element, {
                        textField: '',
                        valueField: '',
                        template: '',
                        valueUpdate: CHANGE,
            ...
            if (target) {
                element.ufaBindingTarget = target;
            }
            if (deep && children) {
        ...
        function bind(dom, object) {
            var idx, length, node, roles = ufa.rolesFromNamespaces([].slice.call(arguments, 2));
            object = ufa.observable(object);
            dom = $(dom);
        ...
        function unbindElement(element, destroyWidget) {
            var bindingTarget = element.ufaBindingTarget;
            if (bindingTarget) {
                bindingTarget.destroy();
                if (deleteExpando) {
                    delete element.ufaBindingTarget;
                } else if (element.removeAttribute) {
                    element.removeAttribute('ufaBindingTarget');
                } else {
                    element.ufaBindingTarget = null;
                }
            }
            if (destroyWidget) {
                var widget = ufa.widgetInstance($(element));
        ...
        function notify(widget, namespace) {
            var element = widget.element, bindingTarget = element[0].ufaBindingTarget;
            if (bindingTarget) {
        ...
        ufa.unbind = unbind;
        ufa.bind = bind;
        ufa.data.binders = binders;
        ufa.data.Binder = Binder;
        ufa.notify = notify;
        ufa.observable = function (object) {
            if (!(object instanceof ObservableObject)) {
                object = new ObservableObject(object);
            }
            return object;
        };
        ufa.observableHierarchy = function (array) {
            var dataSource = ufa.data.HierarchicalDataSource.create(array);
...
(function (f, define) {
    define('ufa.validator', ['ufa.core'], f);
}(function () {
    var __meta__ = {
        id: 'validator',
        name: 'Validator',
    ...
    (function ($, undefined) {
        ...
        function resolveRules(element) {
            var resolvers = ufa.ui.validator.ruleResolvers || {}, rules = {}, name;
        ...
        function searchForMessageContainer(elements, fieldName) {
            ...
            for (var idx = 0, length = elements.length; idx < length; idx++) {
                ...
                if (invalidMsgRegExp.test(element.className)) {
                    attr = element.getAttribute(ufa.attr('for'));
                    if (attr === fieldName) {
        ...
        var Validator = Widget.extend({
            init: function (element, options) {
                var that = this, resolved = resolveRules(element), validateAttributeSelector = '[' + ufa.attr('validate') + '!=false]';
                options = options || {};
                options.rules = $.extend({}, ufa.ui.validator.rules, resolved.rules, options.rules);
                options.messages = $.extend({}, ufa.ui.validator.messages, resolved.messages, options.messages);
                Widget.fn.init.call(that, element, options);
                that._errorTemplate = ufa.template(that.options.errorTemplate);
            ...
            options: {
                ...
                rules: {
                    ...
                    min: function (input) {
                        if (input.filter(NUMBERINPUTSELECTOR + ',[' + ufa.attr('type') + '=number]').filter('[min]').length && input.val() !== '') {
                            var min = parseFloat(input.attr('min')) || 0, val = ufa.parseFloat(input.val());
                            return min <= val;
                        }
                        return true;
                    },
                    max: function (input) {
                        if (input.filter(NUMBERINPUTSELECTOR + ',[' + ufa.attr('type') + '=number]').filter('[max]').length && input.val() !== '') {
                            var max = parseFloat(input.attr('max')) || 0, val = ufa.parseFloat(input.val());
                            return max >= val;
                        }
                        return true;
                    },
                    step: function (input) {
                        if (input.filter(NUMBERINPUTSELECTOR + ',[' + ufa.attr('type') + '=number]').filter('[step]').length && input.val() !== '') {
                    ...
                    email: function (input) {
                        return matcher(input, '[type=email],[' + ufa.attr('type') + '=email]', emailRegExp);
                    },
                    url: function (input) {
                        return matcher(input, '[type=url],[' + ufa.attr('type') + '=url]', urlRegExp);
                    },
                    date: function (input) {
                        if (input.filter('[type^=date],[' + ufa.attr('type') + '=date]').length && input.val() !== '') {
                            return ufa.parseDate(input.val(), input.attr(ufa.attr('format'))) !== null;
                        }
                        return true;
                    }
                },
            ...
            validateInput: function (input) {
                ...
                var that = this, template = that._errorTemplate, result = that._checkValidity(input), valid = result.valid, className = '.' + INVALIDMSG, fieldName = input.attr(NAME) || '', lbl = that._findMessageContainer(fieldName).add(input.next(className).filter(function () {
                        var element = $(this);
                        if (element.filter('[' + ufa.attr('for') + ']').length) {
                            return element.attr(ufa.attr('for')) === fieldName;
                        }
                        return true;
                    })).hide(), messageText;
            ...
            _findMessageContainer: function (fieldName) {
                var locators = ufa.ui.validator.messageLocators, name, containers = $();
            ...
            _decorateMessageContainer: function (container, fieldName) {
                var locators = ufa.ui.validator.messageLocators, name;
                container.addClass(INVALIDMSG).attr(ufa.attr('for'), fieldName || '');
                for (name in locators) {
            ...
            _extractMessage: function (input, ruleKey) {
                ...
                customMessage = ufa.isFunction(customMessage) ? customMessage(input) : customMessage;
                return ufa.format(input.attr(ufa.attr(ruleKey + '-msg')) || input.attr('validationMessage') || input.attr('title') || customMessage || '', fieldName, input.attr(ruleKey));
            },
...
(function (f, define) {
    ...
    (function ($, undefined) {
        var ufa = window.ufa, CHANGE = 'change', BACK = 'back', SAME = 'same', support = ufa.support, location = window.location, history = window.history, CHECK_URL_INTERVAL = 50, BROKEN_BACK_NAV = ufa.support.browser.msie, hashStrip = /^#*/, document = window.document;
        ...
        var HistoryAdapter = ufa.Class.extend({
            back: function () {
                if (BROKEN_BACK_NAV) {
                    setTimeout(function () {
                        history.back();
        ...
        var PushStateAdapter = HistoryAdapter.extend({
            ...
            change: function (callback) {
                $(window).bind('popstate.ufa', callback);
            },
            stop: function () {
                $(window).unbind('popstate.ufa');
            },
        ...
        var History = ufa.Observable.extend({
        ...
        ufa.History = History;
        ufa.History.HistoryAdapter = HistoryAdapter;
        ufa.History.HashAdapter = HashAdapter;
        ufa.History.PushStateAdapter = PushStateAdapter;
        ufa.absoluteURL = absoluteURL;
        ufa.history = new History();
    }(window.ufa.jQuery));
    (function () {
        var ufa = window.ufa, history = ufa.history, Observable = ufa.Observable, INIT = 'init', ROUTE_MISSING = 'routeMissing', CHANGE = 'change', BACK = 'back', SAME = 'same', optionalParam = /\((.*?)\)/g, namedParam = /(\(\?)?:\w+/g, splatParam = /\*\w+/g, escapeRegExp = /[\-{}\[\]+?.,\\\^$|#\s]/g;
        ...
        var Route = ufa.Class.extend({
            ...
            callback: function (url, back) {
                var params, idx = 0, length, queryStringParams = ufa.parseQueryStringParams(url);
        ...
        var Router = Observable.extend({
            ...
            navigate: function (url, silent) {
                ufa.history.navigate(url, silent);
            },
            replace: function (url, silent) {
                ufa.history.replace(url, silent);
            },
            _back: function (e) {
            ...
            _urlChanged: function (e) {
                ...
                if (this.trigger(CHANGE, {
                        url: e.url,
                        params: ufa.parseQueryStringParams(e.url),
                ...
                if (this.trigger(ROUTE_MISSING, {
                        url: url,
                        params: ufa.parseQueryStringParams(url),
                        backButtonPressed: back
                    })) {
                    e.preventDefault();
                }
...
(function (f, define) {
    ...
    (function ($, undefined) {
        var ufa = window.ufa, Observable = ufa.Observable, SCRIPT = 'SCRIPT', INIT = 'init', SHOW = 'show', HIDE = 'hide', TRANSITION_START = 'transitionStart', TRANSITION_END = 'transitionEnd', ATTACH = 'attach', DETACH = 'detach', sizzleErrorRegExp = /unrecognized expression/;
        var View = Observable.extend({
            ...
            render: function (container) {
                ...
                if (notInitialized) {
                    ufa.bind(that.element, that.model);
                    that.trigger(INIT);
                }
                if (container) {
            ...
            _createElement: function () {
                ...
                if (typeof content === 'string') {
                    ...
                    if (that._evalTemplate) {
                        content = ufa.template(content)(that.model || {});
                    }
                    ...
                    element = content;
                    if (that._evalTemplate) {
                        var result = $(ufa.template($('<div />').append(element.clone(true)).html())(that.model || {}));
        ...
        var ViewClone = ufa.Class.extend({
            init: function (view) {
                $.extend(this, {
        ...
        var ViewContainer = Observable.extend({
            ...
            show: function (view, transition, locationID) {
                ...
                if (!theTransition || !ufa.effects.enabled) {
                    view.showStart();
                    that.end();
                } else {
                    ...
                    that.effect = ufa.fx(view.element).replace(current.element, transitionData.type).beforeTransition(function () {
        ...
        ufa.ViewContainer = ViewContainer;
        ufa.Fragment = Fragment;
        ufa.Layout = Layout;
        ufa.View = View;
        ufa.ViewClone = ViewClone;
    }(window.ufa.jQuery));
    return window.ufa;
}, typeof define == 'function' && define.amd ? define : function (a1, a2, a3) {
    (a3 || a2)();
...
(function (f, define) {
    define('ufa.userevents', ['ufa.core'], f);
}(function () {
    var __meta__ = {
        id: 'userevents',
        name: 'User Events',
    ...
    (function ($, undefined) {
        var ufa = window.ufa, support = ufa.support, document = window.document, Class = ufa.Class, Observable = ufa.Observable, now = $.now, extend = $.extend, OS = support.mobileOS, invalidZeroEvents = OS && OS.android, DEFAULT_MIN_HOLD = 800, DEFAULT_THRESHOLD = support.browser.msie ? 5 : 0, PRESS = 'press', HOLD = 'hold', SELECT = 'select', START = 'start', MOVE = 'move', END = 'end', CANCEL = 'cancel', TAP = 'tap', RELEASE = 'release', GESTURESTART = 'gesturestart', GESTURECHANGE = 'gesturechange', GESTUREEND = 'gestureend', GESTURETAP = 'gesturetap';
        var THRESHOLD = {
            'api': 0,
            'touch': 0,
            'mouse': 9,
        ...
        function withEachUpEvent(callback) {
            var downEvents = ufa.eventMap.up.split(' '), idx = 0, length = downEvents.length;
            for (; idx < length; idx++) {
        ...
        var UserEvents = Observable.extend({
            init: function (element, options) {
                var that = this, filter, ns = ufa.guid();
                options = options || {};
                ...
                that.surface.handler(that).on(ufa.applyEventMap('move', ns), '_move').on(ufa.applyEventMap('up cancel', ns), '_end');
                element.on(ufa.applyEventMap('down', ns), filter, '_start');
                if (that.useClickAsTap) {
                    element.on(ufa.applyEventMap('click', ns), filter, '_click');
                }
                ...
                if (options.preventDragEvent) {
                    element.on(ufa.applyEventMap('dragstart', ns), ufa.preventDefault);
                }
                element.on(ufa.applyEventMap('mousedown', ns), filter, { root: element }, '_select');
            ...
            destroy: function () {
                ...
                that.element.ufaDestroy(that.eventNS);
                that.surface.ufaDestroy(that.eventNS);
                that.element.removeData('handler');
        ...
        ufa.getTouches = getTouches;
        ufa.touchDelta = touchDelta;
        ufa.UserEvents = UserEvents;
    }(window.ufa.jQuery));
    return window.ufa;
}, typeof define == 'function' && define.amd ? define : function (a1, a2, a3) {
    (a3 || a2)();
...
(function (f, define) {
    ...
    (function ($, undefined) {
        var ufa = window.ufa, support = ufa.support, document = window.document, $window = $(window), Class = ufa.Class, Widget = ufa.ui.Widget, Observable = ufa.Observable, UserEvents = ufa.UserEvents, proxy = $.proxy, extend = $.extend, getOffset = ufa.getOffset, draggables = {}, dropTargets = {}, dropAreas = {}, lastDropTarget, elementUnderCursor = ufa.elementUnderCursor, KEYUP = 'keyup', CHANGE = 'change', DRAGSTART = 'dragstart', HOLD = 'hold', DRAG = 'drag', DRAGEND = 'dragend', DRAGCANCEL = 'dragcancel', HINTDESTROYED = 'hintDestroyed', DRAGENTER = 'dragenter', DRAGLEAVE = 'dragleave', DROP = 'drop';
        ...
        var TapCapture = Observable.extend({
            init: function (element, options) {
                ...
                if (domElement.addEventListener) {
                    $.each(ufa.eventMap.down.split(' '), function () {
                        domElement.addEventListener(this, proxy(that._press, that), true);
                    });
                    $.each(ufa.eventMap.up.split(' '), function () {
        ...
        var Movable = Observable.extend({
            ...
            refresh: function () {
                ...
                if (newCoordinates != that.coordinates) {
                    if (ufa.support.browser.msie && ufa.support.browser.version < 10) {
        ...
        var Draggable = Widget.extend({
            ...
            options: {
                name: 'Draggable',
                distance: ufa.support.touch ? 0 : 5,
                group: 'default',
                cursorOffset: null,
                axis: null,
                container: null,
            ...
            _start: function (e) {
                ...
                if (hint) {
                    ...
                    that.hint = ufa.isFunction(hint) ? $(hint.call(that, that.currentTarget)) : hint;
                    ...
                    that.angular('compile', function () {
                        ...
                        while (!scopeTarget.data('$$ufaScope') && scopeTarget.length) {
                            scopeTarget = scopeTarget.parent();
                        }
                        return {
                            elements: that.hint.get(),
                            scopeFrom: scopeTarget.data('$$ufaScope')
                        };
                    });
                }
        ...
        ufa.ui.plugin(DropTarget);
        ufa.ui.plugin(DropTargetArea);
        ufa.ui.plugin(Draggable);
        ufa.TapCapture = TapCapture;
        ufa.containerBoundaries = containerBoundaries;
        extend(ufa.ui, {
            Pane: Pane,
        ...
        function scrollableRoot() {
            return $(ufa.support.browser.chrome ? document.body : document.documentElement);
        }
        function findScrollableParent(element) {
            ...
            while (parent && !ufa.isScrollable(parent) && parent !== document.body) {
    ...
    (function ($, undefined) {
        ...
        var Popup = Widget.extend({
            init: function (element, options) {
                ...
                that.downEvent = ufa.applyEventMap(MOUSEDOWN, ufa.guid());
            ...
            destroy: function () {
                ...
                ufa.destroy(that.element.children());
                element.removeData();
            ...
            open: function (x, y) {
                ...
                if (!that.visible()) {
                    if (options.copyAnchorStyles) {
                        ...
                        element.css(ufa.getComputedStyles(anchor[0], styles));
                    }
                    ...
                    that.wrapper = wrapper = ufa.wrap(element, options.autosize).css({
                        overflow: HIDDEN,
                        display: 'block',
                        position: ABSOLUTE
                    });
                    ...
                    element.data(EFFECTS, animation.effects).ufaStop(true).ufaAnimate(animation);
                }
            },
            _openAnimation: function () {
                var animation = extend(true, {}, this.options.animation.open);
                animation.effects = ufa.parseEffects(animation.effects, this.flipped);
                return animation;
            },
            _hideDirClass: function () {
                ...
                this.element.removeClass(ACTIVEBORDER + '-' + ufa.directions[direction].reverse);
            },
            _showDirClass: function (animation) {
                var direction = animation.effects.slideIn ? animation.effects.slideIn.direction : 'down';
                var dirClass = ACTIVEBORDER + '-' + direction;
                $(this.options.anchor).addClass(dirClass).children(ACTIVECHILDREN).addClass(ACTIVE).addClass(dirClass);
                this.element.addClass(ACTIVEBORDER + '-' + ufa.directions[direction].reverse);
            },
            position: function () {
                if (this.visible()) {
            ...
            close: function (skipEffects) {
                ...
                if (that.visible()) {
                    wrap = that.wrapper[0] ? that.wrapper : ufa.wrap(that.element).hide();
                    that._toggleResize(false);
                    ...
                    that.element.find('.k-popup').each(function () {
                        var that = $(this), popup = that.data('ufaPopup');
                        if (popup) {
                    ...
                    if (skipEffects) {
                        ...
                        if (!closeEffects && !ufa.size(closeEffects) && openEffects && ufa.size(openEffects)) {
                            animation.effects = openEffects;
                            animation.reverse = true;
                        }
                        that._closing = true;
                    }
                    that.element.ufaStop(true);
                    wrap.css({ overflow: HIDDEN });
                    that.element.ufaAnimate(animation);
                }
            },
            ...
            _mousedown: function (e) {
                var that = this, container = that.element[0], options = that.options, anchor = $(options.anchor)[0], toggleTarget = options.toggleTarget, target = ufa.eventTarget(e), popup = $(target).closest('.k-popup'), mobile = popup.parent().parent('.km-shim').length;
                popup = popup[0];
            ...
            _scrollableParents: function () {
                return $(this.options.anchor).parentsUntil('body').filter(function (index, element) {
                    return ufa.isScrollable(element);
                });
            },
            _position: function (fixed) {
                ...
                if (isWindow && docEl.scrollHeight - docEl.clientHeight > 0) {
                    viewportWidth -= ufa.support.scrollbar();
                }
...
(function (f, define) {
    ...
    var __meta__ = {
        ...
        name: 'Touch',
        category: 'mobile',
        description: 'The ufa Touch widget provides a cross-platform compatible API for handling user-initiated touch events, multi-touch gestures and event sequences (drag, swipe, etc.). ',
        depends: [
            'core',
            'userevents'
        ]
    ...
    (function ($, undefined) {
        var ufa = window.ufa, Widget = ufa.ui.Widget, proxy = $.proxy, abs = Math.abs, MAX_DOUBLE_TAP_DISTANCE = 20;
        var Touch = Widget.extend({
            init: function (element, options) {
                ...
                that.events = new ufa.UserEvents(element, {
            ...
            _tap: function (e) {
                ...
                if (lastTap && touch.endTime - lastTap.endTime < that.options.doubleTapTimeout && ufa.touchDelta(touch, lastTap).distance < MAX_DOUBLE_TAP_DISTANCE) {
...
(function (f, define) {
    ...
    var __meta__ = {
        ...
        name: 'Scroller',
        category: 'mobile',
        description: 'The ufa Mobile Scroller widget enables touch friendly kinetic scrolling for the contents of a given DOM element.',
        depends: [
            'fx',
            'draganddrop'
        ]
    ...
    (function ($, undefined) {
        var ufa = window.ufa, mobile = ufa.mobile, fx = ufa.effects, ui = mobile.ui, proxy = $.proxy, extend = $.extend, Widget = ui.Widget, Class = ufa.Class, Movable = ufa.ui.Movable, Pane = ufa.ui.Pane, PaneDimensions = ufa.ui.PaneDimensions, Transition = fx.Transition, Animation = fx.Animation, abs = Math.abs, SNAPBACK_DURATION = 500, SCROLLBAR_OPACITY = 0.7, FRICTION = 0.96, VELOCITY_MULTIPLIER = 10, MAX_VELOCITY = 55, OUT_OF_BOUNDS_FRICTION = 0.5, ANIMATED_SCROLLER_PRECISION = 5, RELEASECLASS = 'km-scroller-release', REFRESHCLASS = 'km-scroller-refresh', PULL = 'pull', CHANGE = 'change', RESIZE = 'resize', SCROLL = 'scroll', MOUSE_WHEEL_ID = 2;
        ...
        var AnimatedScroller = Animation.extend({
            init: function (options) {
                var that = this;
                ufa.effects.Animation.fn.init.call(this);
                extend(that, options, {
                    origin: {},
                    destination: {},
                    offset: {}
            ...
            setCallback: function (callback) {
                if (callback && ufa.isFunction(callback)) {
        ...
        var Scroller = Widget.extend({
            init: function (element, options) {
                ...
                Widget.fn.init.call(that, element, options);
                element = that.element;
                that._native = that.options.useNative && ufa.support.hasNativeScrolling;
                if (that._native) {
                ...
                var inner = element.children().eq(1), tapCapture = new ufa.TapCapture(element), movable = new Movable(inner), dimensions = new PaneDimensions({
                        element: inner,
                        container: element,
                        forcedEnabled: that.options.zoom
                    }), avoidScrolling = this.options.avoidScrolling, userEvents = new ufa.UserEvents(element, {
                        fastTap: true,
                        allowSelection: true,
            ...
            _initPullToRefresh: function () {
                ...
                that.pullTemplate = ufa.template(that.options.messages.pullTemplate);
                that.releaseTemplate = ufa.template(that.options.messages.releaseTemplate);
                that.refreshTemplate = ufa.template(that.options.messages.refreshTemplate);
...
(function (f, define) {
    ...
    (function ($, undefined) {
        var ufa = window.ufa, mobile = ufa.mobile, ui = mobile.ui, attr = ufa.attr, Widget = ui.Widget, ViewClone = ufa.ViewClone, INIT = 'init', UI_OVERLAY = '<div style="height: 100%; width: 100%; position: absolute; top: 0; left: 0; z-index: 20000; display: none" />', BEFORE_SHOW = 'beforeShow', SHOW = 'show', AFTER_SHOW = 'afterShow', BEFORE_HIDE = 'beforeHide', TRANSITION_END = 'transitionEnd', TRANSITION_START = 'transitionStart', HIDE = 'hide', DESTROY = 'destroy', attrValue = ufa.attrValue, roleSelector = ufa.roleSelector, directiveSelector = ufa.directiveSelector, compileMobileDirective = ufa.compileMobileDirective;
        function initPopOvers(element) {
            var popovers = element.find(roleSelector('popover')), idx, length, roles = ui.roles;
            for (idx = 0, length = popovers.length; idx < length; idx++) {
                ufa.initWidget(popovers[idx], {}, roles);
            }
        }
        function preventScrollIfNotInput(e) {
            if (!ufa.triggeredByInput(e)) {
                e.preventDefault();
            }
        }
        var View = Widget.extend({
            ...
            _padIfNativeScrolling: function () {
                if (mobile.appLevelNativeScrolling()) {
                    var isAndroid = ufa.support.mobileOS && ufa.support.mobileOS.android, skin = mobile.application.skin() || '', isAndroidForced = mobile.application.os.android || skin.indexOf('android') > -1, hasPlatformIndependentSkin = skin === 'flat' || skin.indexOf('material') > -1, topContainer = (isAndroid || isAndroidForced) && !hasPlatformIndependentSkin ? 'footer' : 'header', bottomContainer = (isAndroid || isAndroidForced) && !hasPlatformIndependentSkin ? 'header' : 'footer';
                    this.content.css({
            ...
            _scroller: function () {
                ...
                if (that.options.stretch) {
                    ...
                    that.content.ufaMobileScroller($.extend(that.options.scroller, {
                        zoom: that.options.zoom,
                        useNative: that.options.useNativeScrolling
                    }));
                    that.scroller = that.content.data('ufaMobileScroller');
                    that.scrollerContent = that.scroller.scrollElement;
                }
                if (ufa.support.kineticScrollNeeded) {
            ...
            _model: function () {
                ...
                if (typeof model === 'string') {
                    model = ufa.getter(model)(that.options.modelScope);
                }
                that.model = model;
                ...
                if (that.options.initWidgets) {
                    if (model) {
                        ufa.bind(element, model, ui, ufa.ui, ufa.dataviz.ui);
                    } else {
        ...
        var Layout = Widget.extend({
            init: function (element, options) {
                ...
                initPopOvers(element);
                if (!this.options.$angular) {
                    ufa.mobile.init(this.element.children());
                }
                this.element.detach();
        ...
        var ViewEngine = Observable.extend({
            init: function (options) {
                ...
                if (!that.rootView[0] && options.rootNeeded) {
                    if (container[0] == ufa.mobile.application.element[0]) {
                        errorMessage = 'Your ufa mobile application element does not contain any direct child elements with data-role="view" attribute set. Make sure that you instantiate the mobile application using the correct container.';
                    } else {
                ...
                that.layouts = {};
                that.viewContainer = new ufa.ViewContainer(that.container);
            ...
            showView: function (url, transition, params) {
                ...
                var that = this, showClosure = function (view) {
                    ...
                    }, element = that._findViewElement(url), view = ufa.widgetInstance(element);
            ...
            _loadView: function (url, callback) {
                ...
                this._xhr = $.get(ufa.absoluteURL(url, this.remoteViewURLPrefix), 'html').always($.proxy(this, '_xhrComplete', callback, url));
            },
            ...
            _setupLayouts: function (element) {
                ...
                element.children(that._locate('layout')).each(function () {
                    if (that.$angular) {
                        ...
                        layout = ufa.initWidget($(this), {}, ui.roles);
                    }
        ...
        ufa.mobile.ViewEngine = ViewEngine;
        ui.plugin(View);
        ui.plugin(Layout);
    }(window.ufa.jQuery));
    return window.ufa;
...
(function (f, define) {
    define('ufa.mobile.loader', ['ufa.core'], f);
}(function () {
    var __meta__ = {
        id: 'mobile.loader',
        name: 'Loader',
    ...
    (function ($, undefined) {
        var ufa = window.ufa, ui = ufa.mobile.ui, Widget = ui.Widget, CAPTURE_EVENTS = $.map(ufa.eventMap, function (value) {
                return value;
            }).join(' ').split(' ');
...
(function (f, define) {
    ...
    (function ($, undefined) {
        var ufa = window.ufa, mobile = ufa.mobile, roleSelector = ufa.roleSelector, ui = mobile.ui, Widget = ui.Widget, ViewEngine = mobile.ViewEngine, View = ui.View, Loader = mobile.ui.Loader, EXTERNAL = 'external', HREF = 'href', DUMMY_HREF = '#!', NAVIGATE = 'navigate', VIEW_SHOW = 'viewShow', SAME_VIEW_REQUESTED = 'sameViewRequested', OS = ufa.support.mobileOS, SKIP_TRANSITION_ON_BACK_BUTTON = OS.ios && !OS.appMode && OS.flatVersion >= 700, WIDGET_RELS = /popover|actionsheet|modalview|drawer/, BACK = '#:back', attrValue = ufa.attrValue;
        var Pane = Widget.extend({
            init: function (element, options) {
                ...
                this._historyNavigate = function (url) {
                    ...
                    that.historyCallback(url, ufa.parseQueryStringParams(url));
                };
                this._historyReplace = function (url) {
                    var params = ufa.parseQueryStringParams(url);
            ...
            closeActiveDialogs: function () {
                ...
                dialogs.each(function () {
                    ufa.widgetInstance($(this), ui).close();
                });
            },
            ...
            bindToRouter: function (router) {
                ...
                router.bind('init', function (e) {
                    ...
                    viewEngine.rootView.attr(ufa.attr('url'), attrUrl);
            ...
            _setPortraitWidth: function () {
                ...
                if (portraitWidth) {
                    width = ufa.mobile.application.element.is('.km-vertical') ? portraitWidth : 'auto';
                    this.element.css('width', width);
                }
            },
            _setupAppLinks: function () {
                var that = this, linkRoles = 'tab', pressedButtonSelector = '[data-' + ufa.ns + 'navigate-on-press]', buttonSelectors = $.map([
                        'button',
                        'backbutton',
                        'detailbutton',
                        'listview-link'
                ...
                this.userEvents = new ufa.UserEvents(this.element, {
                    fastTap: true,
            ...
            _mouseup: function (e) {
                ...
                if (rel.match(WIDGET_RELS)) {
                    ufa.widgetInstance($(href), ui).openFor(link);
                    ...
                    if (target === '_top') {
                        ...
                        pane = $('#' + target).data('ufaMobilePane');
                    }
        ...
        Pane.wrap = function (element) {
            if (!element.is(roleSelector('view'))) {
                element = element.wrap('<div data-' + ufa.ns + 'role="view" data-stretch="true"></div>').parent();
            }
...
(function (f, define) {
    ...
    (function ($, undefined) {
        var ufa = window.ufa, mobile = ufa.mobile, ui = mobile.ui, HIDE = 'hide', OPEN = 'open', CLOSE = 'close', WRAPPER = '<div class="km-popup-wrapper" />', ARROW = '<div class="km-popup-arrow" />', OVERLAY = '<div class="km-popup-overlay" />', DIRECTION_CLASSES = 'km-up km-down km-left km-right', Widget = ui.Widget, DIRECTIONS = {
                'down': {
        ...
        var Popup = Widget.extend({
            init: function (element, options) {
                ...
                that.popup = new ufa.ui.Popup(that.wrapper, $.extend(true, popupOptions, ANIMATION, DIRECTIONS[options.direction]));
            },
            options: {
                name: 'Popup',
                width: 240,
...
(function (f, define) {
    ...
    var __meta__ = {
        ...
        name: 'ModalView',
        category: 'mobile',
        description: 'The ufa ModalView is used to present self-contained functionality in the context of the current task.',
        depends: [
            'mobile.shim',
            'mobile.view'
        ]
    ...
    (function ($, undefined) {
        var ufa = window.ufa, ui = ufa.mobile.ui, Shim = ui.Shim, Widget = ui.Widget, BEFORE_OPEN = 'beforeOpen', OPEN = 'open', CLOSE = 'close', INIT = 'init', WRAP = '<div class="km-modalview-wrapper" />';
...
(function (f, define) {
    ...
    var __meta__ = {
        ...
        name: 'Drawer',
        category: 'mobile',
        description: 'The ufa Mobile Drawer widget provides slide to reveal global application toolbox',
        depends: [
            'mobile.view',
            'userevents'
        ]
    ...
    (function ($, undefined) {
        var ufa = window.ufa, mobile = ufa.mobile, os = ufa.support.mobileOS, Transition = ufa.effects.Transition, roleSelector = ufa.roleSelector, AXIS = 'x', ui = mobile.ui, SWIPE_TO_OPEN = !(os.ios && os.majorVersion == 7 && !os.appMode), BEFORE_SHOW = 'beforeShow', INIT = 'init', SHOW = 'show', HIDE = 'hide', AFTER_HIDE = 'afterHide', NULL_VIEW = { enable: $.noop };
        var Drawer = ui.View.extend({
            init: function (element, options) {
                ...
                this._model();
                var pane = this.element.closest(roleSelector('pane')).data('ufaMobilePane'), userEvents;
                if (pane) {
                    this.pane = pane;
                    ...
                    userEvents = this.userEvents = new ufa.UserEvents(pane.element, {
                        fastTap: true,
                    ...
                    userEvents = this.userEvents = new ufa.UserEvents(container, {
                        fastTap: true,
                        allowSelection: true
                    });
            ...
            _attachTransition: function (element) {
                ...
                movable = this.movable = new ufa.ui.Movable(element);
            ...
            _start: function (e) {
                ...
                if (Math.abs(e.x.velocity) < Math.abs(e.y.velocity) || ufa.triggeredByInput(e.event) || !this._currentViewIncludedIn(this.options.swipeToOpenViews)) {
                    userEvents.cancel();
                    return;
                }
...
(function (f, define) {
    ...
    (function ($, undefined) {
        var ufa = window.ufa, mobile = ufa.mobile, support = ufa.support, Widget = mobile.ui.Widget, Pane = mobile.ui.Pane, DEFAULT_OS = 'ios7', OS = support.mobileOS, BERRYPHONEGAP = OS.device == 'blackberry' && OS.flatVersion >= 600 && OS.flatVersion < 1000 && OS.appMode, FONT_SIZE_COEF = 0.93, VERTICAL = 'km-vertical', CHROME = OS.browser === 'chrome', BROKEN_WEBVIEW_RESIZE = OS.ios && OS.flatVersion >= 700 && OS.flatVersion < 800 && (OS.appMode || CHROME), INITIALLY_HORIZONTAL = Math.abs(window.orientation) / 90 == 1, HORIZONTAL = 'km-horizontal', MOBILE_PLATFORMS = {
                ios7: {
                    ios: true,
                    browser: 'default',
                    device: 'iphone',
        ...
        function isOrientationHorizontal(element) {
            return OS.wp ? element.css('animation-name') == '-ufa-landscape' : Math.abs(window.orientation) / 90 == 1;
        }
        ...
        var Application = Widget.extend({
            ...
            bootstrap: function (element, options) {
                ...
                Widget.fn.init.call(this, element, options);
                this.element.removeAttr('data-' + ufa.ns + 'role');
                this._setupPlatform();
                this._attachMeta();
            ...
            _hideBar: function () {
                var that = this, element = that.element;
                element.height(ufa.support.transforms.css + 'calc(100% + ' + BARCOMPENSATION + 'px)');
                $(window).trigger(ufa.support.resize);
            }
        });
        ufa.mobile.Application = Application;
        ufa.ui.plugin(Application, ufa.mobile, 'Mobile');
    }(window.ufa.jQuery));
    return window.ufa;
}, typeof define == 'function' && define.amd ? define : function (a1, a2, a3) {
    (a3 || a2)();
...
(function (f, define) {
    ...
    (function ($, undefined) {
        var ufa = window.ufa, support = ufa.support, ui = ufa.mobile.ui, Shim = ui.Shim, Popup = ui.Popup, Widget = ui.Widget, OPEN = 'open', CLOSE = 'close', COMMAND = 'command', BUTTONS = 'li>a', CONTEXT_DATA = 'actionsheetContext', WRAP = '<div class="km-actionsheet-wrapper" />', cancelTemplate = ufa.template('<li class="km-actionsheet-cancel"><a href="\\#">#:cancel#</a></li>');
        var ActionSheet = Widget.extend({
            init: function (element, options) {
                ...
                if (options.cancelTemplate) {
                    cancelTemplate = ufa.template(options.cancelTemplate);
                }
                element.addClass('km-actionsheet').append(cancelTemplate({ cancel: that.options.cancel })).wrap(WRAP).on('up', BUTTONS, '_click').on('click', BUTTONS, ufa.preventDefault);
            ...
            destroy: function () {
                Widget.fn.destroy.call(this);
                ufa.unbindResize(this._closeProxy);
                this.shim.destroy();
            },
            _click: function (e) {
                ...
                if (action) {
                    ...
                    if ($angular) {
                        ...
                        ufa.getter(action)(window)(actionData);
                    }
                }
...
(function (f, define) {
    define('ufa.mobile.button', ['ufa.userevents'], f);
}(function () {
    var __meta__ = {
        id: 'mobile.button',
        name: 'Button',
    ...
    (function ($, undefined) {
        var ufa = window.ufa, mobile = ufa.mobile, ui = mobile.ui, Widget = ui.Widget, support = ufa.support, os = support.mobileOS, ANDROID3UP = os.android && os.flatVersion >= 300, CLICK = 'click', DISABLED = 'disabled', DISABLEDSTATE = 'km-state-disabled';
        ...
        var Button = Widget.extend({
            init: function (element, options) {
                ...
                that._userEvents = new ufa.UserEvents(that.element, {
...
(function (f, define) {
    define('ufa.mobile.buttongroup', ['ufa.core'], f);
}(function () {
    var __meta__ = {
        id: 'mobile.buttongroup',
        name: 'ButtonGroup',
        category: 'mobile',
        description: 'The ufa mobile ButtonGroup widget is a linear set of grouped buttons.',
        depends: ['core']
    };
    (function ($, undefined) {
        var ufa = window.ufa, ui = ufa.mobile.ui, Widget = ui.Widget, ACTIVE = 'km-state-active', DISABLE = 'km-state-disabled', SELECT = 'select', SELECTOR = 'li:not(.' + ACTIVE + ')';
        function createBadge(value) {
            return $('<span class="km-badge">' + value + '</span>');
        }
        var ButtonGroup = Widget.extend({
            ...
            _button: function () {
                var button = $(this).addClass('km-button'), icon = ufa.attrValue(button, 'icon'), badge = ufa.attrValue(button, 'badge'), span = button.children('span'), image = button.find('img').addClass('km-image');
                if (!span[0]) {
...
(function (f, define) {
    define('ufa.mobile.collapsible', ['ufa.core'], f);
}(function () {
    var __meta__ = {
        id: 'mobile.collapsible',
        name: 'Collapsible',
        category: 'mobile',
        description: 'The ufa mobile Collapsible widget provides ability for creating collapsible blocks of content.',
        depends: [
            'core',
            'userevents'
        ]
    ...
    (function ($, undefined) {
        var ufa = window.ufa, ui = ufa.mobile.ui, Widget = ui.Widget, COLLAPSIBLE = 'km-collapsible', HEADER = 'km-collapsible-header', CONTENT = 'km-collapsible-content', INSET = 'km-collapsibleinset', HEADER_WRAPPER = '<div data-role=\'collapsible-header\' class=\'' + HEADER + '\'></div>', CONTENT_WRAPPER = '<div data-role=\'collapsible-content\' class=\'' + CONTENT + '\'></div>', COLLAPSED = 'km-collapsed', EXPANDED = 'km-expanded', ANIMATED = 'km-animated', LEFT = 'left', EXAPND = 'expand', COLLAPSE = 'collapse';
        var Collapsible = Widget.extend({
            init: function (element, options) {
                ...
                that._userEvents = new ufa.UserEvents(that.header, {
                    fastTap: true,
                    tap: function () {
                        that.toggle();
                    }
            ...
            expand: function (instant) {
                var icon = this.options.collapseIcon, content = this.content, ios = ufa.support.mobileOS.ios;
...
(function (f, define) {
    ...
    var __meta__ = {
        ...
        name: 'ListView',
        category: 'mobile',
        description: 'The ufa Mobile ListView widget is used to display flat or grouped list of items.',
        depends: [
            'data',
            'userevents',
            'mobile.button'
    ...
    (function ($, undefined) {
        var ufa = window.ufa, Node = window.Node, mobile = ufa.mobile, ui = mobile.ui, DataSource = ufa.data.DataSource, Widget = ui.DataBoundWidget, ITEM_SELECTOR = '.km-list > li, > li:not(.km-group-container)', HIGHLIGHT_SELECTOR = '.km-listview-link, .km-listview-label', ICON_SELECTOR = '[' + ufa.attr('icon') + ']', proxy = $.proxy, attrValue = ufa.attrValue, GROUP_CLASS = 'km-group-title', ACTIVE_CLASS = 'km-state-active', GROUP_WRAPPER = '<div class="' + GROUP_CLASS + '"><div class="km-text"></div></div>', GROUP_TEMPLATE = ufa.template('<li><div class="' + GROUP_CLASS + '"><div class="km-text">#= this.headerTemplate(data) #</div></div><ul>#= ufa.render(this.template, data.items)#</ul></li>'), WRAPPER = '<div class="km-listview-wrapper" />', SEARCH_TEMPLATE = ufa.template('<form class="km-filter-form"><div class="km-filter-wrap"><input type="search" placeholder="#=placeholder#"/><a href="\\#" class="km-filter-reset" title="Clear"><span class="km-icon km-clear"></span><span class="km-text">Clear</span></a></div></form>'), NS = '.ufaMobileListView', STYLED = 'styled', DATABOUND = 'dataBound', DATABINDING = 'dataBinding', ITEM_CHANGE = 'itemChange', CLICK = 'click', CHANGE = 'change', PROGRESS = 'progress', FUNCTION = 'function', whitespaceRegExp = /^\s+$/, buttonRegExp = /button/;
        function whitespace() {
        ...
        function enhanceLinkItem(item) {
            var parent = item.parent(), itemAndDetailButtons = item.add(parent.children(ufa.roleSelector('detailbutton'))), otherNodes = parent.contents().not(itemAndDetailButtons).not(whitespace);
            if (otherNodes.length) {
                return;
            }
            item.addClass('km-listview-link').attr(ufa.attr('role'), 'listview-link');
        ...
        var HeaderFixer = ufa.Class.extend({
        ...
        var RefreshHandler = ufa.Class.extend({
            init: function (listView) {
        ...
        var VirtualList = ufa.Observable.extend({
            init: function (options) {
                var list = this;
                ufa.Observable.fn.init.call(list);
        ...
        ufa.mobile.ui.VirtualList = VirtualList;
        var VirtualListViewItem = ufa.Class.extend({
        ...
        var VirtualListViewLoadingIndicator = ufa.Class.extend({
            init: function (listView) {
        ...
        var VirtualListViewPressToLoadMore = VirtualListViewLoadingIndicator.extend({
            init: function (listView, buffer) {
                ...
                var loadMore = this;
                this._loadButton.ufaMobileButton().data('ufaMobileButton').bind('click', function () {
        ...
        var VirtualListViewItemBinder = ufa.Class.extend({
            init: function (listView) {
                var binder = this;
            ...
            setDataSource: function (dataSource, empty) {
                ...
                buffer = new ufa.data.Buffer(dataSource, Math.floor(pageSize / 2), pressToLoadMore);
                if (pressToLoadMore) {
        ...
        var ListViewItemBinder = ufa.Class.extend({
            init: function (listView) {
                var binder = this;
                this.listView = listView;
                this.options = listView.options;
        ...
        var ListViewFilter = ufa.Class.extend({
            init: function (listView) {
        ...
        var ListView = Widget.extend({
            init: function (element, options) {
                ...
                this._userEvents = new ufa.UserEvents(element, {
                    fastTap: true,
            ...
            scroller: function () {
                if (!this._scrollerInstance) {
                    this._scrollerInstance = this.element.closest('.km-scroll-wrapper').data('ufaMobileScroller');
                }
            ...
            replaceGrouped: function (groups) {
                ...
                this._angularItems('cleanup');
                this.element.empty();
                var items = $(ufa.render(this.groupTemplate, groups));
            ...
            findByDataItem: function (dataItems) {
                ...
                for (var idx = 0, length = dataItems.length; idx < length; idx++) {
                    selectors[idx] = '[data-' + ufa.ns + 'uid=' + dataItems[idx].uid + ']';
                }
            ...
            _renderItems: function (dataItems, callback) {
                var items = $(ufa.render(this.template, dataItems));
                callback(items);
            ...
            _templates: function () {
                ...
                this.template = proxy(ufa.template('<li' + dataIDAttribute + '>' + template + '</li>'), templateProxy);
                groupTemplateProxy.template = this.template;
                if (typeof headerTemplate === FUNCTION) {
                    groupTemplateProxy._headerTemplate = headerTemplate;
                    headerTemplate = '#=this._headerTemplate(data)#';
                }
                groupTemplateProxy.headerTemplate = ufa.template(headerTemplate);
            ...
            _click: function (e) {
                ...
                var dataItem, item = e.target, target = $(e.event.target), buttonElement = target.closest(ufa.roleSelector('button', 'detailbutton', 'backbutton')), button = ufa.widgetInstance(buttonElement, ui), id = item.attr(ufa.attr('uid'));
                if (id) {
...
(function (f, define) {
    ...
    var __meta__ = {
        ...
        name: 'ScrollView',
        category: 'mobile',
        description: 'The ufa Mobile ScrollView widget is used to scroll content wider than the device screen.',
        depends: [
            'fx',
            'data',
            'draganddrop'
    ...
    (function ($, undefined) {
        var ufa = window.ufa, mobile = ufa.mobile, ui = mobile.ui, proxy = $.proxy, Transition = ufa.effects.Transition, Pane = ufa.ui.Pane, PaneDimensions = ufa.ui.PaneDimensions, Widget = ui.DataBoundWidget, DataSource = ufa.data.DataSource, Buffer = ufa.data.Buffer, BatchBuffer = ufa.data.BatchBuffer, math = Math, abs = math.abs, ceil = math.ceil, round = math.round, max = math.max, min = math.min, floor = math.floor, CHANGE = 'change', CHANGING = 'changing', REFRESH = 'refresh', CURRENT_PAGE_CLASS = 'km-current-page', VIRTUAL_PAGE_CLASS = 'km-virtual-page', FUNCTION = 'function', ITEM_CHANGE = 'itemChange', CLEANUP = 'cleanup', VIRTUAL_PAGE_COUNT = 3, LEFT_PAGE = -1, CETER_PAGE = 0, RIGHT_PAGE = 1, LEFT_SWIPE = -1, NUDGE = 0, RIGHT_SWIPE = 1;
        var Pager = ufa.Class.extend({
        ...
        ufa.mobile.ui.ScrollViewPager = Pager;
        var TRANSITION_END = 'transitionEnd', DRAG_START = 'dragStart', DRAG_END = 'dragEnd';
        var ElasticPane = ufa.Observable.extend({
            init: function (element, options) {
                var that = this;
                ufa.Observable.fn.init.call(this);
                this.element = element;
                ...
                movable = new ufa.ui.Movable(that.element);
                transition = new Transition({
                    axis: 'x',
                    movable: movable,
                    onEnd: function () {
                ...
                userEvents = new ufa.UserEvents(element, {
                    fastTap: true,
                    start: function (e) {
                        if (abs(e.x.velocity) * 2 >= abs(e.y.velocity)) {
                            userEvents.capture();
        ...
        ufa.mobile.ui.ScrollViewElasticPane = ElasticPane;
        var ScrollViewContent = ufa.Observable.extend({
            ...
            _getPages: function () {
                this.pageElements = this.element.find(ufa.roleSelector('page'));
                this._paged = this.pageElements.length > 0;
            }
        });
        ufa.mobile.ui.ScrollViewContent = ScrollViewContent;
        var VirtualScrollViewContent = ufa.Observable.extend({
            init: function (element, pane, options) {
                var that = this;
                ufa.Observable.fn.init.call(this);
                that.element = element;
            ...
            _templates: function () {
                ...
                this.template = proxy(ufa.template(template), templateProxy);
                if (typeof emptyTemplate === FUNCTION) {
                    emptyTemplateProxy.emptyTemplate = emptyTemplate;
                    emptyTemplate = '#=this.emptyTemplate(data)#';
                }
                this.emptyTemplate = proxy(ufa.template(emptyTemplate), emptyTemplateProxy);
            },
            _initPages: function () {
            ...
            paneMoved: function (swipeType, bounce, callback, instant) {
                var that = this, pane = that.pane, width = pane.size().width, offset = pane.offset(), thresholdPassed = Math.abs(offset) >= width / 3, ease = bounce ? ufa.effects.Transition.easeOutBack : ufa.effects.Transition.easeOutExpo, isEndReached = that.page + 2 > that.buffer.total(), nextPage, delta = 0;
            ...
            _moveForward: function (instant) {
                this.pane.transitionTo(-this.width, ufa.effects.Transition.easeOutExpo, instant);
            },
        ...
        ufa.mobile.ui.VirtualScrollViewContent = VirtualScrollViewContent;
        var Page = ufa.Class.extend({
            init: function (container) {
        ...
        ufa.mobile.ui.VirtualPage = Page;
...
(function (f, define) {
    ...
    (function ($, undefined) {
        var ufa = window.ufa, ui = ufa.mobile.ui, Widget = ui.Widget, support = ufa.support, CHANGE = 'change', SWITCHON = 'km-switch-on', SWITCHOFF = 'km-switch-off', MARGINLEFT = 'margin-left', ACTIVE_STATE = 'km-state-active', DISABLED_STATE = 'km-state-disabled', DISABLED = 'disabled', TRANSFORMSTYLE = support.transitions.css + 'transform', proxy = $.proxy;
        ...
        var Switch = Widget.extend({
            init: function (element, options) {
                ...
                Widget.fn.init.call(that, element, options);
                options = that.options;
                that.wrapper = $(ufa.format(SWITCH_MARKUP, options.onLabel, options.offLabel));
            ...
            _toggle: function (checked) {
                var that = this, handle = that.handle, element = that.element[0], value = element.checked, duration = ufa.mobile.application && ufa.mobile.application.os.wp ? 100 : 200, distance;
                that.wrapper.toggleClass(SWITCHON, checked).toggleClass(SWITCHOFF, !checked);
                that.position = distance = checked * that.constrain;
                if (that._animateBackground) {
                    that.background.ufaStop(true, true).ufaAnimate({
                        effects: 'slideMargin',
                        offset: distance,
                        reset: true,
                        reverse: !checked,
                ...
                handle.ufaStop(true, true).ufaAnimate({
                    effects: 'slideTo',
                    duration: duration,
            ...
            _drag: function () {
                var that = this;
                that.userEvents = new ufa.UserEvents(that.wrapper, {
                    fastTap: true,
                    tap: function () {
                        if (that.options.enable) {
...
(function (f, define) {
    define('ufa.mobile.tabstrip', ['ufa.core'], f);
}(function () {
    var __meta__ = {
        id: 'mobile.tabstrip',
        name: 'TabStrip',
    ...
    (function ($, undefined) {
        var ufa = window.ufa, ui = ufa.mobile.ui, Widget = ui.Widget, ACTIVE_STATE_CLASS = 'km-state-active', SELECT = 'select';
        function createBadge(value) {
            return $('<span class="km-badge">' + value + '</span>');
        }
        var TabStrip = Widget.extend({
            ...
            _buildButton: function () {
                var button = $(this), icon = ufa.attrValue(button, 'icon'), badge = ufa.attrValue(button, 'badge'), image = button.find('img'), iconSpan = $('<span class="km-icon"/>');
                button.addClass('km-button').attr(ufa.attr('role'), 'tab').contents().not(image).wrapAll('<span class="km-text"/>');
                if (image[0]) {
...
(function (f, define) {
    define('ufa.angular', ['ufa.core'], f);
}(function () {
    var __meta__ = {
        id: 'angular',
        name: 'AngularJS Directives',
        category: 'framework',
        description: 'Adds ufa UI for AngularJS directives',
        depends: ['core'],
        defer: true
    };
    (function ($, angular, undefined) {
        ...
        var module = angular.module('ufa.directives', []), $injector = angular.injector(['ng']), $parse = $injector.get('$parse'), $timeout = $injector.get('$timeout'), $defaultCompile, $log = $injector.get('$log');
        ...
        var createDataSource = function () {
            ...
            var toDataSource = function (dataSource, type) {
                ...
                return ufa.data[type].create(dataSource);
            };
            return function (scope, element, role, source) {
                ...
                scope.$watch(source, function (mew) {
                    var widget = ufaWidgetInstance(element);
        ...
        function createWidget(scope, element, attrs, widget, origAttr, controllers) {
            if (!(element instanceof jQuery)) {
                throw new Error('The ufa UI directives require jQuery to be available before AngularJS. Please include jquery before angular in the document.');
            }
            ...
            function createIt() {
                ...
                scope.$emit('ufaWidgetCreated', object);
        ...
        function bindToKNgDisabled(widget, scope, element, kNgDisabled) {
            if (ufa.ui.PanelBar && widget instanceof ufa.ui.PanelBar || ufa.ui.Menu && widget instanceof ufa.ui.Menu) {
        ...
        function exposeWidget(widget, scope, attrs, ufaWidget, origAttr) {
            if (attrs[origAttr]) {
                var set = $parse(attrs[origAttr]).assign;
                if (set) {
                    set(scope, widget);
                } else {
                    throw new Error(origAttr + ' attribute used but expression in it is not assignable: ' + attrs[ufaWidget]);
                }
            }
        }
        ...
        function bindToNgModel(widget, scope, element, ngModel, ngForm) {
            ...
            if (!(ufa.ui.AutoComplete && widget instanceof ufa.ui.AutoComplete)) {
        ...
        function bindToKNgModel(widget, scope, kNgModel) {
            ...
            var updating = false;
            var valueIsCollection = ufa.ui.MultiSelect && widget instanceof ufa.ui.MultiSelect;
        ...
        function destroyWidgetOnScopeDestroy(scope, widget) {
            var deregister = scope.$on('$destroy', function () {
                ...
                if (widget) {
                    if (widget.element) {
                        widget = ufaWidgetInstance(widget.element);
                        if (widget) {
                            widget.destroy();
                        }
                    }
                    ...
                    case 'class':
                        ...
                        currClassList.forEach(function (cls) {
                            if (prevClassList.indexOf(cls) < 0) {
                                w.classList.add(cls);
                                if (ufa.ui.ComboBox && widget instanceof ufa.ui.ComboBox) {
                ...
                if (!widget._muteRebind && newValue !== oldValue) {
                    ...
                    if (templateOptions) {
                        templateOptions.forEach(function (name) {
                            ...
                            if (templateContents) {
                                originalElement.append($(templateContents).attr(ufa.toHyphens('k' + name), ''));
                            }
                        });
                    }
                ...
                var create = function (role, origAttr) {
                    return {
                        ...
                        link: function (scope, element, attrs, controllers) {
                            ...
                            if (ufaRenderedTimeout) {
                                clearTimeout(ufaRenderedTimeout);
                            }
                            ufaRenderedTimeout = setTimeout(function () {
                                scope.$emit('ufaRendered');
                                if (!RENDERED) {
                                    RENDERED = true;
                                    $('form').each(function () {
        ...
        function createDirectives(klass, isMobile) {
            ...
            var className = name;
            var shortcut = 'ufa' + name.charAt(0) + name.substr(1).toLowerCase();
            name = 'ufa' + name;
            var dashed = name.replace(/([A-Z])/g, '-$1');
            if (SKIP_SHORTCUTS.indexOf(name.replace('ufa', '')) == -1) {
            ...
            if (MANUAL_DIRECTIVES.indexOf(name.replace('ufa', '')) > -1) {
                return;
            }
            make(name, name);
            if (shortcut != name) {
        ...
        function ufaWidgetInstance(el) {
            el = $(el);
            return ufa.widgetInstance(el, ufa.ui) || ufa.widgetInstance(el, ufa.mobile.ui) || ufa.widgetInstance(el, ufa.dataviz.ui);
        }
        ...
        function destroyScope(scope, el) {
            scope.$destroy();
            if (el) {
                $(el).removeData('$scope').removeData('$$ufaScope').removeData('$isolateScope').removeData('$isolateScopeNoTemplate').removeClass('ng-scope');
            }
        }
        var pendingPatches = [];
        ...
        ufa.onWidgetRegistered(function (entry) {
                    ...
                    if (elements.length > 0) {
                        ...
                        case 'cleanup':
                            angular.forEach(elements, function (el) {
                                var itemScope = $(el).data('$$ufaScope');
                                if (itemScope && itemScope !== scope && itemScope.$$ufaScope) {
                            ...
                            angular.forEach(elements, function (el, i) {
                                ...
                                if (x.scopeFrom) {
                                    ...
                                    if (vars !== undefined) {
                                        ...
                                        itemScope.$$ufaScope = true;
                                    } else {
                                        itemScope = scope;
                                    }
                                }
                                $(el).data('$$ufaScope', itemScope);
        ...
        defadvice([
            ...
            return function (ev) {
                ...
                var options = widget.options;
                var cell, multiple, locals = { ufaEvent: ev }, elems, items, columns, colIdx;
                ...
                if (!multiple) {
                    ...
                    locals.angularDataItem = ufa.proxyModelSetters(locals.dataItem);
        ...
        defadvice('ui.Grid', '$angular_init', function (element, options) {
            this.next();
            if (options.columns) {
                var settings = $.extend({}, ufa.Template, options.templateSettings);
                angular.forEach(options.columns, function (col) {
                    if (col.field && !col.template && !col.format && !col.values && (col.encoded === undefined || col.encoded)) {
                        col.template = '<span ng-bind=\'' + ufa.expr(col.field, 'dataItem') + '\'>#: ' + ufa.expr(col.field, settings.paramName) + '#</span>';
                    }
                });
            }
        });
        ...
        module.directive('ufaMobileApplication', function () {
            return {
                terminal: true,
                link: function (scope, element, attrs) {
                    createWidget(scope, element, attrs, 'ufaMobileApplication', 'ufaMobileApplication');
                }
            };
        }).directive('ufaMobileView', function () {
            return {
                scope: true,
                link: {
                    pre: function (scope, element, attrs) {
                        attrs.defaultOptions = scope.viewOptions;
                        attrs._instance = createWidget(scope, element, attrs, 'ufaMobileView', 'ufaMobileView');
                    },
        ...
        }).directive('ufaMobileDrawer', function () {
            return {
                scope: true,
                link: {
                    pre: function (scope, element, attrs) {
                        attrs.defaultOptions = scope.viewOptions;
                        attrs._instance = createWidget(scope, element, attrs, 'ufaMobileDrawer', 'ufaMobileDrawer');
                    },
                    post: function (scope, element, attrs) {
                        attrs._instance._layout();
                        attrs._instance._scroller();
        ...
        }).directive('ufaMobileModalView', function () {
            return {
                scope: true,
                link: {
                    pre: function (scope, element, attrs) {
                        attrs.defaultOptions = scope.viewOptions;
                        attrs._instance = createWidget(scope, element, attrs, 'ufaMobileModalView', 'ufaMobileModalView');
                    },
                    post: function (scope, element, attrs) {
                        attrs._instance._layout();
                        attrs._instance._scroller();
        ...
        }).directive('ufaMobileSplitView', function () {
            return {
                terminal: true,
                link: {
                    pre: function (scope, element, attrs) {
                        attrs.defaultOptions = scope.viewOptions;
                        attrs._instance = createWidget(scope, element, attrs, 'ufaMobileSplitView', 'ufaMobileSplitView');
                    },
                    post: function (scope, element, attrs) {
                        attrs._instance._layout();
                    }
        ...
        }).directive('ufaMobilePane', function () {
            return {
                terminal: true,
                link: {
                    pre: function (scope, element, attrs) {
                        attrs.defaultOptions = scope.viewOptions;
                        createWidget(scope, element, attrs, 'ufaMobilePane', 'ufaMobilePane');
                    }
                }
            };
        }).directive('ufaMobileLayout', function () {
            return {
                link: {
                    pre: function (scope, element, attrs) {
                        createWidget(scope, element, attrs, 'ufaMobileLayout', 'ufaMobileLayout');
                    }
                }
            };
        }).directive('ufaMobileActionSheet', function () {
            return {
                restrict: 'A',
                link: function (scope, element, attrs) {
                    element.find('a[k-action]').each(function () {
                        $(this).attr('data-' + ufa.ns + 'action', $(this).attr('k-action'));
                    });
                    createWidget(scope, element, attrs, 'ufaMobileActionSheet', 'ufaMobileActionSheet');
                }
            };
        }).directive('ufaMobilePopOver', function () {
            return {
                terminal: true,
                link: {
                    pre: function (scope, element, attrs) {
                        attrs.defaultOptions = scope.viewOptions;
                        createWidget(scope, element, attrs, 'ufaMobilePopOver', 'ufaMobilePopOver');
                    }
                }
            };
        }).directive('ufaViewTitle', function () {
            return {
                restrict: 'E',
                replace: true,
                template: function (element) {
                    return '<span data-' + ufa.ns + 'role=\'view-title\'>' + element.html() + '</span>';
                }
            };
        }).directive('ufaMobileHeader', function () {
            return {
                restrict: 'E',
        ...
        }).directive('ufaMobileFooter', function () {
            return {
                restrict: 'E',
                link: function (scope, element) {
        ...
        }).directive('ufaMobileScrollViewPage', function () {
            return {
                restrict: 'E',
                replace: true,
                template: function (element) {
                    return '<div data-' + ufa.ns + 'role=\'page\'>' + element.html() + '</div>';
                }
            };
        });
        angular.forEach([
            ...
            module.directive(kAttr, function () {
                return {
                    ...
                    priority: 2,
                    link: function (scope, element, attrs) {
                        element.attr(ufa.attr(ufa.toHyphens(attr)), scope.$eval(attrs[kAttr]));
                    }
                };
            });
        });
        ...
        (function () {
            ...
            angular.forEach(WIDGET_TEMPLATE_OPTIONS, function (templates, widget) {
                angular.forEach(templates, function (template) {
                    ...
                    templateDirectives[template].push('?^^ufa' + widget);
                });
            });
            angular.forEach(templateDirectives, function (parents, directive) {
                var templateName = 'k' + directive;
                var attrName = ufa.toHyphens(templateName);
    ...
    }(window.ufa.jQuery, window.angular));
    return window.ufa;
}, typeof define == 'function' && define.amd ? define : function (a1, a2, a3) {
    (a3 || a2)();
}));
(function (f, define) {
    define('ufa.webcomponents', ['ufa.core'], f);
}(function () {
    var __meta__ = {
        id: 'webcomponents',
        name: 'Web Components',
        category: 'framework',
        description: 'Adds ufa UI custom elements for Web Components',
        depends: ['core']
    };
    (function ($, angular, undefined) {
        if (!ufa.support.customElements || ufa.webComponents.length) {
            return;
        }
        ...
        function parseOptions(element, options) {
            ...
            Object.keys(options).concat('dataSource').forEach(function (name) {
                if (element.hasAttribute(ufa.toHyphens(name))) {
                    result[name] = parseOption(element, ufa.toHyphens(name));
                }
            });
            return result;
        }
        ...
        function registerElement(name, widget) {
            ...
            ufa.webComponents.push('ufa-' + name);
            document.registerElement('ufa-' + name, { prototype: prototype });
        }
    }(window.ufa.jQuery, window.angular));
    return window.ufa;
}, typeof define == 'function' && define.amd ? define : function (a1, a2, a3) {
...
(function (f, define) {
    define('ufa.mobile', [
        ...
        'ufa.mobile.shim',
        'ufa.mobile.view',
        'ufa.mobile.modalview',
        'ufa.mobile.drawer',
        'ufa.mobile.splitview',
        'ufa.mobile.pane',
        'ufa.mobile.application',
        'ufa.mobile.actionsheet',
        'ufa.mobile.button',
        'ufa.mobile.buttongroup',
        'ufa.mobile.collapsible',
        'ufa.mobile.listview',
        'ufa.mobile.navbar',
        'ufa.mobile.scrollview',
        'ufa.mobile.switch',
        'ufa.mobile.tabstrip',
        'ufa.angular',
        'ufa.webcomponents',
