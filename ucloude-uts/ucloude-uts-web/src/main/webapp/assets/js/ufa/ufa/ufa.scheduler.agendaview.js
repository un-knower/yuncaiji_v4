(function (f, define) {
    define('ufa.scheduler.agendaview', ['ufa.scheduler.view'], f);
}(function () {
    var __meta__ = {
    ...
    (function ($) {
        ...
        ui.AgendaView = ui.SchedulerView.extend({
            init: function (element, options) {
                ...
                this._dateTemplate = ufa.template(options.eventDateTemplate);
                this._groupTemplate = ufa.template(options.eventGroupTemplate);
                this._timeTemplate = ufa.template(options.eventTimeTemplate);
            ...
            _remove: function (e) {
                e.preventDefault();
                this.trigger('remove', { uid: $(e.currentTarget).closest('.k-task').attr(ufa.attr('uid')) });
            },
            nextDate: function () {
                return ufa.date.nextDay(this.startDate());
            },
            startDate: function () {
            ...
            _renderLayout: function (date) {
                ...
                this._endDate = ufa.date.addDays(date, 7);
            ...
            _tasks: function (events) {
                ...
                for (var idx = 0; idx < events.length; idx++) {
                    ...
                    var end = event.end;
                    var eventDurationInDays = (ufa.date.getDate(end) - ufa.date.getDate(start)) / ufa.date.MS_PER_DAY + 1;
                    var task = event.clone();
                    task.startDate = ufa.date.getDate(start);
                    if (task.startDate >= this.startDate()) {
                        tasks.push(task);
                    }
                    if (eventDurationInDays > 1) {
                        task.end = ufa.date.nextDay(start);
                        task.head = true;
                        for (var day = 1; day < eventDurationInDays; day++) {
                            ...
                            if (ufa.date.getDate(task.end) <= this.endDate() && task.start >= this.startDate() || ufa.date.getDate(task.start).getTime() == this.endDate().getTime()) {
                                tasks.push(task);
                            }
                        }
                    }
                }
                return new ufa.data.Query(tasks).sort([
                    {
                        field: 'start',
                        dir: 'asc'
                    },
            ...
            _renderTaskGroups: function (tasksGroups, groups) {
                ...
                for (var taskGroupIndex = 0; taskGroupIndex < tasksGroups.length; taskGroupIndex++) {
                    ...
                    var today = ufa.date.isToday(date);
                    for (var taskIndex = 0; taskIndex < tasks.length; taskIndex++) {
                        ...
                        if (taskGroupIndex === 0 && taskIndex === 0 && groups.length) {
                            for (var idx = 0; idx < groups.length; idx++) {
                                headerCells.push(ufa.format('<td class="k-scheduler-groupcolumn{2}" rowspan="{0}">{1}</td>', groups[idx].rowSpan, this._groupTemplate({ value: groups[idx].text }), groups[idx].className));
                            }
                        }
                        if (taskIndex === 0) {
                            if (isPhoneView) {
                                headerCells.push(ufa.format('<td class="k-scheduler-datecolumn" colspan="2">{0}</td>', this._dateTemplate({ date: date })));
                                tableRows.push('<tr role="row" aria-selected="false"' + (today ? ' class="k-today">' : '>') + headerCells.join('') + '</tr>');
                            } else {
                                tableRow.push(ufa.format('<td class="k-scheduler-datecolumn{3}{2}" rowspan="{0}">{1}</td>', tasks.length, this._dateTemplate({ date: date }), taskGroupIndex == tasksGroups.length - 1 && !groups.length ? ' k-last' : '', !groups.length ? ' k-first' : ''));
                            }
                        }
                        if (task.head) {
                            task.format = '{0:t}';
                        ...
                        tableRow.push(ufa.format('<td class="k-scheduler-timecolumn"><div>{0}{1}{2}</div></td><td>{3}</td>', task.tail || task.middle ? '<span class="k-icon k-i-arrow-w"></span>' : '', this._timeTemplate(task.clone({
            ...
            _angularItems: function (table, items) {
                this.angular('compile', function () {
                    var data = [], elements = items.map(function (item) {
                            ...
                            return table.find('.k-task[' + ufa.attr('uid') + '=' + item.uid + ']');
                        });
                    return {
                        elements: elements,
                        data: data
            ...
            _createGroupConfiguration: function (events, resources, parent) {
                ...
                for (var dataIndex = 0; dataIndex < data.length; dataIndex++) {
                    ...
                    if (tmp.length) {
                        ...
                        var obj = {
                            text: ufa.getter(resource.dataTextField)(data[dataIndex]),
                            value: value,
                            rowSpan: 0,
                            className: className
                        };
            ...
            options: {
                ...
                eventTemplate: '#:title#',
                eventTimeTemplate: '#if(data.isAllDay) {#' + '#=this.options.messages.allDay#' + '#} else { #' + '#=ufa.format(format, start, end)#' + '# } #',
                eventDateTemplate: '<strong class="k-scheduler-agendaday">' + '#=ufa.toString(date, "dd")#' + '</strong>' + '<em class="k-scheduler-agendaweek">' + '#=ufa.toString(date,"dddd")#' + '</em>' + '<span class="k-scheduler-agendadate">' + '#=ufa.toString(date, "y")#' + '</span>',
