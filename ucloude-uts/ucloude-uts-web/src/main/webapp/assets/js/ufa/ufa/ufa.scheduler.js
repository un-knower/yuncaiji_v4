...
(function (f, define) {
    define('ufa.scheduler', [
        'ufa.dropdownlist',
        'ufa.editable',
        'ufa.multiselect',
        'ufa.window',
        'ufa.datetimepicker',
        'ufa.scheduler.recurrence',
        'ufa.scheduler.view',
        'ufa.scheduler.dayview',
        'ufa.scheduler.agendaview',
        'ufa.scheduler.monthview',
        'ufa.scheduler.timelineview',
    ...
    (function ($, undefined) {
        var ufa = window.ufa, date = ufa.date, input_support = ufa.support.input, MS_PER_DAY = date.MS_PER_DAY, getDate = date.getDate, getMilliseconds = ufa.date.getMilliseconds, recurrence = ufa.recurrence, keys = ufa.keys, ui = ufa.ui, Widget = ui.Widget, DataBoundWidget = ui.DataBoundWidget, STRING = 'string', Popup = ui.Popup, Calendar = ui.Calendar, DataSource = ufa.data.DataSource, isPlainObject = $.isPlainObject, extend = $.extend, proxy = $.proxy, toString = Object.prototype.toString, isArray = $.isArray, NS = '.ufaScheduler', CLICK = 'click', CHANGE = 'change', CANCEL = 'cancel', REMOVE = 'remove', SAVE = 'save', ADD = 'add', EDIT = 'edit', valueStartEndBoundRegex = /(?:value:start|value:end)(?:,|$)/, TODAY = getDate(new Date()), RECURRENCE_EXCEPTION = 'recurrenceException', DELETECONFIRM = 'Are you sure you want to delete this event?', DELETERECURRING = 'Do you want to delete only this event occurrence or the whole series?', EDITRECURRING = 'Do you want to edit only this event occurrence or the whole series?', DELETERECURRINGCONFIRM = 'Are you sure you want to delete this event occurrence?', DELETESERIESCONFIRM = 'Are you sure you want to delete the whole series?', COMMANDBUTTONTMPL = '<a class="k-button #=className#" #=attr# href="\\#">#=text#</a>', VIEWBUTTONTEMPLATE = ufa.template('<li class="k-current-view" data-#=ns#name="#=view#"><a role="button" href="\\#" class="k-link">${views[view].title}</a></li>'), TOOLBARTEMPLATE = ufa.template('<div class="k-floatwrap k-header k-scheduler-toolbar">' + '# if (pdf) { #' + '<ul class="k-reset k-scheduler-tools">' + '<li><a role="button" href="\\#" class="k-button k-pdf"><span class="k-icon k-i-pdf"></span>${messages.pdf}</a></li>' + '</ul>' + '# } #' + '<ul class="k-reset k-scheduler-navigation">' + '<li class="k-state-default k-header k-nav-today"><a role="button" href="\\#" class="k-link">${messages.today}</a></li>' + '<li class="k-state-default k-header k-nav-prev"><a role="button" href="\\#" class="k-link"><span class="k-icon k-i-arrow-w"></span></a></li>' + '<li class="k-state-default k-header k-nav-next"><a role="button" href="\\#" class="k-link"><span class="k-icon k-i-arrow-e"></span></a></li>' + '<li class="k-state-default k-nav-current">' + '<a role="button" href="\\#" class="k-link">' + '<span class="k-icon k-i-calendar"></span>' + '<span class="k-sm-date-format" data-#=ns#bind="text: formattedShortDate"></span>' + '<span class="k-lg-date-format" data-#=ns#bind="text: formattedDate"></span>' + '</a>' + '</li>' + '</ul>' + '#if(viewsCount === 1){#' + '<a role="button" data-#=ns#name="#=view#" href="\\#" class="k-link k-scheduler-refresh">' + '<span class="k-icon k-i-refresh"></span>' + '</a>' + '#}else{#' + '<ul class="k-reset k-header k-scheduler-views">' + '#for(var view in views){#' + '<li class="k-state-default k-view-#= view.toLowerCase() #" data-#=ns#name="#=view#"><a role="button" href="\\#" class="k-link">${views[view].title}</a></li>' + '#}#' + '</ul>' + '#}#' + '</div>'), MOBILETOOLBARTEMPLATE = ufa.template('<div class="k-floatwrap k-header k-scheduler-toolbar">' + '<ul class="k-reset k-header k-scheduler-navigation">' + '<li class="k-state-default k-nav-today"><a role="button" href="\\#" class="k-link">${messages.today}</a></li>' + '</ul>' + '#if(viewsCount === 1){#' + '<a role="button" data-#=ns#name="#=view#" href="\\#" class="k-link k-scheduler-refresh">' + '<span class="k-icon k-i-refresh"></span>' + '</a>' + '#}else{#' + '<ul class="k-reset k-header k-scheduler-views">' + '#for(var view in views){#' + '<li class="k-state-default k-view-#= view.toLowerCase() #" data-#=ns#name="#=view#"><a role="button" href="\\#" class="k-link">${views[view].title}</a></li>' + '#}#' + '</ul>' + '#}#' + '</div>' + '<div class="k-floatwrap k-header k-scheduler-toolbar">' + '<ul class="k-reset k-header k-scheduler-navigation">' + '<li class="k-state-default k-nav-prev"><a role="button" href="\\#" class="k-link"><span class="k-icon k-i-arrow-w"></span></a></li>' + '<li class="k-state-default k-nav-current">' + '<span class="k-sm-date-format" data-#=ns#bind="text: formattedShortDate"></span>' + '<span class="k-lg-date-format" data-#=ns#bind="text: formattedDate"></span>' + '</li>' + '<li class="k-state-default k-nav-next"><a role="button" href="\\#" class="k-link"><span class="k-icon k-i-arrow-e"></span></a></li>' + '</ul>' + '</div>'), MOBILEDATERANGEEDITOR = function (container, options) {
                ...
                $('<div ' + ufa.attr('bind') + '="value:' + options.field + '" />').attr({ name: options.field }).toggle(options.visible).appendTo(container).ufaMobileTimezoneEditor({ optionLabel: options.noTimezone });
            }, TIMEZONEEDITOR = function (container, options) {
                $('<div ' + ufa.attr('bind') + '="value:' + options.field + '" />').attr({ name: options.field }).toggle(options.visible).appendTo(container).ufaTimezoneEditor({ optionLabel: options.noTimezone });
            };
        ...
        function appendTimezoneAttr(attrs, options) {
            ...
            if (timezone) {
                attrs[ufa.attr('timezone')] = timezone;
            }
        }
        function appendDateCompareValidator(attrs, options) {
            ...
            if (validationRules) {
                ...
                if (dateCompareRule && isPlainObject(dateCompareRule) && dateCompareRule.message) {
                    attrs[ufa.attr('dateCompare-msg')] = dateCompareRule.message;
                }
            }
        }
        ...
        function wrapDataSerialization(originalFunction, timezone) {
            return function (data) {
                if (data) {
                    if (toString.call(data) !== '[object Array]' && !(data instanceof ufa.data.ObservableArray)) {
                        data = [data];
                    }
                }
            ...
            for (idx = 0, length = data.length; idx < length; idx++) {
                ...
                if (removeUid) {
                    if (event.startTimezone || event.endTimezone) {
                        if (timezone) {
                            event.start = ufa.timezone.convert(event.start, event.startTimezone || event.endTimezone, timezone);
                            event.end = ufa.timezone.convert(event.end, event.endTimezone || event.startTimezone, timezone);
                            event.start = ufa.timezone[method](event.start, timezone);
                            event.end = ufa.timezone[method](event.end, timezone);
                        } else {
                            event.start = ufa.timezone[method](event.start, event.startTimezone || event.endTimezone);
                            event.end = ufa.timezone[method](event.end, event.endTimezone || event.startTimezone);
                        }
                    } else if (timezone) {
                        event.start = ufa.timezone[method](event.start, timezone);
                        event.end = ufa.timezone[method](event.end, timezone);
        ...
        var SchedulerDataReader = ufa.Class.extend({
        ...
        function applyZone(date, fromZone, toZone) {
            if (toZone) {
                date = ufa.timezone.convert(date, fromZone, toZone);
            } else {
                date = ufa.timezone.remove(date, fromZone);
            }
            return date;
        }
        function dateCompareValidator(input) {
            if (input.filter('[name=end]').length) {
                ...
                if (endInput[0] && startInput[0]) {
                    var start, end;
                    var startPicker = ufa.widgetInstance(startInput, ufa.ui);
                    var endPicker = ufa.widgetInstance(endInput, ufa.ui);
                    var editable = container.data('ufaEditable');
                    var model = editable ? editable.options.model : null;
                    if (startPicker && endPicker) {
                        start = startPicker.value();
                        end = endPicker.value();
                    } else {
                        start = ufa.parseDate(startInput.val());
                        end = ufa.parseDate(endInput.val());
                    }
                    if (start && end) {
                        if (model) {
                            var timezone = startInput.attr(ufa.attr('timezone'));
        ...
        var SchedulerEvent = ufa.data.Model.define({
            init: function (value) {
                var that = this;
            ...
            _time: function (field) {
                ...
                if (this[fieldTime]) {
                    return this[fieldTime] - ufa.date.toUtcTime(ufa.date.getDate(date));
                }
            ...
            _date: function (field) {
                ...
                return ufa.date.getDate(this[field]);
            },
            ...
            duration: function () {
                ...
                var offset = (end.getTimezoneOffset() - start.getTimezoneOffset()) * ufa.date.MS_PER_MINUTE;
            ...
            update: function (eventInfo) {
                ...
                if (this._startTime) {
                    this.set('_startTime', ufa.date.toUtcTime(this.start));
                }
                if (this._endTime) {
                    this.set('_endTime', ufa.date.toUtcTime(this.end));
                }
            },
            isMultiDay: function () {
                return this.isAllDay || this.duration() >= ufa.date.MS_PER_DAY;
            },
            ...
            toJSON: function () {
                var obj = ufa.data.Model.fn.toJSON.call(this);
                obj.uid = this.uid;
            ...
            shouldSerialize: function (field) {
                return ufa.data.Model.fn.shouldSerialize.call(this, field) && field !== '_defaultId';
            },
            set: function (key, value) {
                var isAllDay = this.isAllDay || false;
                ufa.data.Model.fn.set.call(this, key, value);
                if (key == 'isAllDay' && value != isAllDay) {
                    var start = ufa.date.getDate(this.start);
                    var end = new Date(this.end);
                    var milliseconds = ufa.date.getMilliseconds(end);
                    if (milliseconds === 0 && value) {
                        milliseconds = MS_PER_DAY;
                    }
                    this.set('start', start);
                    if (value === true) {
                        ufa.date.setTime(end, -milliseconds);
                        if (end < start) {
                            end = start;
                        }
                    } else {
                        ufa.date.setTime(end, MS_PER_DAY - milliseconds);
                    }
                    this.set('end', end);
                }
            },
        ...
        var SchedulerDataSource = DataSource.extend({
            ...
            expand: function (start, end) {
                ...
                if (start && end) {
                    ...
                    data = new ufa.data.Query(expandAll(data, start, end, this.reader.timezone)).filter(filter).toArray();
                }
                return data;
            },
        ...
        SchedulerDataSource.create = function (options) {
            if (isArray(options) || options instanceof ufa.data.ObservableArray) {
                options = { data: options };
            }
            var dataSource = options || {}, data = dataSource.data;
            dataSource.data = data;
            if (!(dataSource instanceof SchedulerDataSource) && dataSource instanceof ufa.data.DataSource) {
        ...
        function dropDownResourceEditor(resource, model) {
            ...
            return function (container) {
                $(ufa.format('<select data-{0}bind="value:{1}">', ufa.ns, resource.field)).appendTo(container).attr(attr).ufaDropDownList({
                    dataTextField: resource.dataTextField,
                    dataValueField: resource.dataValueField,
                    dataSource: resource.dataSource,
                    valuePrimitive: resource.valuePrimitive,
                    optionLabel: 'None',
                    template: ufa.format('<span class="k-scheduler-mark" style="background-color:#= data.{0}?{0}:"none" #"></span>#={1}#', resource.dataColorField, resource.dataTextField)
                });
            };
        }
        ...
        function multiSelectResourceEditor(resource, model) {
            var attr = createValidationAttributes(model, resource.field);
            return function (container) {
                $(ufa.format('<select data-{0}bind="value:{1}">', ufa.ns, resource.field)).appendTo(container).attr(attr).ufaMultiSelect({
                    dataTextField: resource.dataTextField,
                    dataValueField: resource.dataValueField,
                    dataSource: resource.dataSource,
                    valuePrimitive: resource.valuePrimitive,
                    itemTemplate: ufa.format('<span class="k-scheduler-mark" style="background-color:#= data.{0}?{0}:"none" #"></span>#={1}#', resource.dataColorField, resource.dataTextField),
                    tagTemplate: ufa.format('<span class="k-scheduler-mark" style="background-color:#= data.{0}?{0}:"none" #"></span>#={1}#', resource.dataColorField, resource.dataTextField)
                });
            };
        }
        function multiSelectResourceEditorMobile(resource, model) {
            ...
            return function (container) {
                ...
                for (var idx = 0, length = view.length; idx < length; idx++) {
                    options += ufa.format('<option value="{0}">{1}</option>', ufa.getter(resource.dataValueField)(view[idx]), ufa.getter(resource.dataTextField)(view[idx]));
                }
                $(ufa.format('<select data-{0}bind="value:{1}" multiple="multiple" data-{0}value-primitive="{3}">{2}</select>', ufa.ns, resource.field, options, resource.valuePrimitive)).appendTo(container).attr(attr);
            };
        }
        function moveEventRange(event, distance) {
            var duration = event.end.getTime() - event.start.getTime();
            var start = new Date(event.start.getTime());
            ufa.date.setTime(start, distance);
            var end = new Date(start.getTime());
            ufa.date.setTime(end, duration, true);
            return {
                start: start,
                end: end
            };
        ...
        var Editor = ufa.Observable.extend({
            ...
            _toggleDateValidation: function (e) {
                if (e.field == 'isAllDay') {
                    var container = this.container, isAllDay = this.editable.options.model.isAllDay, bindAttribute = ufa.attr('bind'), element, isDateTimeInput, shouldValidate;
                    container.find('[' + bindAttribute + '*=end],[' + bindAttribute + '*=start]').each(function () {
                        ...
                        if (valueStartEndBoundRegex.test(element.attr(bindAttribute))) {
                            isDateTimeInput = element.is('[' + ufa.attr('role') + '=datetimepicker],[type*=datetime]');
                            shouldValidate = isAllDay !== isDateTimeInput;
                            element.attr(ufa.attr('validate'), shouldValidate);
                        }
                    });
                }
            },
            ...
            _buildEditTemplate: function (model, fields, editableFields) {
                ...
                var settings = extend({}, ufa.Template, this.options.templateSettings);
                ...
                if (template) {
                    ...
                    html += ufa.template(template, settings)(model);
                } else {
                    for (var idx = 0, length = fields.length; idx < length; idx++) {
                        ...
                        if (!model.editable || model.editable(field.field)) {
                            ...
                            html += '<div ' + ufa.attr('container-for') + '="' + field.field + '" class="k-edit-field"></div>';
                        } else {
                            var tmpl = '#:';
                            if (field.field) {
                                field = ufa.expr(field.field, paramName);
                                tmpl += field + '==null?\'\':' + field;
                            } else {
                                tmpl += '\'\'';
                            }
                            tmpl += '#';
                            tmpl = ufa.template(tmpl, settings);
        ...
        var MobileEditor = Editor.extend({
            init: function () {
                ...
                this.pane = ufa.mobile.ui.Pane.wrap(this.element);
                this.pane.element.parent().css('height', this.options.height);
                this.view = this.pane.view();
                this._actionSheetButtonTemplate = ufa.template('<li><a #=attr# class="k-button #=className#" href="\\#">#:text#</a></li>');
            ...
            _initTimezoneEditor: function (model) {
                ...
                if (!timezoneView) {
                    ...
                    timezoneView.element.on(CLICK + NS, '.k-scheduler-cancel, .k-scheduler-update', function (e) {
                        ...
                        var editView = pane.element.find('#edit').data('ufaMobileView');
            ...
            showDialog: function (options) {
                ...
                var target = this.element.find('.k-event[' + ufa.attr('uid') + '=\'' + options.model.uid + '\']');
                if (this.container) {
                ...
                html += '</ul>';
                var actionSheet = $(html).appendTo(this.pane.view().element).ufaMobileActionSheet({
                    type: type,
            ...
            editEvent: function (model) {
                ...
                html += '<div data-role="view" class="k-popup-edit-form k-scheduler-edit-form k-mobile-list" id="edit" ' + ufa.attr('uid') + '="' + model.uid + '">' + '<div data-role="header" class="k-header"><a href="#" class="k-button k-scheduler-cancel">' + cancelText + '</a>' + titleText + '<a href="#" class="k-button k-scheduler-update">' + updateText + '</a></div>';
                ...
                this.editable = container.ufaEditable({
            ...
            _views: function () {
                return this.pane.element.find(ufa.roleSelector('view')).not(this.view.element);
            },
            close: function () {
                if (this.container) {
                    ...
                    for (var idx = 0, length = views.length; idx < length; idx++) {
                        view = views.eq(idx).data('ufaMobileView');
                        if (view) {
                            view.purge();
                        }
                    }
        ...
        var PopupEditor = Editor.extend({
            ...
            editEvent: function (model) {
                ...
                var html = '<div ' + ufa.attr('uid') + '="' + model.uid + '" class="k-popup-edit-form k-scheduler-edit-form"><div class="k-edit-form-container">';
                var messages = that.options.messages;
                var updateText = messages.save;
                var cancelText = messages.cancel;
                ...
                html += '</div></div></div>';
                var container = this.container = $(html).appendTo(that.element).eq(0).ufaWindow(extend({
                    modal: true,
                    resizable: false,
                    draggable: true,
                ...
                that.editable = container.ufaEditable({
                    fields: editableFields,
                    model: model,
                    clearContainer: false,
                    validateOnBlur: true,
                ...
                if (!that.trigger(EDIT, {
                    ...
                    container.data('ufaWindow').center().open();
            ...
            close: function () {
                ...
                if (that.editable) {
                    if (that._timezonePopup && that._timezonePopup.data('ufaWindow')) {
                        that._timezonePopup.data('ufaWindow').destroy();
                        that._timezonePopup = null;
                    }
                    if (that.container.is(':visible')) {
                        that.container.data('ufaWindow').bind('deactivate', destroy).close();
                    } else {
                        destroy();
                    }
                } else {
            ...
            showDialog: function (options) {
                var html = ufa.format('<div class=\'k-popup-edit-form\'><div class=\'k-edit-form-container\'><p class=\'k-popup-message\'>{0}</p>', options.text);
                html += '<div class="k-edit-buttons k-state-default">';
                for (var buttonIndex = 0; buttonIndex < options.buttons.length; buttonIndex++) {
            ...
            _initTimezoneEditor: function (model, activator) {
                ...
                that._endTimezone = model.endTimezone;
                if (!timezonePopup) {
                    that._timezonePopup = timezonePopup = container.ufaWindow({
                        modal: true,
                        resizable: false,
                        draggable: true,
                ...
                wnd = timezonePopup.data('ufaWindow');
                wnd.center().open();
            }
        });
        var Scheduler = DataBoundWidget.extend({
            ...
            _isMobile: function () {
                ...
                return options.mobile === true && ufa.support.mobileOS || options.mobile === 'phone' || options.mobile === 'tablet';
            },
            _isMobilePhoneView: function () {
                var options = this.options;
                return options.mobile === true && ufa.support.mobileOS && !ufa.support.mobileOS.tablet || options.mobile === 'phone';
            },
            _groupsByResource: function (resources, groupIndex, groupsArray, parentFieldValue, parentField) {
                ...
                if (resource) {
                    ...
                    for (var dataIndex = 0; dataIndex < data.length; dataIndex++) {
                        var fieldValue = ufa.getter(resource.dataValueField)(data[dataIndex]);
            ...
            select: function (options) {
                ...
                if (options.resources) {
                    ...
                    selectedGroups = new ufa.data.Query(groupsByResource).filter(filters).toArray();
                }
                ...
                if (groups && (options.start && options.end)) {
                    ...
                    var rangeEnd = ufa.date.addDays(getDate(view._endDate), 1);
                    var group;
                    var ranges;
                    if (options.start < rangeEnd && rangeStart <= options.end) {
                        ...
                        if (ranges.length) {
                            that._selection = {
                                start: ufa.timezone.toLocalDate(ranges[0].start.start),
                                end: ufa.timezone.toLocalDate(ranges[ranges.length - 1].end.end),
            ...
            _selectEvents: function (eventsUids, selectedGroups) {
                ...
                for (idx = 0; idx < eventsLength; idx++) {
                    if (groups && isGrouped) {
                        ...
                        events = new ufa.data.Query(events).filter({
                        ...
                        var element = view.element.find(ufa.format('.k-event[data-uid={0}], .k-task[data-uid={0}]', eventsUids[idx]));
                        if (element.length) {
            ...
            _selectable: function () {
                var that = this, wrapper = that.wrapper, selectEvent = ufa.support.mobileOS ? 'touchend' : 'mousedown';
                ...
                wrapper.on(selectEvent + NS, '.k-scheduler-header-all-day td, .k-scheduler-content td, .k-event', function (e) {
                    var which = e.which;
                    var button = e.button;
                    var browser = ufa.support.browser;
                    var isRight = which && which === 3 || button && button == 2;
                    if (ufa.support.mobileOS && e.isDefaultPrevented()) {
                        return;
                    }
                    if (!isRight) {
            ...
            _select: function () {
                ...
                if (current && that._old !== current) {
                    ...
                    if (events[0]) {
                        ...
                        labelFormat = ufa.format(that.options.messages.ariaEventLabel, data.title, data.start, data.start);
                    } else {
                        labelFormat = ufa.format(that.options.messages.ariaSlotLabel, data.start, data.end);
                    }
                ...
                if (key === keys.TAB) {
                    ...
                    if (selection.events.length) {
                        ...
                        if (selection.isAllDay) {
                            selection = $.extend({}, selection, { end: ufa.date.addDays(selection.end, -1) });
                        }
                ...
                if (dataItem && selection) {
                    ...
                    if (this._shiftKey && selection.start && selection.end) {
                        ...
                        if (backward && view._timeSlotInterval) {
                            ufa.date.setTime(selection.end, -view._timeSlotInterval());
                        }
                    } else {
            ...
            _movable: function () {
                ...
                if (movable || resizable && isMobile) {
                    if (isMobile && ufa.support.mobileOS.android) {
                        distance = 5;
                    }
                    that._moveDraggable = new ufa.ui.Draggable(that.element, {
                        distance: distance,
                        filter: '.k-event',
                    ...
                    if (movable) {
                        that._moveDraggable.bind('dragstart', function (e) {
                            ...
                            event = that.occurrenceByUid(eventElement.attr(ufa.attr('uid')));
                            ...
                            if (slot.isDaySlot !== startSlot.isDaySlot) {
                                ...
                                clonedEvent.start = ufa.timezone.toLocalDate(startTime);
                                clonedEvent.end = ufa.timezone.toLocalDate(endTime);
                            ...
                            if (!prevented && (event.start.getTime() !== start.getTime() || event.end.getTime() !== end.getTime() || originSlot.isDaySlot !== endSlot.isDaySlot || ufa.stringify(endResources) !== ufa.stringify(startResources))) {
            ...
            _resizable: function () {
                ...
                if (that._isMobile() && ufa.support.mobileOS.android) {
                    distance = 5;
                }
                that._resizeDraggable = new ufa.ui.Draggable(that.element, {
                    distance: distance,
                    filter: '.k-resize-handle',
                    dragstart: function (e) {
                        var dragHandle = $(e.currentTarget);
                        var eventElement = dragHandle.closest('.k-event');
                        var uid = eventElement.attr(ufa.attr('uid'));
                        var view = that.view();
                        ...
                        startTime = ufa.date.toUtcTime(clonedEvent.start);
                        endTime = ufa.date.toUtcTime(clonedEvent.end);
                    },
                    drag: function (e) {
                        if (!slot) {
                            return;
                        ...
                        if (dir == 'south') {
                            if (!slot.isDaySlot && slot.end - ufa.date.toUtcTime(clonedEvent.start) >= view._timeSlotInterval()) {
                            ...
                            if (!slot.isDaySlot && ufa.date.toUtcTime(clonedEvent.end) - slot.start >= view._timeSlotInterval()) {
                                startTime = slot.startOffset(e.x.location, e.y.location, that.options.snap);
                            }
                        } else if (dir == 'east') {
                            if (slot.isDaySlot && ufa.date.toUtcTime(ufa.date.getDate(slot.endDate())) >= ufa.date.toUtcTime(ufa.date.getDate(clonedEvent.start))) {
                                if (clonedEvent.isAllDay) {
                                    endTime = slot.startOffset(e.x.location, e.y.location, that.options.snap);
                                } else {
                                    endTime = slot.endOffset(e.x.location, e.y.location, that.options.snap);
                                }
                            } else if (!slot.isDaySlot && slot.end - ufa.date.toUtcTime(clonedEvent.start) >= view._timeSlotInterval()) {
                                endTime = slot.endOffset(e.x.location, e.y.location, that.options.snap);
                            }
                        } else if (dir == 'west') {
                            if (slot.isDaySlot && ufa.date.toUtcTime(ufa.date.getDate(clonedEvent.end)) >= ufa.date.toUtcTime(ufa.date.getDate(slot.startDate()))) {
                                startTime = slot.startOffset(e.x.location, e.y.location, that.options.snap);
                            } else if (!slot.isDaySlot && ufa.date.toUtcTime(clonedEvent.end) - slot.start >= view._timeSlotInterval()) {
                                startTime = slot.startOffset(e.x.location, e.y.location, that.options.snap);
                            }
                        }
                        if (!that.trigger('resize', {
                                ...
                                start: ufa.timezone.toLocalDate(startTime),
                                end: ufa.timezone.toLocalDate(endTime),
                                resources: view._resourceBySlot(slot)
                            })) {
                    ...
                    dragend: function (e) {
                        ...
                        if (dir == 'south') {
                            ...
                            if (slot.isDaySlot) {
                                end = ufa.date.getDate(ufa.timezone.toLocalDate(endTime));
                            } else {
                                end = ufa.timezone.toLocalDate(endTime);
                            }
                        } else if (dir == 'west') {
                            if (slot.isDaySlot) {
                                start = new Date(ufa.timezone.toLocalDate(startTime));
                                start.setHours(0);
                                start.setMinutes(0);
                            } else {
                                start = ufa.timezone.toLocalDate(startTime);
            ...
            _updateEvent: function (dir, event, eventInfo) {
                ...
                var updateSeries = function () {
                    ...
                    if (dir == 'south' || dir == 'north') {
                        if (eventInfo.start) {
                            var start = ufa.date.getDate(head.start);
                            ufa.date.setTime(start, getMilliseconds(eventInfo.start));
                            eventInfo.start = start;
                        }
                        if (eventInfo.end) {
                            var end = ufa.date.getDate(head.end);
                            ufa.date.setTime(end, getMilliseconds(eventInfo.end));
                            eventInfo.end = end;
                        }
                    }
            ...
            _modelForContainer: function (container) {
                container = $(container).closest('[' + ufa.attr('uid') + ']');
                return this.dataSource.getByUid(container.attr(ufa.attr('uid')));
            },
            showDialog: function (options) {
            ...
            _confirmation: function (callback, model) {
                ...
                if (editable === true || editable.confirmation !== false) {
                    ...
                    if (!(this._isMobile() && ufa.mobile.ui.Pane)) {
                        buttons.push({
                            name: 'canceledit',
                            text: messages.cancel,
                            click: function () {
            ...
            _createButton: function (command) {
                ...
                return ufa.template(template)(options);
            },
            _convertDates: function (model, method) {
                ...
                if (startTimezone) {
                    if (timezone) {
                        if (method === 'apply') {
                            start = ufa.timezone.convert(model.start, timezone, startTimezone);
                            end = ufa.timezone.convert(model.end, timezone, endTimezone);
                        } else {
                            start = ufa.timezone.convert(model.start, startTimezone, timezone);
                            end = ufa.timezone.convert(model.end, endTimezone, timezone);
                        }
                    } else {
                        start = ufa.timezone[method](model.start, startTimezone);
                        end = ufa.timezone[method](model.end, endTimezone);
                    }
            ...
            occurrencesInRange: function (start, end) {
                return new ufa.data.Query(this._data).filter({
                    logic: 'or',
                    filters: [
                        {
                            logic: 'and',
            ...
            _adjustSelectedDate: function () {
                ...
                if (start && !ufa.date.isInDateRange(date, getDate(start), getDate(selection.end))) {
            ...
            _initializeView: function (name) {
                ...
                if (view) {
                    ...
                    if (typeof type === STRING) {
                        type = ufa.getter(view.type)(window);
                    }
                    if (type) {
            ...
            _dataSource: function () {
                ...
                that.dataSource = ufa.data.SchedulerDataSource.create(dataSource).bind(CHANGE, that._refreshHandler).bind('progress', that._progressHandler).bind('error', that._errorHandler);
            ...
            _progress: function (toggle) {
                ...
                ufa.ui.progress(element, toggle);
            },
            _resources: function () {
                var that = this;
                ...
                for (var idx = 0; idx < resources.length; idx++) {
                    ...
                    that.resources.push({
                        ...
                        dataSource: ufa.data.DataSource.create(dataSource)
                    });
                }
            ...
            _toolbar: function () {
                ...
                ufa.bind(that.toolbar, that._model);
                ...
                toolbar.on(CLICK + NS, '.k-scheduler-views li, .k-scheduler-refresh', function (e) {
                    e.preventDefault();
                    var name = $(this).attr(ufa.attr('name'));
                    if (!that.trigger('navigate', {
                            view: name,
                            action: 'changeView',
                            date: that.date()
        ...
        var defaultViews = {
            day: { type: 'ufa.ui.DayView' },
            week: { type: 'ufa.ui.WeekView' },
            workWeek: { type: 'ufa.ui.WorkWeekView' },
            agenda: { type: 'ufa.ui.AgendaView' },
            month: { type: 'ufa.ui.MonthView' },
            timeline: { type: 'ufa.ui.TimelineView' },
            timelineWeek: { type: 'ufa.ui.TimelineWeekView' },
            timelineWorkWeek: { type: 'ufa.ui.TimelineWorkWeekView' },
            timelineMonth: { type: 'ufa.ui.TimelineMonthView' }
        };
        ui.plugin(Scheduler);
        if (ufa.PDFMixin) {
            ufa.PDFMixin.extend(Scheduler.prototype);
        ...
        var TimezoneEditor = Widget.extend({
            init: function (element, options) {
                var that = this, zones = ufa.timezone.windows_zones;
                if (!zones || !ufa.timezone.zones_titles) {
                    throw new Error('ufa.timezones.min.js is not included.');
                }
                Widget.fn.init.call(that, element, options);
                that.wrapper = that.element;
                that._zonesQuery = new ufa.data.Query(zones);
                that._zoneTitleId = ufa.guid();
            ...
            _zoneTitlePicker: function () {
                ...
                that._zoneTitle = new ufa.ui.DropDownList(zoneTitle, {
                    dataSource: ufa.timezone.zones_titles,
            ...
            _zonePicker: function () {
                ...
                that._zone = new ufa.ui.DropDownList(zone, {
                    dataValueField: 'zone',
        ...
        var ZONETITLEOPTIONTEMPLATE = ufa.template('<option value="#=other_zone#">#=name#</option>');
        var ZONEOPTIONTEMPLATE = ufa.template('<option value="#=zone#">#=territory#</option>');
        var MobileTimezoneEditor = Widget.extend({
            ...
            _zoneTitlePicker: function () {
                var that = this;
                var options = that._options(ufa.timezone.zones_titles, ZONETITLEOPTIONTEMPLATE, that.options.optionLabel);
