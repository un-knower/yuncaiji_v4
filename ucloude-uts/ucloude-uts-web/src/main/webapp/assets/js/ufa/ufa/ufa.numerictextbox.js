(function (f, define) {
    ...
    (function ($, undefined) {
        var ufa = window.ufa, caret = ufa.caret, keys = ufa.keys, ui = ufa.ui, Widget = ui.Widget, activeElement = ufa._activeElement, extractFormat = ufa._extractFormat, parse = ufa.parseFloat, placeholderSupported = ufa.support.placeholder, getCulture = ufa.getCulture, round = ufa._round, CHANGE = 'change', DISABLED = 'disabled', READONLY = 'readonly', INPUT = 'k-input', SPIN = 'spin', ns = '.ufaNumericTextBox', TOUCHEND = 'touchend', MOUSELEAVE = 'mouseleave' + ns, HOVEREVENTS = 'mouseenter' + ns + ' ' + MOUSELEAVE, DEFAULT = 'k-state-default', FOCUSED = 'k-state-focused', HOVER = 'k-state-hover', FOCUS = 'focus', POINT = '.', SELECTED = 'k-state-selected', STATEDISABLED = 'k-state-disabled', ARIA_DISABLED = 'aria-disabled', ARIA_READONLY = 'aria-readonly', INTEGER_REGEXP = /^(-)?(\d*)$/, NULL = null, proxy = $.proxy, extend = $.extend;
        var NumericTextBox = Widget.extend({
            ...
            _arrows: function () {
                ...
                that._upArrowEventHandler = new ufa.UserEvents(that._upArrow, { release: _release });
                that._downArrow = arrows.eq(1);
                that._downArrowEventHandler = new ufa.UserEvents(that._downArrow, { release: _release });
            },
            _blur: function () {
                var that = this;
                that._toggleText(true);
            ...
            _update: function (value) {
                ...
                that._placeholder(ufa.toString(value, format, culture));
                if (isNotNull) {
