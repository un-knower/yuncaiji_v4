(function (f, define) {
    ...
    (function ($, undefined) {
        var ufa = window.ufa, ui = ufa.ui, List = ui.List, keys = ufa.keys, activeElement = ufa._activeElement, ObservableArray = ufa.data.ObservableArray, proxy = $.proxy, ID = 'id', LI = 'li', ACCEPT = 'accept', FILTER = 'filter', REBIND = 'rebind', OPEN = 'open', CLOSE = 'close', CHANGE = 'change', PROGRESS = 'progress', SELECT = 'select', ARIA_DISABLED = 'aria-disabled', ARIA_READONLY = 'aria-readonly', FOCUSEDCLASS = 'k-state-focused', HIDDENCLASS = 'k-loading-hidden', HOVERCLASS = 'k-state-hover', STATEDISABLED = 'k-state-disabled', DISABLED = 'disabled', READONLY = 'readonly', ns = '.ufaMultiSelect', CLICK = 'click' + ns, KEYDOWN = 'keydown' + ns, MOUSEENTER = 'mouseenter' + ns, MOUSELEAVE = 'mouseleave' + ns, HOVEREVENTS = MOUSEENTER + ' ' + MOUSELEAVE, quotRegExp = /"/g, isArray = $.isArray, styles = [
                'font-family',
                'font-size',
                'font-stretch',
                'font-style',
        ...
        var MultiSelect = List.extend({
            ...
            _listOptions: function (options) {
                ...
                if (!template) {
                    template = '#:' + ufa.expr(listOptions.dataTextField, 'data') + '#';
                }
            ...
            _wrapperMousedown: function (e) {
                ...
                if (notInput && !(closeButton && ufa.support.mobileOS)) {
                    e.preventDefault();
                }
                if (!closeButton) {
            ...
            _preselect: function (data, value) {
                var that = this;
                if (!isArray(data) && !(data instanceof ufa.data.ObservableArray)) {
                    data = [data];
                }
                if ($.isPlainObject(data[0]) || data[0] instanceof ufa.data.ObservableObject || !that.options.dataValueField) {
            ...
            _dataSource: function () {
                ...
                that.dataSource = ufa.data.DataSource.create(dataSource).bind(PROGRESS, that._progressHandler).bind('error', that._errorHandler);
            },
            _reset: function () {
            ...
            _keydown: function (e) {
                ...
                var isRtl = ufa.support.isRtl(that.wrapper);
            ...
            _option: function (dataValue, dataText, selected) {
                ...
                if (dataText !== undefined) {
                    option += ufa.htmlEncode(dataText);
                }
            ...
            _tagTemplate: function () {
                ...
                defaultTemplate = isMultiple ? ufa.template('#:' + ufa.expr(options.dataTextField, 'data') + '#', { useWithBlock: false }) : ufa.template('#:values.length# item(s) selected');
                that.tagTextTemplate = tagTemplate = tagTemplate ? ufa.template(tagTemplate) : defaultTemplate;
            ...
            _textContainer: function () {
                var computedStyles = ufa.getComputedStyles(this.input[0], styles);
