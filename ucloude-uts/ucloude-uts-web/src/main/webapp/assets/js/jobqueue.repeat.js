					...
					.ufaGrid(
							{
								dataSource : {
									transport : {
										read : {
											url : 'api/job-queue/repeat-job-get',
											data : function() {
												var params = {
													taskId : $("#txtTaskID")
															.val(),
								...
								columns : [{
									...
									width : 350,
									command:[{
										text:"日志", //job-logger.htm?taskId=xx &taskTrackerNodeGroup=xx1
										click:function(e){
										...
										click:function(e){
								            ...
								            $("#editWindow").ufaWindow({
						                        ...
						                        title: "任务编辑",
						                        visible: false,
						                        content: "job-add.htm?jobqueue=repeat&jobId="+data.jobId
										...
										click:function(e){
											var tr = $(e.target).closest("tr"); 
								            var data = this.dataItem(tr);
								            ufa.confirm({title:"系统提示",content:"是否暂停当前任务？"}).then(function () {
												$.ajax({
													type : "post",
													url : "api/job-queue/repeat-job-suspend",
													data : {
														jobId:data.jobId,
