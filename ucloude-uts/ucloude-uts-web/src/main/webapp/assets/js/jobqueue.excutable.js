										...
										read : {
											...
											data : function() {
												var params = {
													...
													needFeedback : $("#swtichFeedBackClient").data("ufaMobileSwitch").value(),
													submitNodeGroup : $("#ddlClientNodeGroups").data("ufaDropDownList").value(),
													taskTrackerNodeGroup : $("#ddlTaskTrackeNodeGroup").data("ufaDropDownList").value(),
												};
...
//									title : "用户参数"
									...
									command:[{
										...
										click:function(e){
								            ...
								            $("#editWindow").ufaWindow({
						                        ...
						                        title: "任务编辑",
						                        visible: false,
						                        content: "job-add.htm?jobqueue=executable&jobId="+data.jobId
			...
			function triggerClick(e){
				var tr = $(e.target).closest("tr"); 
	            var data = this.dataItem(tr);
	            ufa.confirm({title:"系统提示",content:"是否执行当前任务？"}).then(function () {
					 $.ajax({
