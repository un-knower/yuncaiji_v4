package cn.uway.ucloude.uts.web.admin.filter;


import org.springframework.util.AntPathMatcher;

import cn.uway.ucloude.utils.Base64;
...
public class LoginAuthFilter implements Filter {
    ...
    public void init(final FilterConfig filterConfig) throws ServletException {
        ...
        if (StringUtil.isNotEmpty(excludedURLs)) {
            String[] arr = excludedURLs.split(",");
            excludedURLArray = new String[arr.length];
            for (int i = 0; i < arr.length; i++) {
                excludedURLArray[i] = StringUtil.trim(arr[i]);
            }
        }
    }
    ...
    private void needAuthenticate(final HttpServletRequest request, final HttpServletResponse response) {
        ...
        response.setHeader("Cache-Control", "no-store");
        response.setDateHeader("Expires", 0);
        response.setHeader("WWW-authenticate", AUTH_PREFIX + "Realm=\"uts admin need auth\"");
    }
