...
import cn.uway.framework.cache.store.impl.MemoryLocalDiskStore;
import cn.uway.framework.cache.store.impl.MemoryMappintDiskStore;
...
public class BlockingCacher extends AbstractCacher {
	...
	// LoggerManager.getLogger(BlockingCacher.class);
	...
	public Element getElement(ElementIndex elementIndex) throws Exception {
		...
		try {
			...
			// 对从系列化中读出的BlockData对象，要调用一下processOnAfterSerialRead方法
			Object elValue = null;
			if (element != null && ((elValue = element.getElementValue()) != null)) {
				BlockData blockData = (BlockData)elValue;
				blockData.processOnAfterSerialRead(this.repository);
			}
