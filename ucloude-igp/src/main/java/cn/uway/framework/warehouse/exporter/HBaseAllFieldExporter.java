...
import cn.uway.util.HbaseOperateTableUtils;
import cn.uway.util.StringUtil;

/**
 * @author liuwx HBASE数据库输出器
 * 
 */
public class HBaseAllFieldExporter extends AbstractExporter {

	/**
	 * 日志
	 */
	protected static final ILogger LOGGER = LoggerManager
			.getLogger(HBaseAllFieldExporter.class);
	...
	private HbaseOperateTableUtils hbaseTableUtils = new HbaseOperateTableUtils();
	 ...
	 *            输出参数
	 */
	public HBaseAllFieldExporter(DbExportTemplateBean dbExportTempletBean,
		...
		this.table = dest;
	    this.id =dbExportTempletBean.getId();
	    
	    this.tableAndId =this.table+"-"+this.id;
...
//						.getExportTableForIdConf(this.table);
				
				//如果*hbase_conf.xml中有多个表名相同，但是id号不同的，则需要通过table-id来获取，其中id的配置必须和export下xml中的id值是相同的
				this.exportTargetTableProperty = hbaseExporDBConf
						.getExportTableForIdConf(this.tableAndId);
				
				if(this.exportTargetTableProperty ==null ){
					this.exportTargetTableProperty = hbaseExporDBConf.getExportTableForIdConf(this.table);
				}
	...
	public void export(Map<String, String> record) throws Exception {
		...
		try {
			...
			// 入主表
			{
				...
				buildCSVFormatLine2(record, put, defColumnFamilyBytes);
				// 入库附加输出字段
				if (tableProp.exportFields != null
						&& tableProp.exportFields.size() > 0) {
					for (HBaseExportField exportField : tableProp.exportFields) {
						String fieldCellText = record
								.get(exportField.propertyName);
						if (fieldCellText != null && fieldCellText.length() > 0) {
							put.add(defColumnFamilyBytes,
									exportField.columnFamilyFieldBytes,
...
//				put.add(defColumnFamilyBytes, defColumnFamilyFieldBytes, null);
				Map<String, String> indexRecord = new HashMap<String, String>();
				List<HBaseExportField> efList = tableProp.exportFields;

				if (efList != null) {
					for (HBaseExportField field : efList) {
						indexRecord.put(field.fieldName,
								record.get(field.fieldName));
					}
				}

				buildCSVFormatLine2(indexRecord, put, defColumnFamilyBytes);
				// 入库附加输出字段
				if (tableProp.exportFields != null
						&& tableProp.exportFields.size() > 0) {
					for (HBaseExportField exportField : tableProp.exportFields) {
			...
			if (this.currNum >= batchNum) {
				...
				// objStatus.updateBySynchronized(statusDAO,
				// exporterArgs.getObjStatus().get(0).getId());
			}
	...
	public void close() {
		...
		try {
			...
			// .getId());
			status.updateBySynchronized(statusDAO, exporterArgs.getObjStatus()
					.get(0).getId());
		} catch (Exception e) {
			...
			// 优先根据config.ini中的system.logcltinsert.flag进行判断，如果开启（1）， 添加日志入库操作
			// log_clt_insert ,数据库汇总需要依赖此表,
	...
	byte[] buildRowKey(Map<String, String> record,
		...
		byte[] rowKey = exportTargetTableProperty.getKeyBuilder().buildKey(
				(byte) tableProp.tabIndex, nRawFileKey1, nRawFileKey2,
				(int) this.current,
	 ...
	 * @return
	 */
	protected void buildCSVFormatLine2(Map<String, String> record, Put put,
			byte[] columnFamily) {
		for (ColumnTemplateBean bean : this.columns) {
			String column = bean.getPropertyName();
