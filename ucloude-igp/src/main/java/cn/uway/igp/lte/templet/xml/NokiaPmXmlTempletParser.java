...
public class NokiaPmXmlTempletParser extends TempletParser {
	...
	public void personalHandler(Templet templet, Node templetNode) throws Exception {
		Node measurementTypeNode = templetNode.getAttributes().getNamedItem("measurementType");
		if (measurementTypeNode != null) {
			...
			if (StringUtil.isEmpty(measurementTypeName))
				throw new Exception("measurementType属性值不能为空");
			templet.setDataName(measurementTypeName.trim());
		} else
			throw new Exception("缺少measurementType属性");
	}
}
