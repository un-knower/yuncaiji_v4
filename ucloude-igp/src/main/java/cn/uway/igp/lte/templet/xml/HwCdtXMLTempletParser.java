 ...
 * 朗讯cm xml解析模板
 * 
 * @author yanbo @ 2014年8月8日
 */
public class HwCdtXMLTempletParser extends TempletParser{
		...
		for(Node node = fieldsNode.getFirstChild(); node != null; node = node.getNextSibling()){
			if(node.getNodeType() == Node.ELEMENT_NODE){
				...
				if(nodeName.equalsIgnoreCase("field")){
					...
					Node isHexNode = node.getAttributes().getNamedItem("isHex");
					if(null != isHexNode){
						String isHex = isHexNode.getNodeValue().trim();
						if(isHex.equalsIgnoreCase("0") || isHex.equalsIgnoreCase("true")){
							field.setHex(true);
						}else{
							field.setHex(false);
						}
					...
					if(null != typeNode){
						field.setType(typeNode.getNodeValue().trim());
					}
					...
					if(isSplitNode != null){
						...
						if(value != null && "true".equals(value.trim())){
							...
							if(hasOtherRegexsNode != null && "yes".equals(hasOtherRegexsNode.getNodeValue())){
								...
								field.setRegexsNum(Integer.parseInt(node.getAttributes().getNamedItem("regexsNum")
										.getNodeValue().trim()));
								field.setRegexsSplitSign(node.getAttributes().getNamedItem("regexsSplitSign")
										.getNodeValue().trim());
							}
